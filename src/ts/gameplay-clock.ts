...
export class GameplayClock {
    lastUpdateMs = 0;
    dailyDate = '';
    completedTasks: Record<string, string> = Object.create(null);
    freightGenerationMinute = 0;
    static validDate(value: unknown): value is string {
        return civilDayIndex(value) != null;
    }
    static parisDate(ms: number): string {
        if (!Number.isFinite(ms) || !Number.isFinite(new Date(ms).getTime())) return '';
    ...
    load(raw: unknown, legacySaveTime?: unknown): void {
        ...
        if (data.completedTasks && typeof data.completedTasks === 'object') {
            for (const [key, date] of Object.entries(data.completedTasks)) {
                if (GameplayClock.validDate(date)) this.completedTasks[key] = date;
            }
        }
    ...
    advance(date: string, nowMs = Date.now()): { elapsedMinutes: number; dailyDates: string[] } {
        if (!Number.isFinite(nowMs) || !Number.isFinite(new Date(nowMs).getTime()) || !GameplayClock.validDate(date)) return { elapsedMinutes: 0, dailyDates: [] };
        ...
        if (!this.dailyDate) this.dailyDate = date;
    ...
    runDailyTask(key: string, date: string, task: () => void): void {
        if (!key || !GameplayClock.validDate(date) || (this.completedTasks[key] || '') >= date) return;
        task(); // Failed tasks remain retryable; successful siblings do not run twice.
        this.completedTasks[key] = date;
    }
    markSettled(date: string): void { if (GameplayClock.validDate(date) && date > this.dailyDate) this.dailyDate = date; }
