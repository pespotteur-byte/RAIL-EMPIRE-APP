...
export const CATALOG_CARGO_ALIASES: Record<string, string> = {
...
const norm = (list: readonly string[] | null | undefined): string[] => [...new Set((list || []).map((x: string) => CATALOG_CARGO_ALIASES[x] || x))];
export interface CargoCatalogItem { [key:string]: unknown; cargoTypes?: string[]; technicallyCompatibleCargoTypes?: string[]; }
export function normalizeCatalogCargoKeys<T extends CargoCatalogItem>(catalog: readonly T[]): Array<T & {cargoTypes:string[]; technicallyCompatibleCargoTypes:string[]}> {
