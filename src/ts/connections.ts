...
type TransferStats = { totalTransfers: number; successfulTransfers: number; missedTransfers: number };
type PendingTransfer = Record<string, unknown>;
type ConnectionStop = { stationId?: string; technicalLocationId?: string; type?: string; arrivalTime?: number; departureTime?: number; locationOccurrenceId?: string };
type ConnectionService = { id: string; name?: string; state?: string; active?: boolean; completed?: boolean; cancelled?: boolean;
    currentStopIndex?: number; stops?: ConnectionStop[]; getCurrentStops?: () => ConnectionStop[];
    train?: { delay?: number; stoppedAt?: { id?: unknown }; _stoppedSinceGameTime?: number | null };
    delay?: number; _v2BaseDate?: string; _v2OccurrenceId?: string; _currentDate?: string; _tripCount?: number; isReturnLeg?: boolean };
type ConnectionArrival = { key: string; serviceId: string; stationId: string; at: number };
type ConnectionSettlement = { key: string; at: number };
type StationServiceEntry = { serviceId: string; serviceName: string; arrival: number; departure: number; canArrive: boolean; canDepart: boolean };
const serviceKey = (id: unknown): string => String(id ?? '').replace(/^(v2:.+):\d{4}-\d{2}-\d{2}$/, '$1');
const DAY_MIN = 1440;
...
export class Connections {
     ...
     */
    addTransfer(fromServiceId:string, toServiceId:string, stationId:string) {
        fromServiceId = serviceKey(fromServiceId); toServiceId = serviceKey(toServiceId); stationId = String(stationId ?? '');
        if (!fromServiceId || !toServiceId || fromServiceId === toServiceId || !stationId) return null;
    ...
    removeTransfer(id:string) {
        this.transfers = this.transfers.filter((t: { id: unknown }) => t.id !== id);
    }
    _getWaitTime() {
        return ({ strict: 0, moderate: 5, flexible: 15 } as Record<string, number>)[String(this.waitPolicy)] ?? 5;
    }
    /**
    ...
    _booked(service: ConnectionService, value: unknown, now: number): number {
        ...
        if (service._v2BaseDate && service._v2OccurrenceId) return this._clock(t, service._v2BaseDate);
        const day = Math.floor(now / DAY_MIN) * DAY_MIN;
        let booked = day + ((t % DAY_MIN) + DAY_MIN) % DAY_MIN;
        if (booked - now > DAY_MIN / 2) booked -= DAY_MIN;
        if (now - booked > DAY_MIN / 2) booked += DAY_MIN;
        return booked;
    }
    ...
    _evaluate(transfer: TransferConfig, receiver: ConnectionService | undefined, stop: ConnectionStop | undefined,
              now: number, services: ConnectionService[]) {
        ...
        if (events.length) arrival = Math.max(...events.map(e => e.at));
        for (const feeder of services) {
            if (!feeder || serviceKey(feeder.id) !== serviceKey(transfer.fromServiceId) || feeder.cancelled || feeder.active === false) continue;
            ...
            if (!next || next.stationId !== transfer.stationId || next.technicalLocationId || next.type !== 'arret') continue;
            const planned = this._booked(feeder, next.arrivalTime ?? next.departureTime, now);
            if (!Number.isFinite(planned) || Math.abs(planned - bookedDep) > MAX_TRANSFER_AGE_MIN) continue;
            ...
            if (stationary && arrival == null) {
                ...
                if (Number.isFinite(actual) && actual <= now) arrival = actual;
    ...
    shouldWait(serviceId: string, stationId: string, currentTime: unknown, services: unknown, dateStr?: string) {
        if (!Array.isArray(services) || !serviceId || !stationId) return false;
        const live = services as ConnectionService[];
    ...
    onArrival(service: ConnectionService, stop: ConnectionStop, currentTime: unknown, dateStr?: string) {
        if (!service || !stop || stop.type !== 'arret' || stop.technicalLocationId || !stop.stationId) return;
        if (!this.transfers.some(t => serviceKey(t.fromServiceId) === serviceKey(service.id) && t.stationId === stop.stationId)) return;
        ...
        if (!this.arrivals.some(e => e.key === key)) this.arrivals.push({ key, serviceId: service.id, stationId: stop.stationId, at: now });
    }
    onDeparture(service: ConnectionService, stop: ConnectionStop, currentTime: unknown, dateStr: string | undefined, services: unknown) {
        if (!service || !stop || stop.type !== 'arret' || stop.technicalLocationId || !stop.stationId || !Array.isArray(services)) return;
        const now = this._clock(currentTime, dateStr || service._currentDate);
        if (!Number.isFinite(now)) return;
        this._prune(now);
        for (const t of this.transfers) {
            if (serviceKey(t.toServiceId) !== serviceKey(service.id) || t.stationId !== stop.stationId) continue;
            const result = this._evaluate(t, service, stop, now, services as ConnectionService[]);
            ...
            if (this.settled.some(e => e.key === key)) continue;
     ...
     */
    recordTransfer(fromServiceId:string, toServiceId:string, stationId:string, success: unknown) {
    ...
    autoDetect(services: ActiveService[], world: unknown) {
        const stationServices: Record<string, StationServiceEntry[]> = Object.create(null);
        for (const svc of services) {
            const stops = svc.getCurrentStops?.() || svc.stops || [];
            stops.forEach((stop, i) => {
                if (!stop.stationId || stop.type !== 'arret' || stop.technicalLocationId) return;
        ...
        for (const [stationId, svcs] of Object.entries(stationServices)) for (const arriving of svcs) for (const departing of svcs) {
            if (!arriving.canArrive || !departing.canDepart || serviceKey(arriving.serviceId) === serviceKey(departing.serviceId)) continue;
    ...
    render(container: HTMLElement, game: RailEmpire) {
        if (!container)
            return;
        const services = (game.scheduleCreator?.services || []).filter((s: ActiveService) => s.active !== false && !s.cancelled &&
            (s.serviceType === 'passager' || ['PASSENGER','passager','voyageur'].includes(String(s.category || ''))));
        const usedStations = new Set<string>();
        for (const s of services) for (const stop of s.getCurrentStops?.() || s.stops || []) {
            if (stop.type === 'arret' && !stop.technicalLocationId && stop.stationId) usedStations.add(String(stop.stationId));
        }
        // Only served stations: no 30,000-option selector just to configure two trains.
        const stations = (game.world?.stations || []).filter((s: {id: unknown}) => usedStations.has(String(s.id)));
      ...
      <div class="dash-section">
        ...
        <div class="dash-train-table">
          ...
          ${this.transfers.length === 0 ? '<div style="padding:8px;color:var(--text3)">Aucune correspondance configurée</div>' :
            this.transfers.map((t: { fromServiceId: unknown; toServiceId: unknown; stationId: unknown; id: unknown }) => {
                const fromSvc = services.find((s: { id: unknown }) => serviceKey(s.id) === serviceKey(t.fromServiceId));
                const toSvc = services.find((s: { id: unknown }) => serviceKey(s.id) === serviceKey(t.toServiceId));
                const station = stations.find((s: { id: unknown }) => s.id === t.stationId);
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
          <div>
            ...
            <select id="conn-from" style="width:150px;font-size:11px;padding:4px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:4px">
              ${services.map((s: { id: unknown; name: unknown }) => `<option value="${escapeHtml(s.id)}">${escapeHtml(s.name)}</option>`).join('')}
            </select>
          </div>
          <div>
          ...
          <div>
            ...
            <select id="conn-station" style="width:150px;font-size:11px;padding:4px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:4px">
              ${stations.map((s: { id: unknown; name: unknown }) => `<option value="${escapeHtml(s.id)}">${escapeHtml(s.name)}</option>`).join('')}
            </select>
          </div>
    ...
    `;
        ...
        container.querySelectorAll('.conn-policy').forEach((node) => {
            ...
            btn.addEventListener('click', () => {
                ...
                if (policy !== 'strict' && policy !== 'moderate' && policy !== 'flexible') return;
                this.waitPolicy = policy;
                this.transfers.forEach((t: { waitTime: unknown }) => t.waitTime = this._getWaitTime());
                game.saveState?.();
    ...
    loadFromSave(s: unknown) {
        ...
        const seen = new Set<string>();
        const rawTransfers: UnknownRecord[] = (Array.isArray(s.transfers) ? s.transfers : []).filter(isRecord);
        ...
        }).filter((t): t is TransferConfig => t !== null);
        const raw: UnknownRecord = isRecord(s.stats) ? s.stats : {};
