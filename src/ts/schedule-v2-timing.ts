import { segmentsFromRoute, simulateProfile, type RoutePoint, type TrainPhysicsParams } from './train-physics.js';

type TimingWeather = string | { current?: unknown; type?: unknown; condition?: unknown } | null;

interface ElectricSystemLike { voltage?: unknown; frequency?: unknown; }
...
interface TimingVersion {
  locations?: TimingLocation[];
  outboundPath?: TimingPath | null;
  performanceProfile: TimingProfile;
...
export function calculatePhysicalTravelSeconds(route: RoutePoint[], profile: TimingProfile, opts: PhysicalTravelOptions = {}): number {
...
function weatherTimingKey(weather: TimingWeather): string {
...
function profileTimingKey(profile: TimingProfile): string {
  const p=profile||{};
...
function legTimingKey(leg: TimingLeg, profile: TimingProfile, weather: TimingWeather): string {
...
export function physicalTravelSecondsForLeg(leg: TimingLeg, profile: TimingProfile, { weather = null }: RecalculateTimingOptions = {}): number {
  if(!leg?.routePoints?.length || leg.routePoints.length<2) return 0;
  const signature=legTimingKey(leg,profile,weather);
  const cached=Number(leg.physicalTravelSec);
  if(leg.physicalTravelSignature===signature && Number.isFinite(cached) && cached>0) return cached;
...
export function recalculateScheduleTiming<T>(version: T, { firstDepartureSec = null, weather = null }: RecalculateTimingOptions = {}): T {
  const v = version as unknown as TimingVersion;
  if (!v?.locations?.length) return version;
  v.normalize?.();
  ...
  if (!locs[0].departureOverride) locs[0].departureSec = Math.round(cursor);
  ...
  for (let i=0;i<locs.length-1;i++) {
    ...
    if(!leg?.routePoints?.length || leg.routePoints.length<2){
      if(v.outboundPath) v.outboundPath.error=`Liaison ORM manquante entre ${locs[i].name||locs[i].id} et ${locs[i+1].name||locs[i+1].id}`;
      return version;
    }
    const travelSec = physicalTravelSecondsForLeg(leg, v.performanceProfile, { weather });
    if (!Number.isFinite(travelSec)) {
      if (v.outboundPath) v.outboundPath.error = `Traction insuffisante entre ${locs[i].name || locs[i].id} et ${locs[i+1].name || locs[i+1].id}.`;
      for (let j = i + 1; j < locs.length; j++) {
        locs[j].computedArrivalSec = locs[j].computedDepartureSec = null;
        if (!locs[j].arrivalOverride) locs[j].arrivalSec = null;
        if (!locs[j].departureOverride) locs[j].departureSec = null;
      }
      return version;
    }
    ...
    if (!next.arrivalOverride) next.arrivalSec = computedArrival;
    ...
    if (!next.departureOverride) next.departureSec = computedDeparture;
...
export function shiftScheduleFromLocation<T>(version: T, locationIndex: number, deltaSec: number): T {
  const v = version as unknown as TimingVersion;
  if (!v?.locations || !deltaSec) return version;
