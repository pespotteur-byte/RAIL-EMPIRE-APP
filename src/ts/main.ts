import { LiveryLibrary, copyLiveryAppearance, type LiveryTarget } from './livery-model.js';
import {ReplayPump} from './replay-pump.js';
import { acquireImportUiLock } from './import-ui-lock.js';
import { StateTransaction, validateSaveDocument, type SaveDocument } from './state-transaction.js';
...
type __KPA57 = { "phase"?: unknown; "totalShards": number; "shard": number; "count": unknown };
type __KPA58 = { "phase"?: string; "done": unknown; "total": unknown; "stations": unknown };
type __KPA59 = { "rameId": unknown };
type __KPA60 = { "findRameTrack": (...args: unknown[]) => unknown };
type __KPA61 = { "_state": Record<string, unknown>; "_currentFrontKm": (...args: unknown[]) => unknown };
type __KPA62 = { "_state": Record<string, unknown>; "_currentFrontKm": (...args: unknown[]) => unknown };
...
import { ScheduleCreator, cantonManager, type ActiveService } from './schedule-creator.js';
...
import { applyExternalCatalogBundle, clearExternalCatalogBundle, loadExternalCatalogBundle, parseExternalCatalogFile, saveExternalCatalogBundle, type ExternalCatalogApplyResult } from './catalog-external.js';
import { GlobalStationCatalog } from './global-stations.js';
import { RailReferenceSync, type RailReferenceSyncProgress } from './rail-reference-sync.js';
...
type PlayerSettings = { logoUrl?: string; companyColor?: string; incidentsEnabled?: boolean; realism?: Partial<RailEmpire['realismSettings']> };
type AdminIncidentDetail = { name?: unknown; description?: unknown; duration?: unknown };
type ParisTick = { seconds?: unknown };
type PositionedService = { position?: { lat: number; lon: number } | null };

export class RailEmpire {
  liveries = new LiveryLibrary();
  private _liveryWriting = false;
  declare storage: GameStorage;
  declare incidentManager: IncidentManager;
  declare economy: Economy;
  declare engine: SimulationEngine;
  declare rng: SeededRng;
  ...
  private _externalCatalogLastResult: ExternalCatalogApplyResult | null = null;
  ...
  constructor() {
    ...
    this._railGraphReady = this.world.railNetworkReady?.().catch((err: unknown) => { console.warn('RailGraph local indisponible:', err); return null; })
      || this.railGraphPack.ready().catch((err: unknown) => { console.warn('RailGraph local indisponible:', err); return null; });
    ...
    this.industrialClients.freightAccessProvider = (id: string) => this.stationUpgrades.canHandleFreight(id);
  ...
  async init() {
    ...
    const launch = async (action: () => Promise<void>) => {
      if (this._launching || this._started) return;
      this._launching = true; btnNew.disabled = true; btnLoad.disabled = true;
      try { await action(); }
      catch (error) { alert('Chargement refusé : ' + (error instanceof Error ? error.message : String(error))); }
      finally { this._launching = false; btnNew.disabled = false; btnLoad.disabled = false; }
    };
    btnNew.addEventListener('click', () => void launch(async () => {
      const name = nameInput.value.trim();
      if (!name) throw new Error('Entrez un nom de compagnie');
      if (!await this._ensureAllZoomGameplayStations()) throw new Error('Catalogue européen de gares gameplay absent ou invalide.');
      this.account.companyName = name;
      this.startGame(null);
    }));
    btnLoad.addEventListener('click', () => void launch(async () => {
      const saved = await this.storage.loadGame();
      if (!saved) throw new Error('Aucune sauvegarde lisible');
      if (!await this._ensureAllZoomGameplayStations()) throw new Error('Catalogue européen de gares gameplay absent ou invalide.');
    ...
    importInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        if (!ready) throw new Error('Catalogue européen de gares gameplay absent ou invalide');
        await this.importState(saved);
        nameInput.value = saved.companyName;
        this.startGame(saved);
      } catch (err) {
        alert('Erreur: fichier de sauvegarde invalide.\n' + (err as { message?: string }).message);
      } finally { this._launching = false; btnNew.disabled = false; btnLoad.disabled = false; importInput.value = ''; }
    });
    ...
    this._ensureAllZoomGameplayStations().catch((err: unknown) => console.warn('All-zoom station preload failed:', err));
  }

  startGame(savedState: unknown) {
    ...
    if (!this.engine.chronologicalClock) this.engine.enableChronologicalReplay();
    ...
    if (!document.getElementById('btn-storage')) {
      ...
      button.id = 'btn-storage'; button.className = 'btn-header'; button.type = 'button';
      button.textContent = '🗜'; button.title = 'Stockage : compression et quota';
      button.setAttribute('aria-label', 'Stockage : compression et quota');
      button.addEventListener('click', () => openStoragePanel(this.storage, {
        save: async () => {
          if (this._saveWritePromise) await this._saveWritePromise;
    ...
    window.addEventListener('admin-incident', (event: Event) => {
      const inc = (event as CustomEvent<AdminIncidentDetail>).detail || {};
    ...
    if (this._backgroundSimInterval) clearInterval(this._backgroundSimInterval);
    this._backgroundSimInterval = setInterval(() => {
      if (!this.running || this.engine?.paused || typeof document==='undefined' || !document.hidden) return;
      try { this.engine.update(); } catch (e) { this.diagnostics.record('BACKGROUND_TICK', e); }
    }, 500);
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        ...
        if (gapSec > 8 && !this.engine.chronologicalClock) {
          ...
          } catch (e) { this.diagnostics.record('BACKGROUND_CATCHUP', e); }
        }
      }
    });
  }

  _setWorldStationsStatus(message: unknown, state: StatusState = 'loading') {
  ...
  async _indexAllZoomGameplayStations(stations: unknown, source : unknown = '') {
    if (!Array.isArray(stations) || !stations.length) return 0;
    this._setWorldStationsStatus('Gares gameplay Europe : création 0 %');
    const count = await this.world.setBuiltInGameplayStationsAsync(stations, ({ indexed, total, count: ready }: { indexed: number; total: number; count: number }) => {
  ...
  _ensureAllZoomGameplayStations() {
    if (this._europeGameplayReady) return this._europeGameplayReady;
    ...
    this._europeGameplayReady = this.globalStations.load(onProgress).then(async (stations: unknown) => {
      ...
      try {
        ...
        if (cached.stations || cached.freightSites) this._setWorldStationsStatus(`Référentiel rail : ${count.toLocaleString('fr-FR')} points natifs · cache complet chargé`, 'done');
        return count;
      } catch (err) {
  ...
  _startRailReferenceSync() {
    if (this._railReferenceSyncStarted) return;
    this._railReferenceSyncStarted = true;
    if (this._railReferenceSyncTimer) clearTimeout(this._railReferenceSyncTimer);
    this._railReferenceSyncTimer = setTimeout(() => {
      this._railReferenceSyncTimer = null;
      void this.railReferenceSync.syncAll(this.world, (p: RailReferenceSyncProgress) => {
      ...
      }).then((r: {countries:number;stations:number;freightSites:number;errors:number}) => {
        this.renderer?.invalidateStatic?.();
  ...
  _streamNativeOSMViewport(force : unknown = false) {
    // Replaying elapsed simulation must not multiply external viewport requests.
    if (this.engine?.getReplayDebtSeconds?.() > 2) return Promise.resolve(this.world?._builtInStationCount || 0);
    const tm = this.renderer?.tileMap;
    if (!tm || !this.world || !this.orm) return Promise.resolve(0);
    ...
    if (zoom < 10) {
      ...
      if (force) this._setWorldStationsStatus(`Gares gameplay Europe : ${Number(count).toLocaleString('fr-FR')} visibles · OSM local à zoom 10+`, count ? 'done' : 'loading');
    ...
    if (!tl || !br) return Promise.resolve(this.world._builtInStationCount || 0);
    let south = Math.min(tl.lat, br.lat), north = Math.max(tl.lat, br.lat);
    let west = Math.min(tl.lon, br.lon), east = Math.max(tl.lon, br.lon);
    if (![south, north, west, east].every(Number.isFinite)) return Promise.resolve(0);
    ...
    south = Math.max(-90, south - padLat); north = Math.min(90, north + padLat);
    west = Math.max(-180, west - padLon); east = Math.min(180, east + padLon);
    const q = (v: number) => (Math.round(v * 20) / 20).toFixed(2); // 0.05° cells
    const key = `${q(south)},${q(west)},${q(north)},${q(east)}`;
    if (!force && key === this._osmNativeViewportKey) {
      return this._osmNativeStreamPromise || Promise.resolve(this.world._builtInStationCount || 0);
    }
    if (this._osmNativeStreamPromise) return this._osmNativeStreamPromise;
    ...
    const p = (async () => {
      ...
      let stations = [];
      try {
        stations = await this.orm.fetchStationsTiled(south, west, north, east, (info: __KPA58) => {
  ...
  _seedCatalogCargoTypes() {
    if (this._catalogCargoTypesSeeded) return;
    this._catalogCargoTypesSeeded = true;
    if (Array.isArray(CATALOG_CARGO_TYPES_BASE) && this.cargoTypes?.ensureType) {
      for (const ct of CATALOG_CARGO_TYPES_BASE) this.cargoTypes.ensureType(ct.category, ct);
    }
    if (Array.isArray(BATCH186_FREIGHT_CARGO_TYPES) && this.cargoTypes?.ensureType) {
      for (const ct of BATCH186_FREIGHT_CARGO_TYPES) this.cargoTypes.ensureType(ct.category, ct);
    }
    if (Array.isArray(BATCH186_FREIGHT_PASS2_CARGO_TYPES) && this.cargoTypes?.ensureType) {
      for (const ct of BATCH186_FREIGHT_PASS2_CARGO_TYPES) this.cargoTypes.ensureType(ct.category, ct);
    }
    if (Array.isArray(CATALOG_NORMALIZATION_EXTRA_CARGO_TYPES) && this.cargoTypes?.ensureType) {
      for (const ct of CATALOG_NORMALIZATION_EXTRA_CARGO_TYPES) this.cargoTypes.ensureType(ct.category, ct);
    }
  }

  async _ensureBaseCatalogModulesLoaded() {
    if (this._baseCatalogModules) return this._baseCatalogModules;
    if (this._baseCatalogModulesPromise) return this._baseCatalogModulesPromise;
  ...
  async _ensureBaseCatalogSeeded() {
    if (this._baseCatalogSeeded) return 0;
    this._seedCatalogCargoTypes();
  ...
  seedCatalog() {
    ...
    if (this._batch186FullCatalogLoaded && Array.isArray(this._batch186FullCatalogAdditions) && this._batch186FullCatalogAdditions.length) allCatalog = allCatalog.concat(this._batch186FullCatalogAdditions);
    ...
    const existing = this._batch186ExistingIds || (this._batch186ExistingIds = new Set(this.rollingStock.getAll().map((i: { id: unknown }) => i.id)));
    let added = 0;
    ...
    if (added && this.ui?.activePage === 'rolling-stock') this.ui.renderStockList();
    return added;
  }
  ...
  _applyAdminCatalogOverridesIncremental() {
    ...
    if (!deletions.length && !modifications.length && !imports.length) return 0;

    let changed = 0;
    if (deletions.length) {
      for (const id of deletions) {
        if (!id || !this.rollingStock.getById(id)) continue;
    ...
    for (const mod of modifications) {
      ...
      if (item && item._catalog && !item._edited) {
        ...
        if (this.rollingStock.update(item.id, mod, { markEdited: false })) changed++;
      }
    }
    for (const imp of imports) {
      if (!imp?.id || this.rollingStock.getById(imp.id)) continue;
      this.rollingStock.add({ ...imp, _catalog: true });
      changed++;
    }
    this._batch186ExistingIds = new Set(this.rollingStock.getAll().map((i: { id: unknown }) => i.id));
    if (changed && this.ui?.activePage === 'rolling-stock') this.ui.renderStockList();
    return changed;
  }

  _seedBatch186CatalogChunk(chunk: unknown) {
  ...
  _setCatalogLoadStatus(message: unknown, state: StatusState = 'loading') {
    if (typeof document === 'undefined') return;
    ...
    if (state === 'done') setTimeout(() => { if (el) el.style.display = 'none'; }, 1800);
  }

  async _applyStoredExternalCatalog() {
    if (this._externalCatalogApplied) return this._externalCatalogLastResult;
  ...
  async importExternalCatalogFile(file: File) {
    if (!file || typeof file.name !== 'string') throw new Error('Fichier catalogue absent.');
  ...
  _loadBatch186FullCatalogInBackground() {
    if (this._batch186FullCatalogLoaded) return Promise.resolve(this._batch186FullCatalogAdditions);
    if (this._batch186FullCatalogLoading) return this._batch186FullCatalogLoading;

    const run = (async () => {
      await this._ensureBaseCatalogSeeded();
      if (this._batch186FullCatalogLoaded) return this._batch186FullCatalogAdditions;
      this._setCatalogLoadStatus('Catalogue complet : chargement 0 %');
      this._batch186FullCatalogAdditions = [];
      const additions = await loadBatch186FullCatalogAdditions((done, total, count, chunk: unknown) => {
      ...
      try { await this._applyStoredExternalCatalog(); } catch (error) { console.error('Catalogue externe non appliqué:', error); this._setCatalogLoadStatus('Catalogue externe invalide ou inaccessible', 'error'); }
      if (this.ui?.activePage === 'rolling-stock') this.ui.renderStockList();
  ...
  _setupSettings() {
    ...
    const railReferenceSyncBtn = document.getElementById('settings-sync-rail-reference') as HTMLButtonElement | null;
    const saveBtn = document.getElementById('settings-save');

    // Load saved settings
    let settings: PlayerSettings = {};
    try {
      { const parsed: unknown = JSON.parse(localStorage.getItem('re_player_settings') || '{}'); settings = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as PlayerSettings : {}; }
    } catch (e) {}
    if (settings.realism) {
    ...
    const fmtPrice = (v: unknown, unit: unknown) => `${Number(v).toFixed(2)} ${unit}`;
    ...
    railReferenceSyncBtn?.addEventListener('click', () => {
      if (railReferenceSyncBtn.disabled) return;
      ...
      }, true).then((r: {countries:number;stations:number;freightSites:number;errors:number}) => {
        this.renderer?.invalidateStatic?.();
        const total = Number(this.world._builtInStationCount || this.world.stations.length || 0);
    ...
    saveBtn.addEventListener('click', () => {
      ...
      // Apply logo
      const logoEl = document.getElementById('company-logo') as HTMLImageElement;
      if (logoUrl) {
        logoEl.src = logoUrl;
  ...
  async exportSaveFile() {
    ...
    try {
      ...
      alert('Erreur lors de la sauvegarde: ' + (e as { message?: string }).message);
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = oldText || 'Enregistrer'; }
    }
  }

  _collectPinnedNativeStationIds() {
    const ids = new Set();
    const add = (id: unknown) => { if (typeof id === 'string' && id) ids.add(id); };
    for (const line of this.lineManager?.getAll?.() || []) for (const id of line.stops || []) add(id);
    for (const svc of this.scheduleCreator?.services || []) {
      for (const st of svc.stops || []) add(st?.stationId);
      for (const st of svc._returnStopsData || svc.returnStops || []) add(st?.stationId);
    }
    for (const schedule of this.scheduleV2?.schedules || this.scheduleV2?.getAll?.() || []) {
      for (const ver of schedule?.versions || []) {
        for (const loc of ver?.locations || []) add(loc?.stationId);
      }
    }
    for (const d of this.depotManager?.getAll?.() || []) add(d?.stationId);
    for (const sl of this.sillonManager?.getAll?.() || []) { add(sl?.fromStationId); add(sl?.toStationId); }
    for (const vp of this.voiePointManager?.getAll?.() || []) add(vp?.stationId);
    for (const t of this.world?.tracks || []) { add(t?.stationA); add(t?.stationB); }
    return ids;
  }
  ...
  private _captureImportCheckpoint(): StateTransaction {
    ...
    for (const key of keys) { const value: unknown = this[key]; if (value && typeof value === 'object') tx.captureRoot(value); }
    return tx;
  }

  loadState(input: unknown): void {
    if (this._liveryWriting) throw new Error('Enregistrement d’une livrée en cours.');
    validateSaveDocument(input);
    const tx = this._captureImportCheckpoint();
    const cadence = this.incidentManager.getCadenceSave();
    try { this._loadStateUnchecked(input); tx.release(); this.ui?.resetLiveryEditor?.(); }
    catch (error) { tx.rollback(); this.incidentManager.loadCadenceSave(cadence); throw error; }
  }
  ...
  async importState(input: unknown): Promise<void> {
    if (this._importing || this._liveryWriting) throw new Error('Un import ou enregistrement de livrée est déjà en cours.');
    validateSaveDocument(input);
    if (typeof input.companyName !== 'string' || !input.companyName.trim()) throw new Error('Nom de compagnie manquant.');
    this._importing = true;
    const wasRunning = this.running, wasPaused = this.engine.paused;
    this.running = false; this.engine.paused = true;
    let tx: StateTransaction | null = null;
    let releaseUiLock: () => void = () => {};
    const cadence = this.incidentManager.getCadenceSave();
    try {
      ...
      if (written !== true) throw new Error('Import non enregistré : quota ou stockage indisponible. Partie précédente conservée.');
      this.account.companyName = input.companyName;
      tx.release(); tx = null; this.ui?.resetLiveryEditor?.();
    } catch (error) {
      tx?.rollback(); this.incidentManager.loadCadenceSave(cadence);
      throw error;
    } finally {
      releaseUiLock();
  ...
  private _loadStateUnchecked(s: SaveDocument) {
    ...
    if (s.scheduleV2 && !this.scheduleV2.loadFromSave(s.scheduleV2)) throw new Error('Schedule V2 : schéma incompatible ou sauvegarde invalide.');
    if (s.rotationsV2 && !this.rotationV2.loadFromSave(s.rotationsV2)) throw new Error('Roulements V2 : schéma incompatible ou sauvegarde invalide.');
    this.refreshLiveryImages(true, false);
    if (this.scheduleV2Runtime?.loadFromSave?.(s.v2Runtime || {schemaVersion:1,services:[]}) === false) throw new Error('Circulations V2 : schéma incompatible ou sauvegarde invalide.');
    ...
    this.scheduleCreator.services = this.scheduleCreator.services.filter((x: { _v2OccurrenceId: unknown }) => !x._v2OccurrenceId);
    this.scheduleCreator._invalidateActiveCache?.();
    if (this.scheduleV2Runtime) { this.scheduleV2Runtime.alerts=[]; this.scheduleV2Runtime._alertKeys?.clear?.(); this.scheduleV2Runtime._alertTimes?.clear?.(); this.scheduleV2Runtime._timedVersionCache?.clear?.(); this.scheduleV2Runtime._lastSyncKey=''; }
    ...
    { const queued=new Set([...(this.depotManager.maintenanceQueue||[]), ...(this.depotManager.repairQueue||[])].map(m =>String(m.rameId??'')).filter(Boolean));
      const depotOps=new Map<string, { rameId?: unknown; id?: unknown }>((this.depotManager.depotOperations||[]).map((o: { rameId?: unknown; id?: unknown }) =>[String(o.rameId??''),o]));
      for(const rame of this.rameManager.getAll?.()||[]) {
        rame.inMaintenance=queued.has(String(rame.id??''));
        const op=depotOps.get(String(rame.id??''));rame.depotOperationId=op?.id||'';
        // Track occupancy is authoritative for physical depot presence after reload.
        const d=(this.depotManager.getDepots?.()||[]).find((x: __KPA60) =>x.findRameTrack?.(rame.id));
        if(d) rame.currentLocation={...(rame.currentLocation||{}),depotId:d.id,stationId:'',serviceId:'',lat:d.location?.lat??rame.currentLocation?.lat??null,lon:d.location?.lon??rame.currentLocation?.lon??null};
    ...
    if (s.lines) this.lineManager.loadFromSave(s.lines, this.world);
    ...
    if (Array.isArray(s.sillons)) this.sillonManager.loadFromSave(s.sillons, this.world);
    else this.sillonManager.loadFromSave([], this.world);
    if (s.voiePoints) this.voiePointManager.loadFromSave(s.voiePoints, this.world);
    ...
    if (s.junctions) this.junctionManager.loadFromSave(s.junctions, this.world, this.rameManager);
    if (s.cargoTypes) this.cargoTypes.loadFromSave(s.cargoTypes);
    if (s.iteModules) this.iteModules.loadFromSave(s.iteModules);
    this.iteModules.pruneInstallations?.(this.depotManager.getITEs().map((d: { id: unknown }) => d.id));
    if (s.industrialClients) this.industrialClients.loadFromSave(s.industrialClients);
    if ((s as Record<string, unknown>).marketing) this.marketingManager?.loadFromSave?.((s as Record<string, unknown>).marketing);
    this.industrialClients.pruneClients?.(this.depotManager.getITEs());
    this.freightManager.pruneIndustrialClientContracts?.(this.industrialClients.clients?.map((c: { id: unknown }) => c.id) || []);
    ...
    for (const trc of this.voiePointManager.getAllTroncons()) { trc.occupiedBy = null; trc.reservedBy = null; }
  ...
  liveryTargets(includeRuntime = true): LiveryTarget[] {
    const targets = new Set<LiveryTarget>();
    for (const rame of this.rameManager.getAll()) for (const e of rame.elementDetails) targets.add(e);
    for (const v of this.rotationV2.vehicles) targets.add(v);
    if (includeRuntime) for (const svc of this.scheduleCreator.services) for (const e of svc.rame?.elementDetails || []) targets.add(e);
    return [...targets];
  }
  refreshLiveryImages(strict = false, includeRuntime = true): void {
    for (const target of this.liveryTargets(false)) this.liveries.apply(target, undefined, strict);
    if(includeRuntime){
      const physical=new Map<string,LiveryTarget>();
      for(const v of this.rotationV2.vehicles as Array<LiveryTarget & {id:string}>)physical.set(v.id,v);
      for(const svc of this.scheduleCreator.services){
        const parent=this.rameManager.getById(svc.rame?.id||svc.rameId||'');
        const sourceElements=new Map<string,LiveryTarget>();
        for(const element of (parent?.elementDetails||[]) as LiveryTarget[])if(element.elementId)sourceElements.set(element.elementId,element);
        for(const target of (svc.rame?.elementDetails||[]) as LiveryTarget[]){
          ...
          if(source)copyLiveryAppearance(target,source);
  ...
  async commitLiveryChange(change: () => void): Promise<void> {
    if (this._liveryWriting || this._importing) throw new Error('Un enregistrement ou import est déjà en cours.');
    this._liveryWriting = true;
    const release = acquireImportUiLock();
    let backup: ReturnType<LiveryLibrary['toSave']> | null = null;
    const saved: Array<{target:LiveryTarget; fields:PropertyDescriptorMap}> = [];
    try {
      while (this._saveWritePromise) await this._saveWritePromise;
      ...
      for (const target of this.liveryTargets()) {
        ...
        for (const key of ['liveryId','imageData','originalImageData']) {
          const descriptor=Object.getOwnPropertyDescriptor(target,key); if(descriptor)fields[key]=descriptor;
        }
        saved.push({target,fields});
      }
      change(); this.refreshLiveryImages(true);
      this._saveStateNow();
      const pending = this._saveWritePromise;
      if (!pending || await pending !== true) throw new Error('Livrée non enregistrée : quota ou stockage indisponible. La version précédente est conservée.');
    } catch(error) {
      if(backup)this.liveries.loadFromSave(backup);
      for(const {target,fields} of saved){for(const key of ['liveryId','imageData','originalImageData'])Reflect.deleteProperty(target,key);Object.defineProperties(target,fields);}
      this.refreshLiveryImages(false);
      this.renderer?._trainImageCache?.clear();
      throw error;
    } finally { this._liveryWriting=false;release(); }
  }

  _saveStateNow(options: SaveOptions | null = null) {
    if (this._importing) return null;
    ...
    for (const svc of this.scheduleCreator.getActiveServices()) {
      if (svc.rame && svc.train) syncMaterialMileage(svc.rame, svc.train);
    }
    this.gameplayClock.freightGenerationMinute = this.freightManager.lastGenTime;
    const state = {
      gameplayClock: this.gameplayClock.toSave(),
    ...
    try {
      ...
      writePromise = Promise.resolve(this.storage.saveGame(state,{lowMemory}))
        .catch((e: unknown) => { console.warn('Auto-save failed:', e); return false; })
        .finally(() => {
          if (this._saveWritePromise === writePromise) this._saveWritePromise = null;
          if (this._saveAfterWrite) {
            ...
            this._saveAfterWrite = false;this._saveAfterWriteLowMemory=false;this._saveAfterWriteRoutePointCount=0;
  ...
  saveState(options: SaveOptions | true | null = null) {
    if (this._importing) return null;
    const force = options === true || (typeof options === 'object' && options?.force === true);
    const opts: SaveOptions = typeof options === 'object' && options ? options : {};
    const lowMemory = opts.lowMemory === true;
    const routePointCount = Math.max(0, Number(opts.routePointCount || 0));
    if (force) return this._saveStateNow({lowMemory,routePointCount});
    ...
    if (this._saveIdleHandle) return this._lastSaveState || null;
    const run = () => {
      ...
      this._saveLowMemoryPending=false;this._saveRoutePointCountPending=0;
      try { this._lastSaveState = this._saveStateNow(opts); } catch (e) { console.warn('Deferred save failed:', e); }
    };
  ...
  moveTick(dt: number, timeOfDay: unknown) {
    ...
    const activeServices = [...this.scheduleCreator.getActiveServices().filter((s: ActiveService)=>!this.depotManager.ownsServiceMovement?.(s.id)), ...(this.depotManager.getPhysicalRescueServices?.() || [])];
    const movingSvcs = this.scheduleCreator.getMovingServices().filter((svc: ActiveService) => !this.depotManager.ownsServiceMovement?.(svc.id));
    ...
    if (movingCount === 0) {
      if (tm && tm.zoomLevel >= 10 && lowPhase === 0) this._streamNativeOSMViewport(false);
      for (let i = 0; i < activeServices.length; i++) {
        const svc = activeServices[i];
        if (svc.state === 'stopped_at_station') svc.moveUpdate(dt, timeOfDay, activeServices);
      }
      ...
      if (idleNow - this._lastCantonCleanup > 30000) { this._lastCantonCleanup = idleNow; this.cantonManager.cleanup(); }
      return;
    }
    ...
    if (hasViewport && lowPhase === 0) this._streamNativeOSMViewport(false);
    ...
    for (const svc of activeServices) {
      ...
      if (!svc.position || ['completed','cancelled'].includes(svc.state)) continue;
    ...
    this._safetyFleetFrame = {
      ...
      members: new Map(activeServices.map((svc: ActiveService, index: number) => [svc, index])),
    };
    // Clear route-local neighbours even when a train is demoted to macro LOD.
    for (const svc of activeServices) svc._nearbyServices = undefined;
    ...
    for (const [key, group] of routeIndex) {
      if (!group.some((s: { _lod: unknown }) => s._lod === 'high')) continue;
      let sortGroup = group;
      if (sortGroup.length > MAX_GROUP_SORT) {
        sortGroup = sortGroup.filter((s: { _lod: unknown }) => s._lod === 'high');
      }
      ...
      sortGroup.sort((a: __KPA61, b: __KPA62) => {
    ...
    for (let i = 0; i < highCandidates.length; i++) {
      ...
      if (svc._macroElapsed) { svc._macroElapsed.medium = 0; svc._macroElapsed.low = 0; }
    ...
    const runMacroCadence = (list: ActiveService[], intervalTicks: number, bucket: 'medium' | 'low') => {
      const intervalSec = dt * intervalTicks;
      for (let i = 0; i < list.length; i++) {
        const svc = list[i];
        if (!svc) continue;
        if (!svc._macroElapsed) svc._macroElapsed = { medium: 0, low: 0 };
        const elapsed = svc._macroElapsed.medium + svc._macroElapsed.low + dt;
        svc._macroElapsed.medium = 0; svc._macroElapsed.low = 0;
        svc._macroElapsed[bucket] = elapsed;
        if (elapsed + 1e-9 < intervalSec) continue;
  ...
  _forceV2RuntimeSyncNow() {
    ...
    if(!date||!this.scheduleV2Runtime)return null;
  ...
  _updateV2RuntimeStatus() {
    ...
    if (!fault && catchingUp === this._replayStatusWasCatchingUp && paintingNow - this._lastReplayStatusPaint < 200) return;
    this._lastReplayStatusPaint = paintingNow; this._replayStatusWasCatchingUp = catchingUp;
    if (fault || catchingUp) {
      el.style.display='block'; el.style.color=fault ? '#fecaca' : '#fde68a';
    ...
    el.style.display='none'; el.textContent=''; el.title='';
  }

  secondTick(timeOfDay: unknown, dateStr: unknown, pt: unknown) {
    const tick: ParisTick | null = pt && typeof pt === 'object' ? pt as ParisTick : null;
    // Minute 00 is already handled by tick(); avoid duplicate heavy legacy work.
    if (!dateStr || Number(tick?.seconds || 0) === 0) return;
    this.timeOfDay = timeOfDay;
    this._gameTime = timeOfDay;
    this._currentDate = dateStr;
    try { this.seasonal.checkSeason(dateStr); } catch (e) { this.diagnostics.record('SEASONAL', e); }
    this.scheduleV2Runtime?.sync(timeOfDay, dateStr);
    this._updateV2RuntimeStatus();
    const services = this.scheduleCreator.getActiveServices();
    for (const svc of services) {
      if (!svc._v2OccurrenceId) continue;
  ...
  tick(timeOfDay: number, dateStr: string, pt: unknown) {
    ...
    try { this.staffManager.tickWorkforce(this, activeSchedules, timeOfDay, dateStr); } catch (e) { this.diagnostics.record('STAFF_WORKFORCE', e); }
    ...
    try { this.staffManager.tickRegulateurs(this.scheduleCreator.getActiveServices(), timeOfDay, dateStr, this.realismSettings); } catch (e) { this.diagnostics.record('STAFF_REGULATION', e); }
    ...
    const incidentVisualKey = () => (this.incidentManager.activeIncidents || [])
      .filter((inc: { active: unknown; serviceId: unknown }) => inc?.active !== false && !inc?.serviceId)
    ...
    try { this.staffManager.tickConductors(activeSchedules, timeOfDay, dateStr, this); } catch (e) { this.diagnostics.record('STAFF_CONDUCTORS', e); }
    ...
    for (const { serviceId, repairType, rameId, vehicleIds } of repaired) {
      ...
      if (storedRame) { storedRame.inMaintenance = false; storedRame.pendingDefects = (storedRame.pendingDefects || []).filter((type: string) => type !== repairType); }
      for (const id of vehicleIds || []) { const vehicle = this.rotationV2?.getVehicle?.(id); if (vehicle) vehicle.available = true; }
      const svc = this.scheduleCreator.services.find((s: { id: unknown }) => s.id === serviceId);
      if (svc) {
    ...
    try { this.staffManager.tickControleurs(this.economy, activeSchedules, timeOfDay); } catch (e) { this.diagnostics.record('STAFF_INSPECTIONS', e); }

    // Revenue collected inside service.completeService -> economy.processServiceRevenue

    for (const settlementDate of dailyDates) {
      try {
        const settle = (key: string, work: () => void) => this.gameplayClock.runDailyTask(key, settlementDate, work);
        ...
        settle('ite', () => {
          ...
          if (cost > 0) this.economy.addExpense(cost, 'maintenance', 'Maintenance ITE');
        });
    ...
    } catch (e) { this.diagnostics.record('WEATHER', e); }

    // Shunting operations update every minute
    try { this.shuntingManager.update(elapsedMinutes); } catch (e) { this.diagnostics.record('SHUNTING', e); }
    ...
    } catch (e) { this.diagnostics.record('WEATHER_TILES', e); }

    // Dashboard + Graph hooks (every minute, wrapped in try/catch for safety)
    try { this.dashboard.record(this); } catch (e) { this.diagnostics.record('DASHBOARD', e); }
    try { this.graphMarche.record(this, timeOfDay); } catch (e) { this.diagnostics.record('TIMETABLE_GRAPH', e); }
  ...
  gameLoop() {
    ...
    try {
      ...
      if (catchingUp && !this._importing && !this.engine.paused && !document.hidden) {
        if (!this._replayPump) this._replayPump = new ReplayPump(() => {
          if (!this.running || this._importing || this.engine.paused || document.hidden || this.engine.getReplayDebtSeconds() <= 2) return false;
          this.engine.update();
          this._updateV2RuntimeStatus();
          return this.engine.getReplayDebtSeconds() > 2;
        }, error => this.diagnostics.record('REPLAY_PUMP', error));
        this._replayPump.request();
      } else this._replayPump?.cancel();
      if (this.engine.chronologicalClock?.failed) this._updateV2RuntimeStatus();
      ...
      if (frameDelta >= (catchingUp ? 200 : 30)) {
        ...
        if (mapActive && this.renderer) {
          ...
          const inView = (s: PositionedService) => !!s?.position && (!bounds || (s.position.lat >= bounds.minLat && s.position.lat <= bounds.maxLat && s.position.lon >= bounds.minLon && s.position.lon <= bounds.maxLon));
          ...
          if (threeDFollowActive && followed) this.ui?.applyCameraFollow?.();
          ...
          if (threeDFollowActive && (!this._last3DMarkerSync || now - this._last3DMarkerSync >= 100)) {
            ...
            if (document.getElementById('toggle-trains')?.checked !== false) this.renderer.sync3DMarkers?.(activeServices);
          }
          ...
          if (shouldRender) {
            ...
            for (let i = 0; i < movingSvcs.length; i++) {
              const svc = movingSvcs[i];
              if (inView(svc)) allVisibleServices.push(svc);
            }
            ...
            if (!this._lastStoppedScan || now - this._lastStoppedScan > 500) {
              ...
              for (let i = 0; i < activeServices.length; i++) {
                const svc = activeServices[i];
                if ((svc.state === 'stopped_at_station' || svc.state === 'blocked_route' || (svc.state === 'waiting' && svc.position)) && svc.position) stoppedBuf.push(svc);
              }
            }
            for (let i = 0; i < stoppedBuf.length; i++) if (inView(stoppedBuf[i])) allVisibleServices.push(stoppedBuf[i]);
            for (let i = 0; i < rescueServices.length; i++) if (inView(rescueServices[i])) allVisibleServices.push(rescueServices[i]);
            ...
            if (!threeDFollowActive) this.ui?.applyCameraFollow?.();
            else if (threeDAnchorDue) this.ui?.prepare3DMapAnchorForRender?.();
            ...
            if (!hasVisibleMotion) this._lastIdleMapRender = now;
          }
        }
