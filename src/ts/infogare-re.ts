...
import { clockText, setText, type BoardRow } from './operations-view-model.js';
export interface StationBoardData { station: string; clock: string; date: string; rows: BoardRow[]; }
export class RailEmpireBoard {
    private list: WindowedList<BoardRow>;
    private horizon = 1440;
    private active = false;
    private timer: ReturnType<typeof setInterval> | null = null;
    private morePending = false;
    private request = 0;
    private stationKey = '';
    private arrivals = false;
    private search: HTMLInputElement;
    constructor(private root: HTMLElement, private data: (horizonMinutes: number, arrivals: boolean) => StationBoardData) {
        ...
        this.search = (root.querySelector('#re-board-search') as HTMLInputElement);
        this.list = new WindowedList((root.querySelector('#re-board-list') as HTMLElement), 112, r => r.key, (r, old) => this.row(r, old), () => this.more());
    ...
    show(stationId: string): void {
        ...
        if (reset) { this.horizon = 1440; this.search.value = ''; }
        this.setActive(true); this.refresh(reset);
    }
    private mode(arrivals: boolean): void {
    ...
    private more(): void {
        if (!this.active || this.morePending) return;
        this.morePending = true;
        const button = (this.root.querySelector('#re-board-more') as HTMLButtonElement); button.disabled = true;
        // One additional day per gesture; never an automatic unbounded background loop.
        this.request = window.setTimeout(() => {
            this.request = 0;
            if (this.active) { this.horizon += 1440; this.refresh(); }
            this.morePending = false; button.disabled = false;
        }, 0);
    }
    private row(row: BoardRow, old?: HTMLElement): HTMLElement {
        const node = old || document.createElement('article');
        if (!old) { node.className = 're-board-row'; node.innerHTML = `<div class="re-board-time"><strong></strong><small></small></div><div class="re-board-destination"><strong></strong><span class="re-board-train-name"></span><small class="re-board-via"></small></div><div class="re-board-status"><strong></strong><small></small></div><div class="re-board-platform"></div>`; }
        node.dataset.serviceId = row.serviceId;
        node.classList.toggle('is-cancelled', row.cancelled); node.classList.toggle('is-delayed', !row.cancelled && row.delay >= 1);
        ...
        setText(node, '.re-board-via', via); (node.querySelector('.re-board-via') as HTMLElement).title = via;
        ...
        (node.querySelector('.re-board-status') as HTMLElement).title = row.reason;
    ...
    refresh(reset = false): void {
        ...
        setText(this.root, '.re-board-station', d.station); setText(this.root, '.re-board-clock strong', d.clock); setText(this.root, '.re-board-clock span', d.date);
        ...
        (this.root.querySelector('#re-board-empty') as HTMLElement).hidden = rows.length > 0;
    ...
    setActive(active: boolean): void {
        ...
        if (!active && this.request) { clearTimeout(this.request); this.request = 0; this.morePending = false; (this.root.querySelector('#re-board-more') as HTMLButtonElement).disabled = false; }
        if (active) this.timer = setInterval(() => this.refresh(), 5000);
    }
    dispose(): void { this.setActive(false); this.list.dispose(); }
}
