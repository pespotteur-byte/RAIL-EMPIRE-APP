...
export class Economy {
    balance: number; revenue: number; expenses: number; penalties: number; history: Array<Record<string, unknown>>;
    ticketPricePerKm: number; freightPricePerTKm: number; passengerPriceByClass: CountryRateMap; dailyProcessed: Record<string, boolean>;
    totalPassengers: number; totalFreightTonnes: number; revenueByCategory: Record<string, number>; expenseByCategory: Record<string, number>;
    lineRevenue: Record<string, number>; lineExpense: Record<string, number>; dailySnapshots: Array<Record<string, unknown>>;
    _currentDayKey: string | null; _currentDayRevenue: number; _currentDayExpenses: number; _currentDayByCategory: Record<string, number>;
    totalTicketsSold: number; totalTicketRevenue: number; totalFraudFines: number; fraudRate: number; _companyLogo: string | null = null; _lastBulletinDate: string | null = null;
    ...
    addRevenue(amount:number, category:string, description: unknown) {
    ...
    addExpense(amount:number, category:string, description: unknown) {
        amount = Number(amount);
        if (!Number.isFinite(amount) || amount <= 0)
            return false;
        this.balance -= amount;
    ...
    addPenalty(amount:number, description: unknown) {
        amount = Number(amount);
        if (!Number.isFinite(amount) || amount <= 0)
            return false;
        this.balance -= amount;
    ...
    getPassengerPricePerKm(maxSpeed:number = 100) {
    ...
    processServiceRevenue(service: unknown) { }
    /**
     ...
     */
    _calcInfrastructureToll(distanceKm:number, route: Array<{ maxSpeed?: number; tracks?: number; electrified?: unknown; usage?: unknown }>, rame: Rame, country:string = 'FR') {
        ...
        for(const p of route){
            ...
            if(Number.isFinite(speed)&&speed>0){speedSum+=speed;speedCount++;maxLineSpeed=Math.max(maxLineSpeed,speed);}
            if(Number.isFinite(tracks)&&tracks>0){trackSum+=tracks;trackCount++;}
            if(p.electrified)electrifiedCount++;
            if(p.usage==='main')mainUsage=true;
        }
        if(!speedCount)maxLineSpeed=rame.maxSpeed||160;
        ...
        const countryRates: CountryRateMap = {
     ...
     */
    processStopRevenue(service: ActiveService, stationName:string, distFromPrev: number, isFirst:boolean, isTerminus:boolean, stationId:string, legRoute: Array<{ maxSpeed?: number; tracks?: number; electrified?: unknown; usage?: unknown }>) {
        ...
        const assignedContract = service.assignedContractId && g?.freightManager?.contracts
            ? g.freightManager.contracts.find((c: { id: unknown; active: unknown }) => c.id === service.assignedContractId && c.active)
            : null;
        ...
        if (!isFirst && distFromPrev > 0) {
            ...
            if (carriedContract?.active && stationId === carriedContract.toId && service._contractFreight > 0 && !service._iteCargoMismatch && freightAccess) {
                ...
                if (g?.freightManager?.fulfillAtStation) {
                    ...
                    contractRevenue = (res.fulfilled || []).reduce((sum: number, f: { payment: unknown }) => sum + (Number(f.payment) || 0), 0);
                }
                ...
                if (service._contractFreight === 0) service._contractCargoId = '';
            ...
            if (paxDescend > 0) {
                const satisfaction = passengerSatisfactionScore(service.rame, service.train?.delay,
                    g?.stationUpgrades?.getSatisfactionBonus?.(stationId));
    ...
    processDailyCharges(services: ActiveService[], depots: Array<{ getMaintenanceCost?: () => number; tracks: number; name: string }> | null | undefined, dateKey:string) {
        ...
        while (keys.length > 7) {
            const k = keys.shift(); if (k !== undefined) delete this.dailyProcessed[k];
        }
    }
    // Get effective fraud rate (reduced by contrôleur presence)
    getEffectiveFraudRate(hasControleur:boolean) {
    ...
    addLineRevenue(lineId:string, amount:number) {
        if (!lineId || amount <= 0)
            return;
        this.lineRevenue[lineId] = (this.lineRevenue[lineId] || 0) + amount;
    }
    addLineExpense(lineId:string, amount:number) {
        if (!lineId || amount <= 0)
            return;
    ...
    loadFromSave(s: unknown) {
        ...
        const num = (v: unknown, fallback = 0, min = -Infinity, max = Infinity) => {
            const n = Number(v);
        ...
        if (data.passengerPriceByClass && typeof data.passengerPriceByClass === 'object' && !Array.isArray(data.passengerPriceByClass)) {
            const savedPrices = data.passengerPriceByClass as Record<string, unknown>;
        ...
        this.history = (Array.isArray(data.history) ? data.history : []).filter((h): h is Record<string, unknown> => !!h && typeof h === 'object' && !Array.isArray(h) && Number.isFinite(Number((h as Record<string, unknown>).amount))).map((h) => ({ ...h, amount: Number(h.amount) }));
        ...
        const numMap = (raw: unknown) => Object.fromEntries(Object.entries(raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {}).map(([k, v]) => [k, num(v, 0)]));
        ...
        this.dailySnapshots = (Array.isArray(data.dailySnapshots) ? data.dailySnapshots : []).filter((x): x is Record<string, unknown> => !!x && typeof x === 'object' && !Array.isArray(x)).map((x) => ({
            ...x,
...
// S3_STRUCT_V2_TEMP
type __S3Struct37 = { "maxSpeed": number };
type __S3Struct38 = { "maxSpeed": number };
type __S3Struct39 = { "tracks": number };
type __S3Struct40 = { "tracks": number };
