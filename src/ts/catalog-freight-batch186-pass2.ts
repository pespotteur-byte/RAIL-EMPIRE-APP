...
export const BATCH186_FREIGHT_PASS2_CARGO_TYPES: CargoDefinition[] = [
  {
    "type": "sugar-bulk",
    "name": "Sucre en vrac",
...
export const BATCH186_FREIGHT_PASS2_WAGON_PATCH: Record<string, FreightPatch> = {
  "cat-100": {
    "cargoTypes": [
      "automotive-parts"
    ],
...
export const BATCH186_FREIGHT_PASS2_INDUSTRY_ADDITIONS: Record<string, string[]> = {
  "grain_terminal": [
    "grain",
    "potatoes-bulk"
  ],
...
export function applyBatch186FreightPass2ToCatalog(catalog: unknown[]): unknown[] {
...
export function applyBatch186FreightPass2IndustryPatch(industrialClients: unknown) {
  const host = industryPatchHost(industrialClients);
  const types=host?.getIndustryTypes?.(); if (!Array.isArray(types)) return {updated:0}; let updated=0;
  for (const [type,cargos] of Object.entries(BATCH186_FREIGHT_PASS2_INDUSTRY_ADDITIONS)) {
    const ind=types.find(x=>x.type===type); if (!ind) continue; if(!Array.isArray(ind.cargoTypes)) ind.cargoTypes=[]; let changed=false;
    for(const cargo of cargos){ if(!ind.cargoTypes.includes(cargo)){ind.cargoTypes.push(cargo);changed=true;} } if(changed) updated++;
  }
  return {updated};
}
