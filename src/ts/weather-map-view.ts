...
import type { RailWeatherEvaluation, WeatherThreshold } from './weather-thresholds.js';
...
type WeatherPoint = {
    [key: string]: unknown;
    lat?: number; lon?: number; type?: string; label?: string; temperature?: number; apparentTemp?: number;
    humidity?: number; windSpeed?: number; windGust?: number; windDirection?: number; precipitation?: number;
    rain6hMm?: number; rain24hMm?: number; snowfall?: number; snowDepthCm?: number; visibility?: number; visibilityM?: number;
    pressure?: number; cloudCover?: number; weatherCode?: number; risk?: RailWeatherEvaluation;
};
type WeatherGridPoint = WeatherPoint & { lat: number; lon: number; windSpeed: number };
type DragState = { x: number; y: number; moved: boolean };
type WeatherCanvas = HTMLCanvasElement & { _weatherDpr?: number };
...
const esc = (s: unknown) => String(s ?? '').replace(/[&<>"']/g, (c: string) => HTML_ESCAPES[c] ?? c);
const clamp = (v: unknown, a: number, b: number) => Math.max(a, Math.min(b, Number(v) || 0));
function layerValue(p: WeatherPoint, layer: unknown) {
    switch (layer) {
...
function colorFor(layer: unknown, value:number) {
...
function fmtLayer(layer: unknown, v: number) {
...
export class WeatherMapView {
    ...
    constructor(weather: Weather) {
        ...
        this.map = null as unknown as TileMapType;
        this.layer = 'radar';
        this.grid = [];
    ...
    render(container: HTMLElement, game: RailEmpire | null = null) {
      ...
      <div class="weather-page-v2">
        ...
        const canvas = container.querySelector('#weather-world-map') as WeatherCanvas | null;
        if (!canvas)
            return;
        if (!this.map) {
        ...
        if (typeof rp === 'string' && rp) {
            ...
            if (typeof radarUrl === 'string' && radarUrl) this.map.setRadarTileUrl(radarUrl);
        }
    ...
    _wire(container: HTMLElement, canvas: WeatherCanvas, token: number) {
        const layerButtons = container.querySelectorAll('.weather-layer-btn');
        layerButtons.forEach((node) => { const btn = node as HTMLButtonElement; btn.addEventListener('click', () => { this.layer = btn.dataset.layer || 'radar'; layerButtons.forEach((other) => other.classList.toggle('active', other === btn)); this.map.radarEnabled = this.layer === 'radar'; this.map.markDirty(); this._scheduleGrid(container, 0); this._updateLegend(container); }); });
        let resizeTimer: number | undefined;
        const onResize = () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(() => { if (token !== this._token)
            return; this._resize(canvas); this._scheduleGrid(container, 100); }, 100); };
        window.addEventListener('resize', onResize, { once: false });
        canvas.addEventListener('wheel', (e: WheelEvent) => { e.preventDefault(); const r = canvas.getBoundingClientRect(); this.map.applyZoom(e.deltaY < 0 ? 1 : -1, e.clientX - r.left, e.clientY - r.top); this.map.markDirty(); this._scheduleGrid(container, 300); }, { passive: false });
        canvas.addEventListener('pointerdown', (e: PointerEvent) => { canvas.setPointerCapture?.(e.pointerId); this._drag = { x: e.clientX, y: e.clientY, moved: false }; });
        canvas.addEventListener('pointermove', (e: PointerEvent) => { const r = canvas.getBoundingClientRect(), w = this.map.screenToWorld(e.clientX - r.left, e.clientY - r.top, canvas.clientWidth, canvas.clientHeight); const coords = container.querySelector('#weather-map-coords'); if (coords)
            coords.textContent = `${w.lat.toFixed(3)}°, ${w.lon.toFixed(3)}°`; if (!this._drag)
            return; const dx = e.clientX - this._drag.x, dy = e.clientY - this._drag.y; if (Math.abs(dx) + Math.abs(dy) > 2)
            this._drag.moved = true; this.map.pan(dx, dy); this.map.markDirty(); this._drag.x = e.clientX; this._drag.y = e.clientY; this._scheduleGrid(container, 350); });
        canvas.addEventListener('pointerup', async (e: PointerEvent) => { const was = this._drag; this._drag = null; if (was?.moved)
            return; const r = canvas.getBoundingClientRect(); const w = this.map.screenToWorld(e.clientX - r.left, e.clientY - r.top, canvas.clientWidth, canvas.clientHeight); await this._inspectPoint(container, w.lat, w.lon, `Point ${w.lat.toFixed(3)}°, ${w.lon.toFixed(3)}°`); });
        const search = container.querySelector('#weather-map-search'), results = container.querySelector('#weather-map-search-results');
        let st: number | undefined;
        search?.addEventListener('input', () => { clearTimeout(st); const q = search.value.trim(); if (q.length < 2) {
            results.classList.add('hidden');
            return;
        } st = setTimeout(async () => { try {
            const rows = await this.weather.searchLocation(q);
            if (token !== this._token)
                return;
            results.innerHTML = rows.length ? rows.map((r: { name: unknown; admin1: unknown; country: unknown }, i:number) => `<button data-i="${htmlText(i)}"><b>${esc(r.name)}</b><span>${esc([r.admin1, r.country].filter(Boolean).join(', '))}</span></button>`).join('') : '<div class="weather-search-empty">Aucun résultat</div>';
            results.classList.remove('hidden');
            results.querySelectorAll('button').forEach((b: __S3Struct82) => b.onclick = async () => { const r = rows[Number(b.dataset.i)]; this.map.centerLat = r.lat; this.map.centerLon = r.lon; this.map.zoomLevel = Math.max(this.map.zoomLevel, 7); this.map.markDirty(); results.classList.add('hidden'); search.value = [r.name, r.country].filter(Boolean).join(', '); this._scheduleGrid(container, 50); await this._inspectPoint(container, r.lat, r.lon, [r.name, r.admin1, r.country].filter(Boolean).join(', ')); });
        }
        catch (e) {
    ...
    _resize(canvas: WeatherCanvas) { const r = canvas.getBoundingClientRect(); const dpr = Math.min(2, window.devicePixelRatio || 1); const w = Math.max(300, Math.round(r.width)), h = Math.max(320, Math.round(r.height)); canvas.width = Math.round(w * dpr); canvas.height = Math.round(h * dpr); canvas._weatherDpr = dpr; this.map.viewportWidth = w; this.map.viewportHeight = h; this.map.markDirty(); }
    _drawLoop(container: HTMLElement, canvas: WeatherCanvas, token: number) {
    ...
    _drawTrains(ctx: CanvasRenderingContext2D, w: number, h: number) { const services = this._game?.scheduleCreator?.services || []; ctx.save(); for (const s of services) {
        const t = s?.train;
    ...
    _bounds(canvas: WeatherCanvas) { const w = canvas.clientWidth || 800, h = canvas.clientHeight || 500; const nw = this.map.screenToWorld(0, 0, w, h), se = this.map.screenToWorld(w, h, w, h); return { minLat: Math.min(nw.lat, se.lat), maxLat: Math.max(nw.lat, se.lat), minLon: Math.min(nw.lon, se.lon), maxLon: Math.max(nw.lon, se.lon) }; }
    _scheduleGrid(container: HTMLElement, delay:number = 250) { if (this._gridTimer !== null) clearTimeout(this._gridTimer); this._gridTimer = setTimeout(() => this._loadGrid(container), delay); }
    async _loadGrid(container: HTMLElement) { const canvas = container.querySelector('#weather-world-map') as WeatherCanvas | null; if (!canvas || this.layer === 'radar') {
        this.grid = [];
        return;
    } const load = container.querySelector('#weather-map-loading'); load?.classList.remove('hidden'); this.gridLoading = true; try {
        this.grid = await this.weather.fetchGrid(this._bounds(canvas), this.layer, 7, 5) as WeatherGridPoint[];
    }
    catch (e) {
    ...
    async _inspectPoint(container: HTMLElement, lat:number, lon:number, title:string) { const panel = container.querySelector('#weather-point-panel'), timeline = container.querySelector('#weather-forecast-timeline'); if (panel)
        panel.innerHTML = '<div class="weather-panel-loading">Lecture météo du point…</div>'; try {
        ...
        const state = stateRaw as WeatherPoint;
        const forecast = forecastRaw as ForecastRow[];
    ...
    _pointHtml(s: WeatherPoint, lat:number, lon:number, title:string) {
    ...
    _forecastHtml(rows: __S3Struct90) { if (!rows?.length)
        return '<span>Prévisions indisponibles</span>'; const now = Date.now(); const future = rows.filter((r: __S3Struct91) => new Date(r.time).getTime() >= now - 3600000); const picks = [0, 1, 3, 6, 12, 24].map((h) => future[Math.min(future.length - 1, h)]).filter(Boolean); return picks.map((r: __S3Struct92, i) => { const h = [0, 1, 3, 6, 12, 24][i]; return `<div class="weather-time-chip"><span>${h === 0 ? 'Maintenant' : `+${h}h`}</span><b>${htmlText(Math.round(Number(r.temperature_2m) || 0))}°</b><small>${htmlText((Number(r.precipitation) || 0).toFixed(1))} mm · ${htmlText(Math.round(Number(r.wind_gusts_10m) || 0))} km/h</small></div>`; }).join(''); }
    _thresholdHtml() { const cats = this.weather.getThresholdCatalog() as Record<string, WeatherThreshold>; return `<div class="weather-panel-title"><div><span>MOTEUR FERROVIAIRE</span><h3>Seuils d'impact</h3></div></div><p class="weather-panel-copy">Ces seuils sont utilisés par la page et par les trains : l'interface n'affiche plus une règle différente de la physique.</p><div class="weather-threshold-list">${Object.values(cats).map((c) => `<div><b>${esc(c.label)}</b><span>${htmlText(c.bands.map((v: number, i:number) => `${['Vigilance', 'Dégradé', 'Sévère', 'Extrême'][i]} ${c.direction === 'low' ? '≤' : '≥'} ${v} ${c.unit}`).join(' · '))}</span></div>`).join('')}</div>`; }
    _updateLegend(container: HTMLElement) { const el = container.querySelector('#weather-map-legend'); if (!el)
        return; if (this.layer === 'radar') {
...
// S3_STRUCT_V2_TEMP
type __S3Struct76 = { "temperature": unknown; "precipitation": unknown; "windSpeed": number; "windGust": unknown; "snowfall": unknown; "visibilityM": unknown; "humidity": unknown; "pressure": unknown; "cloudCover": unknown; "risk": { "rank": unknown } };
type __S3Struct82 = { "onclick": unknown; "dataset": { "i": unknown } };
type ForecastRow = { time: string | number | Date; temperature_2m?: unknown; precipitation?: unknown; wind_gusts_10m?: unknown };
type __S3Struct90 = ForecastRow[];
type __S3Struct91 = ForecastRow;
type __S3Struct92 = ForecastRow;
