  ...
  for (let i = 0; i < count; i++) {
    if (i > 0 && transitionUp[i]) {
      ...
      while (child > 0) {
        ...
        if (caps[parent] <= cap) break;
        caps[child] = caps[parent]; until[child] = until[parent]; child = parent;
      }
      caps[child] = cap; until[child] = end;
    }
    while (size > 0 && position >= until[0]) {
      ...
      for (;;) {
        ...
        if (child >= size) break;
        if (child + 1 < size && caps[child + 1] < caps[child]) child++;
        if (cap <= caps[child]) break;
        caps[parent] = caps[child]; until[parent] = until[child]; parent = child;
      }
      caps[parent] = cap; until[parent] = end;
    }
    if (size > 0) result[i] = caps[0];
    position += distance[i];
  }
  return result;
}
