...
import { canonicalTrackRef, normalizeStationTrackIdentity, stationTrackIdentityFromBinding,
  resolveNativeStationTrack, stationTrackResourceKey, type NativeStationTrack, type StationTrackIdentity } from './station-track-identity.js';
...
const label = (value: unknown): string => (typeof value === 'string' || typeof value === 'number') ? String(value).trim().slice(0, 256) : '';

/** Detached copies only; saved locations must not share mutable editor bindings. */
export function materialTrackLocation(value: unknown): MaterialTrackLocation {
  ...
  if (!identity && !voiePointId && !platform) return {};
...
function key(loc: MaterialTrackLocation, stationId: string, nativeTracks: readonly NativeStationTrack[]): string {
...
export function materialTrackMismatch(current: unknown, expected: unknown, stationId: string,
  nativeTracks: readonly NativeStationTrack[] = []): boolean {
  const from = materialTrackLocation(current), to = materialTrackLocation(expected);
  for (const loc of [from, to]) {
    if (loc.trackIdentity?.kind !== 'native') continue;
    const native = nativeTracks.find(t => t.id === loc.trackIdentity!.id);
    if (native && String(native.stationId ?? '') !== stationId) return true;
  }
  ...
  if (a?.kind === 'osm' && b?.kind === 'osm') {
    ...
    if (ref && ref === canonicalTrackRef(b.trackRef) && locals.length <= 1) return false;
  }
  const source = key(from, stationId, nativeTracks), target = key(to, stationId, nativeTracks);
  if (!source) return false;
  if (!target) return !!from.trackIdentity;
