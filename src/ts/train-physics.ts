...
// Source TypeScript autoritative; js/train-physics.js is generated by `npm run build:ts`.
...
export interface RoutePoint {
  ...
  incline?: unknown;
  tags?: { incline?: unknown; [key: string]: unknown };
...
const WEATHER_ADHESION: Record<string, number> = {
  clear: 0.33, cloudy: 0.33, wind: 0.33,
  light_rain: 0.28, rain: 0.25, heavy_rain: 0.22,
  storm: 0.20, snow: 0.15,
};

export function weatherAdhesion(weather: WeatherCondition): number {
...
export function tractiveEffortN(powerW: number, vMs: number, adhesionMassKg: number, weather: WeatherCondition): number {
...
export function accelerationMs2(params: TrainPhysicsParams, vMs: number, gradePermille = 0): number {
...
export function brakingDecelMs2(params: TrainPhysicsParams, weather: WeatherCondition, gradePermille = 0): number {
...
function routeInclinePermille(a: RoutePoint | undefined, b: RoutePoint | undefined): number {
  const raw = b?.tags?.incline ?? a?.tags?.incline ?? b?.incline ?? a?.incline;
  if (raw == null || raw === '') return 0;
  const lower = String(raw).trim().toLowerCase();
  if (lower === 'up' || lower === 'down') return 0; // no numeric magnitude available
  const txt = lower.replace(',', '.');
  let value = Number.parseFloat(txt);
  if (!Number.isFinite(value)) return 0;
  if (!txt.includes('‰')) value *= 10; // OSM plain / percent incline -> permille
...
export function segmentsFromRoute(route: RoutePoint[], rameMaxSpeedKmh: number, haversineKm: HaversineKm): PhysicsSegment[] {
  const segs: PhysicsSegment[] = [];
...
export function simulateProfile(segments: PhysicsSegment[], params: SimulationParams = {} as SimulationParams): SimulationResult {
  ...
  let N = 0;
  for (const seg of segments) N += Math.max(1, Math.ceil(seg.distM / dsStep));
  if(!Number.isSafeInteger(N)||N<1)throw new RangeError('Profil physique : taille numérique non représentable');
  ...
  if(streamed)return simulateProfileStream(segments,params,{cellCount:N,capacity,dsStep,preBrakeMarginM,brakeBuildSec,brake:brakingDecelMs2,accel:accelerationMs2,power:profileSegmentPower});
  ...
  for (let i = 0; i < segments.length; i++) segmentPower[i] = profileSegmentPower(segments[i],params);
  ...
  const localParams: TrainPhysicsParams = { ...params, powerW: 0 };
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (a < 0 && nextSquared <= 0 && v > 0 && accelerationMs2(localParams, 0, grade) > 0) {
      ...
      for (let k = 0; k < 24; k++) {
        const mid = (lo + hi) / 2;
        if (accelerationMs2(localParams, mid, grade) > 0) lo = mid; else hi = mid;
      }
      nextSquared = lo * lo;
    }
    ...
    timeSec += dtCell;
    if(segmentTimeSec)segmentTimeSec[sourceIndexes[segIndex]] += dtCell;
...
function profileSegmentPower(cell:PhysicsSegment,params:SimulationParams):number {
    const ep=Math.max(0,Number(params.electricPowerW||0)),dp=Math.max(0,Number(params.dieselPowerW||0));
    if(!ep&&!dp)return Number(params.powerW||0);
    ...
    if(cell?.electrified!==false&&ep>0){
      ...
      if(!systems.length||!volts.length)return Math.min(Number(params.powerW)||ep+dp,ep+dp);
      for(const sys of systems)for(const vv of volts)for(const ff of freqs){const vok=!Number(sys?.voltage)||Math.abs(Number(sys.voltage)-Number(vv))<=Math.max(50,Number(vv)*0.03);const fok=!Number(sys?.frequency)||!Number(ff)||Math.abs(Number(sys.frequency)-Number(ff))<=1;if(vok&&fok)return Math.min(Number(params.powerW)||ep+dp,ep+dp);}
    }
    if(dp>0)return dp;
    return 0;
  }
