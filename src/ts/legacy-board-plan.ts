...
export interface ForecastStop { stationId: unknown; type?: unknown; departureTime?: number | null; arrivalTime?: number | null; platform?: unknown; stopCode?: unknown; }
export interface ForecastService {
 id: string; stops: readonly ForecastStop[]; returnStops?: readonly ForecastStop[]; roundTrip?: boolean; multiDepartures?: number; terminusWait?: number;
 runDays?: readonly number[]; runDates?: readonly string[]; operatingDay?: string; completedDay?: string; completed?: boolean;
 isReturnLeg?: boolean; tripCount?: number; currentStops?: readonly ForecastStop[];
}
export interface ForecastRun { key: string; day: string; isReturn: boolean; live: boolean; stops: ForecastStop[]; }
const DAY = 1440;
const finite = (n: unknown): n is number => typeof n === 'number' && Number.isFinite(n);
const forward = (a: number, b: number) => { const d = b - a; return d >= 0 ? d : ((d % DAY) + DAY) % DAY; };
/** Resolve a leg chronologically. A midnight crossing belongs to its departure's civil date. */
function unroll(stops: readonly ForecastStop[], start?: number): ForecastStop[] {
 ...
 return stops.map((s, i) => {
  ...
  if (i > 0) now += forward(raw!, arr);
  const arrival = now;
  const dep = finite(s.departureTime) ? s.departureTime : arr;
  if (i > 0) now += forward(arr, dep);
  raw = dep;
...
export function legacyBoardRuns(service: ForecastService, date: string, now: number, horizon: number): ForecastRun[] {
 const today = civilDayIndex(date); if (today == null || !finite(now) || !finite(horizon) || horizon < 0) return [];
 const outbound = unroll(service.stops); if (outbound.length < 2) return [];
 ...
 for (let offset = -Math.min(366, Math.ceil(span / DAY) + 1); offset <= Math.floor((now + horizon) / DAY); offset++) {
  ...
  if (!pinned && (!days.includes(((today + offset + 4) % 7 + 7) % 7) || dates.length && !dates.includes(day))) continue;
  if (service.completedDay && day <= service.completedDay || service.completed && day === service.operatingDay) continue;
  for (let turn = 0; turn < count; turn++) {
   const shift = offset * DAY + cycle * turn;
   if (origin + shift > now + horizon) break;
   const add = (leg: ForecastStop[], isReturn: boolean) => {
    ...
    if (stops.at(-1)!.arrivalTime! < now - 3 && !live) return;
    out.push({key:`${service.id}:${day}:${turn}:${isReturn?'R':'A'}`,day,isReturn,live,stops});
   };
   add(outbound,false); add(ret,true);
  }
 }
 return out;
}
