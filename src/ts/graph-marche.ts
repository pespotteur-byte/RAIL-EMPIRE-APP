...
type __KPM56 = { "name"?: unknown; "number"?: unknown; "train"?: Record<string, unknown> };
type __KPA21 = { "stationId"?: unknown; "name"?: unknown; "arrivalSec"?: unknown; "computedArrivalSec"?: unknown; "departureSec"?: unknown; "computedDepartureSec"?: unknown; "track"?: Record<string, unknown>; "stopCode"?: unknown };
type __KPA22 = { "id": unknown; "name": string; "country": string };
type __KPA24 = { "stationId": unknown; "name"?: unknown };
type __KPA25 = { "serviceId": unknown; "time": unknown; "lat": unknown; "lon": unknown };
 ...
 */
type GraphStationStop = { stationId: string; name?: string; [key: string]: unknown };
type StationSearchRow = { id: string; name: string; norm: string; country: string };
type GraphPad = { top: number; right?: number; bottom?: number; left?: number };
type TimeRange = { start: number; end: number };
type LineSegment = { x1: number; y1: number; x2: number; y2: number };
type RouteDistancePoint = { lat: number; lon: number; dist: number };
type LegendItem = { name: string; color: string };
type GraphWorldStation = { id?: unknown; name?: string; country?: string; lat: number; lon: number; [key: string]: unknown };
type GraphGame = { _currentDate?: string; engine?: { currentDate?: string; getParisDate?: () => string }; scheduleCreator: { getActiveServices: () => ActiveService[]; services?: ActiveService[] }; scheduleV2?: ScheduleV2Manager; world: { stations?: GraphWorldStation[]; getStationById: (id: unknown) => GraphWorldStation | null | undefined }; saveState?: (...args: unknown[]) => unknown; };
type GraphLiveRecord = { serviceId: string; time: number; date?: string; lat?: number | null; lon?: number | null; [key: string]: unknown };
type GraphSave = { records?: unknown; colorMap?: unknown; colorIdx?: unknown; stationAId?: unknown; stationBId?: unknown; mode?: unknown };
type GraphRoutePoint = { lat: number; lon: number; [key: string]: unknown };
type GraphStopLike = GraphStationStop & { arrivalTime?: unknown; departureTime?: unknown; type?: unknown; platform?: unknown; stopCode?: unknown };
type GraphServiceLike = {
  id: string; name: string; number?: string; active?: unknown; train?: Record<string, unknown> | null;
  position?: { lat?: unknown; lon?: unknown } | null; currentStopIndex?: number; state?: unknown;
  stops?: GraphStopLike[]; routes?: GraphRoutePoint[][]; _returnRoutes?: GraphRoutePoint[][]; isReturnLeg?: unknown;
  serviceType?: unknown; isWorkTrain?: unknown; _v2OccurrenceId?: unknown;
  getCurrentStops?: () => GraphStopLike[]; [key: string]: unknown;
};


export class GraphMarche {
  declare records: GraphLiveRecord[];
  declare maxRecords: number;
  declare _lastRecordTime: number;
  declare _lastRecordKey: string;
  declare _colorMap: Record<string, string>;
  declare _colorIdx: number;
  declare _colors: string[];
  declare stationAId: string | null;
  declare stationBId: string | null;
  declare mode: string;
  declare viewZoom: number;
  declare viewPanX: number;
  declare viewPanY: number;
  declare _viewMinZoom: number;
  declare _viewMaxZoom: number;
  constructor() {
    this.records = [];
  ...
  record(game: GraphGame, timeOfDay: number) {
    const date = game._currentDate || game.engine?.currentDate || game.engine?.getParisDate?.() || '';
    const key = `${date}|${timeOfDay}`;
    if (key === this._lastRecordKey) return;
    this._lastRecordKey = key; this._lastRecordTime = timeOfDay;
    ...
    for (const svc of activeServices) {
      ...
      this.records.push({
        ...
        stops: stops?.map((s: { stationId: unknown }) => s.stationId) || [],
  ...
  _v2GraphServices(game: GraphGame): GraphServiceLike[] {
    ...
    const date = game?._currentDate || game?.engine?.currentDate || game?.engine?.getParisDate?.() || '';
    const out = [];
    const typeFor = (category: unknown) => ({
      PASSENGER:'passager', FREIGHT:'fret', W:'w', HLP:'hlp', TM:'tm', INFRA:'work', TTX:'work',
    }[String(category || '').toUpperCase()] || 'passager');

    for (const rec of records) {
      let ver = null;
      try { ver = date && typeof manager.resolveVersion === 'function' ? manager.resolveVersion(rec.id, date) : null; } catch {}
      if (!ver && rec?.currentVersion?.state === 'VALID') ver = rec.currentVersion;
      if (!ver || ver.state !== 'VALID') continue;
      const locs = Array.isArray(ver.locations) ? [...ver.locations].sort((a: { order: unknown },b: { order: unknown })=>(Number(a?.order)||0)-(Number(b?.order)||0)) : [];
      ...
      for (let i = 0; i < locs.length - 1; i++) {
        ...
        const leg = ver?.outboundPath?.legs?.find?.((x: { fromLocationId: unknown; toLocationId: unknown }) => x?.fromLocationId === a?.id && x?.toLocationId === b?.id);
      ...
      out.push({
        ...
        stops,
        routes: routes as GraphRoutePoint[][],
        active: true,
      });
    }
    return out;
  ...
  _graphServices(game: GraphGame, mode: unknown = this.mode): GraphServiceLike[] {
    const sc = game?.scheduleCreator;
    if (mode === 'live') return (typeof sc?.getActiveServices === 'function' ? (sc.getActiveServices() || []) : []) as unknown as GraphServiceLike[];
    ...
    return [...(legacy as unknown as GraphServiceLike[]), ...this._v2GraphServices(game)];
  }

  /** Get all stations referenced by the theoretical timetable. */
  _getServiceStations(game: GraphGame) {
    ...
    for (const svc of this._graphServices(game, 'theoretical')) {
      ...
      for (const stop of stops) if (stop?.stationId) stationSet.add(String(stop.stationId));
    }
    return [...stationSet].map((id: unknown) => game.world.getStationById(id)).filter((st): st is GraphWorldStation => Boolean(st));
  }
  ...
  _findServicesThrough(game: GraphGame, stAId: unknown, stBId: unknown) {
    ...
    for (const svc of this._graphServices(game, this.mode)) {
      const stops = typeof svc.getCurrentStops === 'function' ? (svc.getCurrentStops() || []) : (svc.stops || []);
      const idxA = stops.findIndex((s: { stationId: unknown }) => String(s?.stationId ?? '') === aId);
      const idxB = stops.findIndex((s: { stationId: unknown }) => String(s?.stationId ?? '') === bId);
      if (idxA >= 0 && idxB >= 0) {
  ...
  _getStationsBetween(stops: unknown, idxA: number, idxB: number) {
  ...
  _routeForLeg(svc: GraphServiceLike, legIdx: number) {
    if (!svc || legIdx < 0) return null;
    if (!svc.isReturnLeg) return svc.routes?.[legIdx] || null;
    if (Array.isArray(svc._returnRoutes) && svc._returnRoutes.length) return svc._returnRoutes[legIdx] || null;
  ...
  _calcDistances(stationStops: GraphStationStop[], game: GraphGame, refSvc: GraphServiceLike | null, routeStartIndex: number = 0) {
    const dists = [0];
  ...
  _haversine(lat1: number, lon1: number, lat2: number, lon2: number) {
    const R = 6371;
  ...
  _normalizeStationSearch(value: unknown) {
    return String(value ?? '')
  ...
  _stationSearchIndex(game: GraphGame, extraStations: unknown = []) {
    ...
    for (const st of [...base, ...(Array.isArray(extraStations) ? extraStations : [])]) {
      if (!st || st.id == null || typeof st.name !== 'string' || !st.name.trim()) continue;
    ...
    return stations.map((st: __KPA22) => ({
      id: String(st.id),
      name: st.name.trim(),
  ...
  _stationSearchMatches(index: StationSearchRow[], query: unknown, preferredIds: Set<string> = new Set(), limit: number = 12) {
    ...
    for (const st of rows) {
      if (!st.norm.includes(q)) continue;
      ...
      if (preferredIds.has(st.id)) rank -= 0.25;
      scored.push({ st, rank });
    }
  ...
  _stationSearchControlHtml(side: unknown, label: unknown, selectedName: unknown = '') {
  ...
  _escapeHtml(value: unknown) {
    return String(value ?? '').replace(/[&<>"']/g, (ch) => (({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'} as Record<string,string>)[String(ch)]));
  }

  _bindStationSearch(container: HTMLElement, game: GraphGame, side: 'a' | 'b', index: StationSearchRow[], preferredIds: Set<string>) {
    const input = container.querySelector(`#gm-station-${side}-search`) as HTMLInputElement | null;
    const list = container.querySelector(`#gm-station-${side}-results`) as HTMLElement | null;
    const clear = container.querySelector(`#gm-station-${side}-clear`) as HTMLButtonElement | null;
    ...
    const selectStation = (st: StationSearchRow) => {
      if (!st) return;
    ...
    input.addEventListener('keydown', (e: KeyboardEvent) => {
      if ((e.key === 'ArrowDown' || e.key === 'ArrowUp') && list.style.display === 'none') drawResults();
      const buttons = [...list.querySelectorAll('button[data-station-id]')] as HTMLButtonElement[];
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        if (activeIndex >= 0 && buttons[activeIndex]) {
          ...
          if (hits.length) { e.preventDefault(); selectStation(hits[0]); }
        }
  ...
  render(container: HTMLElement, game: GraphGame) {
    if (!container) return;
    ...
    const btnTheo = document.getElementById('gm-mode-theo') as HTMLElement | null;
    const btnLive = document.getElementById('gm-mode-live') as HTMLElement | null;
    const updateModeUI = () => {
      if(!btnTheo||!btnLive)return;
  ...
  _clampGraphView(W: number, H: number) {
  ...
  _setGraphZoom(game: GraphGame, nextZoom: number, focalX: number | null = null, focalY: number | null = null) {
    ...
    if (newZoom <= 1.0001) { this.viewPanX = 0; this.viewPanY = 0; }
    this._clampGraphView(W, H);
    this._updateGraphZoomLabel();
    this._draw(game);
  }

  _panGraph(game: GraphGame, dx: unknown, dy: unknown) {
    const canvas = document.getElementById('gm-canvas');
    if (!canvas) return;
    const W = canvas.clientWidth || 900;
    const H = canvas.clientHeight || 600;
  ...
  _timeStepForZoom(zoom: unknown = this.viewZoom) {
  ...
  _visibleTimeRange(H: number, pad: GraphPad, chartH: number) {
    ...
    const toMinute = (y: number) => ((y - pad.top) / Math.max(1, chartH)) * 1440;
  ...
  _bestVisibleTrainLabelSegment(points: LineSegment[], W: number, H: number) {
    ...
    for (const p of points) {
      ...
      if (len > bestLen) { bestLen = len; best = clipped; }
    }
    return best;
  }

  _drawScreenTimeScale(ctx: CanvasRenderingContext2D, W: number, H: number, pad: GraphPad, chartH: number, step: number, range: TimeRange) {
    const z = Math.max(1, Number(this.viewZoom) || 1);
    ...
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(58.5, 0); ctx.lineTo(58.5, H); ctx.stroke();
  ...
  _updateGraphZoomLabel() {
    ...
    if (el) el.textContent = `${Math.round((Number(this.viewZoom) || 1) * 100)}% · ${this._timeStepForZoom()} min`;
  }

  _bindGraphViewport(game: GraphGame) {
    const viewport = document.getElementById('gm-viewport');
    const canvas = document.getElementById('gm-canvas');
    if (!viewport || !canvas) return;
    const zoomStep = (factor: number) => this._setGraphZoom(game, (Number(this.viewZoom) || 1) * factor);
    ...
    document.getElementById('gm-zoom-reset')?.addEventListener('click', () => {
      this.viewZoom = 1; this.viewPanX = 0; this.viewPanY = 0; this._updateGraphZoomLabel(); this._draw(game);
    });
    ...
    viewport.addEventListener('pointermove', (e) => {
      ...
      dragLastX = e.clientX;
      dragLastY = e.clientY;
      if (dx || dy) this._panGraph(game, dx, dy);
      e.preventDefault();
    });
    ...
    viewport.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowLeft') { e.preventDefault(); this._panGraph(game, step, 0); }
      else if (e.key === 'ArrowRight') { e.preventDefault(); this._panGraph(game, -step, 0); }
      else if (e.key === 'ArrowUp') { e.preventDefault(); this._panGraph(game, 0, step); }
      else if (e.key === 'ArrowDown') { e.preventDefault(); this._panGraph(game, 0, -step); }
      else if (e.key === '+' || e.key === '=') { e.preventDefault(); zoomStep(1.5); }
      else if (e.key === '-' || e.key === '_') { e.preventDefault(); zoomStep(1 / 1.5); }
      else if (e.key === '0') { e.preventDefault(); this.viewZoom = 1; this.viewPanX = 0; this.viewPanY = 0; this._updateGraphZoomLabel(); this._draw(game); }
    });
  ...
  _draw(game: GraphGame) {
    const canvas = document.getElementById('gm-canvas') as HTMLCanvasElement | null;
    if (!canvas) return;
    ...
    for (let t = gridFirst; t <= gridLast; t += timeStep) {
      ...
      if (!is10) ctx.setLineDash([2 / zLine, 2 / zLine]);
    ...
    const stationNames = refStops.map((s: __KPA24) => {
    ...
    if (this.mode === 'theoretical') {
      for (const m of matches) {
        ...
        for (let i = start; i <= end; i++) {
          ...
          const refIdx = refStops.findIndex((rs: { stationId: unknown }) => rs.stationId === stop.stationId);
          ...
          if (i === start) {
            ...
            const prevRef = refStops.findIndex((rs: { stationId: unknown }) => rs.stationId === prevStop.stationId);
            if (prevRef >= 0) {
              ...
              for (let pi=0; pi<pieces.length; pi++) {
                ...
                if (pi>0 || Math.floor(piece.t1/1440)!==Math.floor(prevDep/1440)) ctx.moveTo(piece.x1, pad.top + (p1/1440)*chartH);
                ...
                if (dur>0) labelPoints.push({x1:piece.x1,y1:pad.top+(p1/1440)*chartH,x2:piece.x2,y2:piece.t2%1440===0&&piece.t2>piece.t1?H-pad.bottom:pad.top+(p2/1440)*chartH,duration:dur,name:m.svc.name});
                if (piece.t2 % 1440 === 0 && pi < pieces.length-1) ctx.moveTo(piece.x2,pad.top);
              }
            } else ctx.lineTo(x, yArr);
          }
          if (dep !== arr) {
            ...
            for(let pi=0;pi<dwellPieces.length;pi++){
              ...
              if(piece.t2%1440===0&&pi<dwellPieces.length-1)ctx.moveTo(x,pad.top);
            }
          }
        }
        ctx.stroke();
        ...
        if (labelPoints.length > 0) trainLabelOverlays.push({ m, labelPoints });
      ...
      const byService: Record<string, GraphLiveRecord[]> = {};
      ...
      for (const m of matches) {
        ...
        for (let i = start; i <= end; i++) {
          ...
          const refIdx = refStops.findIndex((rs: { stationId: unknown }) => rs.stationId === m.stops[i].stationId);
  ...
  _serviceDisplayName(svc: Pick<GraphServiceLike, 'name' | 'number' | 'train'>) {
  ...
  _drawTrainLabel(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number, label: string) {
  ...
  _centerText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, color: string) {
    ctx.fillStyle = color;
  ...
  _styleForService(svc: Pick<GraphServiceLike, 'serviceType' | 'isWorkTrain'>, ctx: CanvasRenderingContext2D) {
  ...
  _interpolateDist(lat: number, lon: number, routeCoords: RouteDistancePoint[]) {
  ...
  loadFromSave(s: GraphSave) {
    if (!s || typeof s !== 'object') return;
    const validColor = (v: unknown) => typeof v === 'string' && /^#[0-9a-f]{6}$/i.test(v);
    this.records = (Array.isArray(s.records) ? s.records : []).filter((r: __KPA25) => {
      if (!r || typeof r !== 'object' || !r.serviceId) return false;
      const time=Number(r.time), lat=Number(r.lat), lon=Number(r.lon);
      if (!Number.isFinite(time) || time < 0 || time >= 1440) return false;
      if ((r.lat != null || r.lon != null) && (!Number.isFinite(lat)||!Number.isFinite(lon)||lat < -90||lat > 90||lon < -180||lon > 180)) return false;
      return true;
    }).slice(-this.maxRecords).map((r: { totalStops: unknown; stopIndex: unknown; date: unknown; serviceId: unknown; name: unknown; time: unknown; lat: unknown; lon: unknown; stops: unknown; color: unknown; state: unknown }) => {
      ...
      return {
        ...
        totalStops,
        stops:Array.isArray(r.stops)?r.stops.map((x: unknown)=>x==null?'':String(x)).filter(Boolean):[],
        color:validColor(r.color)?r.color:'#64748b', state:typeof r.state==='string'?r.state:'',
      };
    });
    this._colorMap = {};
    if (s.colorMap && typeof s.colorMap==='object' && !Array.isArray(s.colorMap)) for (const [k,v] of Object.entries(s.colorMap)) if (k && validColor(v)) this._colorMap[k]=v;
    const ci=Number(s.colorIdx); this._colorIdx=Number.isFinite(ci)&&ci>=0?Math.floor(ci):0;
