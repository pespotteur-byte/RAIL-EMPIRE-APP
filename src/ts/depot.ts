...
import { validRescueMotion, RescueMovement, type RescueMotionSnapshot } from './rescue-movement.js';
...
type __KPStruct38 = { "depotOperationId": unknown; "id": unknown; "currentLocation": Record<string, unknown>; "inMaintenance": unknown };
import type { Economy } from './economy.js';
import type { World } from './world.js';
import { normalizeRailSection, cleanRailRoute, cleanRailBinding, cloneRailValue, type RailSectionLike, type NormalizedRailSection, type RailBinding, type RailPoint } from './rail-section-geometry.js';
import { Rame, type RameManager } from './rame.js';
let nextDepotId = 1;
let nextRescueId = 1;
type DepotServiceRuntime = {
  rame?: { id?: unknown };
  train?: { inDepot?: boolean; inMaintenance?: boolean };
  completed?: unknown; cancelled?: unknown; state?: unknown;
};
type DepotStaffBridge = {
  checkDepotStaff?: (depotId: string, needs: Record<string, unknown>) => { ok: boolean; shortages: Array<{ label: unknown; free: unknown; need: unknown }> };
  reserveDepotStaff?: (depotId: string, needs: Record<string, unknown>, operationId: string, label: string) => { ok: boolean; staffIds?: string[] };
  releaseDepotTask?: (operationId: string) => unknown;
};
type BrokenServiceForRescue = {
  id?: unknown; position?: { lat?: unknown; lon?: unknown } | null;
  getCurrentRoute?: () => Array<{ electrified?: unknown }> | null | undefined;
  _state?: { cachedRoute?: Array<{ electrified?: unknown }> };
  train?: { breakdown?: { type?: unknown } };
};

export interface DepotResourceDefinition { label: string; unit: string; price: number; stock: boolean; defaultCapacity: number; category: string; }
export interface DepotEquipmentDefinition { label: string; category: string; price: number; maxCount: number; description: string; }
export interface DepotOperationDefinition {
  label: string; group: string; duration: number; staff: Record<string, number>;
  dynamic?: 'fuel' | 'sand' | 'fluids' | 'washer' | 'engine';
  resources?: Record<string, number>; parts?: Record<string, number>; equipment?: string[]; requires?: string;
}
...
export const DEPOT_RESOURCE_CATALOG: Readonly<Record<string, DepotResourceDefinition>> = Object.freeze({
...
export const DEPOT_EQUIPMENT_CATALOG: Readonly<Record<string, DepotEquipmentDefinition>> = Object.freeze({
...
export const DEPOT_OPERATION_CATALOG: Readonly<Record<string, DepotOperationDefinition>> = Object.freeze({
...
function depotFinite(v: unknown,fb: number=0){const n=Number(v);return Number.isFinite(n)?n:fb;}
function depotClamp(v: unknown,a: number,b: number){return Math.max(a,Math.min(b,depotFinite(v,a)));}
function materialSearchText(rame: Rame){return [rame?.name,rame?.serialNumber,...(rame?.elementDetails||[]).flatMap((e: __S3Struct43) =>[e?.name,e?.instanceName,e?.seriesName])].filter(Boolean).join(' ');}
type DepotInput = Record<string, unknown> & {
  location?: { lat?: unknown; lon?: unknown } | null;
  spareParts?: Record<string, unknown> | null;
  resourceUsage?: Record<string, unknown> | null;
  resourceCapacities?: Record<string, unknown> | null;
  resourceStocks?: Record<string, unknown> | null;
  utilityTotals?: { electricityKWh?: unknown; waterL?: unknown; expenseEur?: unknown } | null;
};

type DepotIteTrack = {
  name: string; length: number; cargoType: string; startBinding: RailBinding | null; endBinding: RailBinding | null;
  constraints: unknown[]; route: RailPoint[]; segments: unknown[]; distanceKm: number; role: string;
};
type DepotRescueLoco = { stockId: string; stockName: string; traction: string; deployed: boolean; consumables?: Rame['consumables']; totalKmRun?: number };
type DepotTrackSlot = { track: number; rameId: string; purpose: string; operationId: string };
type DepotExpense = { time: number; label: string; amount: number; category: string };
type DepotUtilityTotals = { electricityKWh: number; waterL: number; expenseEur: number };
type DepotLegacySpareParts = { moteur: number; climatisation: number; fanaux: number; freins: number; portes: number };

function partForEngine(rame: Rame){const txt=materialSearchText(rame);return DEPOT_PART_CATALOG.find((p) =>p.realModel&&p.match?.test(txt)) || DEPOT_PART_CATALOG.find((p) =>p.id==='engine_generic');}
...
export class Depot {
  ...
  declare stationUpgradeSource: string;
  declare stationUpgradeTracks: number;
  declare tracks: number;
  declare cost: number;
  declare built: boolean;
  declare infrastructure: string[];
  declare equipmentInventory: Record<string, number>;
  declare ramesStored: string[];
  declare schemaVersion: 1 | 2;
  declare location: { lat: number; lon: number } | null;
  declare placementOnly: boolean;
  declare railSections: NormalizedRailSection[];
  declare iteTracks: DepotIteTrack[];
  declare iteCargoTypes: string[];
  declare rescueLocos: DepotRescueLoco[];
  declare partInventory: Record<string, number>;
  declare spareParts: DepotLegacySpareParts;
  declare resourceStocks: Record<string, number>;
  declare resourceCapacities: Record<string, number>;
  declare resourceUsage: Record<string, number>;
  declare trackOccupancy: DepotTrackSlot[];
  declare utilityTotals: DepotUtilityTotals;
  declare expenseLedger: DepotExpense[];
  constructor(data: unknown = {}) {
    const src: DepotInput = data && typeof data === 'object' && !Array.isArray(data) ? data as DepotInput : {};
    this.id = src.id == null || src.id === '' ? `depot-${nextDepotId++}` : String(src.id);
    this.type = (src.type || 'depot') as string; // depot, ite-fret, ite-industrie, ite-logistique
    this.name = (src.name || 'Depot') as string;
    ...
    this.infrastructure = [...new Set((Array.isArray(src.infrastructure) ? src.infrastructure : []).filter((x: unknown) =>typeof x==='string'&&infraAllowed.has(x)))] as string[];
    ...
    this.equipmentInventory={} as Record<string, number>;
    if(src.equipmentInventory&&typeof src.equipmentInventory==='object'&&!Array.isArray(src.equipmentInventory)){
      for(const [key,value] of Object.entries(src.equipmentInventory)){const def=DEPOT_EQUIPMENT_CATALOG[key],n=Math.floor(Number(value));if(def&&Number.isFinite(n)&&n>0)this.equipmentInventory[key]=Math.min(def.maxCount||1,n);}
    }
    if(!Object.keys(this.equipmentInventory).length){
      if(this.infrastructure.includes('technicentre')) Object.assign(this.equipmentInventory,{inspection_pit:1,lifting_jacks:1,overhead_crane:1,underfloor_lathe:1,brake_bench:1,electrical_bench:1,compressor_station:1,fuel_station:1,sand_station:1,fluid_station:1,wash_plant:1,interior_cleaning:1,roof_access:1,battery_station:1});
      if(this.infrastructure.includes('remisage')) this.equipmentInventory.interior_cleaning=Math.max(1,this.equipmentInventory.interior_cleaning||0);
    }
    this.ramesStored = [...new Set((Array.isArray(src.ramesStored) ? src.ramesStored : []).filter((x: unknown) =>typeof x==='string'&&x))] as string[];
    ...
    this.railSections = (Array.isArray(src.railSections) ? src.railSections : [])
      .map((x: unknown,i: number)=>normalizeRailSection(x as RailSectionLike,{id:`${this.id}-section-${i+1}`,name:`Section ${i+1}`,direction:'both'}))
      .filter((x: __S3Struct44) =>x.route.length>=2 && x.startBinding && x.endBinding);
    // ITE track footprints keep their real ORM geometry when available, while
    // preserving legacy {name,length,cargoType} saves.
    this.iteTracks = (Array.isArray(src.iteTracks) ? src.iteTracks : []).filter((t: __S3Struct45) =>t&&typeof t==='object'&&Number.isFinite(Number(t.length))&&Number(t.length)>0).map((t: { name: unknown; length: unknown; cargoType: unknown; startBinding: unknown; endBinding: unknown; constraints: unknown; route: unknown; segments: unknown; distanceKm: unknown; role: unknown },i: unknown)=>({
    ...
    if (!this.railSections.length) {
      this.railSections = this.iteTracks.filter((t: __S3Struct46) =>t.route?.length>=2&&t.startBinding&&t.endBinding).map((t: __S3Struct47,i: number)=>normalizeRailSection({...t,id:`${this.id}-section-${i+1}`,name:t.name,role:'ite-track',direction:'both'}));
    }
    this.iteCargoTypes = [...new Set((Array.isArray(src.iteCargoTypes) ? src.iteCargoTypes : []).filter((x: unknown) =>typeof x==='string'&&x))] as string[];
    // Rescue locomotives: array of { stockId, stockName, deployed }
    this.rescueLocos = (Array.isArray(src.rescueLocos) ? src.rescueLocos : []).filter((r: { stockId: unknown }) => r && r.stockId).map((r: { stockId: unknown; stockName: unknown; traction: unknown; deployed: unknown; consumables?: unknown; totalKmRun?: unknown }) => ({
      ...
      consumables:r.consumables && typeof r.consumables==='object' && !Array.isArray(r.consumables)
        ?Object.fromEntries(Object.entries(r.consumables).filter(([k,n])=>!['__proto__','constructor','prototype'].includes(k) && typeof n==='number' && Number.isFinite(n) && n>=0)) as Rame['consumables']:undefined,
    }));
    ...
    this.partInventory = {} as Record<string, number>;
    if (src.partInventory && typeof src.partInventory === 'object' && !Array.isArray(src.partInventory)) {
      for (const [key,value] of Object.entries(src.partInventory)) {
        const n=Math.floor(Number(value)); if(key && Number.isFinite(n) && n>=0) this.partInventory[String(key)]=n;
      }
    }
    ...
    if (src.spareParts && typeof src.spareParts === 'object' && !Array.isArray(src.spareParts)) {
      for (const [oldKey,newKey] of Object.entries(legacyMap)) {const n=Math.floor(Number(src.spareParts[oldKey]));if(Number.isFinite(n)&&n>0)this.partInventory[newKey]=(this.partInventory[newKey]||0)+n;}
    }
    ...
    this.resourceStocks={} as Record<string, number>; this.resourceCapacities={} as Record<string, number>; this.resourceUsage={} as Record<string, number>;
    ...
    this.trackOccupancy=Array.from({length:this.tracks},(_: unknown,i)=>{const raw=slots[i]||{};return {track:i+1,rameId:String(raw.rameId||''),purpose:String(raw.purpose||'garage'),operationId:String(raw.operationId||'')};});
    ...
    this.expenseLedger=(Array.isArray(src.expenseLedger)?src.expenseLedger:[]).filter((x: unknown) =>x&&typeof x==='object').slice(-120).map((x: { time: unknown; label: unknown; amount: unknown; category: unknown }) =>({time:Number(x.time)||0,label:String(x.label||''),amount:Math.max(0,depotFinite(x.amount,0)),category:String(x.category||'exploitation')}));
  }
  ...
  addPart(partId: unknown, qty: unknown) {
    const key=String(partId||''); const amount=Math.floor(Number(qty)); if(!key||!Number.isFinite(amount)||amount<=0)return false;
    this.partInventory[key]=(Number(this.partInventory[key])||0)+amount;this._syncLegacySpareParts();return true;
  }

  consumeParts(parts: unknown) {
    const entries: Array<[string, number]>=Object.entries(parts||{}).map(([k,v])=>[String(k),Math.floor(Number(v))]);
    if(!entries.length)return true;for(const [k,n] of entries){if(!k||!Number.isFinite(n)||n<=0||(Number(this.partInventory[k])||0)<n)return false;}
    for(const [k,n] of entries)this.partInventory[k]-=n;this._syncLegacySpareParts();return true;
  }

  // Legacy API retained for old repair/rescue paths.
  addSpareParts(type: unknown, qty: unknown) {const map: Record<string,string>={moteur:'engine_generic',climatisation:'hvac_unit',fanaux:'headlight_unit',freins:'brake_pad_set',portes:'door_drive'};const key=String(type||'');return this.addPart(map[key]||key,qty);}
  consumeSpareParts(parts: unknown) {const map: Record<string,string>={moteur:'engine_generic',climatisation:'hvac_unit',fanaux:'headlight_unit',freins:'brake_pad_set',portes:'door_drive'};const next: Record<string,unknown>={};for(const [k,v] of Object.entries(parts||{}))next[map[k]||k]=v;return this.consumeParts(next);}

  freeTrackCount(){return this.trackOccupancy.filter((x: { rameId: unknown }) =>!x.rameId).length;}
  occupancyCount(){return this.trackOccupancy.filter((x: { rameId: unknown }) =>!!x.rameId).length;}
  findRameTrack(rameId: unknown){return this.trackOccupancy.find((x: { rameId: unknown }) =>x.rameId===String(rameId||''))||null;}
  ...
  hasInfrastructure(type: unknown) {
    return Array.isArray(this.infrastructure) && this.infrastructure.includes(type as string);
  }

  equipmentCount(id: unknown){const key=String(id||'');return Math.max(0,Math.floor(Number(this.equipmentInventory?.[key])||0));}
  hasEquipment(id: unknown){return this.equipmentCount(id)>0;}
  addEquipment(id: unknown,count: unknown=1){const key=String(id||'');const def=DEPOT_EQUIPMENT_CATALOG[key],n=Math.floor(Number(count));if(!def||!Number.isFinite(n)||n<=0)return false;const cur=this.equipmentCount(id),next=Math.min(def.maxCount||1,cur+n);if(next<=cur)return false;this.equipmentInventory[key]=next;return true;}
  ...
  getMaintenanceDuration(baseMinutes: unknown) {
    const base=Math.max(1, Number(baseMinutes)||60);
    if (this.hasInfrastructure('rotonde')) return Math.max(5, Math.round(base * 0.8));
    return base;
  }

  // Can dispatch rescue locomotive (has available non-deployed locos)
  canRescue() {
    return this.type === 'depot' && this.built && this.rescueLocos.some((r: { deployed: unknown }) => !r.deployed);
  }

  // Get first available rescue loco
  getAvailableRescueLoco(preferredTraction: unknown) {
    const preferred = String(preferredTraction || '').toLowerCase();
    if (preferred) {
      const match = this.rescueLocos.find((r: { deployed?: boolean; traction?: string }) => !r.deployed && r.traction && r.traction.toLowerCase().includes(preferred));
      if (match) return match;
    }
    return this.rescueLocos.find((r: { deployed: unknown }) => !r.deployed);
  }

  // Mark a rescue loco as deployed
  deployRescue(stockId: unknown) {
    const loco = this.rescueLocos.find((r: { stockId: unknown; deployed: unknown }) => r.stockId === stockId && !r.deployed);
    if (loco) {
      loco.deployed = true;
      return loco;
    }
  ...
  returnRescue(stockId: unknown) {
    const loco = this.rescueLocos.find((r: { stockId: unknown; deployed: unknown }) => r.stockId === stockId && r.deployed);
    if (loco) loco.deployed = false;
  }
}

type DepotRoutePoint = { lat: number; lon: number; [key:string]:unknown };
...
type DepotRepairQueueEntry = {
  serviceId: string; depotId: string; remainingMin: number; totalMin: number; serviceName: string; repairType: string; rameId?: string; vehicleIds?: string[];
};
type DepotRescuedFormation = { id: string; name: string; depotId: string; serviceId: string; vehicleIds: string[] };
type DepotMaintenanceQueueEntry = {
  rameId: string; depotId: string; remainingMin: number; totalMin: number; rameName: string;
};
type DepotOperation = {
  id: string; depotId: string; rameId: string; rameName: unknown; opId: unknown; label: string; group: string;
  remainingMin: number; totalMin: number; state: string; staff: Record<string, unknown>; staffIds: string[];
  resources: Record<string, unknown>; parts: Record<string, unknown>; equipment: string[]; startedAt?: number;
};

export class DepotManager {
  private _rescueMovements = new Map<string, RescueMovement>();
  declare depots: Depot[];
  declare activeRescues: DepotActiveRescue[];
  declare repairQueue: DepotRepairQueueEntry[];
  declare maintenanceQueue: DepotMaintenanceQueueEntry[];
  declare depotOperations: DepotOperation[];
  rescuedOccurrenceIds = new Set<string>();
  ...
  add(data: unknown, economy: Economy) {
    ...
    if (economy) {
      if (!Number.isFinite(Number(economy.balance)) || economy.balance < cost) return null;
  ...
  remove(id: unknown) {
    id=String(id??'');
    if (this.getDepotById(id)?.stationUpgradeSource) return false; // paid module still owns these slots
    if (this.activeRescues.some((r: { depotId: unknown }) => r.depotId === id) || this.repairQueue.some((r: { depotId: unknown }) => r.depotId === id) || this.maintenanceQueue.some((r: { depotId: unknown }) => r.depotId === id) || this.depotOperations.some((r: { depotId: unknown }) => r.depotId === id) || this.getDepotById(id)?.trackOccupancy?.some((x: { rameId: unknown }) =>x.rameId)) return false;
    const before = this.depots.length;
    this.depots = this.depots.filter((d: { id: unknown }) => d.id !== id);
  ...
  getDepots() {
    return this.depots.filter((d: { type: unknown; built: unknown }) => d.type === 'depot' && d.built);
  }

  getDepotById(id: unknown) {
    id=String(id??'');
    return this.depots.find((d: { id: unknown }) => d.id === id) || null;
  }

  getITEs() {
    return this.depots.filter((d: { built: unknown; type: unknown }) => d.built && typeof d.type === 'string' && d.type.startsWith('ite'));
  }

  getByStation(stationId: unknown) {
    stationId=String(stationId??'');
    return this.depots.filter((d: { stationId: unknown }) => d.stationId === stationId);
  }

  addITETrack(depotId: unknown, track: { length: unknown; name: unknown; cargoType: unknown; startBinding: unknown; endBinding: unknown; constraints: unknown; route: unknown; segments: unknown; distanceKm: unknown; role: unknown }) {
    const depot = this.depots.find((d: { id: unknown }) => d.id === depotId);
    ...
    if (depot?.built && depot.type.startsWith('ite') && name && Number.isFinite(length) && length > 0) {
      ...
      if(rec.route.length>=2&&rec.startBinding&&rec.endBinding) depot.railSections.push(normalizeRailSection({...rec,id:`${depot.id}-section-${depot.railSections.length+1}`,direction:'both'}));
      return true;
    }
    return false;
  }

  removeITETrack(depotId: unknown, index: unknown) {
    const depot = this.depots.find((d: { id: unknown }) => d.id === depotId);
  ...
  getTotalITELength(depotId: unknown) {
    const depot = this.depots.find((d: { id: unknown }) => d.id === depotId);
    return depot ? depot.iteTracks.reduce((s: number, t: { length: unknown }) => s + (Number(t.length) || 0), 0) : 0;
  }

  // Section VI — ITE : récupère le dépôt ITE lié à une gare (s'il existe)
  getITEByStation(stationId: unknown) {
    stationId=String(stationId??'');
    return this.depots.find((d: { built: unknown; type: unknown; stationId: unknown }) => d.built && typeof d.type==='string' && d.type.startsWith('ite') && d.stationId === stationId) || null;
  }
  ...
  getITEInfo(stationId: unknown, trainLengthM: unknown, cargoType: unknown) {
    ...
    if (!ite) return { isITE: false, depotId: '', totalLength: Infinity, trancheCount: 1 };
    const totalLength = ite.iteTracks.reduce((s: number, t: { length: unknown }) => s + (Number(t.length) || 0), 0);
    ...
    if (cargoType != null && String(cargoType).trim() && ite.iteCargoTypes && ite.iteCargoTypes.length > 0) {
      ...
      cargoMatch = ite.iteCargoTypes.some((ct: unknown) => { const accepted=String(ct||'').trim(); return accepted && (accepted===cargo || cargo.startsWith(accepted) || accepted.startsWith(cargo)); });
    }
    if (totalLength <= 0) {
      ...
      if (ite.placementOnly) return { isITE: true, depotId: ite.id, totalLength: Infinity, canFit: true, usable: true, trancheCount: 1, cargoMatch, placementOnly: true };
      return { isITE: true, depotId: ite.id, totalLength: 0, canFit: false, usable: false, trancheCount: 1, cargoMatch };
    }
    const rawLength=Number(trainLengthM); const safeLength=Number.isFinite(rawLength)?Math.max(0,rawLength):0;
  ...
  assignRameHome(rame: Rame, depotId: string, {assignElements=true}: { assignElements?: boolean }={}) {
    const depot=this.getDepotById(depotId); if(!rame||!depot||depot.type!=='depot'||!depot.built)return false;
    rame.depotId=depot.id;
    if(assignElements) for(const el of rame.elementDetails||[]) el.homeDepotId=depot.id;
    return true;
  }

  assignElementHome(rame: Rame, elementId: unknown, depotId: unknown) {
    const depot=this.getDepotById(depotId); const el=rame?.elementDetails?.find((e: { elementId: unknown }) =>e.elementId===String(elementId||''));
    if(!el||!depot||depot.type!=='depot'||!depot.built)return false;el.homeDepotId=depot.id;return true;
  }

  clearRameHome(rame: Rame) {if(!rame)return false;const old=rame.depotId;rame.depotId='';for(const el of rame.elementDetails||[])if(el.homeDepotId===old)el.homeDepotId='';return true;}

  _rameHasActiveService(rameId: unknown, services: DepotServiceRuntime[] = []) {
  ...
  enterRame(depotId: unknown, rame: __S3Struct54, services: DepotServiceRuntime[] = []) {
    const depot=this.getDepotById(depotId); if(!depot||depot.type!=='depot'||!depot.built||!rame)return {ok:false,reason:'Dépôt invalide'};
    if(this._rameHasActiveService(rame.id,services))return {ok:false,reason:'Cette rame est encore engagée dans un service.'};
    if(rame.depotOperationId)return {ok:false,reason:'Une opération dépôt est déjà en cours.'};
    const existing = depot.findRameTrack(rame.id);
    const slot = existing || depot.trackOccupancy.find(x => !x.rameId);
    if (!slot) return {ok:false,reason:'Dépôt complet : aucune voie de garage libre.'};
    ...
    for (const d of this.getDepots()) {
      for (const occupied of d.trackOccupancy) {
        if (occupied !== slot && occupied.rameId === rame.id) {
          occupied.rameId=''; occupied.purpose='garage'; occupied.operationId='';
        }
      }
    }
    slot.rameId=rame.id;slot.purpose='garage';slot.operationId='';
    ...
    for(const svc of services||[])if(svc?.rame?.id===rame.id&&svc.train){svc.train.inDepot=true;svc.train.inMaintenance=!!rame.inMaintenance;}
    return {ok:true,track:slot.track,already:!!existing};
  }

  leaveRame(depotId: unknown, rame: __KPStruct38, services: DepotServiceRuntime[] = []) {
    const depot=this.getDepotById(depotId); if(!depot||!rame)return {ok:false,reason:'Dépôt / rame invalide'};
    if(rame.depotOperationId||this.depotOperations.some((o: { rameId: unknown; state: unknown }) =>o.rameId===rame.id&&o.state==='running'))return {ok:false,reason:'Opération en cours : sortie impossible.'};
    if (this.isRameInMaintenance(rame.id) || this.repairQueue.some(q => q.rameId === rame.id)) return {ok:false,reason:'Réparation ou entretien en cours : sortie impossible.'};
    const slot=depot.findRameTrack(rame.id);
    if (!slot && rame.currentLocation?.depotId !== depot.id) return {ok:false,reason:'Cette rame ne se trouve pas dans ce dépôt.'};
    if (slot) { slot.rameId='';slot.purpose='garage';slot.operationId=''; }
    if(rame.currentLocation?.depotId===depot.id) rame.currentLocation={...(rame.currentLocation||{}),depotId:'',lat:null,lon:null};
    for(const svc of services||[])if(svc?.rame?.id===rame.id&&svc.train){svc.train.inDepot=false;svc.train.inMaintenance=!!rame.inMaintenance;}
    return {ok:true};
  }

  getDepotOccupancy(depotId: unknown){const d=this.getDepotById(depotId);return d?{used:d.occupancyCount(),capacity:d.tracks,free:d.freeTrackCount(),tracks:d.trackOccupancy.map((x: { track: unknown; rameId: unknown; purpose: unknown; operationId: unknown }) =>({...x}))}:{used:0,capacity:0,free:0,tracks:[]};}

  addDepotTracks(depotId: unknown,count: number,economy: Economy){
    const depot=this.getDepotById(depotId);count=Math.floor(Number(count));if(!depot||depot.type!=='depot'||!economy||!Number.isFinite(count)||count<=0)return {ok:false,reason:'Extension invalide.'};
    const allowed=Math.min(count,Math.max(0,60-depot.tracks));if(allowed<=0)return {ok:false,reason:'Capacité maximale atteinte (60 voies).'};const cost=allowed*DEPOT_TRACK_EXPANSION_COST;if(Number(economy.balance)<cost)return {ok:false,cost,reason:'Fonds insuffisants.'};
    const old=depot.tracks;for(let i=0;i<allowed;i++)depot.trackOccupancy.push({track:old+i+1,rameId:'',purpose:'garage',operationId:''});depot.tracks+=allowed;economy.addExpense(cost,'construction',`Extension ${depot.name} — ${allowed} voie(s) de garage`);depot.utilityTotals.expenseEur+=cost;depot.expenseLedger.push({time:Date.now(),label:`Extension +${allowed} voie(s)`,amount:cost,category:'infrastructure'});depot.expenseLedger=depot.expenseLedger.slice(-120);return {ok:true,cost,count:allowed,tracks:depot.tracks};
  }

  buyEquipment(depotId: unknown,equipmentId: unknown,economy: Economy){
    const equipmentKey=String(equipmentId||'');
    const depot=this.getDepotById(depotId),def=DEPOT_EQUIPMENT_CATALOG[equipmentKey];if(!depot||depot.type!=='depot'||!def||!economy)return {ok:false,reason:'Équipement invalide.'};const cur=depot.equipmentCount(equipmentKey);if(cur>=(def.maxCount||1))return {ok:false,reason:'Nombre maximal déjà installé.'};if(Number(economy.balance)<def.price)return {ok:false,cost:def.price,reason:'Fonds insuffisants.'};economy.addExpense(def.price,'construction',`${depot.name} — ${def.label}`);depot.addEquipment(equipmentKey,1);depot.utilityTotals.expenseEur+=def.price;depot.expenseLedger.push({time:Date.now(),label:`Équipement : ${def.label}`,amount:def.price,category:'équipement'});depot.expenseLedger=depot.expenseLedger.slice(-120);return {ok:true,cost:def.price,count:depot.equipmentCount(equipmentKey)};
  }

  _equipmentBusyCount(depotId: unknown,equipmentId: unknown){return this.depotOperations.filter((o: { depotId: unknown; state: unknown; equipment: unknown }) =>o.depotId===String(depotId||'')&&o.state==='running'&&Array.isArray(o.equipment)&&o.equipment.includes(equipmentId)).length;}
  getEquipmentAvailability(depotId: unknown,equipmentId: unknown){const depot=this.getDepotById(depotId),installed=depot?.equipmentCount?.(equipmentId)||0,busy=this._equipmentBusyCount(depotId,equipmentId);return {installed,busy,free:Math.max(0,installed-busy)};}

  buyResource(depotId: unknown,key: unknown,qty: number,economy: Economy){
    const resourceKey=String(key||''); const depot=this.getDepotById(depotId),def=DEPOT_RESOURCE_CATALOG[resourceKey];qty=Number(qty);
    if(!depot||!def?.stock||!economy||!Number.isFinite(qty)||qty<=0)return {ok:false,cost:0};
    ...
    if(add<=0)return {ok:false,cost:0,reason:'Stock déjà plein'};const cost=add*def.price;if(Number(economy.balance)<cost)return {ok:false,cost,reason:'Fonds insuffisants'};
    economy.addExpense(cost,'maintenance',`Approvisionnement ${depot.name} — ${def.label} ${add.toFixed(def.unit==='L'?0:1)} ${def.unit}`);depot.resourceStocks[resourceKey]=cur+add;depot.utilityTotals.expenseEur+=cost;depot.expenseLedger.push({time:Date.now(),label:`Achat ${def.label}`,amount:cost,category:'approvisionnement'});depot.expenseLedger=depot.expenseLedger.slice(-120);return {ok:true,cost,qty:add};
  }

  buyPart(depotId: unknown,partId: unknown,qty: number,economy: Economy){
    const depot=this.getDepotById(depotId),part=DEPOT_PART_CATALOG.find((p) =>p.id===partId);qty=Math.floor(Number(qty));
    if(!depot||!part||!economy||!Number.isFinite(qty)||qty<=0)return {ok:false,cost:0};const cost=part.price*qty;if(Number(economy.balance)<cost)return {ok:false,cost,reason:'Fonds insuffisants'};
    economy.addExpense(cost,'maintenance',`Pièces ${depot.name} — ${part.label} x${qty}`);depot.addPart(partId,qty);depot.utilityTotals.expenseEur+=cost;depot.expenseLedger.push({time:Date.now(),label:`Pièce : ${part.label} x${qty}`,amount:cost,category:'pièces'});depot.expenseLedger=depot.expenseLedger.slice(-120);return {ok:true,cost};
  }

  _operationNeeds(rame: Rame, opId: unknown){
    const opKey=String(opId||''); const def=DEPOT_OPERATION_CATALOG[opKey];if(!def)return null;const resources={...(def.resources||{})},parts={...(def.parts||{})};const c=rame?.consumables||{};
    if(def.dynamic==='fuel'){const missing=Math.max(0,Number(c.fuelCapacityL||0)-Number(c.fuelL||0));if(missing<=0)return {def,resources,parts,nothing:true};resources.diesel_l=missing;}
    if(def.dynamic==='sand'){const missing=Math.max(0,Number(c.sandCapacityKg||0)-Number(c.sandKg||0));if(missing<=0)return {def,resources,parts,nothing:true};resources.sand_kg=missing;}
    if(def.dynamic==='fluids'){
      resources.engine_oil_l=Math.max(0,Number(c.oilCapacityL||0)-Number(c.oilL||0));resources.coolant_l=Math.max(0,Number(c.coolantCapacityL||0)-Number(c.coolantL||0));if(Number(c.adblueCapacityL||0)>0)resources.adblue_l=Math.max(0,Number(c.adblueCapacityL||0)-Number(c.adblueL||0));if(Number(c.gearboxOilCapacityL||0)>0)resources.gearbox_oil_l=Math.max(0,Number(c.gearboxOilCapacityL||0)-Number(c.gearboxOilL||0));if(Number(c.hydraulicOilCapacityL||0)>0)resources.hydraulic_oil_l=Math.max(0,Number(c.hydraulicOilCapacityL||0)-Number(c.hydraulicOilL||0));
    }
    if(def.dynamic==='washer'){const missing=Math.max(0,Number(c.washerCapacityL||0)-Number(c.washerL||0));if(missing<=0)return {def,resources,parts,nothing:true};resources.washer_fluid_l=missing;}
    if(def.dynamic==='engine'){const part=partForEngine(rame);parts[part?.id||'engine_generic']=1;}
    return {def,resources,parts,nothing:false};
  }

  startDepotOperation(depotId: unknown, rame: Rame, opId: unknown, economy: Economy, staffManager: DepotStaffBridge | null=null){
    const depot=this.getDepotById(depotId),need=this._operationNeeds(rame,opId);if(!depot||depot.type!=='depot'||!rame||!need)return {ok:false,reason:'Opération invalide'};
    if(rame.currentLocation?.depotId!==depot.id||!depot.findRameTrack(rame.id))return {ok:false,reason:'La rame doit d’abord être envoyée manuellement dans ce dépôt.'};
    if(rame.depotOperationId||this.depotOperations.some((o: { rameId: unknown; state: unknown }) =>o.rameId===rame.id&&o.state==='running'))return {ok:false,reason:'Une opération est déjà en cours sur cette rame.'};
    if(need.def.requires&&!depot.hasInfrastructure(need.def.requires))return {ok:false,reason:`Équipement historique requis : ${need.def.requires}.`};
    const requiredEquipment: string[]=[...new Set<string>((Array.isArray(need.def.equipment)?need.def.equipment:[]).map(String))];
    for(const equipmentId of requiredEquipment){const def=DEPOT_EQUIPMENT_CATALOG[equipmentId],availability=this.getEquipmentAvailability(depot.id,equipmentId);if(availability.installed<=0)return {ok:false,reason:`Équipement requis : ${def?.label||equipmentId}.`};if(availability.free<=0)return {ok:false,reason:`Équipement occupé : ${def?.label||equipmentId}.`};}
    if(staffManager?.checkDepotStaff){const staffCheck=staffManager.checkDepotStaff(depot.id,need.def.staff||{});if(!staffCheck.ok){const txt=staffCheck.shortages.map((x: { label: unknown; free: unknown; need: unknown }) =>`${x.label} : ${x.free}/${x.need} disponible(s)`).join(' · ');return {ok:false,reason:`Personnel insuffisant : ${txt}. Affectez ou embauchez les agents nécessaires dans Personnel.`};}}
    if(need.nothing)return {ok:false,reason:'Aucun complément nécessaire.'};
    for(const [key,qty] of Object.entries(need.resources)){const def=DEPOT_RESOURCE_CATALOG[key];if(def?.stock&&(Number(depot.resourceStocks[key])||0)+1e-9<Number(qty||0))return {ok:false,reason:`Stock insuffisant : ${def.label}.`};}
    for(const [key,qty] of Object.entries(need.parts)){if((Number(depot.partInventory[key])||0)<Number(qty||0)){const p=DEPOT_PART_CATALOG.find((x) =>x.id===key);return {ok:false,reason:`Pièce manquante : ${p?.label||key}.`};}}
    let utilityCost=0;for(const [key,qty] of Object.entries(need.resources)){const def=DEPOT_RESOURCE_CATALOG[key];if(def&&!def.stock)utilityCost+=Number(qty||0)*def.price;}
    if(utilityCost>0&&Number(economy?.balance)<utilityCost)return {ok:false,reason:'Fonds insuffisants pour les fluides/énergies.'};
    const duration=depot.getMaintenanceDuration(need.def.duration);const id=`depot-op-${nextDepotOperationId++}`;
    ...
    if(!staffReservation?.ok)return {ok:false,reason:'Personnel devenu indisponible avant le lancement de l’opération.'};
    ...
    for(const [key,qty] of Object.entries(need.resources)){const def=DEPOT_RESOURCE_CATALOG[key];if(!def)continue;const used=Math.max(0,Number(qty||0));depot.resourceUsage[key]=(Number(depot.resourceUsage[key])||0)+used;if(def.stock)depot.resourceStocks[key]=Math.max(0,(Number(depot.resourceStocks[key])||0)-used);if(key==='water_l')depot.utilityTotals.waterL+=used;if(key==='electricity_kwh')depot.utilityTotals.electricityKWh+=used;}
    if(utilityCost>0){economy.addExpense(utilityCost,'maintenance',`${depot.name} — ${need.def.label} (eau/électricité)`);depot.utilityTotals.expenseEur+=utilityCost;depot.expenseLedger.push({time:Date.now(),label:need.def.label,amount:utilityCost,category:'énergie/eau'});}
    ...
    this.depotOperations.push(op);rame.depotOperationId=id;const slot=depot.findRameTrack(rame.id);if(slot){slot.purpose=need.def.group;slot.operationId=id;}return {ok:true,operation:op,utilityCost};
  }

  updateDepotOperations(dt: unknown, rameManager: RameManager, staffManager: DepotStaffBridge | null=null){
    const step=Math.max(0,Number(dt)||0),finished=[];for(const op of this.depotOperations){if(op.state!=='running')continue;op.remainingMin-=step;if(op.remainingMin<=0){op.remainingMin=0;op.state='done';finished.push(op);}}
    for(const op of finished){const rame=rameManager?.getById?.(op.rameId),depot=this.getDepotById(op.depotId);if(rame){const c=rame.consumables||{};if(op.opId==='refuel')c.fuelL=Number(c.fuelCapacityL||0);else if(op.opId==='sand_fill')c.sandKg=Number(c.sandCapacityKg||0);else if(op.opId==='fluid_service'){c.oilL=Number(c.oilCapacityL||0);c.coolantL=Number(c.coolantCapacityL||0);c.adblueL=Number(c.adblueCapacityL||0);c.gearboxOilL=Number(c.gearboxOilCapacityL||0);c.hydraulicOilL=Number(c.hydraulicOilCapacityL||0);rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-3);}else if(op.opId==='washer_fill')c.washerL=Number(c.washerCapacityL||0);else if(op.opId==='lubrication')rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-2);else if(op.opId==='exterior_wash'||op.opId==='deicing')rame.cleanliness={...(rame.cleanliness||{}),exterior:100};else if(op.opId==='interior_clean'||op.opId==='deep_disinfection')rame.cleanliness={...(rame.cleanliness||{}),interior:100};else if(op.opId==='routine_service'){rame.kmSinceLastMaint=0;rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-25);rame.recommendedMaintenance=false;}else if(op.opId==='brake_overhaul'){rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-12);rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='freins');}else if(op.opId==='wheel_reprofile'){rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-10);}else if(op.opId==='engine_replace'){rame.wearLevel=Math.max(0,Number(rame.wearLevel||0)-20);rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='moteur');}else if(op.opId==='traction_motor_replace')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>!['traction','moteur_traction'].includes(String(x)));else if(op.opId==='pantograph_replace')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='pantographe');else if(op.opId==='compressor_replace')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='compresseur');else if(op.opId==='hvac_replace')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='climatisation');else if(op.opId==='door_repair')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='portes');else if(op.opId==='battery_service')rame.pendingDefects=(rame.pendingDefects||[]).filter((x: unknown) =>x!=='batterie');rame.depotOperationId='';}
      const slot=depot?.findRameTrack?.(op.rameId);if(slot){slot.purpose='garage';slot.operationId='';}
      staffManager?.releaseDepotTask?.(op.id);
    }
    this.depotOperations=this.depotOperations.filter((o: { state: unknown }) =>o.state==='running');return finished;
  }

  getOperationsForDepot(depotId: unknown){return this.depotOperations.filter((o: { depotId: unknown }) =>o.depotId===String(depotId||''));}

  // Add a rescue loco to a depot — max 2 per depot (Annexe 9)
  addRescueLoco(depotId: unknown, stockId: unknown, stockName: unknown, traction: unknown) {
    const depot = this.depots.find((d: { id: unknown }) => d.id === depotId);
    if (depot?.built && depot.type === 'depot' && stockId) {
      if (depot.rescueLocos.length >= 2 || depot.rescueLocos.some((r: __S3Struct61) => r.stockId === stockId) || this.depots.some((d: __S3Struct62) => d.id !== depotId && d.rescueLocos?.some((raw: unknown) => (raw as { stockId?: unknown })?.stockId === stockId))) return false;
      depot.rescueLocos.push({ stockId: stockId as string, stockName: stockName as string, traction: (traction || '') as string, deployed: false });
      return true;
    }
    return false;
  }
  ...
  removeRescueLoco(depotId: unknown, stockId: unknown) {
    ...
    const loco = depot.rescueLocos.find((r: { stockId: unknown }) => r.stockId === stockId);
    if (!loco || loco.deployed || this.activeRescues.some((r: { depotId: unknown; stockId: unknown }) => r.depotId === depotId && r.stockId === stockId)) return false;
    depot.rescueLocos = depot.rescueLocos.filter((r: { stockId: unknown }) => r.stockId !== stockId);
    return true;
  }

  // Find nearest depot that can rescue
  findNearestRescueDepot(world: World, lat: number, lon: number) {
  ...
  dispatchRescue(world: World, brokenService: BrokenServiceForRescue) {
    const targetId = String(brokenService?.id ?? '');
    const targetPosition = rescuePoint(brokenService?.position);
    if (!targetId || !targetPosition) return null;
    ...
    if (this.repairQueue.some(r => r.serviceId === targetId)) return null;
    ...
    if (route?.length > 1) {
      const electCount = route.filter((p: { electrified?: unknown }) => p.electrified !== false).length;
      const nonElectCount = route.filter((p: { electrified?: unknown }) => p.electrified === false).length;
    ...
    if (!rescuePoint(depotOrigin) || !depot.deployRescue(loco.stockId)) return null;
    const repairType = (brokenService?.train?.breakdown?.type || 'moteur') as string;
    const rescue: DepotActiveRescue = {
  ...
  _requestRescueRoute(rescue: DepotActiveRescue, from: { lat?: unknown; lon?: unknown } | null, to: { lat: unknown; lon: unknown }, returning: unknown=false) {
    const origin=rescuePoint(from), target=rescuePoint(to);
    if (!rescue || !this.activeRescues.includes(rescue) || rescue._routeRequestInFlight || rescue._routeAccessDenied || !origin || !target) return false;
    if (Number(rescue._routeRetryAtMs || 0)>Date.now()) return false;
    const phase=returning?'routing_return':'routing';
    rescue.state=phase;rescue.speed=0;
    ...
    const fail=(error:unknown)=>{
      ...
      rescue._routeFailureCount=f.attempts;rescue._routeRetrySec=f.retrySec;rescue._routeRetryAtMs=f.retryAtMs;
      rescue._routeAccessDenied=f.accessDenied;rescue.routeStatusMessage=f.message;
    };
    const orm=globalThis.window?.game?.orm;
    if(!orm?.findRoute){fail('Service de routage indisponible');return false;}
    ...
    Promise.resolve().then(()=>current()?orm.findRoute(origin.lat,origin.lon,target.lat,target.lon):null)
      .then((raw:unknown)=>{
        ...
        if(!route){fail('Tracé refusé : géométrie ou extrémités incohérentes');return;}
        const liveService=!returning ? globalThis.window?.game?.scheduleCreator?.services?.find((s:{id?:unknown})=>s.id===rescue.targetServiceId) : null;
        ...
        if(!movedOrigin || !movedTarget || rescueDistanceKm(origin,movedOrigin)>RESCUE_ENDPOINT_TOLERANCE_KM || rescueDistanceKm(target,movedTarget)>RESCUE_ENDPOINT_TOLERANCE_KM || (liveTarget && rescueDistanceKm(target,liveTarget)>RESCUE_ENDPOINT_TOLERANCE_KM)) {
          if(liveTarget)rescue.targetPosition=liveTarget;
          fail('Position modifiée pendant le calcul : tracé à recalculer');return;
        }
        if(returning)rescue.returnRoute=route;else rescue.route=route;
        rescue.routeIndex=0;rescue._routeRetrySec=0;rescue._routeRetryAtMs=0;rescue._routeFailureCount=0;
        rescue.routeStatusMessage='';rescue.state=returning?'returning':'en_route';
  ...
  resumeRescueRouting(id: unknown): boolean {
    ...
    if(!rescue || !['routing','routing_return'].includes(rescue.state) || rescue._routeRequestInFlight || Number(rescue._routeRetryAtMs||0)>Date.now())return false;
    rescue._routeAccessDenied=false;rescue._routeRetrySec=0;rescue._routeRetryAtMs=0;
  ...
  hasRescuedOccurrence(serviceId: unknown): boolean { return this.rescuedOccurrenceIds.has(String(serviceId)); }
  ...
  reconcileRescuedFormations(): void {
    ...
    if (!rotation?.getVehicle) return;
    this.rescuedFormations = this.rescuedFormations.filter(f => {
      if (this.repairQueue.some(q => q.serviceId === f.serviceId)) return true;
  ...
  isRescueCouplingAuthorized(a: unknown, b: unknown): boolean {
    ...
    if (!r || r.state === 'done' || !r.position) return false;
    const target = this._rescueTarget(r);
    if (!target?.position || target.cancelled || target.completed) return false;
    if (r.towAttached) return rescueDistanceKm(r.position,target.position) <= RESCUE_ENDPOINT_TOLERANCE_KM;
  ...
  getPhysicalRescueService(id: unknown): RescueMovement | undefined { return this._rescueMovements.get(String(id)); }

  /** Occupation liveness remains true during load, before controllers exist. */
  hasPhysicalRescue(id: unknown): boolean { return this.activeRescues.some(r=>r.id===id && r.state!=='done' && !!r.position); }

  getPhysicalRescueServices(): RescueMovement[] {
    for (const rescue of this.activeRescues) this._prepareRescueMovement(rescue);
  ...
  _prepareRescueMovement(rescue: DepotActiveRescue): RescueMovement | null {
    ...
    if (rescue._motionInvalid) {rescue.speed=0;rescue.routeStatusMessage='Secours suspendu : état physique sauvegardé invalide';return null;}
    ...
    if (!route || route.length < 2 || !rescue.position || !game) return controller || null;
    if (!controller) {
      const rames: Rame[] = game.rameManager?.getAll?.() || [];
      ...
      if (!detail || !(Number(detail.power)>0) || !(Number(detail.mass || detail.tonnage)>0) || !(Number(detail.length)>0) || !(Number(detail.maxSpeed)>0)) {
        rescue.speed=0;rescue.routeStatusMessage='Secours retenu : caractéristiques physiques du matériel manquantes';return null;
      }
    ...
    if (controller.routeIdentity !== route || controller.returning !== returning) {
      ...
      if (rescue.route) cantonManager.createRouteCantons(rescue.route);
      const target = this._rescueTarget(rescue);
      if (target?.getCurrentRoute?.()) cantonManager.createRouteCantons(target.getCurrentRoute()!);
      controller.configure(route,rescue.position,returning,target,snapshot);
      if (!snapshot) controller.speed = Math.max(0,Number(rescue.speed) || 0);
      ...
      if (returning && target) {
        ...
        if (!snapshot) controller._carryoverStartTravelKm = controller.totalDistance;
        for (const trc of game.voiePointManager?.troncons || []) {
          if (trc.occupiedBy === target.id) {trc.occupiedBy=controller.id;controller._occupiedTronconIds.add(trc.id);controller._tronconExitTravelKm.set(trc.id,controller.totalDistance);}
          if (trc.reservedBy === target.id) trc.reservedBy=controller.id;
        }
  ...
  _moveRescue(rescue: DepotActiveRescue, dt: number, timeOfDay: number, fleet: ActiveService[]): boolean {
    ...
    if (returning && !rescue.towAttached) return false;
    ...
    if (returning) {
      if (!this._syncRescueTow(rescue,Math.max(0,controller.totalDistance-before))) return false;
      if (controller.arrived) {
        if (this._deliverRescueTow(rescue)) {controller._releaseAllPhysicalResources();this._rescueMovements.delete(rescue.id);}
      }
      ...
      if (!target?.position || !rescue.position || rescueDistanceKm(rescue.position,target.position)>RESCUE_ENDPOINT_TOLERANCE_KM) {
        rescue.state='routing';rescue.route=null;rescue.speed=0;rescue.routeIndex=0;rescue.routeStatusMessage='Le train cible a bougé : nouveau tracé requis';
      } else {rescue.state='recovering';rescue.speed=0;rescue._recoverTimer=5;}
    }
    return true;
  }

  _rescueTarget(rescue: DepotActiveRescue) {
    return globalThis.window?.game?.scheduleCreator?.services?.find((s: {id?: unknown}) => String(s.id) === rescue.targetServiceId) || null;
  }
   ...
   */
  _syncRescueTow(rescue: DepotActiveRescue, movedKm = 0): boolean {
    const target = this._rescueTarget(rescue);
    ...
    if (route && route.length >= 2 && target._state) {
      ...
      if (target._state.cachedRoute === route) {
        ...
        if (!this._rescueMovements.has(rescue.id)) target._syncCantonFootprint?.(route);
    ...
    if (movedKm > 0 && Number.isFinite(movedKm)) {
      ...
      if (material && target.rame && material !== target.rame) {
        for (const key of ['totalKmRun', 'kmSinceLastMaint', 'wearLevel']) target.rame[key] = material[key];
      }
      for (const id of rescue.towVehicleIds || []) {
        const v = game?.rotationV2?.getVehicle?.(id);
        if (v) v.odometerKm = Math.max(0, Number(v.odometerKm) || 0) + movedKm;
      }
    }
    if (material) material.currentLocation = {depotId:'', stationId:'', serviceId:target.id, lat:rescue.position.lat, lon:rescue.position.lon};
    target._movementStop?.('RESCUE_TOW', 'Traction propre neutralisée : remorquage', 'depot');
    return true;
  }
  ...
  _deliverRescueTow(rescue: DepotActiveRescue): boolean {
    ...
    if (!rescue.towAttached || !target?.train || !depot?.built || !rescue.position || !rescue.depotPosition ||
      ...
      rescue.speed = 0; rescue.routeStatusMessage = 'Réception au dépôt suspendue : convoi ou dépôt indisponible'; return false;
    }
    if (!material || (rescue.towVehicleIds || []).some(id => !game?.rotationV2?.getVehicle?.(id))) { rescue.speed = 0; rescue.routeStatusMessage = 'Réception suspendue : matériel introuvable'; return false; }
    ...
    if (material) {
      ...
      if (!result.ok) { rescue.speed = target.speed = target.train.speed = 0; rescue.routeStatusMessage = result.reason || 'Dépôt indisponible'; return false; }
      ...
      if (target.rame && target.rame !== material) { target.rame.currentLocation = {...material.currentLocation}; target.rame.inMaintenance = true; }
    }
    ...
    for (const id of rescue.towVehicleIds || []) {
      const v = game?.rotationV2?.getVehicle?.(id);
      if (v) { v.available = false; v.location = {kind:'DEPOT', id:depot.id, depotId:depot.id, lat:rescue.depotPosition.lat, lon:rescue.depotPosition.lon}; }
    }
    target._releaseAllPhysicalResources?.();
    target._finishLegacyOperatingDay?.();
    target.speed = target.train.speed = 0;
    target.completed = true; target.cancelled = true; target.state = 'cancelled';
    target.train.inDepot = true; target.train.inMaintenance = true;
    target.train.state = 'au dépôt'; target.train.delayReason = 'Service interrompu — rame remorquée au dépôt';
    target.train.stoppedAt = null; target.position = null;
    ...
    if (target._v2OccurrenceId) { target._v2MaterialFinalized = true; this.rescuedOccurrenceIds.add(target.id); }
    if (!this.repairQueue.some(q => q.serviceId === rescue.targetServiceId)) this.repairQueue.push({
    ...
    rescue.state = 'done'; rescue.speed = 0; depot.returnRescue(rescue.stockId);
    game?.scheduleCreator?._invalidateActiveCache?.();
    return true;
  }

  // Update active rescues (called each move tick)
  updateRescues(dt: number, timeOfDay = Number(globalThis.window?.game?.timeOfDay || 0), physicalFleet?: ActiveService[]) {
    if(!Number.isFinite(dt) || dt<=0)return;
    this.reconcileRescuedFormations();
    const fleet = physicalFleet || [...(globalThis.window?.game?.scheduleCreator?.services || []).filter((s: ActiveService)=>!this.ownsServiceMovement(s.id)),...this.getPhysicalRescueServices()];
    for (const rescue of this.activeRescues) {
      ...
      if (rescue.state === 'routing' || rescue.state === 'routing_return') {
        ...
        if (!rescue._routeAccessDenied && !rescue._routeRequestInFlight && rescue._routeRetrySec <= 0 && Number(rescue._routeRetryAtMs||0)<=Date.now()) {
          const liveService = rescue.state === 'routing' ? globalThis.window?.game?.scheduleCreator?.services?.find((s:{id?:unknown})=>s.id===rescue.targetServiceId) : null;
          const liveTarget=rescuePoint(liveService?.position);
          if(liveTarget)rescue.targetPosition=liveTarget;
      ...
      if (rescue.state === 'en_route') {
        ...
        if(actualTarget && rescueDistanceKm(actualTarget,rescue.targetPosition)>RESCUE_ENDPOINT_TOLERANCE_KM) {
          rescue.targetPosition=actualTarget;rescue.state='routing';rescue.route=null;rescue.speed=0;rescue.routeIndex=0;
          rescue.movementSnapshot=undefined;const old=this._rescueMovements.get(rescue.id);if(old){old.speed=old.train.speed=0;}
          rescue.routeStatusMessage='Train cible déplacé : recalcul du trajet depuis la position réelle';continue;
        }
        if (!rescue.route || rescue.route.length < 2) {
          rescue.state='routing';rescue.speed=0;rescue._routeRetrySec=0;continue;
        }
        this._moveRescue(rescue,dt,timeOfDay,fleet);
      } else if (rescue.state === 'recovering') {
        const target=globalThis.window?.game?.scheduleCreator?.services?.find((s:{id?:unknown})=>s.id===rescue.targetServiceId);
        const position=rescuePoint(target?.position);
        if(position && rescueDistanceKm(rescue.position,position)>RESCUE_ENDPOINT_TOLERANCE_KM) {
          rescue.targetPosition=position;rescue.state='routing';rescue.speed=0;rescue.route=null;rescue.routeIndex=0;rescue._recoverTimer=5;
          rescue.routeStatusMessage='Accostage interrompu : le train cible a bougé';continue;
        }
        ...
        if (rescue._recoverTimer <= 0) {
          if (!this._attachRescueTow(rescue)) continue;
        ...
        if (!rescue.towAttached) {
          ...
          if (target) {
            rescue.targetPosition = target; rescue.state = 'routing'; rescue.route = null;
            rescue.returnRoute = null; rescue.routeIndex = 0; rescue._routeRetrySec = 0;
            rescue.routeStatusMessage = 'Ancien retour sans train : reprise du secours à la position réelle';
          } else rescue.routeStatusMessage = 'Ancien retour suspendu : train cible absent';
          continue;
        }
        ...
        if (!towTarget?.position || towTarget.cancelled || towTarget.completed || !rescue.depotPosition || !this.getDepotById(rescue.depotId)?.built) {
          rescue.speed = 0; rescue.routeStatusMessage = 'Remorquage suspendu : train cible ou dépôt absent'; continue;
        }
        ...
        if (rescueDistanceKm(rescue.position, towTarget.position) > RESCUE_ENDPOINT_TOLERANCE_KM ||
          ...
          rescue.routeStatusMessage = 'Remorquage suspendu : localisation ou composition du matériel incohérente'; continue;
        }
    ...
    this.activeRescues = this.activeRescues.filter((r: { state: unknown }) => r.state !== 'done');
  }
  ...
  updateRepairs(dt: unknown) {
    ...
    for (const r of finished) {
      ...
      const depot = this.depots.find((d: { id: unknown }) => d.id === r.depotId);
      const type = r.repairType || 'moteur';
      const needed: Record<string,number> = { moteur: 1, freins: 1, climatisation: 1, portes: 1, fanaux: 1 };
      ...
      if (r.remainingMin <= 0) {
        this.repairQueue = this.repairQueue.filter((q: { serviceId: unknown }) => q.serviceId !== r.serviceId);
      }
    }
    const finishedM = [];
    ...
    for (const m of finishedM) {
      this.maintenanceQueue = this.maintenanceQueue.filter((q: { rameId: unknown }) => q.rameId !== m.rameId);
    }
    return {
      repaired: finished.filter((r: __S3Struct66) => r.remainingMin <= 0).map((r: DepotRepairQueueEntry) => ({ serviceId: r.serviceId, depotId: r.depotId, repairType: r.repairType, rameId:r.rameId, vehicleIds:[...(r.vehicleIds || [])] })),
      maintainedIds: finishedM.map((m: { rameId: unknown }) => m.rameId),
    };
  }

  // MNT-06 : vérification mensuelle de la maintenance préventive recommandée
  checkPreventiveMaintenance(dateStr: string, rameManager: RameManager) {
  ...
  getLowStockDepots(threshold: unknown = 2) {
    const result = [];
    for (const depot of this.depots) {
  ...
  buyBulkSpareParts(type: unknown, qty: number, economy: Economy) {
    qty = Number(qty);
    if (!Number.isInteger(qty) || qty <= 0 || !economy) return { ok: false, totalCost: 0, invalid: true };
    ...
    const targets = this.depots.filter((d: { type: unknown; built: unknown }) => d.type === 'depot' && d.built);
  ...
  sendRameToMaintenance(rameId: unknown, rameName: unknown, depotId: unknown) {
    rameId=String(rameId??''); depotId=String(depotId??'');
    if(!rameId||!depotId||this.maintenanceQueue.some((m: { rameId: unknown }) => m.rameId === rameId)) return false;
    const depot = this.depots.find((d: { id: unknown }) => d.id === depotId);
    if (!depot || depot.type !== 'depot' || !depot.built) return false;
    const game = typeof window !== 'undefined' ? window.game : null;
    const rame = game?.rameManager?.getById?.(rameId);
    if (rame) {
      const inService = (game?.scheduleCreator?.services || []).some((s: {rame?: {id?: unknown}; _v2DirectRameId?: unknown; _v2AssignedRameId?: unknown; completed?: unknown; cancelled?: unknown; position?: unknown; state?: string; currentStopIndex?: unknown}) =>
        (s.rame?.id === rameId || s._v2DirectRameId === rameId || s._v2AssignedRameId === rameId) &&
        !s.completed && !s.cancelled && s.position && (s.state === 'moving' || s.state === 'departing' || Number(s.currentStopIndex) > 0));
      if (inService || (rame.currentLocation?.depotId !== depotId && !depot.findRameTrack?.(rameId))) return false;
    }
    const duration = depot.getMaintenanceDuration(20);
    this.maintenanceQueue.push({
      rameId: rameId as string,
      depotId: depotId as string,
      remainingMin: duration,
      totalMin: duration,
      rameName: (rameName || rameId) as string,
    });
    return true;
  }

  isRameInMaintenance(rameId: unknown) {
    return this.maintenanceQueue.some((m: { rameId: unknown }) => m.rameId === rameId);
  }

  getRameMaintenanceInfo(rameId: unknown) {
    return this.maintenanceQueue.find((m: { rameId: unknown }) => m.rameId === rameId) || null;
  }

  isInRepairOrMaintenance(serviceId: unknown) {
    return this.repairQueue.some((r: { serviceId: unknown }) => r.serviceId === serviceId);
  }

  getRepairInfo(serviceId: unknown) {
    return this.repairQueue.find((r: { serviceId: unknown }) => r.serviceId === serviceId) || null;
  }

  // Get active rescues as pseudo-services for rendering on map
  getRescueServices() {
    return this.activeRescues.filter((r: __S3Struct69) => r.position).map((r) => ({
      id: r.id,
  ...
  toSave() {
    return {
      ...
      depots: this.depots.map((d: Depot) => ({
        id: d.id,
        type: d.type,
        name: d.name,
  ...
  loadFromSave(data: unknown) {
    for (const s of this._rescueMovements.values()) s._releaseAllPhysicalResources();
    ...
    const obj: Record<string, unknown> = data&&typeof data==='object'&&!Array.isArray(data) ? data as Record<string, unknown> : {};
    this.rescuedOccurrenceIds = new Set(Array.isArray(obj.rescuedOccurrenceIds) ? obj.rescuedOccurrenceIds.filter((x:unknown): x is string => typeof x === 'string' && x.startsWith('v2:')) : []);
    ...
    for (const d of arr) {
      ...
      if (!depot.id || seenDepotIds.has(depot.id) || !depot.stationId || !['depot','ite-fret','ite-industrie','ite-logistique'].includes(depot.type)) continue;
      seenDepotIds.add(depot.id); this.depots.push(depot);
    ...
    if (!Array.isArray(data) && Array.isArray(obj.activeRescues)) {
      ...
      const validCoord=(p: unknown): p is {lat:number;lon:number} => rescuePoint(p)!==null;
      const seenRescueIds=new Set(), seenRescueLocos=new Set();
      this.activeRescues = obj.activeRescues.filter((r: unknown) => r && typeof r === 'object' && !Array.isArray(r)).map((raw: unknown) => {
        const r = raw as Record<string, unknown>;
        const depotId=String(r.depotId??''),stockId=String(r.stockId??''),id=String(r.id??''); if(!depotId||!stockId||!id||seenRescueIds.has(id)||seenRescueLocos.has(`${depotId}|${stockId}`))return null;
        seenRescueIds.add(id); seenRescueLocos.add(`${depotId}|${stockId}`);
        const rawState=String(r.state||''); const state=rescueStates.has(rawState)?rawState:'routing'; const finite=(v: unknown,fb=0)=>Number.isFinite(Number(v))?Number(v):fb;
      ...
      for (const r of this.activeRescues) {
        ...
        if(route && endpoint && rescueDistanceKm(route[route.length-1]!,endpoint)>RESCUE_ENDPOINT_TOLERANCE_KM) {
          if(r.state==='returning')r.returnRoute=null;else r.route=null;
          route=null;
        }
        if(r._routeAccessDenied && !r._routeRetryAtMs)r._routeRetryAtMs=Date.now()+900000;
        if((r.state==='en_route'||r.state==='returning') && !route) {
          r.state=r.state==='returning'?'routing_return':'routing';r.speed=0;r.routeIndex=0;
          ...
          if(r.state==='en_route'||r.state==='returning') {
            r.state=r.state==='returning'?'routing_return':'routing';r.speed=0;
            r.routeStatusMessage='Progression sauvegardée incohérente : recalcul nécessaire';
          }
          r.routeIndex=0;
        } else if(route)r.routeIndex=Math.min(r.routeIndex,route.length-1);
    ...
    if (!Array.isArray(data) && Array.isArray(obj.maintenanceQueue)) {
      ...
      this.maintenanceQueue = obj.maintenanceQueue
        ...
        .map((m: { rameId: unknown; serviceId: unknown; depotId: unknown; remainingMin: unknown; totalMin: unknown; rameName: unknown; serviceName: unknown }) => { const rameId=String(m.rameId??m.serviceId??''), depotId=String(m.depotId??''), remaining=Number(m.remainingMin), total=Number(m.totalMin); return {rameId,depotId,remainingMin:remaining,totalMin:Number.isFinite(total)&&total>0?total:20,rameName:String(m.rameName??m.serviceName??'')}; })
        .filter((m: __S3Struct72) => m.rameId && !seenRames.has(m.rameId) && Number.isFinite(m.remainingMin) && m.remainingMin>0 && (seenRames.add(m.rameId), true));
    } else {
    ...
    if (!Array.isArray(data) && Array.isArray(obj.repairQueue)) {
      ...
      this.repairQueue = obj.repairQueue
        ...
        .map((r: Record<string, unknown>) => { const serviceId=String(r.serviceId??''), depotId=String(r.depotId??''), remaining=Number(r.remainingMin), total=Number(r.totalMin); return {serviceId,depotId,rameId:typeof r.rameId==='string'?r.rameId:'',vehicleIds:Array.isArray(r.vehicleIds)?r.vehicleIds.filter((v:unknown):v is string=>typeof v==='string'):[],remainingMin:remaining,totalMin:Number.isFinite(total)&&total>0?total:30,serviceName:String(r.serviceName??''),repairType:['moteur','freins','climatisation','portes','fanaux'].includes(String(r.repairType??r.type))?String(r.repairType??r.type):'moteur'}; })
        .filter((r: __S3Struct74) => r.serviceId && !seenServices.has(r.serviceId) && Number.isFinite(r.remainingMin) && r.remainingMin>0 && (seenServices.add(r.serviceId), true));
    } else {
      this.repairQueue = [];
    }
    ...
    if (!Array.isArray(data) && Array.isArray(obj.depotOperations)) {
      ...
      this.depotOperations=obj.depotOperations.filter((o: unknown) =>o&&typeof o==='object').map((o: { id: unknown; depotId: unknown; rameId: unknown; remainingMin: unknown; totalMin: unknown; rameName: unknown; opId: unknown; label: unknown; group: unknown; staff: unknown; resources: unknown; parts: unknown; equipment: unknown; staffIds: unknown }) =>{
        ...
        if(!id||!depotId||!rameId||seenOps.has(id)||remaining<=0)return null;seenOps.add(id);
        const m=/^depot-op-(\d+)$/.exec(id);if(m&&Number(m[1])>=nextDepotOperationId)nextDepotOperationId=Number(m[1])+1;
        return {...o,id,depotId,rameId,rameName:String(o.rameName||''),opId:String(o.opId||''),label:String(o.label||''),group:String(o.group||''),remainingMin:remaining,totalMin:total,state:'running',staff:o.staff&&typeof o.staff==='object'?{...o.staff}:{},resources:o.resources&&typeof o.resources==='object'?{...o.resources}:{},parts:o.parts&&typeof o.parts==='object'?{...o.parts}:{},equipment:Array.isArray(o.equipment)?o.equipment.filter((x: unknown) =>DEPOT_EQUIPMENT_CATALOG[String(x)]):[],staffIds:Array.isArray(o.staffIds)?o.staffIds.map(String).filter(Boolean):[]};
      }).filter(Boolean) as DepotOperation[];
    }
    ...
    this.activeRescues = this.activeRescues.filter((r) => {
      if(r.state==='done'||(r.targetServiceId && seenTargets.has(r.targetServiceId)))return false;
      const depot = this.getDepotById(r.depotId);
      const valid=!!(depot?.built && depot.type === 'depot' && depot.rescueLocos?.some((l: { stockId: unknown }) => l.stockId === r.stockId));
      if(valid && r.targetServiceId)seenTargets.add(r.targetServiceId);
      return valid;
    });
    const validBuiltDepotIds=new Set(this.depots.filter((d: { built: unknown }) =>d.built).map((d: { id: unknown }) =>d.id));
    const formationIds = new Set<string>();
    for (const value of Array.isArray(obj.rescuedFormations) ? obj.rescuedFormations : []) {
      if (!value || typeof value !== 'object' || Array.isArray(value)) continue;
      const f = value as Record<string, unknown>, id=String(f.id || ''), depotId=String(f.depotId || '');
      const vehicleIds = Array.isArray(f.vehicleIds) ? [...new Set(f.vehicleIds.filter((v: unknown): v is string => typeof v === 'string' && !!v))] : [];
      if (!id || formationIds.has(id) || !validBuiltDepotIds.has(depotId) || !vehicleIds.length || !this.getDepotById(depotId)?.findRameTrack(id)) continue;
      formationIds.add(id); this.rescuedFormations.push({id,depotId,name:String(f.name || id),serviceId:String(f.serviceId || ''),vehicleIds});
    }
    ...
    this.depotOperations=this.depotOperations.filter((o: { depotId: unknown }) =>validBuiltDepotIds.has(o.depotId));
    const opByRame=new Map<string,DepotOperation>(this.depotOperations.map((o) =>([String(o.rameId),o] as [string, DepotOperation])));
    for(const depot of this.getDepots()){
      if(!Array.isArray(depot.trackOccupancy)||depot.trackOccupancy.length!==depot.tracks) depot.trackOccupancy=Array.from({length:depot.tracks},(_: unknown,i)=>({track:i+1,rameId:'',purpose:'garage',operationId:''}));
      for(const slot of depot.trackOccupancy){if(slot.rameId){const op=opByRame.get(slot.rameId);slot.operationId=op?.id||'';slot.purpose=op?.group||'garage';}}
    }
    const rescueKeys = new Set(this.activeRescues.map((r: { depotId: unknown; stockId: unknown }) => `${r.depotId}|${r.stockId}`));
    for (const depot of this.depots) for (const loco of depot.rescueLocos || []) {
...
// S3_STRUCT_V2_TEMP
type __S3Struct43 = { "name": string; "instanceName": string; "seriesName": string };
type __S3Struct44 = { "route": unknown[]; "startBinding": unknown; "endBinding": unknown };
type __S3Struct45 = { "length": number };
type __S3Struct46 = { "route": unknown[]; "startBinding": unknown; "endBinding": unknown };
type __S3Struct47 = { "name": string };
type __S3Struct48 = { "toLowerCase": (...args: unknown[]) => unknown };
type __S3Struct53 = { "rame": { "id": string }; "completed": number; "cancelled": number; "state": string };
type __S3Struct54 = { "id": string; "depotOperationId": string; "currentLocation": unknown; "inMaintenance": number };
type __S3Struct61 = { "stockId": string };
type __S3Struct62 = { "id": string; "rescueLocos": unknown[] };
type __S3Struct66 = { "remainingMin": number };
type __S3Struct67 = { "serviceId": string; "depotId": string; "repairType": string };
type __S3Struct69 = { "position": unknown };
type __S3Struct70 = { "id": string; "stockName": string; "position": unknown; "state": string; "speed": number };
type __S3Struct72 = { "rameId": string; "remainingMin": number };
type __S3Struct73 = { "serviceId": string; "depotId": string; "remainingMin": number; "totalMin": number; "serviceName": string; "repairType": string; "type": string };
type __S3Struct74 = { "serviceId": string; "remainingMin": number };
type __S3Struct75 = { "rameId": string };
