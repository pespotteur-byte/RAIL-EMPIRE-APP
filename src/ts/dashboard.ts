import {renderInteractiveChart, type ChartPoint, type ChartSeries, type ChartView} from './dashboard-chart.js';
import {renderFinancialPanel, type FinanceView} from './financial-panel.js';
import { htmlText } from './html-text.js';
type __KPM10 = unknown[];
type __KPM19 = { "punctualityHistory": unknown; "revenueHistory": unknown; "passengerHistory": unknown; "kmHistory": unknown; expenseHistory?: unknown; lastRecordBucket?: unknown };
type __KPA3 = { "id": unknown; "name": unknown; "code": unknown };
type __KPA4 = { "revenue": number; "expense": number };
type __KPA5 = { "profit": number };
type __KPA6 = { "profit": number };
type __KPA8 = { "impact": unknown; "speedLimit": unknown; "route": unknown[]; "workName": unknown; "name": unknown; "zoneId": unknown; "stationA": unknown; "stationB": unknown };
type __KPA10 = { "name": unknown; "code": unknown; "revenue": unknown; "expense": unknown; "profit": number; "margin": number };
...
export class Dashboard {
    ...
    private _chartViews: Record<string, ChartView> = {};
    private _financeView: FinanceView = {page:0,query:'',type:'',from:'',to:''};
    private _chart(canvasId: string, rows: readonly ChartSeries[], unit: string, options: {bars?: boolean; min?: number; max?: number} = {}) {
        const canvas = document.getElementById(canvasId) as HTMLCanvasElement | null;
        if (!canvas) return;
        const state = this._chartViews[canvasId] ||= {start:0,end:0,cursor:0,follow:true};
        renderInteractiveChart(canvas,rows,unit,state,options);
    }
    punctualityHistory: Array<{ time: string; value: number }>;
    revenueHistory: Array<{ time: string; value: number }>;
    passengerHistory: Array<{ time: string; value: number }>;
    kmHistory: Array<{ time: string; value: number }>;
    ...
    record(game: RailEmpire) {
        ...
        const movingServices = activeServices.filter((s: { state: unknown }) => s.state === 'moving');
        ...
        while(this.expenseHistory.length < this.revenueHistory.length) this.expenseHistory.push({time:this.revenueHistory[this.expenseHistory.length].time,value:null});
    ...
    _push(arr: __KPM10, entry: unknown) {
        arr.push(entry);
    ...
    render(container: HTMLElement, game: RailEmpire) {
        ...
        const activeServices = game.scheduleCreator.getActiveServices();
        const movingServices = activeServices.filter((s: { state: unknown }) => s.state === 'moving');
        const waitingServices = activeServices.filter((s: { state: unknown }) => s.state === 'waiting');
        ...
        const lineProfit = lines.map((l: __KPA3) => {
            const p = eco.getLineProfitability(String(l.id || ''));
            return { name: l.name, code: l.code, ...p };
        }).filter((l: __KPA4) => l.revenue > 0 || l.expense > 0).sort((a: __KPA5, b: __KPA6) => b.profit - a.profit);
        ...
        const fmtE = (n: unknown) => fmt(Number(n) || 0) + ' \u20ac';
        ...
        const worksBulletins = worksRestrictions.map((w: __KPA8) => {
      ...
      <div class="dash-section">
        ...
        <details style="font-size:11px;color:var(--text3)">
          ...
          <div style="margin-top:8px;display:flex;flex-direction:column;gap:6px">
            ${activeServices.slice(0, 8).map((svc: ActiveService) => {
    ...
    `;
      ...
      <div class="dash-section">
        ...
        <div class="dash-train-table">
          ...
          ${lineProfit.map((l: __KPA10) => `<div class="dash-train-row" style="grid-template-columns:2fr 1fr 1fr 1fr 1fr">
      ...
      <div class="dash-section">
        ...
        <div class="dash-train-table">
          ...
          ${activeServices.map((svc: ActiveService) => {
    ...
    `;
        ...
        if(financeHost) renderFinancialPanel(financeHost,eco,game.bank,this._financeView);
        // Draw charts
     ...
     */
    _drawLineChart(canvasId: string, data: Array<{ time: string; value: number }>, unit: string, color: string, fixedMin: number | undefined = undefined, fixedMax: number | undefined = undefined) {
    ...
    _drawHorizontalBar(canvasId: string, data: Array<{ label: string; value: number; color: string }>) {
        const canvas=document.getElementById(canvasId) as HTMLCanvasElement|null;
        if(!canvas||!data.length)return;
        const ctx=canvas.getContext('2d');if(!ctx)return;
        const total=data.reduce((sum,d)=>sum+d.value,0);if(!(total>0))return;
        const width=Math.max(160,canvas.clientWidth||600),dpr=Math.max(1,window.devicePixelRatio||1);
        canvas.style.width='100%';canvas.style.height='70px';canvas.width=Math.round(width*dpr);canvas.height=Math.round(70*dpr);
        canvas.tabIndex=0;canvas.style.cursor='grab';canvas.setAttribute('aria-label','Ventilation financière : glisser pour lire une catégorie, flèches pour changer de catégorie.');
        const view=this._chartViews[canvasId] ||= {start:0,end:data.length-1,cursor:0,follow:false};
        ...
        const output=document.createElement('output');output.className='re-chart-readout';
        const legend=document.createElement('div');legend.className='re-chart-help';
        legend.textContent=data.map(d=>`${d.label} : ${d.value.toLocaleString('fr-FR')} €`).join(' · ');
        controls.append(output,legend);canvas.insertAdjacentElement('afterend',controls);
        const draw=()=>{
            ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,70);let x=10;
            for(let i=0;i<data.length;i++){
                const d=data[i],w=d.value/total*(width-20);ctx.fillStyle=d.color;ctx.fillRect(x,15,w,30);
                if(w>40){ctx.fillStyle='#fff';ctx.font='bold 11px sans-serif';ctx.textAlign='center';ctx.fillText((d.value/total*100).toFixed(1)+'%',x+w/2,34);}
                if(i===view.cursor){ctx.beginPath();ctx.arc(x+w/2,50,5,0,Math.PI*2);ctx.fillStyle=d.color;ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.stroke();}
                x+=w;
            }
            const selected=data[view.cursor];output.textContent=`${selected.label} : ${selected.value.toLocaleString('fr-FR',{maximumFractionDigits:2})} € · ${(selected.value/total*100).toFixed(2)} % · saisir la poignée pour lire`;
        };
        let dragging=false;
        const select=(event:PointerEvent)=>{const r=canvas.getBoundingClientRect(),fraction=Math.max(0,Math.min(1,((event.clientX-r.left)*width/Math.max(1,r.width)-10)/(width-20)));let sum=0;view.cursor=data.length-1;for(let i=0;i<data.length;i++){sum+=data[i].value/total;if(fraction<=sum){view.cursor=i;break;}}draw();};
        canvas.onpointerdown=event=>{if(event.button!==0)return;dragging=true;controls.dataset.reInteracting='true';canvas.setPointerCapture?.(event.pointerId);select(event);};
        canvas.onpointermove=event=>{if(dragging)select(event);};
        const release=()=>{dragging=false;delete controls.dataset.reInteracting;};canvas.onpointerup=release;canvas.onpointercancel=release;canvas.onlostpointercapture=release;
        canvas.onkeydown=event=>{if(event.key!=='ArrowLeft'&&event.key!=='ArrowRight')return;event.preventDefault();view.cursor=Math.max(0,Math.min(data.length-1,view.cursor+(event.key==='ArrowRight'?1:-1)));draw();};
        draw();
    }
    _drawBarChart(canvasId: string, data: Array<{ label: string; value: number }>, unit: string) {
    ...
    loadFromSave(s: __KPM19) {
        if (!s || typeof s !== 'object')
            return;
        const hist = (raw: unknown, min = -Infinity, max = Infinity, retain = this.maxEntries) => (Array.isArray(raw) ? raw : []).filter((x) => x && Number.isFinite(Number(x.value))).slice(-retain).map((x) => ({ time: typeof x.time === 'string' ? x.time : '--:--', value: Math.max(min, Math.min(max, Number(x.value))) }));
