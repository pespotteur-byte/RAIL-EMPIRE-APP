...
type WeatherEffect = { speedMult: number; icon: string; label: string; color: string };
type WmoWeather = { type: string; desc: string };
type WeatherPointState = RailWeatherState & { label?: string; live?: boolean; apparentTemp?: number; windDirection?: number; pressure?: number; cloudCover?: number; dewpoint?: number; risk?: unknown; lat?: number; lon?: number };
type PointQueueItem = { key: string; lat: number; lon: number; ts: number };
type PointCacheEntry = { state: WeatherPointState; expiresAt: number };
type ForecastRow = Record<string, unknown>;
type ForecastCacheEntry = { rows: ForecastRow[]; expiresAt: number };
type GridCacheEntry = { points: WeatherPointState[]; expiresAt: number };

export class Weather {
    declare current: string;
    declare temperature: number;
    declare windSpeed: number;
    declare windGust: number;
    declare humidity: number;
    declare precipitation: number;
    declare rain: number;
    declare showers: number;
    declare snowfall: number;
    declare cloudCover: number;
    declare cloudLow: number;
    declare cloudMid: number;
    declare cloudHigh: number;
    declare pressure: number;
    declare apparentTemp: number;
    declare windDirection: number;
    declare visibility: number;
    declare uvIndex: number;
    declare dewpoint: number;
    declare season: string;
    declare locationName: string;
    declare _lastFetchTime: number;
    declare _fetchCooldown: number;
    declare _lastLat: number;
    declare _lastLon: number;
    declare _fetchInProgress: boolean;
    declare _liveDataAvailable: boolean;
    declare _wmoDescription: string;
    declare weatherCode: number;
    declare _mapView: WeatherMapView | null;
    declare _forecastCache: Map<string, ForecastCacheEntry>;
    declare _gridCache: Map<string, GridCacheEntry>;
    declare radarTimestamps: string[];
    declare radarHost: string;
    declare cloudTimestamps: string[];
    declare _lastRadarFetch: number;
    declare _radarFetchCooldown: number;
    declare _pointCache: Map<string, PointCacheEntry>;
    declare _pointFetchQueue: PointQueueItem[];
    declare _maxPointCache: number;
    declare _pointCooldown: number;
    declare _pointFetchTimer: number | null;
    declare _effects: Record<string, WeatherEffect>;
    declare _wmoMapping: Record<number, WmoWeather>;
    declare _lastForecast: ForecastRow[];
    // A load invalidates in-flight observations from the previous world.
    private _observationGeneration = 0;
    private _historicalReplay(): boolean {
    ...
    constructor() {
        ...
        this.weatherCode = 0;
        this._mapView = null;
        this._forecastCache = new Map<string, ForecastCacheEntry>();
        this._gridCache = new Map<string, GridCacheEntry>();
        ...
        this._pointCache = new Map<string, PointCacheEntry>();
     ...
     */
    update(timeOfDay: unknown, dateStr: string, lat: number, lon: number) {
        lat = Number(lat);
        lon = Number(lon);
        // Season from date
        if (dateStr) {
        ...
        if (this._historicalReplay()) return;
    ...
    async _fetchLiveWeather(lat: number, lon: number) {
        if (this._historicalReplay()) return;
        ...
        try {
            ...
            if (generation !== this._observationGeneration || this._historicalReplay()) return;
            if (data.current) {
                const f = (v: unknown, fb: number, min = -Infinity, max = Infinity) => { const n = Number(v); return Number.isFinite(n) ? Math.max(min, Math.min(max, n)) : fb; };
            ...
            if (generation !== this._observationGeneration || this._historicalReplay()) return;
            console.warn('Open-Meteo fetch failed, using fallback:', (e as { message?: string }).message);
            this._liveDataAvailable = false;
    ...
    async _fetchRadarTimestamps() {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.radar?.past) {
                this.radarTimestamps = data.radar.past.map((p: { path: string }) => p.path);
                this.radarHost = data.host || 'https://tilecache.rainviewer.com';
            }
            // Infrared satellite (cloud cover) data
            if (data.satellite?.infrared) {
                this.cloudTimestamps = data.satellite.infrared.map((p: { path: string }) => p.path);
            }
        }
        catch (e) {
            console.warn('RainViewer fetch failed:', (e as { message?: string }).message);
        }
    }
    getLatestRadarPath() {
    ...
    getRadarTileUrl(path: unknown) {
        if (!path)
            return null;
        return `${this.radarHost}${path}/256/{z}/{x}/{y}/2/1_1.png`;
    }
    getCloudTileUrl(path: unknown) {
        if (path) {
    ...
    _fallbackWeather() {
        ...
        const r = rng.random();
        let cumul = 0;
        for (const [type, prob] of Object.entries(p) as [string, number][]) {
            cumul += prob;
            if (r <= cumul) {
                this.current = type;
                return;
    ...
    _pointKey(lat: number, lon: number) {
        const k = 100;
    ...
    getAt(lat: number, lon: number) {
        ...
        if (cached) return cached.state; // stale local weather is preferable to another city's weather
        if (!this._liveDataAvailable) return this._globalPointState(); // explicit offline simulation model
    ...
    _queuePointFetch(lat: number, lon: number) {
        if (this._historicalReplay()) return;
        lat = Number(lat);
        lon = Number(lon);
        if (!Number.isFinite(lat) || !Number.isFinite(lon) || lat < -90 || lat > 90 || lon < -180 || lon > 180)
            return;
        const key = this._pointKey(lat, lon);
        if (this._pointFetchQueue.some((p: { key: unknown }) => p.key === key))
            return;
    ...
    async _processPointFetchQueue() {
        const generation = this._observationGeneration;
        this._pointFetchTimer = null;
        if (this._historicalReplay()) { this._pointFetchQueue = []; return; }
    ...
    async _fetchLiveWeatherFor(lat: number, lon: number) {
        ...
        let hi = ht.findIndex((t: unknown) => String(t).slice(0, 13) === currentHour);
        if (hi < 0)
            hi = Math.max(0, ht.length - 1);
        const sumBack = (hours: number) => { let total = 0; for (let i = Math.max(0, hi - hours + 1); i <= hi; i++) {
            const v = Number(hp[i]);
     ...
     *  - type: weather type
     */
    getSpeedEffectsAt(lat: number, lon: number, trainSpeedKmh: number = 0) {
    ...
    getRailRiskAt(lat: number, lon: number, trainSpeedKmh: number = 0) {
        const state = this.getAt(lat, lon);
    ...
    _normalizeHourlyForecast(hourly: Record<string, unknown> = {}) {
        ...
        return times.map((time: unknown, i: number) => { const row: AnyRecord = { time }; for (const k of keys)
            row[k] = Array.isArray(hourly[k]) ? hourly[k][i] : null; return row; }).filter((x) => Boolean(x.time));
    }
    getForecast() { return Array.isArray(this._lastForecast) ? this._lastForecast : []; }
    async fetchForecastAt(lat: number, lon: number) {
        lat = Number(lat);
        lon = Number(lon);
    ...
    async searchLocation(query: unknown) {
        ...
        if (!resp.ok)
            throw new Error(`HTTP ${resp.status}`);
        const data = await resp.json();
        return (data.results || []).map((r: { name: unknown; country: unknown; admin1: unknown; latitude: unknown; longitude: unknown; timezone: unknown }) => ({ name: r.name, country: r.country || '', admin1: r.admin1 || '', lat: Number(r.latitude), lon: Number(r.longitude), timezone: r.timezone || '' })).filter((r: { lat: unknown; lon: unknown }) => Number.isFinite(r.lat) && Number.isFinite(r.lon));
    }
    async fetchGrid(bounds: Record<string, unknown>, layer: string = 'temperature', cols: number = 7, rows: number = 5) {
        const b = bounds || {};
    ...
    _seasonLabel() {
        const labels: AnyRecord = { spring: 'Printemps', summer: 'Été', autumn: 'Automne', winter: 'Hiver' }; return labels[this.season] || '';
    }
    renderWidget() {
    ...
    render(container: unknown, game: unknown = null) {
        if (!container)
            return;
        if (!this._mapView)
        ...
        if (!this.radarTimestamps.length && Date.now() - this._lastRadarFetch > 30000)
            this._fetchRadarTimestamps();
        this._mapView.render(container as HTMLElement, game as Parameters<WeatherMapView['render']>[1]);
    }
    toSave() {
        return {
    ...
    loadFromSave(s: Record<string, unknown>) {
        if (!s || typeof s !== 'object')
            return;
        this._observationGeneration++;
        const finite = (v: unknown, fb: number, min = -Infinity, max = Infinity) => { const n = Number(v); return Number.isFinite(n) ? Math.max(min, Math.min(max, n)) : fb; };
        ...
        if (Array.isArray(s.pointObservations)) {
            for (const raw of s.pointObservations.slice(-this._maxPointCache)) {
                ...
                if (typeof entry.key !== 'string' || !/^-?\d+,-?\d+$/.test(entry.key) || !entry.state || typeof entry.state !== 'object' || Array.isArray(entry.state)) continue;
                const state = entry.state as Record<string, unknown>;
                if (typeof state.type !== 'string' || !this._effects[state.type]) continue;
                const clean: Record<string, string | number | boolean> = {};
                for (const [key, value] of Object.entries(state)) {
                    if (typeof value === 'number' && Number.isFinite(value) || typeof value === 'boolean' || typeof value === 'string' && value.length <= 240) {
                        if (key !== '__proto__' && key !== 'constructor' && key !== 'prototype') clean[key] = value;
                    }
                }
                this._pointCache.set(entry.key, {state:clean as WeatherPointState, expiresAt:finite(entry.expiresAt, 0)});
            }
        }
        if (this._pointFetchTimer) {
