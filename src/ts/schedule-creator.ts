...
import {assessTurnback,normalizeTurnbackState,CAB_CHANGE_SECONDS,type TurnbackState} from './formation-turnback.js';
...
import { canonicalTrackRef, normalizeStationTrackIdentity, resolveNativeStationTrack, stationTrackResourceKey, type StationTrackIdentity } from './station-track-identity.js';
import { TailSpeedIndex, normalizePassageTailSpeedHolds, type PassageTailSpeedHold } from './tail-speed-index.js';
...
type __KPM316 = number;
type __KPM317 = number;
...
type __KPA110 = { "stationId": unknown; "type": unknown; "departureTime": unknown; "time": unknown; "arrivalTime": unknown; "voiePointId": unknown; "platform": unknown; "stopCode": unknown; "lat": unknown; "lon": unknown; "locationOccurrenceId": unknown; "technicalLocationId": unknown; "locationName": unknown; "v2OperationSec": unknown; "trackIdentity"?: unknown; "turnBack"?: unknown };
type __KPA111 = { "item": Record<string, unknown> };
type __KPA113 = string;
type __KPA118 = number;
type __KPA121 = unknown[];
type __KPA123 = number;
...
type ServiceRoutePoint = Record<string, unknown> & {
  lat: number; lon: number; electrified?: boolean; wayId?: string | number; way_id?: string | number;
  maxSpeed?: number; speed?: number; maxSpeedSource?: string; direction?: unknown; incline?: unknown; fallback?: unknown; tracks?: number; usage?: unknown;
};
type ServiceRoute = ServiceRoutePoint[];
type ServiceStopInput = Record<string, unknown> & {
  stationId?: unknown; type?: unknown; departureTime?: number; arrivalTime?: number; time?: number;
  voiePointId?: unknown; platform?: unknown; stopCode?: unknown;
};


type PhysicalLeader = { service: ActiveService; frontProgressKm: number; rearGapKm: number; leaderSpeedKmh: number; lengthM: number; tailClearingDeparture: boolean };
type StationReservation = { id: unknown; lat?: number; lon?: number; platforms?: unknown };
type ArrivalStation = StationReservation & { id: string; lat: number; lon: number; name: string };
type ArrivalStopResource = { lat?: number | null; lon?: number | null };


type TronconResource = { id: string | number; occupiedBy?: unknown };
type TronconManager = {
  troncons?: TronconResource[];
  getTronconAtPosition: (pos: { lat: number; lon: number; wayId?: string | number | null }, radiusKm?: number, voie?: unknown, wayIds?: Set<string> | null) => TronconResource | null | undefined;
...
function timeDiff(timeA: unknown, timeB: unknown) {
...
function timeGte(timeA: unknown, timeB: unknown) {
...
function isInServiceWindow(timeOfDay: number, start: __KPM316, end: __KPM317) {
  // Normalize to [0, 1440)
...
function _seeded01(seed: unknown) {
  let h = 0;
  const s = String(seed);
...
export class ServiceStop {
  ...
  declare type: string;
  declare stopCode: string;
  declare departureTime: number;
  declare arrivalTime: number;
  declare voiePointId: string | null;
  declare trackIdentity: StationTrackIdentity | null;
  declare platform: string | number;
  declare lat: number | null;
  declare lon: number | null;
  declare locationOccurrenceId: string;
  declare technicalLocationId: string;
  declare locationName: string;
  declare name?: string;
  declare v2OperationSec: number;
  declare turnBack: boolean;
  declare _skipped?: boolean;
  declare time?: number;
  constructor(stationId: unknown, type: unknown, depTime: unknown, arrTime: unknown, voiePointId: unknown = null, platform: unknown = '', stopCode: unknown = '', extra: Record<string, unknown> | null = null) {
    this.stationId = stationId as string;
    this.type = type as string;
    this.stopCode = (stopCode || '') as string;
    ...
    this.voiePointId = (voiePointId || null) as string | null;
    this.platform = (platform || '') as string | number;
    ...
    this.locationOccurrenceId = (extra?.locationOccurrenceId as string) || '';
    this.technicalLocationId = (extra?.technicalLocationId || '') as string;
    this.locationName = (extra?.locationName || '') as string;
    this.v2OperationSec = Math.max(0, Number(extra?.v2OperationSec || 0));
    this.turnBack = extra?.turnBack === true;
    if (this.turnBack) this.type = 'arret';
  }
}

type ServiceIncident = { effect?: string; name?: string; speedLimit?: number; [key: string]: unknown };
type ServiceBreakdown = { type: string; time?: number; [key: string]: unknown };
type ServiceTrain = {
  id: string; name: string; color: string; category: string; maxSpeed: number;
  speed: number; delay: number; state: string; totalKm: number;
  stoppedAt: ({ id?: unknown; name?: unknown; lat?: unknown; lon?: unknown; platformNames?: string[] } | null); incident: ServiceIncident | null; breakdown: ServiceBreakdown | null;
  blockedBy: boolean; signalAlert: string | null; delayReason: string;
  incidentDelayReasons: Array<Record<string, unknown>>; accel: number; decel: number;
  seriesName: string; number: string; platform: unknown; totalKmRun: number;
  kmSinceLastMaint: number; wearLevel: number; inMaintenance: boolean; inDepot: boolean;
  length?: number; heading?: number; geoHeading?: number; iteInfo?: Record<string, unknown> | null;
  _stoppedSinceGameTime?: number | null; movementAuthority?: PublishedMovementAuthority;
};
type ActiveServiceState = {
  index: number; progress: number; legKey: unknown; cachedRoute: ServiceRoute | null;
  worksLimitCache: { key: string; limit: number | null } | null;
  cumDist: Float64Array | null; segDists: Float64Array | null;
  plannedCumTimeSec: Float64Array | null; plannedSegTimeSec: Float64Array | null; plannedTotalTimeSec: number;
  negativeSpeedTransitions: Array<{ index: number; speed: number; distanceKm: number }>;
};
type PlatformAssignment = { stationId: unknown; platform: unknown; voiePointId?: unknown; source?: string; trackIdentity?: StationTrackIdentity | null; displayName?: string };
type DepartureResourceHold = { stationId: unknown; platform: unknown; platformSource: string; voiePointId: unknown; startTravelKm: number; trackIdentity?: StationTrackIdentity | null; displayName?: string };
type PendingAltRoute = { key: string; completed: boolean; route: ServiceRoute | null; failed: boolean };
type V2OperationEntry = { actionId: string; dependsOn: string[]; started: boolean; applied: boolean; failed: boolean; failure?: string; durationSec?: number; actualStartRelSec?: number; actualEndRelSec?: number; reservedIds: string[]; plannedStartOffsetSec?: number; plannedEndOffsetSec?: number; [key: string]: unknown };
type V2OperationState = { completed: boolean; failed: boolean; locationOccurrenceId?: string; stationId?: string; technicalLocationId?: string; locationName?: string; windowStartRelSec?: number; entries?: V2OperationEntry[]; [key: string]: unknown };
type WeatherEffects = { brakeFactor: number; speedCap: number; speedMult: number; type: string };
...
export class ActiveService {
  ...
  private _stepPhysicalLeader: PhysicalLeader | null = null;
  private _electrificationIndex?: RouteElectrificationIndex;
  declare id: string;
  declare name: string;
  declare _v2OccurrenceId: string;
  declare _v2RotationId: string;
  declare _v2BaseDate: string;
  declare _v2OptionalStopDecisions: Record<string, boolean> | null;
  declare _v2AllowEarlyDeparture: boolean;
  declare rameId: unknown;
  declare rame: ActiveServiceRame | null | undefined;
  declare stops: ServiceStop[];
  declare routes: ServiceRoute[];
  declare world: World;
  declare weather: WeatherEffectsProvider | null;
  declare roundTrip: boolean;
  declare multiDepartures: number;
  declare terminusWait: number;
  declare number: string | number;
  declare returnNumber: string | number;
  declare _returnRoutes: ServiceRoute[] | null;
  declare _returnStopsData: ServiceStop[] | null;
  declare totalDistance: number;
  declare plannedDistance: number;
  declare active: boolean;
  declare serviceType: string;
  declare isTTX: boolean;
  declare isLegacyCatenaryWork: boolean;
  declare isWorkTrain: boolean;
  declare assignedContractId: string;
  declare returnName: string;
  declare returnPlatforms: Record<string, unknown>;
  declare runDays: number[];
  declare runDates: string[];
  declare _tripCount: number;
  declare _adjustedStops: ServiceStop[] | null;
  declare _adjustedReturnStops: ServiceStop[] | null;
  declare currentStopIndex: number;
  declare state: string;
  declare position: { lat: number; lon: number } | null;
  declare speed: number;
  declare targetSpeed: number;
  declare delay: number;
  declare completed: boolean;
  declare direction: number;
  declare currentRouteIndex: number;
  declare routeProgress: number;
  declare lastTickTime: number;
  declare revenueCollected: boolean;
  declare isReturnLeg: boolean;
  declare returnStops: ServiceStop[];
  declare _platformAssignment: PlatformAssignment | null;
  declare _departureResourceHold: DepartureResourceHold | null;
  declare _state: ActiveServiceState;
  declare _routeAnalysis: ReturnType<typeof analyzeRoute> | null;
  declare _cantonAssignments: ReturnType<CantonManager['createRouteCantons']> | null;
  declare _carryoverCantonIds: Set<unknown> | null;
  declare _occupiedTronconIds: Set<string | number>;
  declare _tronconExitTravelKm: Map<string | number, number>;
  declare _reservedTronconId: string | number | null;
  declare _lastTronconId: string | number | null;
  declare _brakeEffort: number;
  declare _tractiveEffort: number;
  declare _stationaryRoute: ServiceRoute | null;
  declare _stationaryRouteIndex: number;
  declare _pendingAltRoute: PendingAltRoute | null;
  declare _altRouteFailedKeys: Set<string>;
  declare category: string;
  declare train: ServiceTrain;
  declare _movementAuthority: MovementAuthority;
  declare _garage: unknown;
  declare _iteHardBlock: boolean;
  declare _iteDwellExtra: number;
  declare _iteCargoMismatch: boolean;
  declare _passageStops: unknown[];
  declare _carreMarginM: number;
  declare _negativeBufferKm: number;
  declare _lastPhysicsDecelMs2?: number;
  declare _resolvedSpeedCache: WeakMap<object, { cap: number; speeds: number[] }> | null;
  declare _cumDistCache: WeakMap<object, Float64Array> | null;
  declare _currentDate: string;
  declare _economy: Economy;
  declare _v2OnOperationTick?: (dateStr: string, timeOfDay: number) => void;
  ...
  declare _cachedDowDate: string;
  declare _cachedFirstDep: number;
  declare _evoCompleted?: boolean;
  declare _evoServiceId?: string | null;
  declare cancelled?: boolean;
  declare completedDate?: string;
  _legacyOperatingDay = '';
  _legacyLastFinishedDay = '';
  declare _nextDepartureTime?: number | null;
  declare _atTerminus?: boolean;
  declare _cachedTroncon?: TronconResource | null;
  declare _cachedTronconTime?: number | null;
  declare _trackKey?: string | null;
  declare _routeKey?: string | null;
  declare _blockedSinceGameTime?: number | null;
  declare _carryoverStartTravelKm?: number | null;
  declare _worksClosureAhead?: boolean;
  declare _lastArrivalTime?: number;
  declare _v2BrakeServiceMs2?: number;
  declare _contractFreight?: number;
  declare _contractCargoId?: string;
  declare _onboardFreight?: number;
  declare _onboardPax?: number;
  declare _onboardPassengerKm?: number;
  declare _genericCargoType?: string;
  declare _contractDelivered?: number;
  declare _evoCreated?: boolean;
  declare _evoForServiceId?: string | null;
  declare _isEVO?: boolean;
  declare _v2OperationWaitingMaterial?: boolean;
  declare _v2PrepReadyMinute?: number;
  declare _v2FormationMembers?: Array<{ vehicleId: string; role: FormationMember['role']; order?: number; sourceCouponId?: string }>;
  declare _v2VehicleIds?: string[];
  declare _v2FormationReversed?: boolean;
  declare _v2OperationFailure?: string;
  declare _v2RandomDepartureKey?: string;
  declare _v2DirectRameId?: string;
  declare _v2AssignedRameId?: string;
  declare _macroElapsed?: { medium: number; low: number };
  declare lineId?: string;
  declare delayReason?: string;
  declare isRescue?: boolean;
  declare rescueState?: string;
  declare rescueCanRetry?: boolean;
  declare _nearbyServices?: ActiveService[];
  declare _garageUntil?: number | null;
  declare _garageUntilDate?: string | null;
  declare _turnbackState?: TurnbackState | null;
  declare _currentTimeOfDay?: number;
  constructor(data: Record<string, unknown>, rame: Rame | null | undefined, world: unknown, weather: unknown) {
    this.id = (data.id || `svc-${nextServiceId++}`) as string;
    this.name = (data.name || 'Service') as string;
    this._v2OccurrenceId = (data.v2OccurrenceId || '') as string;
    this._v2RotationId = (data.v2RotationId || '') as string;
    this._v2BaseDate = (data.v2BaseDate || '') as string;
    this._v2OptionalStopDecisions = (data.optionalStopDecisions || null) as Record<string, boolean> | null;
    this._v2AllowEarlyDeparture = data.allowEarlyDeparture === true;
    this.rameId = data.rameId || rame?.id || ''; 
    this.rame = rame as ActiveServiceRame | null | undefined;
    this.stops = (Array.isArray(data.stops) ? data.stops : []).map((s: __KPA110) =>
      new ServiceStop(
        s.stationId, s.type, s.departureTime ?? s.time, s.arrivalTime ?? s.time,
    ...
    this.routes = (data.routes || []) as ServiceRoute[];
    this.world = world as World;
    this.weather = weather as WeatherEffectsProvider | null;
    this.roundTrip = (data.roundTrip || false) as boolean;
    this.multiDepartures = (data.multiDepartures || 1) as number;
    this.terminusWait = (data.terminusWait ?? DEFAULT_TERMINUS_WAIT_MIN) as number; // preserve explicit 0
    ...
    if (this._v2OccurrenceId) {
      this.number = (data.number != null ? data.number : '') as string | number;
      this.returnNumber = (data.returnNumber != null ? data.returnNumber : '') as string | number;
    } else {
      ...
      this.returnNumber = data.returnNumber != null
        ? (data.returnNumber as string | number) : returnNumberFor(this.number);
    }
    ...
    this._returnRoutes = (data.returnRoutes || null) as ServiceRoute[] | null;
    this._returnStopsData = (data.returnStops || null) as ServiceStop[] | null;
    this.totalDistance = (data.totalDistance || 0) as number;
    this.plannedDistance = (data.plannedDistance || 0) as number;
    ...
    this.serviceType = (data.serviceType || (data.isWorkTrain ? 'work' : 'passager')) as string;
    ...
    this.assignedContractId = (emptyMovement ? '' : (data.assignedContractId || '')) as string;
    this.returnName = (data.returnName || '') as string;
    this.returnPlatforms = (data.returnPlatforms || {}) as Record<string, unknown>; // { stationId: platformName }
    this.runDays = (data.runDays || [0,1,2,3,4,5,6]) as number[]; // days of week (0=Sun..6=Sat), default all
    this.runDates = (data.runDates || []) as string[]; // specific dates (YYYY-MM-DD), empty = every day
    this._tripCount = 0;
    ...
    if (this.serviceType === 'fret') this.category = 'fret';
    else if (this.serviceType === 'w') this.category = 'w';
    else if (this.serviceType === 'hlp' || this.serviceType === 'm-' || this.serviceType === 'evo') this.category = 'hlp';
    else if (this.serviceType === 'tm') this.category = 'tm';
    else if (this.serviceType === 'infra') this.category = 'infra';
    else if (this.isTTX) this.category = 'ttx';
    else this.category = (rame && rame.totalFreightCapacity > rame.totalCapacity) ? 'fret' : 'voyageur';
  ...
  _movementBegin(baseLimitKmh: unknown = Infinity) {
    if (!this._movementAuthority) this._movementAuthority = new MovementAuthority(this.train);
  ...
  _movementStop(code: string = 'RUNTIME_STOP', reason: string = '', source: string = 'runtime', meta: Record<string, unknown> | null = null) {
    if (!this._movementAuthority) this._movementAuthority = new MovementAuthority(this.train);
    const why = reason || this.train?.delayReason || 'arrêt de sécurité';
    this._movementAuthority.stop(code, why, source, meta);
    if (this.train && !this.train.delayReason) this.train.delayReason = why;
    return false;
  }

  _movementCaution(limitKmh: unknown, code: string = 'RUNTIME_CAUTION', reason: string = '', source: string = 'runtime', meta: Record<string, unknown> | null = null) {
    if (!this._movementAuthority) this._movementAuthority = new MovementAuthority(this.train);
  ...
  _computePassageStops() {
      ...
      if (typeof this.world.getStationsNear === 'function') {
        ...
        for (let i = 0; i < route.length; i++) {
          ...
          for (const st of near) {
            ...
            if (!prev || d < prev.dist) bestByStation.set(st.id, { st, idx: i, dist: d });
          }
        }
        for (const { st, idx, dist } of bestByStation.values()) {
          if (dist > thresholdKm || idx < 0) continue;
    ...
    return Array.from(byStation.values()).sort((a: __S3Struct687, b: __S3Struct688) => a.time - b.time);
  }
  ...
  async garageToVoiePoint(vpId: unknown) { return; }
  resumeFromGarage() { return; }

  getColor() {
    const colors = ['#2563eb', '#dc2626', '#16a34a', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16'];
    const hash = this.name.split('').reduce((a: number, c: string) => a + c.charCodeAt(0), 0);
    return colors[hash % colors.length];
  }

  _v2ScheduleNowMinutes(dateStr: unknown, timeOfDay: number) {
    if (!this._v2OccurrenceId || !this._v2BaseDate || !dateStr) return timeOfDay;
    const a = Date.parse(`${this._v2BaseDate}T12:00:00Z`);
    const b = Date.parse(`${dateStr}T12:00:00Z`);
    if (!Number.isFinite(a) || !Number.isFinite(b)) return timeOfDay;
  ...
  _scheduleDiff(a: __KPM321, b: __KPM322) {
    return this._v2OccurrenceId ? (a - b) : timeDiff(a, b);
  }

  _scheduleGte(a: number, b: __KPM323) {
  ...
  _timetableSpeedCap(timeOfDay: unknown, route: unknown) {
    return null;
  }

  _isLiveResourceOwner(ownerId: unknown) {
    if (ownerId == null || ownerId === this.id) return true;
    ...
    const live = (service: { active?: unknown; completed?: unknown; cancelled?: unknown; state?: unknown; position?: unknown } | undefined) => {
      if (!service || service.active === false || service.completed || service.cancelled || service.state === 'completed' || service.state === 'cancelled') return false;
      if (service.state === 'moving' || service.state === 'departing' || service.state === 'stopped_at_station') return true;
      return !!service.position;
    };
    if (live(owner)) return true;
    return services.some((service: { id: unknown; active?: unknown; completed?: unknown; cancelled?: unknown; state?: unknown; position?: unknown }) => String(service?.id) === String(ownerId) && live(service));
  }
  ...
  _arrivalReservationOwnerIsFollower(ownerId: unknown, station: StationReservation, stop: ArrivalStopResource, resource: ArrivalStopResource | null = null) {
    if (ownerId == null || String(ownerId) === String(this.id) || !station || !this.position) return false;
    const services = globalThis.window?.game?.scheduleCreator?.services || [];
    const owner = services.find((s: { id: unknown }) => String(s?.id) === String(ownerId));
    if (!owner || !owner.position || owner.completed || owner.cancelled || owner.active === false) return false;

    const ownerNext = owner.getNextStop?.();
    if (String(ownerNext?.stationId || '') !== String(station.id || '')) return false;
    const ownerPhysicalStation = String(owner.train?.stoppedAt?.id || owner._platformAssignment?.stationId || '');
    if (owner.state === 'stopped_at_station' && ownerPhysicalStation === String(station.id || '')) return false;
    ...
    try {
      ...
      if (leader?.service && String(leader.service.id) === String(this.id)) return true;
    } catch {}
    ...
    if (!/espacement|proximity/i.test(reason)) return false;
  ...
  _revokeFollowerArrivalReservation(ownerId: unknown, stationId: unknown, voiePointId: unknown = null, platform: unknown = null) {
    const services = globalThis.window?.game?.scheduleCreator?.services || [];
    const owner = services.find((s: { id: unknown }) => String(s?.id) === String(ownerId));
    if (!owner) return;
    ...
    if (String(owner.train?.delayReason || '') === 'attente voie libre en gare') owner.train.delayReason = '';
  }

  _purgeStaleStationResources(stationId: unknown) {
    ...
    if (data?.occupied instanceof Map) {
      for (const [plat, ownerId] of (Array.from(data.occupied.entries()) as Array<[unknown, unknown]>)) {
        if (ownerId !== this.id && !this._isLiveResourceOwner(ownerId)) data.occupied.delete(plat);
      }
    }
  }

  _reserveArrivalResources(station: StationReservation, stop: ServiceStop) {
    if (!station || !stop || stop.type !== 'arret') return true;
    ...
    if (vpm) {
      if (stop.voiePointId) {
        ...
        if (!requested || String(requested.stationId || '') !== String(station.id || '')) return false;
      }
      ...
      if (stationVPs.length) {
        ...
        if (stop.trackIdentity && !chosen) return false;
        if (!chosen && stop.voiePointId) chosen = vpm.getVoiePointById(stop.voiePointId);
        if (!chosen && stop.platform) {
          const canonical = (value: unknown) => String(value ?? '').trim().toLowerCase().replace(/^(?:voie|track|gleis|v)\s*(\d+)$/, '$1');
          chosen = stationVPs.find((vp: { voie: unknown }) => canonical(vp.voie) === canonical(stop.platform));
          if (!chosen) return false; // Do not mutate a requested track into a free substitute.
        }
        if (!chosen) chosen = stationVPs.find((vp: { occupiedBy: unknown }) => vp.occupiedBy == null || vp.occupiedBy === this.id);
        if (!chosen) return false;
        if (chosen.occupiedBy != null && chosen.occupiedBy !== this.id) {
          const previousOwner = chosen.occupiedBy;
          if (!this._arrivalReservationOwnerIsFollower(previousOwner, station, stop, chosen)) return false;
        ...
        if (!stop.trackIdentity) stop.platform = chosen.voie;
    ...
    if (stop.trackIdentity?.kind === 'native') return false;
    ...
    if (!stop.trackIdentity && this._platformAssignment?.stationId === station.id &&
        pm.getPlatformForTrain?.(station.id, this.id) != null) return true;
  ...
  _beginDepartureResourceHold(stationId: unknown) {
  ...
  _releaseDepartureResourcesIfTailClear(route: unknown = this._state?.cachedRoute) {
    ...
    if (travelledM + 0.5 < lengthM) return;
    const vpm = globalThis.window?.game?.voiePointManager;
    ...
    if (vpm && hold.voiePointId) vpm.releaseVoiePoint(hold.voiePointId, this.id);
    ...
    if (this._platformAssignment?.stationId === hold.stationId) this._platformAssignment = null;
    ...
    if (!this._platformAssignment) this.train.platform = null;
  }
  ...
  _holdForBookedArrival(timeOfDay: unknown) {
    ...
    if (!stop || stop.type !== 'arret') return false;
    const booked = Number(stop.arrivalTime ?? stop.departureTime);
    if (!Number.isFinite(booked)) return false;
    const now = this._v2ScheduleNowMinutes(this._currentDate, timeOfDay as number);
    ...
    if (diff > (1 / 60)) {
      ...
      if (this.train) {
        this.train.delay = 0;
        if (typeof this._movementGo === 'function') this._movementGo();
        else new MovementAuthority(this.train).begin(); // prototype-level compatibility tests/plugins
  ...
  _commitNearEndpointArrival(route: unknown, timeOfDay: unknown, toleranceKm: number = 0.006) {
    if (!Array.isArray(route) || route.length < 2 || this.state !== 'moving') return false;
    const stop = this.getNextStop?.();
    if (!stop || stop.type !== 'arret') return false;
    const remainingKm = this._getRemainingDistance(route);
    if (!Number.isFinite(remainingKm) || remainingKm > toleranceKm) return false;
    ...
    if (this.train?.blockedBy || (this.train?.breakdown && !['climatisation','portes'].includes(this.train.breakdown.type))) return false;
    const station = this.getTargetStation?.();
    if (!station || !this._reserveArrivalResources(station, stop)) return false;
    const last = route[route.length - 1];
    this._state.index = route.length - 1; this._state.progress = 0;
    this.position = { lat:last.lat, lon:last.lon };
    this.speed = 0; if (this.train) this.train.speed = 0;
    this._syncCantonFootprint?.(route);
    this.arriveAtStation(station, timeOfDay as number, this._economy);
    return true;
  }

  _scheduleInWindow(now: number, start: __KPM329, end: __KPM330) {
  ...
  getTargetStation(): ArrivalStation | null {
    ...
    // Legacy station stop
    if (next.stationId) return (this.world.getStationById(next.stationId) || null) as ArrivalStation | null;
    ...
    if (next.voiePointId && window.game?.voiePointManager) {
      ...
      if (vp) return { id: vp.id, name: `Voie ${vp.voie}`, lat: vp.lat, lon: vp.lon, platforms: 1 } as ArrivalStation;
    }
    return null;
  }
  ...
  _adhesionMassKg() {
    if (!this.rame) return 90000;
    const poweredT = Number(this.rame.adhesionMass || 0) || (this.rame.elementDetails || [])
      .filter((e: { category: unknown; power: unknown }) => (e.category === 'locomotive' || e.category === 'automotrice') && Number(e.power || 0) > 0)
      .reduce((sum: number, e: { mass: unknown; tonnage: unknown }) => sum + Math.max(0, Number(e.mass || e.tonnage || 0)), 0);
  ...
  _gradePermilleAt(route: unknown = this._state?.cachedRoute, segIdx: number = this._state?.index || 0) {
    ...
    if (raw == null || raw === '' || raw === 'up' || raw === 'down') return 0;
    const txt = String(raw).trim().replace(',', '.');
    let value = Number.parseFloat(txt);
    if (!Number.isFinite(value)) return 0;
    if (txt.includes('‰')) value = value;
    else value *= 10; // OSM numeric/plain and '%' incline are percentages.
  ...
  _serviceBrakeMs2() {
    ...
    if (Number.isFinite(formationBrake) && formationBrake > 0) return Math.max(0.1, Math.min(1.6, formationBrake));
    ...
    switch (String(this.serviceType || '').toLowerCase()) {
      ...
      case 'ttx': case 'work': return 0.50;
      case 'infra': return 0.52;
      case 'tm': return 0.62;
      case 'w': return 0.78;
      case 'hlp': case 'm-': case 'evo': return 0.85;
      default: return 0.90;
    }
  }
  ...
  _applyPhysicalSpeedTarget(targetKmh: unknown, accelKmhS: number, decelKmhS: number, dt: unknown) {
  ...
  _buildPlannedTimeProfile(route: unknown) {
    ...
    try {
      ...
      for (let i = 0; i < segTimes.length; i++) cum[i + 1] = cum[i] + segTimes[i];
  ...
  _computePhysicsAccel(weather: __S3Struct695, route: unknown = this._state?.cachedRoute, segIdx: unknown = this._state?.index || 0) {
    const baseAccel = this.train.accel || 3.0; // km/h/s
    const baseDecel = this.train.decel || 4.0; // km/h/s
    const grade = this._gradePermilleAt(route, segIdx as number);
    if (!this.rame) {
  ...
  _resourceSection(route: unknown = this._state?.cachedRoute, index = Number(this._state?.index || 0)) {
    if (!Array.isArray(route) || route.length < 2) return {};
    const i = Math.max(0, Math.min(route.length - 2, index));
    const a = route[i] as ServiceRoute[number], b = route[i + 1] as ServiceRoute[number];
  ...
  _isElectricOnly() {
    ...
    const parts = traction.split('+').map((s: __KPA113) => s.trim().toLowerCase()).filter(Boolean);
    ...
    if (parts.some((p: string) => self.has(p))) return false;
    if (parts.some((p: string) => electric.has(p))) return true;
    return false;
  }

  // MAT-08 — le segment courant est-il compatible avec la traction de la rame ?
  _electrificationMismatch(route: ServiceRoute, segIdx: number) {
  ...
  _distanceAheadToElectrificationMismatch(route: unknown = this._state?.cachedRoute) {
    if (!this._isElectricOnly() || !Array.isArray(route) || route.length < 2) return Infinity;
    ...
    this._electrificationIndex ??= new RouteElectrificationIndex();
  ...
  _getInfraSpeedLimit(route: ServiceRoute, segIdx: number, progress: unknown, trainLengthM: unknown) {
    const segDists = this._state?.segDists;
    const resolved = this._resolvedRouteSpeeds(route);
    if (!segDists || !route?.length) return resolved[Math.max(0, Math.min(route.length - 1, segIdx + 1))] || resolved[segIdx] || this.train.maxSpeed;
    ...
    if(remainingKm>0)minSpeed=Math.min(minSpeed,resolved[0] ?? this.train.maxSpeed);
  ...
  _getNegativeTransitionCap(route: unknown, segIdx: number, progress: number, currentSpeed: __KPM332) {
    const segDists=this._state?.segDists;
    const cumDist=this._state?.cumDist;
    const transitions=this._state?.negativeSpeedTransitions;
    if(!segDists||!cumDist||!transitions?.length||currentSpeed<=0)return null;
    ...
    while(lo<hi){const mid=(lo+hi)>>1;if(transitions[mid].distanceKm<=frontDist)lo=mid+1;else hi=mid;}
    let cap=Infinity;
    for(let ti=lo;ti<transitions.length;ti++){
      const tr=transitions[ti];
      const distToPoint=tr.distanceKm-frontDist;
      if(distToPoint>maxLookahead)break;
  ...
  _yieldToRescue() {
    ...
    if (!dm || !this.position) return false;
    const myRoute = this._state?.cachedRoute || this.getCurrentRoute?.();
    if (!Array.isArray(myRoute) || myRoute.length < 2) return false;
    ...
    if (!Number.isFinite(myProj.progressKm)) return false;
    ...
    for (const rescue of dm.activeRescues || []) {
      if (!rescue.position || !['en_route','recovering','returning'].includes(rescue.state)) continue;
      const rescueRoute = rescue.state === 'returning' ? rescue.returnRoute : rescue.route;
      if (!Array.isArray(rescueRoute) || rescueRoute.length < 2) continue;
      ...
      if (raw > Math.max(2, horizonKm * 1.25)) continue;
      ...
      if (!Number.isFinite(rp.progressKm) || rp.distanceKm > 0.05) continue;
      const rescueWays = this._wayIdsAlong(rescueRoute, ri, 0.3, 0.3);
      const wayKnown = myWays.size > 0 && rescueWays.size > 0;
      if (wayKnown && ![...myWays].some((id: unknown) => rescueWays.has(id))) continue;
      ...
      if (a&&b&&ra&&rb) {
        ...
        let hd=Math.abs(mh-rh); if(hd>Math.PI)hd=2*Math.PI-hd;
  ...
  _isGarageRegulationActive(timeOfDay: number, dateStr: string = this._currentDate || '') {
    if (this._garageUntil == null) return false;
    const now = Number(timeOfDay), until = Number(this._garageUntil);
    if (!Number.isFinite(now) || !Number.isFinite(until)) return false;
    if (!this._garageUntilDate || !dateStr) return now < until;
    if (dateStr < this._garageUntilDate) return true;
    if (dateStr > this._garageUntilDate) return false;
    return now < until;
  }

  _waitForConnection(stop: ServiceStop, timeOfDay: number, dateStr: string): boolean {
    const game = globalThis.window?.game;
    if (!stop?.stationId || stop.type !== 'arret' || stop.technicalLocationId || !this._isPassengerService()) return false;
    if (!game?.connections?.shouldWait(this.id, stop.stationId, timeOfDay, game.scheduleCreator?.services || [], dateStr)) return false;
  ...
  _recordConnectionDeparture(stop: ServiceStop, timeOfDay: number, dateStr: string) {
    const game = globalThis.window?.game;
    if (this._isPassengerService()) game?.connections?.onDeparture(this, stop, timeOfDay, dateStr, game.scheduleCreator?.services || []);
  }
  ...
  _legacyOriginEligible(timeOfDay: number): boolean {
    if (this._v2OccurrenceId || civilDayIndex(this._currentDate) == null || this.currentStopIndex !== 0 || this.isReturnLeg || this._atTerminus) return true;
    if (this.state !== 'waiting' && this.state !== 'completed' && this.state !== 'cancelled') return true;
  ...
  _prepareLegacyOperatingDay(timeOfDay: number, dateStr: string): boolean {
    if (this._v2OccurrenceId || civilDayIndex(dateStr) == null) return true;
    const finished = this.completed || this.cancelled || this.state === 'completed' || this.state === 'cancelled';
    if (!finished && (this.currentStopIndex !== 0 || this.isReturnLeg || this._atTerminus ||
        (this.state !== 'waiting' && this.state !== 'stopped_at_station'))) return true;
    if (finished && !this._legacyLastFinishedDay) {
      this._legacyLastFinishedDay = this._legacyOperatingDay || this.completedDate || '';
    }
    if (finished && this.serviceType === 'evo') return false; // automatic positioning is one-shot
    ...
    if (!day && finished && timeOfDay < Number(this.stops[0]?.departureTime ?? 0)) {
      ...
      if (future === dateStr) day = future;
    }
    if (!day) {
      if (this.position || this._platformAssignment) this._releaseAllPhysicalResources();
    ...
    globalThis.window?.game?.freightManager?.finishServiceCargo?.(this);
      ...
      this.state = 'waiting'; this.completed = false; this.cancelled = false;
      this.currentStopIndex = 0; this.isReturnLeg = false; this.returnStops = [];
      this._tripCount = 0; this._adjustedStops = null; this._adjustedReturnStops = null;
      this._nextDepartureTime = null; this._atTerminus = false; this.revenueCollected = false;
      this._evoCreated = false; this._evoCompleted = false; this._evoServiceId = null;
      this.position = null; this.speed = 0; this.delay = 0; this.totalDistance = 0;
      this._onboardPax = 0; this._onboardPassengerKm = 0; this._onboardFreight = 0;
      this._blockedSinceGameTime = null;
      this.train.speed = 0; this.train.delay = 0; this.train.state = 'waiting';
      this.train.stoppedAt = null; this.train.platform = null; this.train.delayReason = '';
      ...
      this._resetState(); this._movementBegin();
    }
  ...
  _finishLegacyOperatingDay() {
    if (!this._v2OccurrenceId) this._legacyLastFinishedDay = this._legacyOperatingDay || this._currentDate || '';
  }

  /** No station-level teleport after a consist has acquired a physical track. */
  _turnbackDepartureReady(stop: ServiceStop | undefined, timeOfDay: number, dateStr: string, returnLeg=false): boolean {
    if (!stop || (!stop.turnBack && !returnLeg)) return true;
    const nextTrip=this.roundTrip&&(!this.isReturnLeg||(this.multiDepartures&&this._tripCount<this.multiDepartures));
    if(!returnLeg&&this.currentStopIndex>=this.getCurrentStops().length&&nextTrip)return true;
    if(!this._v2BaseDate&&!this._legacyOperatingDay)this._legacyOperatingDay=this._currentDate||dateStr;
    ...
    const hold=(reason:string,code='TURNBACK_REQUIRED')=>{
    ...
    if(this.speed>0.05)return hold('rebroussement : attente de l’arrêt complet');
    ...
    if(day==null||!Number.isFinite(minute))return hold('rebroussement : heure de simulation inconnue');
    ...
    if(now+1e-6<Number(state.readyAtSec))return hold(assessment.reason,'CAB_CHANGE');
    const incoming=this._getStationaryRoute();
    const lengthKm=Number(this.rame?.totalLength||this.train.length||0)/1000;
    if(!this.position||!incoming||!(lengthKm>0))return hold('rebroussement : empreinte ferroviaire de la rame inconnue');
    ...
    if(!tail)return hold('rebroussement : position de la cabine opposée inconnue');
    const outgoing=this.getCurrentRoute();
    const needsRoute=returnLeg||this.currentStopIndex<this.getCurrentStops().length;
    if(needsRoute){
      if(!outgoing||outgoing.length<2)return hold('rebroussement : tracé de départ ferroviaire requis');
      ...
      for(let i=0;i<outgoing.length&&outCum[i]<=lengthKm;i++)if(!matching(outCum[i]))return hold('rebroussement : discontinuité sous la rame');
      for(let i=head.segmentIndex;i>=0&&inCum[i]>=tailKm;i--)if(!matching(head.progressKm-inCum[i]))return hold('rebroussement : discontinuité sous la rame');
      if(!matching(lengthKm/2))return hold('rebroussement : discontinuité sous la rame');
    }
    ...
    const footprint:ServiceRoute=[{...this.position,wayId:incoming[Math.min(incoming.length-1,head.segmentIndex+1)].wayId}];
    for(let i=head.segmentIndex;i>=0&&cum[i]>tailKm;i--)if(cum[i]<head.progressKm)footprint.push(incoming[i]);
    footprint.push(tail);
    if(this.rame){
    ...
    if(this._v2OccurrenceId)this._v2FormationReversed=!this._v2FormationReversed;
    this.position={lat:tail.lat,lon:tail.lon};
    this._stationaryRoute=footprint;this._stationaryRouteIndex=Math.max(0,footprint.length-2);
    this._resetState();state.applied=true;
    if(Number.isFinite(this.train.geoHeading))this.train.geoHeading=(Number(this.train.geoHeading)+Math.PI)%(2*Math.PI);
  ...
  _materialTrackDepartureReady(stop: ServiceStop | undefined): boolean {
    ...
    if (!stop || !location || location.stationId !== stop.stationId) return true;
    if (!materialTrackMismatch(location, stop, stop.stationId, globalThis.window?.game?.voiePointManager?.voiePoints || [])) return true;
    this.speed = 0; this.train.speed = 0;
  ...
  scheduleTick(timeOfDay: number, dateStr: string, economy: Economy) {
    if (globalThis.window?.game?.depotManager?.ownsServiceMovement?.(this.id)) return;
    if (this.state === 'waiting' || this.state === 'stopped_at_station' || this.state === 'preparation' || this.state === 'garage') this._movementBegin();
    if (!this.active || this.stops.length < 2) return;
    if (!this._prepareLegacyOperatingDay(timeOfDay, dateStr)) return;
    const scheduleNow = this._v2ScheduleNowMinutes(dateStr, timeOfDay as number);
    cantonManager.setTime(timeOfDay);
    this._currentDate = dateStr;this._currentTimeOfDay=timeOfDay;
    ...
    if (this._v2OccurrenceId && typeof this._v2OnOperationTick === 'function') {
      try { this._v2OnOperationTick(dateStr, timeOfDay); }
      catch (e: unknown) { console.warn('V2 rotation operation tick:', e); }
    }
    ...
    if (!this._v2OccurrenceId && this.state === 'waiting' && this.currentStopIndex === 0) {
      if (window.game?.seasonal && !window.game.seasonal.isServiceActive(this.id)) {
        this.position = null; this.train.stoppedAt = null;
        this.train.delayReason = 'service saisonnier inactif'; return;
      } else if (this.train.delayReason === 'service saisonnier inactif') this.train.delayReason = '';
    }
    ...
    if (this.state === 'waiting') {
      ...
      if (this.serviceType !== 'evo' && this._evoServiceId && !this._evoCompleted) {
        const evo = window.game?.scheduleCreator?.services.find((s: { id: unknown }) => s.id === this._evoServiceId);
      ...
      if (this.currentStopIndex === 0 && !this._scheduleInWindow(scheduleNow, firstDep - 1, windowEnd)) {
        if (this._scheduleDiff(scheduleNow, windowEnd) > 0) {
          ...
          this.completedDate = dateStr; this._finishLegacyOperatingDay();
          this._releaseAllPhysicalResources();
          this.position = null;
          this.train.stoppedAt = null;
          return;
      ...
      if (this.currentStopIndex === 0 && !this._materialTrackDepartureReady(currentStops[0])) return;
      if (this.currentStopIndex === 0 && !this._turnbackDepartureReady(currentStops[0],timeOfDay,dateStr)) return;
      ...
      if (this.currentStopIndex === 0 && this._scheduleInWindow(scheduleNow, firstDep - 1, firstDep) && !this._scheduleGte(scheduleNow, firstDep)) {
        ...
        if (firstStation || (this._v2OccurrenceId && s0?.lat != null && s0?.lon != null)) {
          ...
          if (originReserved) {
            if (this.train.delayReason === 'attente voie libre au départ') this.train.delayReason = '';
            this._movementGo();
          } else {
      ...
      if (this.currentStopIndex === 0 && this._scheduleGte(scheduleNow, firstDep)) {
        if (!this._v2OccurrenceId && this.rame?.currentLocation?.stationId &&
            ...
            !this._evoServiceId) {
          this.position = null; this.train.stoppedAt = null;
          this.delay = Math.max(0, this._scheduleDiff(scheduleNow, firstDep)); this.train.delay = this.delay;
          this.train.delayReason = 'rame absente de la gare de départ';
          this._movementStop('MATERIAL_POSITION', this.train.delayReason, 'rotation'); return;
        }
        ...
        if (this._scheduleInWindow(scheduleNow, firstDep, windowEnd)) {
          ...
          if (this.rameId && window.game?.scheduleCreator?.isRameInUse?.(this.rameId, this.id, timeOfDay)) {
            ...
            this.train.delayReason = ''; this._movementGo();
          }
          ...
          if (!this._v2OccurrenceId && depStationId && myDep != null && this.serviceType === 'passager' && window.game?.scheduleCreator) {
            ...
            if (earliest && earliest.id !== this.id && timeDiff(myDep, earliest.dep) > 0) {
              const earliestSvc = window.game.scheduleCreator.services.find((s: { id: unknown }) => s.id === earliest.id);
              ...
              if (!earliestBlockedByRame) { this.train.delayReason = 'priorité à un départ voyageur antérieur'; this._movementStop('RUNTIME_STOP', this.train.delayReason || 'arrêt de sécurité'); return; }
            }
          }

          if (this.train.delayReason === 'priorité à un départ voyageur antérieur') { this.train.delayReason = ''; this._movementGo(); }
          ...
          if (this._v2OccurrenceId) {
            ...
            if (!Array.isArray(preflightRoute) || preflightRoute.length < 2 || preflightRoute.some((p: { fallback?: unknown })=>p?.fallback)) {
              this.state='blocked_route';this.train.state='blocked_route';this.speed=0;this.train.speed=0;
          ...
          if (this._waitForConnection(currentStops[0], timeOfDay, dateStr)) return;
          ...
          if (economy) {
            ...
            if (firstStation) {
              try { economy.processStopRevenue(this, firstStation.name, 0, true, false, firstStation.id, []); }
              catch (e: unknown) { console.warn('Economy origin-stop side effect ignored:', e); }
            }
          }
    ...
    if (this.state === 'stopped_at_station') {
      ...
      if (this._nextDepartureTime != null && this.currentStopIndex === 0) {
        const scheduleNowForTerminus = this._v2ScheduleNowMinutes(this._currentDate, timeOfDay as number);
        if (this._scheduleGte(scheduleNowForTerminus, this._nextDepartureTime)) {
          ...
          if (!this._materialTrackDepartureReady(curStops[0])) return;
          if (!this._turnbackDepartureReady(curStops[0],timeOfDay,dateStr,true)) return;
          if (this._waitForConnection(curStops[0], timeOfDay, dateStr)) return;
          this._prepareRandomDeparture(dateStr);
          ...
          if (economy && curStops[0]) {
            ...
            if (depStation) {
              try { economy.processStopRevenue(this, depStation.name, 0, true, false, depStation.id, []); }
              catch (e: unknown) { console.warn('Economy terminus-departure side effect ignored:', e); }
            }
          }
      ...
      if (!this._materialTrackDepartureReady(stop)) return;
      ...
      if (this._iteHardBlock && stop?.type === 'arret' && stop?.stationId && window.game?.depotManager && window.game?.world) {
        ...
        if (station) {
          ...
          const rameCargo = this.rame?.elementDetails?.find((e: { cargoTypes: unknown }) => Array.isArray(e.cargoTypes) && e.cargoTypes.length > 0)?.cargoTypes?.[0] || '';
      ...
      if (!this._turnbackDepartureReady(stop,timeOfDay,dateStr)) return;
      ...
      if (this._v2AllowEarlyDeparture && stop.type === 'arret' && this.train._stoppedSinceGameTime != null) {
        ...
        if(operationFloor!=null)effectiveDep=Math.max(effectiveDep,operationFloor);
      }
      ...
      if (stop.type === 'passage' || stop.type === 'waypoint' || effectiveDep == null || isNaN(effectiveDep) || forceDepart || this._scheduleGte(scheduleNow, effectiveDep)) {
        if (this.currentStopIndex < stops.length) {
          if (this._waitForConnection(stop, timeOfDay, dateStr)) return;
   ...
   */
  _trackWear(distKm: number, timeOfDay: number) {
    if (!isFinite(distKm) || distKm <= 0) return;
    advanceMaterialMileage(this.rame, this.train, distKm);
    if (this.rame) {
      if (!this.rame.cleanliness) this.rame.cleanliness = { exterior: 100, interior: 100 };
    ...
    if (vpm && this.position) {
      const trc = this._cachedTroncon || vpm.getTronconAtPosition(this.position, 0.3, (this.state as string) === 'stopped_at_station' ? this._stationaryPhysicalTrackRef() : null);
      if (trc) {
    ...
    if (!this.train.breakdown) {
      ...
      if (rng.random() < failureProb) {
        ...
        if (this.rame) {
          if (!Array.isArray(this.rame.pendingDefects)) this.rame.pendingDefects = [];
          if (!this.rame.pendingDefects.includes(type)) this.rame.pendingDefects.push(type);
        }
  ...
  resumeAfterRepair() {
    ...
    this.speed = 0; this.train.speed = 0;
    this._brakeEffort = 0; this._tractiveEffort = 0;
   ...
   */
  _initializeState(route: ServiceRoute, legKey: unknown) {
    this._tailSpeedIndex = null;
    this._electrificationIndex?.reset();
    ...
    for(let i=1;i<route.length;i++){
      ...
      if(next<prev)negativeSpeedTransitions.push({index:i,speed:next,distanceKm:totalRouteKm-cumDist[i]});
    }
    ...
    if (this._cantonAssignments.length > 0) {
      ...
      if (!occupied) this._movementStop('RUNTIME_STOP', this.train.delayReason || 'arrêt de sécurité');
    }
  }
  ...
  _cancelBlockedService(timeOfDay: unknown) {
    ...
    this.cancelled = true;
    this.completed = true;
    this.completedDate = (typeof window !== 'undefined' && window.game?._currentDate) || this._currentDate || ''; this._finishLegacyOperatingDay();
    this.position = null;
    this.speed = 0;
    if (this.train) {
      this.train.speed = 0;
  ...
  _updateStuckTimer(timeOfDay: unknown) {
    ...
    const now: number = this._v2OccurrenceId
      ? this._v2ScheduleNowMinutes(this._currentDate, timeOfDay as number)
      : timeOfDay as number;
    if (this._blockedSinceGameTime == null) this._blockedSinceGameTime = now;
  ...
  _updateHeading(a: __S3Struct699, b: __S3Struct700) {
    ...
    } catch (e: unknown) { this.train.heading = 0; }
  }
  ...
  _routeForStopIndex(stopIndex: number): ServiceRoute | null {
    ...
    if (!this.isReturnLeg) return this.routes?.[leg] || null;
    if (this._returnRoutes?.length) return this._returnRoutes[leg] || null;
  ...
  _passageContinuationIssue(): string | null {
    if (!this._isPassThroughStop(this.getNextStop()) || this.currentStopIndex + 1 >= this.getCurrentStops().length) return null;
    const next = this._routeForStopIndex(this.currentStopIndex + 1);
    const current = this._state?.cachedRoute;
    if (!next || next.length < 2) return 'tracé ferroviaire suivant manquant';
    ...
    if (!end || !Number.isFinite(haversineDistance(end.lat, end.lon, start.lat, start.lon)) ||
        haversineDistance(end.lat, end.lon, start.lat, start.lon) > 0.002) return 'discontinuité ferroviaire après passage';
    return null;
  }
  ...
  _passageLookAheadCap(route: ServiceRoute, timeOfDay: unknown): number | null {
    if (!this._isPassThroughStop(this.getNextStop())) return null;
    const stops = this.getCurrentStops();
    if (this._passageContinuationIssue()) return this._brakingCurveCapKmh(this._getRemainingDistance(route) * 1000, 0, 5);
    ...
    for (let si = this.currentStopIndex + 1; si < stops.length && distance <= horizon; si++) {
      ...
      for (let i = 0; i < following.length - 1 && distance <= horizon; i++) {
        ...
        if (electricOnly && (following[i].electrified === false || following[i + 1].electrified === false)) nextLimit = 0;
        if (nextLimit < this.speed) cap = Math.min(cap, this._brakingCurveCapKmh(distance * 1000, nextLimit, nextLimit ? 100 : 5));
      ...
      if (!this._isPassThroughStop(stops[si])) {
        if (distance <= horizon) cap = Math.min(cap, this._brakingCurveCapKmh(distance * 1000, 0, 0, null, 0.78));
        break;
      }
    }
    return Number.isFinite(cap) ? cap : null;
   ...
   */
  moveUpdate(dt: number, timeOfDay: unknown, allServices: { "length": number }) {
    if (globalThis.window?.game?.depotManager?.ownsServiceMovement?.(this.id)) return;
    ...
    if (strikeBlocked) this.train.delayReason = 'grève';
    if (severeBreakdown) {
      this.train.state = 'en panne';
    ...
    if (this._state.index >= route.length - 1) {
      if (this._holdAtInvalidPassage()) return;
      ...
      if (this._holdForBookedArrival(timeOfDay)) return;
      ...
      this._syncCantonFootprint(route);
      this.arriveAtStation(target, timeOfDay as number, this._economy);
      return;
    }
    ...
    if (this.train.delayReason === 'attente tronçon / réservation') this.train.delayReason = '';
    if (this.speed > 0 && this.train.incident?.effect !== 'stop') this._blockedSinceGameTime = null;
    ...
    const worksLimit = this.isWorkTrain ? null : this.getWorksSpeedLimit(timeOfDay as number);
    ...
    if (!this.isWorkTrain && this._worksClosureAhead) {
      this._startAlternateRouteSearch(timeOfDay as number, dateStrWorks);
      const ready = this._pendingAltRoute;
      if (ready?.completed && ready.route) { this._applyAlternateRoute(ready.route, timeOfDay); return; }
    }
    if (worksLimit === 0) {
      const dateStr = dateStrWorks;
      this._startAlternateRouteSearch(timeOfDay as number, dateStr);
    ...
    if (resources.stopReason) {
      ...
      this._movementStop('CONSUMABLE_STOP', resources.stopReason, 'depot');
      this.train.delayReason = resources.stopReason;
    } else if (this.train.delayReason?.includes('épuisé')) this.train.delayReason = '';
    ...
    if (window.game?.voiePointManager && this.position) {
      ...
      const trainVoie = (this.state as string) === 'stopped_at_station' ? this._stationaryPhysicalTrackRef() : null;
      ...
      if (approachConflict) {
        ...
        if (tronconBlocked) this._movementStop('TRONCON_CONFLICT', 'attente itinéraire / cisaillement', 'interlocking');
        else this._movementCaution(approachConflict.limit, 'TRONCON_APPROACH', 'approche itinéraire / cisaillement', 'interlocking');
    ...
    if (this._cantonAssignments && this._cantonAssignments.length > 0) {
      ...
      const preReserved = cantonManager.reserveNextAhead(
        this._cantonAssignments, segIdx, this.id, frontKm, horizonM
      );
      const blocked = preReserved?.blocked ? preReserved : cantonManager.firstUnavailableAhead(
        this._cantonAssignments, segIdx, this.id, frontKm, horizonM
      );
      if (blocked) {
        ...
        if (!tronconBlocked && immediate && distM <= this._carreMarginM) this._movementStop('CANTON_STOP', 'arrêt avant signal de canton fermé', 'canton');
        else this._movementCaution(Math.min(curveCap,visaCap == null ? Infinity : visaCap),'CANTON_APPROACH','freinage vers signal de canton fermé','canton');
      } else {
        this.train.signalAlert = null;
      }
    }
    ...
    if (blockLimit !== null) {
      ...
      if (blockLimit <= 0) this._movementStop('PROXIMITY_STOP', 'espacement avec le train précédent', 'proximity');
      else this._movementCaution(blockLimit, 'PROXIMITY_CAUTION', 'espacement avec le train précédent', 'proximity');
    }
    ...
    if (ipcsLimit !== null) {
      ...
      } else this._movementCaution(ipcsLimit, 'IPCS_CAUTION', 'croisement IPCS en approche', 'ipcs');
    }
      ...
      if (nextStop?.type === 'arret' && target) {
        ...
        if (remainDist <= reserveHorizonKm) {
          ...
          if (!canArrive) {
            ...
            if (stationCurve <= 0.01) this._movementStop('STATION_RESOURCE', 'attente voie libre en gare', 'station');
            else this._movementCaution(stationCurve, 'STATION_APPROACH', 'approche voie occupée en gare', 'station');
            this.train.delayReason = 'attente voie libre en gare';
          }
        }
      }
    ...
    if (this._yieldToRescue()) effectiveMaxSpeed = 0;
    ...
    if (throughCap != null) physicalTargetSpeed = Math.min(physicalTargetSpeed, throughCap);
    ...
    if (stepKm <= 0) {
      if (this._commitNearEndpointArrival(route, timeOfDay)) return;
      this.train.speed = 0;
    ...
    if (this._state.index < route.length - 1) {
      ...
      if (isFinite(finalDist) && finalDist > 0) {
        ...
        this._trackWear(finalDist, timeOfDay as number);
      }
      this._syncCantonFootprint(route);
      if (this._holdAtInvalidPassage()) return;
    ...
    if (allServices && allServices.length > 1) {
      ...
      if (leader) {
        ...
        if (myFrontKm > hardTargetKm) {
          ...
          if (this.speed <= 0.01) this._movementStop('ANTI_OVERLAP_STOP', 'espacement de sécurité', 'proximity');
          else this._movementCaution(this.speed, 'ANTI_OVERLAP_CAUTION', 'espacement de sécurité', 'proximity');
        }
      }
    }
    ...
    if (isFinite(actualDist) && actualDist > 0) {
      this._trackWear(actualDist, timeOfDay as number);
    }
   ...
   */
  _getRemainingDistance(route: ServiceRoute) {
    const idx = this._state.index;
  ...
  _updateDelayReason() {
    ...
    if (t.movementAuthority?.status === 'STOP') {
      ...
      if (ar && ar !== 'arrêt de sécurité') { t.delayReason = ar; return; }
      if (t.delayReason) return; // keep the exact subsystem reason already published this tick
    }
    if (t.signalAlert === 'closed') { t.delayReason = 'Arrêt pour signal fermé'; return; }
    if (t.movementAuthority?.status === 'CAUTION' && t.movementAuthority.reason) { t.delayReason = t.movementAuthority.reason; return; }
  ...
  _updateContinuousDelay(timeOfDay: unknown) {
    const stops = this.getCurrentStops();
    ...
    if (!this._v2OccurrenceId && scheduledTravelTime < 0) scheduledTravelTime += 1440;
    ...
    const scheduleNow = this._v2ScheduleNowMinutes(this._currentDate, timeOfDay as number);
   ...
   */
  _moveDirectToTarget(dt: unknown, timeOfDay: unknown, target: unknown) {
   ...
   */
  _distanceToRouteIndexM(route: ServiceRoute, targetIndex: unknown) {
    if (!route?.length || !this.position) return Infinity;
    const cum = this._routeCumulativeKm(route);
  ...
  _brakingCurveCapKmh(distanceM: unknown, targetKmh: unknown = 0, marginM: unknown = 0, decelMs2: unknown = null, planningFactor: unknown = 0.86) {
  ...
  _setRouteProgressKm(route: ServiceRoute, targetKm: unknown) {
    ...
    while (lo < hi) {
      ...
      if ((cum[mid] || 0) <= target) lo = mid; else hi = mid - 1;
    }
  ...
  _routeCumulativeKm(route: ServiceRoute | null): Float64Array | null {
  ...
  _projectRoutePosition(pos: { lat: number; lon: number }, route: ServiceRoute, hintIndex: unknown = 0, windowKm: number = 6) {
    if (!pos || !route || route.length < 2) return { progressKm:0, distanceKm:Infinity, segmentIndex:0, t:0 };
    const cum = this._routeCumulativeKm(route);
    const hint = Math.max(0, Math.min(route.length - 2, Number(hintIndex) || 0));
    const centerKm = cum[hint] || 0;
    let lo = hint, hi = hint;
    while (lo > 0 && centerKm - cum[lo] < windowKm) lo--;
    while (hi < route.length - 2 && cum[hi + 1] - centerKm < windowKm) hi++;
  ...
  _currentFrontKm(route: unknown = this._state?.cachedRoute) {
    if (!Array.isArray(route) || route.length < 2) return 0;
    const cum = this._routeCumulativeKm(route);
    const rawIdx = Number(this._state?.index) || 0;
    if (rawIdx >= route.length - 1) return Number(cum[route.length - 1] || 0);
  ...
  _routePositionAtKm(route: unknown, targetKm: unknown) {
    if (!Array.isArray(route) || route.length < 2) return this.position ? { ...this.position } : null;
    const cum = this._routeCumulativeKm(route);
    ...
    while (lo < hi) {
      const mid = (lo + hi + 1) >> 1;
      if (Number(cum[mid] || 0) <= target) lo = mid; else hi = mid - 1;
    }
    const i = Math.max(0, Math.min(route.length - 2, lo));
  ...
  _reserveUpcomingTroncon(vpm: TronconManager, route: ServiceRoute, currentTrc: TronconResource | null = null) {
    if (!vpm || !Array.isArray(vpm.troncons) || !vpm.troncons.length || !route?.length) return null;
    const frontKm = this._currentFrontKm(route);
    ...
    for (let d = 0.03; d <= horizonKm + 1e-9; d += stepKm) {
      ...
      if (!trc || trc.id === currentTrc?.id || this._occupiedTronconIds.has(trc.id)) continue;
      candidate = trc; distanceKm = d; break;
    }
    if (!candidate) {
      if (this._reservedTronconId) vpm.releaseTronconReservation?.(this._reservedTronconId, this.id);
      this._reservedTronconId = null;
      return null;
    }
  ...
  _syncTronconTail(vpm: TronconManager, currentTrc: TronconResource | null) {
    ...
    if (currentTrc) {
      ...
      if (ownsOrAcquired) {
        ...
        if (this._reservedTronconId === currentTrc.id) this._reservedTronconId = null;
  ...
  _restoreHeldDepartureSafety() {
    ...
    if (hold.voiePointId && vpm) vpm.occupyVoiePoint?.(hold.voiePointId, this.id);
    if (hold.stationId && hold.platformSource !== 'VOIE_POINT' && typeof window !== 'undefined' && window.game?.platformManager) {
      const station = this.world?.getStationById?.(hold.stationId as string);
      const assigned = window.game.platformManager.assignPlatform(
        hold.stationId, this.id, station?.platforms || 2, hold.platform ?? null, { exactPreferred: !!this._v2OccurrenceId || !!hold.trackIdentity, trackIdentity: hold.trackIdentity, displayName: hold.displayName }
      );
    ...
    if (!Array.isArray(oldRoute) || oldRoute.length < 2) return;
    const lengthM = Math.max(1, Number(this.rame?.totalLength || this.train?.length || 20));
    const travelledM = Math.max(0, (Number(this.totalDistance || 0) - Number(hold.startTravelKm || 0)) * 1000);
    const rearOnOldM = Math.max(0, lengthM - travelledM);
    if (rearOnOldM <= 0.5) return;
    ...
    const oldIds = new Set([...tracked].filter((id: unknown) => !currentIds.has(id)));
  ...
  _restoreStationarySafetyFootprint() {
    if (this.state !== 'stopped_at_station' || !this.position) return;
  ...
  _capturePassageTailSpeedLimits(route: ServiceRoute | null) {
    if (!route?.length || route !== this._state?.cachedRoute || !this._state.segDists) return;
    ...
    const entries: PassageTailSpeedHold[] = [];
    let behindKm = 0;
    for (let i = distances.length - 1; i >= 0 && behindKm < lengthKm; --i) {
      const limitKmh = Math.min(speeds[i] ?? Infinity, speeds[i + 1] ?? Infinity);
      if (Number.isFinite(limitKmh)) entries.push({ endTravelKm: Number(this.totalDistance || 0) - behindKm, limitKmh });
  ...
  _getPassageTailSpeedLimit(): number {
    const queue = this._passageTailSpeedHolds ||= [];
    const tailKm = Number(this.totalDistance || 0) - Math.max(0, Number(this.rame?.totalLength || this.train?.length || 20)) / 1000;
    let expired = 0;
    while (expired < queue.length && tailKm >= queue[expired].endTravelKm) ++expired;
    if (expired) queue.splice(0, expired);
  ...
  _syncCantonFootprint(route: ServiceRoute | null = this._state?.cachedRoute) {
    if (!this._cantonAssignments?.length || !route?.length) return true;
    const frontKm = this._currentFrontKm(route);
    const lengthM = Math.max(1, Number(this.rame?.totalLength || this.train?.length || 20));
    ...
    if (this._carryoverCantonIds?.size && sinceCarryM < lengthM) retain = this._carryoverCantonIds;
  ...
  _getRouteProgressKm(pos: { lat: number; lon: number }, route: ServiceRoute, hintIndex: unknown) {
  ...
  _wayIdsAround(route: ServiceRoute, index: unknown, radiusKm: number = 0.6) {
    const out = new Set();
    ...
    add(i0); add(Math.min(route.length - 1, i0 + 1));
    let acc = 0;
  ...
  _wayIdsAlong(route: ServiceRoute, index: unknown, aheadKm: number = 0.6, behindKm: number = 0.3) {
    const out = new Set();
    if (!route?.length) return out;
    const i0 = Math.max(0, Math.min(route.length - 1, Number(index) || 0));
    const add = (i: number) => {
  ...
  _safetyHorizonKm(speedKmh: unknown = this.speed) {
  ...
  _safetyCandidatePool(allServices: unknown, radiusKm: number = 6): ActiveService[] {
    const game = globalThis.window?.game;
    const fleet = (Array.isArray(allServices) ? allServices : game?.scheduleCreator?.services) as ActiveService[] | undefined;
    const grid = game?._serviceGrid as Map<string, ActiveService[]> | undefined;
    const frame = game?._safetyFleetFrame as {
      services: ActiveService[]; length: number;
      members: Map<ActiveService, number>;
    } | undefined;
    ...
    const map = new Map<string, ActiveService>();
    let stale = false;
    const add = (list: ActiveService[] | undefined) => {
      for (const s of list || []) {
        if (!s || s.id === this.id) continue;
        if (fleet && frame) {
          const index = frame.members.get(s);
          if (index === undefined || fleet[index] !== s) { stale = true; continue; }
        }
    ...
    if (grid && this.position) {
      ...
      for (let a = -latCells; a <= latCells; a++) for (let b = -lonCells; b <= lonCells; b++) add(grid.get(`${latKey + a},${lonKey + b}`));
    }
    // Direct edits during a frame invalidate the acceleration, not the railway.
    if (stale && fleet) return fleet.filter(s => s && s.id !== this.id);
    if (!grid?.size && fleet) add(fleet);
    return [...map.values()];
  }

  _stationaryPhysicalTrackRef(): string | number | null {
    const assignment = this._platformAssignment;
    if (assignment?.source === 'VOIE_POINT') return assignment.platform as string | number | null;
    if (assignment?.trackIdentity) return assignment.trackIdentity.trackRef || null;
    const stops = this.getCurrentStops();
    const stop = stops[Math.max(0, this.currentStopIndex - 1)];
    if (stop?.trackIdentity) return stop.trackIdentity.trackRef || null;
    return this.train.platform == null ? null : String(this.train.platform);
  }

  _findPhysicalLeader(allServices: unknown, route: ServiceRoute, radiusKm: number = 6): PhysicalLeader | null {
    if (!this.position || !route?.length) return null;
    const myProj = this._projectRoutePosition(this.position, route, this._state.index, radiusKm + 1);
    const vpm = typeof window !== 'undefined' ? window.game?.voiePointManager : null;
    const myVoie = (this.state as string) === 'stopped_at_station' ? this._stationaryPhysicalTrackRef() : (vpm ? vpm.getVoieAtPosition(this.position) : null);
    let best: PhysicalLeader | null = null;
    ...
    let latitudeBounds: [number, number] | null | undefined;

    const scan = (candidates: unknown) => {
      for (const other of (Array.isArray(candidates) ? candidates : []) as ActiveService[]) {
        if (!other || samePhysicalMovement(other.id, this.id) || !other.position) continue;
        if (other.state === 'completed' || other.state === 'cancelled') continue;
        const rawDist = haversineDistance(this.position!.lat, this.position!.lon, other.position.lat, other.position.lon);
        if (rawDist > radiusKm) continue;
        if (latitudeBounds === undefined) {
          ...
          if (route.length <= 2048 && Array.isArray(candidates) && candidates.length >= 4) {
            ...
            for (const point of route) {
              if (!Number.isFinite(point?.lat)) { valid = false; break; }
              minLat = Math.min(minLat, point.lat); maxLat = Math.max(maxLat, point.lat);
            }
            if (valid) latitudeBounds = [minLat - 0.0005, maxLat + 0.0005];
          }
        }
        if (latitudeBounds && (other.position.lat < latitudeBounds[0] || other.position.lat > latitudeBounds[1])) continue;
        ...
        const oRoute = (otherStationary
          ...
          : (other._state?.cachedRoute || other.getCurrentRoute?.())) as ServiceRoute | null;
        ...
        if (op.distanceKm > 0.05 || op.progressKm <= myProj.progressKm) continue;
        ...
        const wayOverlap = wayKnown && [...routeWayIdsAtOther].some((id: unknown) => otherWayIds.has(id));
        if (wayKnown && !wayOverlap) continue;
        if (!wayKnown && myVoie && otherVoie && canonicalTrackRef(myVoie) !== canonicalTrackRef(otherVoie)) continue;
        ...
        if (otherPhysicalSpeed > 1 && oRoute && oRoute.length > 1 && oIdx < oRoute.length - 1 && myProj.segmentIndex < route.length - 1) {
          ...
          if (a && b && oa && ob) {
            ...
            let hd = Math.abs(myH - oH); if (hd > Math.PI) hd = 2 * Math.PI - hd;
    ...
    if (!best && Array.isArray(allServices) && pool.length < allServices.length - 1) scan(allServices);
    return best;
  }
  ...
  _clipAdvanceToPhysicalLeader(route: ServiceRoute, requestedKm: number): number {
    ...
    if (!leader || requestedKm <= 0) return requestedKm;
    const frontKm = this._currentFrontKm(route);
    const availableKm = Math.max(0, leader.frontProgressKm - leader.lengthM / 1000 - 0.03 - frontKm);
    if (requestedKm <= availableKm) return requestedKm;
    this.speed = Math.min(this.speed, leader.leaderSpeedKmh);
    this.train.speed = Math.round(this.speed);
    if (this.speed <= 0.01) this._movementStop('ANTI_OVERLAP_STOP', 'espacement de sécurité', 'proximity');
    else this._movementCaution(this.speed, 'ANTI_OVERLAP_CAUTION', 'espacement de sécurité', 'proximity');
  ...
  _proximityBlockCheck(allServices: unknown) {
    ...
    if (leader.tailClearingDeparture && mySpeed <= 5) return 0;
    const fallbackMs2 = Math.max(0.05, Number(this.train?.decel || 2) / 3.6);
    ...
    if (leader.rearGapKm <= 0.03) return 0; // hard 30 m anti-overlap envelope
    ...
    if (leader.rearGapKm <= marginKm) return Math.min(mySpeed, leader.leaderSpeedKmh);
  ...
  _isNearRoute(pos: __S3Struct710, route: ServiceRoute, hintIndex: number = 0) {
  ...
  _ipcsBlockCheck(candidates: unknown = null) {
    ...
    const vpm = globalThis.window?.game?.voiePointManager;
    const myVoie = (this.state as string) === 'stopped_at_station' ? this._stationaryPhysicalTrackRef() : (vpm ? vpm.getVoieAtPosition(this.position, 0.3) : null);
    ...
    for (const other of list) {
      ...
      if (projected.distanceKm > 0.05 || distanceKm <= 0 || distanceKm > horizonKm) continue;
      ...
      const wayOverlap = wayKnown && [...myWayIds].some((id: unknown) => otherWayIds.has(id));
      if (wayKnown && !wayOverlap) continue;
      if (!wayKnown) {
        ...
        if (myVoie && otherVoie && canonicalTrackRef(myVoie) !== canonicalTrackRef(otherVoie)) continue;
      }
      ...
      if (hDiff <= Math.PI / 2) continue; // same direction: leader logic handles it
      ...
      if (availableForMeM <= 0) return 0;
      const safeKmh = Math.sqrt(Math.max(0, 2 * myDecel * availableForMeM)) * 3.6;
      if (safeKmh < mySpeed) limit = Math.min(limit ?? Infinity, Math.max(0, safeKmh));
    }
    return limit;
  }
   ...
   */
  moveMacro(dt: number, timeOfDay: unknown, economy: Economy, allServices: unknown) {
    this._economy = economy;
    ActiveService.prototype.moveUpdate.call(this, dt, timeOfDay, (Array.isArray(allServices) ? allServices : []) as { length: number });
  }

  // Legacy compatibility
  update(timeOfDay: number, dateStr: string, economy: Economy, allServices: unknown) {
    this._economy = economy;
    this.scheduleTick(timeOfDay, dateStr, economy);
  }

  _canSafelyReversePhysicalRoute(route: unknown) {
    ...
    for (const p of route) {
      ...
      if (hasWay || bi || service || oneway) explicit = true;
      if (['yes','1','true','forward','-1','reverse','backward'].includes(oneway)) return false;
      ...
      if (hasWay && bi !== 'regular' && !service) return false;
    }
  ...
  _getStationaryRoute() {
    ...
    if (Array.isArray(this._stationaryRoute) && this._stationaryRoute.length >= 2) return this._stationaryRoute;
    const legIdx = Number(this.currentStopIndex || 0) - 2;
    if (legIdx < 0) return null;
    if (!this.isReturnLeg) return this.routes?.[legIdx] || null;
    if (this._returnRoutes?.length) return this._returnRoutes[legIdx] || null;
    if (!this.routes?.length) return null;
    const routeIdx = this.routes.length - 1 - legIdx;
    const route = this.routes[routeIdx];
    if (!route) return null;
    if (!this._canSafelyReversePhysicalRoute(route)) return null;
  ...
  getWorksSpeedLimit(timeOfDay: number) {
  ...
  _distanceAheadToRestriction(route: unknown, restriction: { route: unknown; stationOnly: unknown; stationId: unknown; stationA: unknown; stationLat: unknown; stationLon: unknown }) {
    ...
    if (restriction?.stationOnly) {
      ...
      for(let i=idx0;i<route.length-1;i++){
        ...
        if(d<=0.12)return Math.max(0,Number(cum[i]||0)-frontKm);
      }
      return Infinity;
    }
    ...
    if (workWays.size) {
      for (let i = idx0; i < route.length; i++) {
        if (workWays.has(String(route[i]?.wayId || ''))) return Math.max(0, Number(cum[i] || 0) - frontKm);
      }
      return Infinity;
    }
    ...
    for (let i = idx0; i < route.length; i += step) {
      ...
      if (d <= 0.04) return Math.max(0, Number(cum[i] || 0) - frontKm);
    }
    return Infinity;
  }

  _computeWorksLimit(route: ServiceRoute, dateStr: unknown, timeOfDay: unknown) {
    ...
    for (const w of blocking) {
      ...
      if (!Number.isFinite(aheadKm)) continue;
      ...
      if (isClosure || powerBlocks) {
        ...
        if (aheadM <= horizonM) limits.push(this._brakingCurveCapKmh(aheadM, 0, this._carreMarginM));
        continue;
      }
      if (w.impact === 'power-off') continue;
  ...
  _minDistToPolyline(lat: unknown, lon: unknown, polyline: ServiceRoute) {
    if (!polyline || polyline.length < 2) return Infinity;
    let minD = Infinity;
    for (let i = 0; i < polyline.length - 1; i++) {
      const d = this._pointToSegmentDistKm(lat as number, lon as number, polyline[i].lat as number, polyline[i].lon as number, polyline[i + 1].lat as number, polyline[i + 1].lon as number);
      if (d < minD) minD = d;
    }
    return minD;
  }

  _routeIntersectsPolyline(route: ServiceRoute, polyline: ServiceRoute, bufferKm: number = 0.1) {
  ...
  _routesShareOsmWay(route: ServiceRoute, polyline: ServiceRoute) {
    ...
    if (!a.size || !b.size) return null; // caller may fall back to geometry
    for (const id of a) if (b.has(id)) return true;
    return false;
  }

  _isFallbackRoute(route: unknown) {
  ...
  _getBlockingWorksForRoute(route: ServiceRoute, dateStr: unknown, timeOfDay: unknown) {
    const blocking: Array<{
      impact: string; speedLimit: number | null | undefined; stationOnly: boolean | undefined; stationId: string | undefined; stationLat: unknown; stationLon: unknown;
      stationA: string | undefined; stationB: string | undefined; route: ServiceRoute | undefined; startBinding?: unknown; endBinding?: unknown;
      direction?: string; workId?: string; zoneId?: string;
    }> = [];
    ...
    if (worksMgr) {
      ...
      for (const w of restrictions) {
        ...
        if (w.stationOnly) {
          ...
          if (!adjacent && !routePassesStation) continue;
        } else if (poly && poly.length >= 2) {
          const sharedWay = this._routesShareOsmWay(route, poly);
          if (sharedWay === false) continue;
          if (sharedWay == null && !this._routeIntersectsPolyline(route, poly, 0.015)) continue;
          ...
          if (!railSectionDirectionMatchesRoute(route, w)) continue;
        } else if (prevId && nextId) {
  ...
  async _findAlternateRoute(prevId: string, nextId: string, dateStr: string, timeOfDay: number) {
    ...
    const candidates = this.world.stations
      .filter((s: { id: unknown }) => s.id !== prevId && s.id !== nextId)
      .map((s: __S3Struct715) => ({ s, d: haversineDistance(s.lat, s.lon, mid.lat, mid.lon) }))
      .sort((a: __S3Struct716, b: __S3Struct717) => a.d - b.d)
      .slice(0, 12);
  ...
  _startAlternateRouteSearch(timeOfDay: number, dateStr: string) {
    const currentStops = this.getCurrentStops();
  ...
  _applyAlternateRoute(route: ServiceRoute, timeOfDay: unknown) {
  ...
  arriveAtStation(station: ArrivalStation, timeOfDay: number, economy: Economy) {
    ...
    if (!economy) economy = this._economy || globalThis.window?.game?.economy || null;
    if (economy) this._economy = economy;
    const stops = this.getCurrentStops();
    ...
    if (this.serviceType === 'evo' && this._evoForServiceId) {
      const passenger = window.game?.scheduleCreator?.services.find((s: { id: unknown }) => s.id === this._evoForServiceId);
      if (passenger) {
    ...
    let arrivalLat: number = station.lat, arrivalLon: number = station.lon;
    ...
    if (stop?.trackIdentity?.kind === 'osm' && boundLat != null && boundLon != null
      ...
      arrivalLat = Number(boundLat); arrivalLon = Number(boundLon);
      ...
      if (stop.voiePointId) {
        ...
        if (vp) { arrivalLat = vp.lat as number; arrivalLon = vp.lon as number; }
      }
      // Priority 2: look up by station + platform name
        ...
        if (svp) { arrivalLat = svp.lat as number; arrivalLon = svp.lon as number; }
      }
    }
    this.position = { lat: arrivalLat, lon: arrivalLon };
    this._movementGo();
    this._lastArrivalTime = scheduleNow;this._currentTimeOfDay=timeOfDay;
    ...
    if (stop?.type === 'arret' && station) {
      try {
        ...
        const rameCargo = this.rame?.elementDetails?.find((e: { cargoTypes: unknown }) =>
        ...
        if (iteInfo?.isITE) {
          ...
          if (this._iteHardBlock) this.train.delayReason = 'ITE : aucune voie utile';
    ...
    if (economy && stop?.type === 'arret') {
      const firstArretIdx = stops.findIndex((s: { type: unknown }) => s.type === 'arret');
      const lastArretIdx = stops.length - 1 - [...stops].reverse().findIndex((s: { type: unknown }) => s.type === 'arret');
      ...
      try { economy.processStopRevenue(this, station.name, distFromPrev, isFirst, isTerminus, String(station.id), legRoute); }
      catch (e: unknown) { console.warn('Economy arrival side effect ignored:', e); }
    }
    ...
    if (this._isPassengerService()) globalThis.window?.game?.connections?.onArrival(this, stop, timeOfDay, this._currentDate);

    if (this._v2OccurrenceId && typeof this._v2OnArrive === 'function') {
      try { this._v2OnArrive(stop, station, timeOfDay); }
      catch (e: unknown) { console.warn('V2 rotation action:', e); }
    }
    ...
    if (isPassThrough) this._captureCantonCarryover();
    ...
    this._resetState();
    if (stop?.turnBack) this._turnbackDepartureReady(stop,timeOfDay,this._currentDate);
  ...
  completeService(economy: Economy, station: ArrivalStation | null = null, arrivalLat: number | null = null, arrivalLon: number | null = null) {
    ...
    if (this._terminalOperationPending()) {
      ...
      this.train.state = 'stopped_at_station';
      this.speed = 0; this.train.speed = 0;
      this._movementStop(this._v2OperationState?.failed ? 'ROTATION_OPERATION_FAILED' : 'ROTATION_OPERATION',
        this._v2OperationWaitingMaterial ? 'opérations au terminus : attente matériel' : 'opérations au terminus', 'rotation');
      return;
    }
    ...
    if (terminalStop?.turnBack && !this._turnbackDepartureReady(terminalStop,Number(this._currentTimeOfDay ?? this._lastArrivalTime ?? 0),this._currentDate)) {
      this.state='stopped_at_station';this.train.state='stopped_at_station';return;
    }
    const completedTrack = materialTrackLocation(terminalStop);
    globalThis.window?.game?.freightManager?.finishServiceCargo?.(this);
    if (!this.revenueCollected && economy) {
      try { economy.processServiceRevenue(this); }
      catch (e: unknown) { console.warn('Economy completion side effect ignored:', e); }
    ...
    if (this.rame) syncMaterialMileage(this.rame, this.train);
    ...
    if (this.roundTrip && !this.isReturnLeg) {
      ...
      if ((this.delay ?? 0) > 0 && rng.random() < 1 / 3) {
        ...
        this.completed = true; this.completedDate = this._currentDate || ''; this._finishLegacyOperatingDay();
        this.train.stoppedAt = station || this.train.stoppedAt;
        if (this.rame) {
          this.rame.currentLocation = {
            ...completedTrack,
            stationId: (station?.id || this.rame.currentLocation?.stationId || '') as string,
            depotId: '', // RC2: home depot is affiliation, not physical presence.
            serviceId: '',
    ...
    if (this.roundTrip && this.isReturnLeg && this.multiDepartures && this._tripCount < this.multiDepartures) {
      ...
      if (this.stops.length > 0 && this.world) {
        ...
        if (firstStation) {
          if (!this.position) this.position = { lat: arrivalLat ?? firstStation.lat, lon: arrivalLon ?? firstStation.lon };
    ...
    this._atTerminus = false;
    this.completed = true;
    this.completedDate = this._currentDate || ''; this._finishLegacyOperatingDay();
    this.isReturnLeg = false;
    this.revenueCollected = false;
    this._tripCount = 0;
    this._adjustedStops = null;
  ...
  _rebuildStopsFromTime(departureTime: __KPM361) {
    const offset = departureTime - (this.stops[0]?.departureTime || 0);
    return this.stops.map((s: ServiceStop) => {
      ...
      return new ServiceStop(
        ...
        dep < 0 ? dep + 1440 : dep,
        arr < 0 ? arr + 1440 : arr,
        s.voiePointId, s.platform, s.stopCode, { ...materialTrackLocation(s), lat:s.lat, lon:s.lon, turnBack:s.turnBack }
      );
    });
  }
  ...
  _buildAdjustedStops(sourceStops: ServiceStop[] = this.stops, rng: () => number = Math.random) {
    return sourceStops.map((s: ServiceStop) => {
      let skip = false;
      ...
      if (s.type === 'arret' && skip) {
        const adjusted = new ServiceStop(
          ...
          s.voiePointId, s.platform, s.stopCode, extra
        );
  ...
  buildReturnStops() {
    ...
    const fwdIndices = fwdStops.map((s: { stationId: unknown }, i: unknown) => ({ stationId: s.stationId, idx: i }));
    ...
    return reversed.map((stop, i) => {
      ...
      if (prevStop) {
        ...
        if (origIdxPrev >= 0 && origIdxCurr >= 0 && origIdxPrev < fwdStops.length && origIdxCurr < fwdStops.length) {
          travelTime = forwardClockMinutes(
            fwdStops[origIdxCurr].departureTime ?? fwdStops[origIdxCurr].arrivalTime,
            fwdStops[origIdxPrev].arrivalTime ?? fwdStops[origIdxPrev].departureTime);
        }
        if (!Number.isFinite(travelTime)) travelTime = 15;
      }
      ...
      const rs = new ServiceStop(stop.stationId, stop.type, depTime, arrTime, physical.voiePointId, returnPlat, stop.stopCode,
        {...physical, lat:sameTrack ? stop.lat : explicitNative?.lat, lon:sameTrack ? stop.lon : explicitNative?.lon, turnBack:stop.turnBack});
      return rs;
    });
  }
  ...
  _buildIndependentReturnStops(fwdStops: ServiceStop[]) {
    const lastStop = fwdStops[fwdStops.length - 1];
    const lastArrival = (lastStop?.arrivalTime ?? lastStop?.departureTime ?? 0);
    ...
    const wrap = (t: __KPA118) => { const m = t % 1440; return m < 0 ? m + 1440 : m; };
    return data.map((s: ServiceStop) => {
      const dep = anchor + ((s.departureTime ?? s.arrivalTime ?? 0) - base);
      const arr = anchor + ((s.arrivalTime ?? s.departureTime ?? 0) - base);
      return new ServiceStop(
        s.stationId, s.type, wrap(dep), wrap(arr), s.voiePointId, s.platform, s.stopCode, { ...materialTrackLocation(s), lat:s.lat, lon:s.lon, turnBack:s.turnBack }
      );
    });
  }
}
...
type RameUsageEntry = { moving: unknown | null; waiting: Map<unknown, number> };
type StationPriorityEntry = { map: Map<unknown, number> };

export class ScheduleCreator {
  declare services: ActiveService[];
  declare weather: unknown | null;
  declare _activeCache?: ActiveService[];
  declare _activeCacheVer?: number;
  declare _movingCache?: ActiveService[];
  declare _movingCacheVer?: number;
  declare _serviceVer?: number;
  declare _indexTime?: unknown;
  declare _rameUsage: Map<unknown, RameUsageEntry>;
  declare _stationPriority: Map<unknown, StationPriorityEntry>;
  constructor() {
    this.services = [];
    this.weather = null; // set by game
  }

  addService(data: unknown, rame: Rame, world: World) {
    const svc = new ActiveService((data && typeof data === 'object' ? data : {}) as Record<string, unknown>, rame, world, this.weather);
    const freight = typeof window !== 'undefined' ? window.game?.freightManager : null;
    if (svc.assignedContractId && freight) {
      const contract = freight.contracts.find((c: FreightContract) => c.id === svc.assignedContractId);
  ...
  ensureEVOForService(svc: ActiveService, world: World, timeOfDay: unknown) {
    // V2: no automatic EVO. Positioning is explicitly the player's responsibility.
    if (svc._v2OccurrenceId) return null;
    ...
    if (orm?._ensureGraph && orm?.findNearestNode && orm?.dijkstra) {
      try {
        ...
        if(Array.isArray(resolved)&&resolved.length>=2&&!orm.isFallbackRoute(resolved))route=resolved;
      }catch (e: unknown){route=[];}
    }
    if(route.length<2){svc._evoCreated=true;console.warn('EVO automatique annulé : aucun trajet ORM réel déjà chargé.');return null;}
    ...
    const wrap = (t: number) => { const m = t % 1440; return m < 0 ? m + 1440 : m; };
  ...
  duplicateService(id: unknown, intervalMin: number, count: number, rame: Rame, world: World) {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const shiftStops = (stops: ServiceStop[]) => stops.map((st: ServiceStop) => ({
  ...
  createAutoRoundTripDuplicates(baseService: ActiveService, requestedCount: number, oneRoundTripMin: number, rame: Rame, world: World) {
    if (requestedCount <= 1) return [];
    const created = [];
    const shiftStops = (stops: ServiceStop[], offset: number) => stops.map((st) => ({
      stationId: st.stationId, type: st.type,
      departureTime: (st.departureTime ?? st.time) + offset,
      arrivalTime: (st.arrivalTime ?? st.time) + offset,
      voiePointId: st.voiePointId || null, platform: st.platform || '', stopCode: st.stopCode || '',
  ...
  _addToTickIndexes(svc: ActiveService, timeOfDay: unknown) {
    if (svc._v2OccurrenceId) return;
    if (svc._legacyOriginEligible?.(Number(timeOfDay)) === false) return;
  ...
  _removeFromTickIndexes(svc: ActiveService) {
  ...
  updateServiceIndexes(svc: ActiveService, timeOfDay: unknown) {
    if (!this._rameUsage) return;
    this._removeFromTickIndexes(svc);
    this._addToTickIndexes(svc, timeOfDay);
  }

  isRameInUse(rameId: unknown, excludeId: string, timeOfDay: unknown) {
    ...
    const blocks = (otherId: string, otherDep: unknown) => {
    ...
    if (!this._rameUsage || this._indexTime !== timeOfDay) {
      for (const svc of this.getActiveServices()) {
        ...
        if (svc.completed || svc._legacyOriginEligible?.(Number(timeOfDay)) === false) continue;
    ...
    for (const [id, dep] of entry.waiting) {
      ...
      if (diff >= -2 && diff <= 5 && blocks(id as string, dep)) return true;
    }
    return false;
  }
  ...
  getEarliestDueServiceAtStation(stationId: unknown, timeOfDay: unknown) {
    ...
    for (const [id, dep] of sEntry.map) {
      ...
      if (indexedSvc?.train?.incident?.effect === 'stop' || (indexedSvc && indexedSvc._legacyOriginEligible?.(Number(timeOfDay)) === false)) continue;
  ...
  _encodeRouteForSave(route: unknown) {
    if (!Array.isArray(route) || route.length === 0) return null;
    const q = 1e6;
    const coords = [];
    const exactCoords: Array<[number,number,number]> = [];
    ...
    for (let i = 0; i < route.length; i++) {
      ...
      if(latQ/q!==Number(pt.lat) || lonQ/q!==Number(pt.lon)) exactCoords.push([i,Number(pt.lat),Number(pt.lon)]);
      if (i === 0) coords.push(latQ, lonQ);
      else coords.push(latQ - prevLat, lonQ - prevLon);
      prevLat = latQ; prevLon = lonQ;

      const sp = Number.isFinite(Number(pt.maxSpeed)) ? Number(pt.maxSpeed) : 30;
      speeds.push(sp);
      if (sp !== 30) hasCustomSpeed = true;
      if (String(pt.maxSpeedSource || '') === 'FALLBACK_30') fallbackSpeedIdx.push(i);
      ...
      directions.push(dc); if (dc) hasDirection = true;

      const ec = pt.electrified === true ? 1 : (pt.electrified === false ? 0 : -1);
      electrification.push(ec); if (ec !== -1) hasElectrification = true;

      const rawIncline = pt.tags?.incline ?? pt.incline;
      if (rawIncline != null && rawIncline !== '') inclines.push([i, String(rawIncline)]);
    }

    const out: Record<string, unknown> = { c: coords, q: 6 };
    if(exactCoords.length)out.rc=exactCoords;
    if (hasCustomSpeed) out.s = speeds;
    if (fallbackSpeedIdx.length) out.f = fallbackSpeedIdx;
    if (wayDict.length) { out.wd = wayDict; out.w = wayIdx; out.wm = wayMeta; }
    if (hasDirection) out.d = directions;
    if (hasElectrification) out.e = electrification;
    if (inclines.length) out.i = inclines;
    return out;
  }

  _runtimeRouteRef(service: ActiveService, route: unknown) {
    if (!service || !Array.isArray(route)) return null;
    const fi = (service.routes || []).indexOf(route);
    if (fi >= 0) return { s:'f', i:fi };
    const ri = (service._returnRoutes || []).indexOf(route);
    if (ri >= 0) return { s:'r', i:ri };
    return null;
  }

  _runtimeRouteFromRef(service: ActiveService, ref: { s: unknown; i: unknown }, encoded: unknown = null): ServiceRoute | null {
    if (ref?.s === 'f') return service.routes?.[Number(ref.i)] || null;
    if (ref?.s === 'r') return service._returnRoutes?.[Number(ref.i)] || null;
    if (encoded) return (this._decodeRoutes([encoded])?.[0] || null) as ServiceRoute | null;
    return null;
  }
  ...
  toSave() {
    ...
    return this.services.filter((s) => !s._v2OccurrenceId).map((s: ActiveService) => {
      try {
        // Compact route encoding preserving the complete physical ORM track.
        const safeRoutes = (s.routes || []).map((route: unknown) => this._encodeRouteForSave(route)).filter(Boolean);
        const safeReturnRoutes = (s._returnRoutes || []).map((route: unknown) => this._encodeRouteForSave(route)).filter(Boolean);
        // Compact stops: short keys
        const compactStops = (s.stops || []).map((st: { stationId: unknown; type: unknown; departureTime: unknown; arrivalTime: unknown; voiePointId: unknown; platform: unknown; stopCode: unknown; trackIdentity?: unknown; lat?: unknown; lon?: unknown; turnBack?:unknown }) => {
          const o: Record<string, unknown> = { si: st.stationId, t: st.type, d: st.departureTime, a: st.arrivalTime };
          if (st.voiePointId) o.vp = st.voiePointId;
          if (st.platform) o.p = st.platform;
          if (st.stopCode) o.sc = st.stopCode;
          if (st.trackIdentity) o.ti = normalizeStationTrackIdentity(st.trackIdentity);
          if (st.turnBack) o.tb = true;
          if (st.lat != null && st.lon != null) { o.la = st.lat; o.lo = st.lon; }
          return o;
        });
        const compactReturnStops = (s._returnStopsData || []).map((st: { stationId: unknown; type: unknown; departureTime: unknown; arrivalTime: unknown; voiePointId: unknown; platform: unknown; stopCode: unknown; trackIdentity?: unknown; lat?: unknown; lon?: unknown; turnBack?:unknown }) => {
          const o: Record<string, unknown> = { si: st.stationId, t: st.type, d: st.departureTime, a: st.arrivalTime };
          if (st.voiePointId) o.vp = st.voiePointId;
          if (st.platform) o.p = st.platform;
          if (st.stopCode) o.sc = st.stopCode;
        ...
        const o: Record<string, unknown> & { _r: Record<string, unknown> } = { id: s.id, n: s.name, ri: s.rameId, st: compactStops, rt: safeRoutes, _r: {} };
        ...
        if (s._stationaryRoute?.length) {
          ...
          if (sr) o._r.sr = sr;
          else o._r.sx = this._encodeRouteForSave(s._stationaryRoute);
        }
        ...
        if (s._adjustedStops) {
          o._r.as = s._adjustedStops.map((st: ServiceStop) => ({
  ...
  _decodeRoutes(routes: unknown) {
    ...
    return routes.map((r: unknown[]) => {
      ...
      if (r && r.c && Array.isArray(r.c)) {
        ...
        const inclineMap = new Map(Array.isArray(r.i) ? r.i.map((x: Record<string, unknown>) => [Number(x?.[0]), x?.[1]]) : []);
        for (let i = 0; i < r.c.length; i += 2) {
          if (i === 0) { lat = r.c[0]; lon = r.c[1]; }
          else { lat += r.c[i]; lon += r.c[i + 1]; }
          const n = i / 2;
          const pt: Record<string, unknown> = { lat: lat / div, lon: lon / div, maxSpeed: 30, tracks: 1 };
          if (r.s && r.s[n] !== undefined) pt.maxSpeed = Number(r.s[n]);
          if (fallbackSet.has(n)) pt.maxSpeedSource = 'FALLBACK_30';
          else if (r.q === 6) pt.maxSpeedSource = 'OSM';
          else pt.maxSpeedSource = 'LEGACY_SAVE';

          const wi = Number(r.w?.[n] || 0);
          if (wi > 0 && r.wd?.[wi - 1] != null) {
            pt.wayId = String(r.wd[wi - 1]);
            const wm = r.wm?.[wi - 1] || {};
            if (wm.s) pt.service = wm.s;
            if (wm.u) pt.usage = wm.u;
            if (wm.o) pt.oneway = wm.o;
            if (wm.b) pt.bidirectional = wm.b;
            if (wm.r) pt.trackRef = wm.r;
          }
          const dc = Number(r.d?.[n] || 0);
          if (dc === 1) pt.travelDirection = 'forward';
          else if (dc === -1) pt.travelDirection = 'backward';
          const ec = r.e?.[n];
          if (ec === 1) pt.electrified = true;
          else if (ec === 0) pt.electrified = false;
          if (inclineMap.has(n)) pt.incline = inclineMap.get(n);
          pts.push(pt);
        }
        if(Array.isArray(r.rc)) for(const item of r.rc as unknown[]) {
          ...
          if(Number.isInteger(index) && index>=0 && index<pts.length && typeof a==='number' && typeof b==='number' && Number.isFinite(a) && Number.isFinite(b) && Math.abs(a)<=90 && Math.abs(b)<=180) {
            pts[index]!.lat=a;pts[index]!.lon=b;
          }
        }
        return pts;
      }
  ...
  _expandCompactService(d: unknown[]) {
    ...
    if (d.n !== undefined && d.ri !== undefined && d.st !== undefined) {
      const expanded: Record<string, unknown> = {
        ...
        name: d.n,
        rameId: d.ri,
        stops: (d.st || []).map((s: { si: unknown; t: unknown; d: unknown; a: unknown; vp: unknown; p: unknown; sc: unknown; ti?: unknown; la?: unknown; lo?: unknown; tb?:unknown }) => ({
        ...
        returnStops: (d.rst || []).map((s: { si: unknown; t: unknown; d: unknown; a: unknown; vp: unknown; p: unknown; sc: unknown; ti?: unknown; la?: unknown; lo?: unknown; tb?:unknown }) => ({
          stationId: s.si, type: s.t, departureTime: s.d, arrivalTime: s.a,
          voiePointId: s.vp || null, platform: s.p || '', stopCode: s.sc || '',
          trackIdentity: normalizeStationTrackIdentity(s.ti), lat:s.la, lon:s.lo,turnBack:s.tb===true,
        })),
      ...
      if (d._r) {
        expanded._runtime = {
          ...
          _adjustedStops: d._r.as ? d._r.as.map((s: { si: unknown; t: unknown; d: unknown; a: unknown; vp?:unknown;p?:unknown;ti?:unknown;la?:unknown;lo?:unknown;tb?:unknown }) => ({
            stationId: s.si, type: s.t, departureTime: s.d, arrivalTime: s.a,
  ...
  loadFromSave(input: unknown, rameManager: RameManager, world: World, timeOfDay: number = 0, dateStr: string = '') {
    ...
    for (let d of arr) {
      ...
      if (d._runtime) {
        ...
        if (Number.isFinite(Number(rt.distanceKm))) svc.totalDistance = Number(rt.distanceKm);
        ...
        if (rt._adjustedStops) {
          svc._adjustedStops = rt._adjustedStops.map((s: { stationId: unknown; type: unknown; departureTime: unknown; arrivalTime: unknown; voiePointId?:unknown; platform?:unknown; stopCode?:unknown; lat?:unknown; lon?:unknown; trackIdentity?:unknown;turnBack?:unknown }) => new ServiceStop(
            s.stationId, s.type, s.departureTime, s.arrivalTime, s.voiePointId, s.platform, s.stopCode,
            {...materialTrackLocation(s),lat:s.lat,lon:s.lon,turnBack:s.turnBack}
          ));
        }
      }
      ...
      if (d._runtime) {
        ...
        svc.train.incidentDelayReasons = (Number(svc.train.delay || svc.delay || 0) >= 0.5 &&
          ...
          ? d._runtime.incidentDelayReasons.map((r: Record<string, unknown>) => ({...r}))
          : [];
      }
      ...
      if ((svc.state === 'moving' || svc.state === 'departing') && svc.position) {
        ...
        if (station && stopAt) {
          ...
          if (d._runtime?.platformAssignment?.platform != null) stopAt.platform = d._runtime.platformAssignment.platform;
          if (d._runtime?.platformAssignment?.voiePointId) stopAt.voiePointId = d._runtime.platformAssignment.voiePointId;
...
// S3_STRUCT_V2_TEMP
type __S3Struct687 = { "time": number };
type __S3Struct688 = { "time": number };
type __S3Struct695 = { "type": string };
type __S3Struct697 = { "length": number };
type __S3Struct699 = { "lat": number; "lon": number };
type __S3Struct700 = { "lat": number; "lon": number };
type __S3Struct702 = { "length": number };
type __S3Struct707 = { "occupyTroncon": (...args: unknown[]) => unknown; "releaseTronconReservation": (...args: unknown[]) => unknown; "releaseTroncon": (...args: unknown[]) => unknown };
type __S3Struct710 = { "lat": number; "lon": number };
type __S3Struct715 = { "lat": number; "lon": number };
type __S3Struct716 = { "d": number };
type __S3Struct717 = { "d": number };
type __S3Struct718 = { "length": number };
type __S3Struct724 = { "map": (...args: unknown[]) => unknown };
type __S3Struct729 = { "map": (...args: unknown[]) => unknown };
