...
type ElectricalSystem = { voltage: number; frequency: number };
type RoutePointLike = AnyRecord & {
  lat?: number | string | null; lon?: number | string | null; wayId?: string | number | null; segmentIndex?: number | null;
  maxSpeed?: number | string | null; maxSpeedSource?: string; maxSpeedForward?: number | string | null; maxSpeedBackward?: number | string | null;
  electrified?: boolean | string | null; electrifiedMode?: string; voltage?: unknown[]; frequency?: unknown[]; gauge?: unknown[]; loadingGauge?: string;
  axleLoad?: number | string | null; metreLoad?: number | string | null; tracks?: number | string; trafficMode?: string; trainProtection?: AnyRecord;
  usage?: string; service?: string; railway?: string; railwayLifecycle?: string; railwayBaseType?: string; preferredDirection?: string;
  bidirectional?: string; oneway?: string; trackRef?: string; name?: string; ref?: string; tags?: AnyRecord; fallback?: boolean; _againstPreferredDirection?: boolean;
};
type ScheduleLeg = {
  id: string; fromLocationId: string; toLocationId: string; constraintIds: string[]; routeInputKey: string; physicsRouteKey: string;
  physicalTravelSec: number | null; physicalTravelSignature: string; routePoints: RoutePointLike[]; segments: RouteSegmentSnapshot[]; distanceKm: number;
};
type PackedScheduleRoute = {
  format: 'SC8P1'; scale: number; count: number; coords: number[]; wayDict: string[]; wayIndex: number[]; segmentIndex: number[]; metaDict: AnyRecord[]; metaIndex: number[];
};
type ValidationIssueLike = { id?: string; level: string; code: string; message: string; data?: unknown };
type ScheduleReference = { scheduleId: string; versionId?: string; locationId?: string };
type RotationManagerBridge = {
  findScheduleReferences?: (id: string) => ScheduleReference[]; removeScheduleReferences?: (id: string) => unknown; removeDirectAssignment?: (id: string) => unknown;
  addOccurrence?: (rotationId: string, data: AnyRecord) => unknown; recalculateRotation?: (rotationId: string) => unknown;
  toSave?: () => unknown; loadFromSave?: (data: unknown) => unknown;
};

type OperatingCalendarInput = AnyRecord & { id?: string; name?: string; startDate?: string; endDate?: string; weekdays?: unknown[]; excludedDates?: unknown[]; enabled?: boolean };
type TrackBindingInput = AnyRecord & { voiePointId?: unknown; wayId?: string | number | null; trackRef?: string; displayName?: string; lat?: unknown; lon?: unknown; snapLat?: unknown; snapLon?: unknown; segmentIndex?: unknown; osmSnapshot?: AnyRecord | null };
type RouteConstraintInput = TrackBindingInput & { id?: string; order?: unknown; legIndex?: unknown };
type RouteSegmentInput = AnyRecord & { from?: AnyRecord | null; to?: AnyRecord | null; distanceKm?: unknown; trainProtection?: AnyRecord; tags?: AnyRecord };
type ScheduleLegInput = AnyRecord & { id?: string; fromLocationId?: string; toLocationId?: string; constraintIds?: unknown[]; routeInputKey?: string; physicsRouteKey?: string; physicalTravelSec?: unknown; physicalTravelSignature?: string; routePoints?: RoutePointLike[]; routePacked?: AnyRecord | PackedScheduleRoute | null; segments?: RouteSegmentInput[]; distanceKm?: unknown };
type SchedulePathInput = AnyRecord & { id?: string; direction?: PathDirectionValue; constraints?: RouteConstraintInput[]; legs?: ScheduleLegInput[]; routePoints?: RoutePointLike[]; segments?: RouteSegmentInput[]; distanceKm?: unknown; validatedAt?: string; ormRevision?: string; topologyRevision?: unknown; resolvedRevision?: unknown; offlineUsable?: boolean; error?: string };
type ScheduledLocationInput = AnyRecord & { id?: string; kind?: LocationKindValue; order?: unknown; stationId?: string; technicalLocationId?: string; name?: string; track?: TrackBindingInput; computedArrivalSec?: unknown; computedDepartureSec?: unknown; arrivalSec?: unknown; departureSec?: unknown; arrivalOverride?: boolean; departureOverride?: boolean; dwellSec?: unknown; stopCode?: StopCodeValue; turnBack?: boolean; notes?: string };
type TechnicalLocationInput = AnyRecord & { id?: string; name?: string; track?: TrackBindingInput; createdAt?: string };
type PerformanceProfileInput = AnyRecord & { id?: string; mode?: PerformanceModeValue; name?: string; referenceRameId?: string; referenceComposition?: AnyRecord[]; category?: TrainCategoryValue; maxSpeed?: unknown; massKg?: unknown; powerW?: unknown; electricPowerW?: unknown; dieselPowerW?: unknown; lengthM?: unknown; adhesionMassKg?: unknown; brakeServiceMs2?: unknown; brakeBuildSec?: unknown; traction?: string; electricSystems?: AnyRecord[]; gauges?: unknown[]; loadingGauge?: string; axleLoad?: unknown; metreLoad?: unknown; source?: string };
type ValidationReportInput = AnyRecord & { timestamp?: string; ormRevision?: string; issues?: ValidationIssueLike[]; deferred?: boolean };
type ScheduleVersionInput = AnyRecord & { id?: string; version?: unknown; createdAt?: string; state?: ScheduleStateValue; calendarIds?: unknown[]; category?: TrainCategoryValue; performanceProfile?: PerformanceProfileInput; locations?: ScheduledLocationInput[]; outboundPath?: SchedulePathInput; returnPath?: SchedulePathInput | null; validationReport?: ValidationReportInput | null; notes?: string; lastRecalculatedAt?: string; allowShorterThanPhysicalTiming?: boolean };
type ScheduleRecordInput = AnyRecord & { id?: string; number?: string; name?: string; createdAt?: string; versions?: ScheduleVersionInput[]; currentVersionId?: string };
type RoundTripGroupInput = AnyRecord & { id?: string; outboundScheduleId?: string; returnScheduleId?: string; terminalLayoverSec?: unknown };
...
let _seq = 1;
export function makeV2Id(prefix = 'v2'): string {
...
function num(value: unknown, fallback: null): number | null;
...
function bool(value: unknown, fallback = false): boolean { return value == null ? fallback : !!value; }

export function isPassengerCategory(category: unknown): boolean {
  return category === TrainCategory.PASSENGER;
}

export function isOptionalStopCode(code: unknown): boolean {
  return code === StopCode.OPTIONAL_C || code === StopCode.OPTIONAL_S;
}

export function formatScheduleClock(totalSec: unknown): string {
...
export function parseScheduleClock(text: unknown, previousSec = 0, options: AnyRecord = {}): number | null {
  if (typeof text === 'number' && Number.isFinite(text)) return Math.max(0, Math.round(text));
  ...
  if (!explicitDay && inferRollover) {
    while (result < previousSec) result += 86400;
  }
  return result;
}

export class OperatingCalendar {
  declare id: string; declare name: string; declare startDate: string; declare endDate: string;
  declare weekdays: number[]; declare excludedDates: string[]; declare enabled: boolean;
  constructor(data: OperatingCalendarInput = {}) {
    ...
    this.weekdays = array(data.weekdays).length ? [...new Set((data.weekdays as unknown[]).map(Number))] : [0,1,2,3,4,5,6];
    this.excludedDates = [...new Set(array(data.excludedDates).map(String))];
    this.enabled = data.enabled !== false;
  }

  matchesDate(dateStr: string): boolean {
    if (!this.enabled || !dateStr) return false;
    if (dateStr < this.startDate || dateStr > this.endDate) return false;
    if (this.excludedDates.includes(dateStr)) return false;
    const d = new Date(`${dateStr}T12:00:00Z`);
    if (Number.isNaN(d.getTime())) return false;
    return this.weekdays.includes(d.getUTCDay());
  }

  toJSON(): this { return deepClone(this); }
}

export class TrackBinding {
  declare voiePointId: string;
  declare wayId: string; declare trackRef: string; declare displayName: string;
  declare lat: number | null; declare lon: number | null; declare snapLat: number | null; declare snapLon: number | null;
  declare segmentIndex: number | null; declare osmSnapshot: AnyRecord | null;
...
export class RouteConstraint {
  declare id: string; declare order: number; declare lat: number | null; declare lon: number | null; declare wayId: string;
  declare snapLat: number | null; declare snapLon: number | null; declare trackRef: string; declare segmentIndex: number | null;
  declare osmSnapshot: AnyRecord | null; declare legIndex: number;
...
export class RouteSegmentSnapshot {
  declare wayId: string; declare from: GeoPoint | null; declare to: GeoPoint | null; declare distanceKm: number; declare maxSpeed: number; declare maxSpeedSource: string;
  declare maxSpeedForward: number | null; declare maxSpeedBackward: number | null; declare electrified: boolean | string | null; declare electrifiedMode: string;
  declare voltage: number[]; declare frequency: number[]; declare gauge: number[]; declare loadingGauge: string; declare axleLoad: number | null; declare metreLoad: number | null;
  declare tracks: number; declare trafficMode: string; declare trainProtection: AnyRecord; declare usage: string; declare service: string; declare railway: string;
  declare railwayLifecycle: string; declare railwayBaseType: string; declare preferredDirection: string; declare bidirectional: string; declare oneway: string;
  declare _againstPreferredDirection: boolean; declare trackRef: string; declare name: string; declare ref: string; declare tags: AnyRecord; declare fallback: boolean;
...
function compactGlobalPathSegment(s: AnyRecord = {}): AnyRecord {
  return {
...
function schedulePointDistanceKm(a: RoutePointLike, b: RoutePointLike): number {
  if (!a || !b || ![a.lat,a.lon,b.lat,b.lon].every(x=>Number.isFinite(Number(x)))) return NaN;
...
function documentedScheduleSpeedMeta(p: RoutePointLike): {speed:number;source:string} | null {
  const speed=Number(p?.maxSpeed),source=String(p?.maxSpeedSource||'');
  if(Number.isFinite(speed)&&speed>0&&source!=='FALLBACK_30')return {speed,source:source||'OSM'};
  return null;
}

function derivedSegmentSpeedMeta(a: RoutePointLike, b: RoutePointLike): {speed:number;source:string} {
  const bm=documentedScheduleSpeedMeta(b),am=documentedScheduleSpeedMeta(a);
  if(bm)return bm;
  const aw=String(a?.wayId||''),bw=String(b?.wayId||'');
  if(am && (!aw || !bw || aw===bw))return am;
  return {speed:Number(b?.maxSpeed??a?.maxSpeed??30)||30,source:'FALLBACK_30'};
}


function normalizeSavedSameWaySpeeds(points: RoutePointLike[]): boolean {
  const pts=Array.isArray(points)?points:[];let changed=false;
  const original=pts.map(p=>documentedScheduleSpeedMeta(p));
  for(const [i,j] of [[0,1],[pts.length-1,pts.length-2]]){
    if(i<0||j<0||i>=pts.length||j>=pts.length||original[i]||!original[j])continue;
    if(!String(pts[i]?.wayId||'')||String(pts[i].wayId)!==String(pts[j].wayId))continue;
    if(railPointDistanceKm(pts[i],pts[j])>1)continue;
    pts[i].maxSpeed=original[j]!.speed;pts[i].maxSpeedSource=original[j]!.source;changed=true;
  }
  return changed;
}

function deriveRouteSegmentsFromPoints(points: RoutePointLike[] = []): RouteSegmentSnapshot[] {
...
function packScheduleRoutePoints(points: RoutePointLike[] = []): PackedScheduleRoute | null {
  const pts=Array.isArray(points)?points:[];
  ...
  for(let i=0;i<pts.length;i++){
    ...
    if(i===0)coords.push(lat,lon);else coords.push(lat-prevLat,lon-prevLon);
    prevLat=lat;prevLon=lon;
    const way=p.wayId!=null?String(p.wayId):'';
    let wi=wayMap.get(way);if(wi===undefined){wi=wayDict.length;wayMap.set(way,wi);wayDict.push(way);}wayIndex.push(wi);
    segmentIndex.push(Number.isFinite(Number(p.segmentIndex))?Number(p.segmentIndex):-1);
    const meta: AnyRecord = {};
    for(const [k,v] of Object.entries(p)){
      if(k==='lat'||k==='lon'||k==='wayId'||k==='segmentIndex')continue;
      meta[k]=v;
    }
    const mk=JSON.stringify(meta);
    let mi=metaMap.get(mk);if(mi===undefined){mi=metaDict.length;metaMap.set(mk,mi);metaDict.push(deepClone(meta));}metaIndex.push(mi);
  }
  return {format:'SC8P1',scale,count:pts.length,coords,wayDict,wayIndex,segmentIndex,metaDict,metaIndex};
}

function unpackScheduleRoutePoints(packed: AnyRecord | PackedScheduleRoute | null): RoutePointLike[] {
  if(!packed||packed.format!=='SC8P1')return [];
  ...
  const out=new Array(count);let lat=0,lon=0;
  for(let i=0;i<count;i++){
    if(i===0){lat=num(coords[0],0);lon=num(coords[1],0);}else{lat+=num(coords[i*2],0);lon+=num(coords[i*2+1],0);}
...
export function assignResolvedLegsToPath(path: SchedulePath, legs: ScheduleLeg[], { validatedAt = null, resolvedRevision = null, error = '' }: AnyRecord = {}): SchedulePath {
  if (!path) throw new Error('SchedulePath manquant.');
  ...
  for (const sourceLeg of input) {
    if (!Array.isArray(sourceLeg?.routePoints) || sourceLeg.routePoints.length < 2) throw new Error('Liaison ORM vide pendant assemblage du trajet.');
    ...
    if(sourceSegments.length!==legPoints.length-1) throw new Error('Liaison ORM incohérente : nombre de segments différent de la géométrie.');
    const legSegments=[];let legDistanceKm=0;
    for(let i=0;i<sourceSegments.length;i++){
      const d=schedulePointDistanceKm(legPoints[i],legPoints[i+1]);
      if(!Number.isFinite(d)) throw new Error('Liaison ORM incohérente : coordonnées invalides.');
    ...
    for (const segment of legSegments) segments.push(shareGlobalGeometry ? segment : compactGlobalPathSegment(segment));
  ...
  if (validatedAt !== null) path.validatedAt = validatedAt || '';
  if (resolvedRevision !== null) path.resolvedRevision = Math.max(0, Number(resolvedRevision || 0));
  return path;
}

export class SchedulePath {
  declare id: string; declare direction: PathDirectionValue; declare constraints: RouteConstraint[]; declare legs: ScheduleLeg[]; declare globalGeometryShared: boolean;
  declare routePoints: RoutePointLike[]; declare segments: Array<RouteSegmentSnapshot | AnyRecord>; declare distanceKm: number; declare validatedAt: string; declare ormRevision: string;
  declare topologyRevision: number; declare resolvedRevision: number; declare offlineUsable: boolean; declare error: string;
  constructor(data: SchedulePathInput = {}) {
    this.id = data.id || makeV2Id('path');
    this.direction = data.direction || PathDirection.OUTBOUND;
    this.constraints = array<AnyRecord>(data.constraints).map(c => new RouteConstraint(c));
    this.legs = array<AnyRecord>(data.legs).map(l => {
      const legPoints=array<RoutePointLike>(l.routePoints).length?array<RoutePointLike>(l.routePoints).map(p=>({...p})):unpackScheduleRoutePoints(l.routePacked);
      ...
      return {
        ...
        constraintIds: array<unknown>(l.constraintIds).map(String), routeInputKey:String(l.routeInputKey||''),physicsRouteKey:String(l.physicsRouteKey||''),
        ...
        segments: array<AnyRecord>(l.segments).length ? array<AnyRecord>(l.segments).map(s => new RouteSegmentSnapshot(s)) : deriveRouteSegmentsFromPoints(legPoints), distanceKm: num(l.distanceKm,0),
      };
    });
    ...
    const explicitPoints=array<RoutePointLike>(data.routePoints);
    ...
    if(explicitPoints.length) this.routePoints=explicitPoints.map(p=>({...p}));
    else {
      this.routePoints=[];
      for(const leg of this.legs) for(let i=0;i<leg.routePoints.length;i++){
        if(this.routePoints.length&&i===0)continue;
        this.routePoints.push(this.globalGeometryShared?leg.routePoints[i]:{...leg.routePoints[i]});
      }
    }
    const explicitSegments=array<AnyRecord>(data.segments);
    ...
    if (explicitSegments.length) {
      ...
      const legPoints: RoutePointLike[] = [];
      for (const leg of this.legs) for (let i = 0; i < leg.routePoints.length; i++) {
        if (legPoints.length && i === 0) continue;
        legPoints.push(leg.routePoints[i]);
      }
      const aligned = canonicalSegments.length === explicitSegments.length &&
        legPoints.length === this.routePoints.length && legPoints.every((p, i) =>
          Number(p.lat) === Number(this.routePoints[i]?.lat) && Number(p.lon) === Number(this.routePoints[i]?.lon));
      this.segments = explicitSegments.map((seg, i) => compactGlobalPathSegment(aligned ? { ...canonicalSegments[i], ...seg } : seg));
    } else this.segments = this.globalGeometryShared ? canonicalSegments : canonicalSegments.map(compactGlobalPathSegment);
...
export class ScheduledLocation {
  declare id: string; declare kind: LocationKindValue; declare order: number; declare stationId: string; declare technicalLocationId: string; declare name: string; declare track: TrackBinding;
  declare computedArrivalSec: number | null; declare computedDepartureSec: number | null; declare arrivalSec: number | null; declare departureSec: number | null;
  declare arrivalOverride: boolean; declare departureOverride: boolean; declare dwellSec: number; declare stopCode: StopCodeValue; declare turnBack: boolean; declare notes: string;
  ...
  setComputedTimes(arrivalSec: unknown, departureSec: unknown): void {
    this.computedArrivalSec = arrivalSec == null ? null : Math.round(arrivalSec as number);
    this.computedDepartureSec = departureSec == null ? null : Math.round(departureSec as number);
    if (!this.arrivalOverride) this.arrivalSec = this.computedArrivalSec;
    if (!this.departureOverride) this.departureSec = this.computedDepartureSec;
  }

  applyManualArrival(sec: unknown): void {
    this.arrivalOverride = true;
    this.arrivalSec = Math.max(0, Math.round(sec as number));
  }

  applyManualDeparture(sec: unknown): void {
    this.departureOverride = true;
    this.departureSec = Math.max(0, Math.round(sec as number));
  }
...
export class TechnicalLocation {
  declare id: string; declare name: string; declare track: TrackBinding; declare createdAt: string;
...
export class PerformanceProfile {
  declare id: string; declare mode: PerformanceModeValue; declare name: string; declare referenceRameId: string; declare referenceComposition: AnyRecord[]; declare category: TrainCategoryValue;
  declare maxSpeed: number; declare massKg: number; declare powerW: number; declare electricPowerW: number; declare dieselPowerW: number; declare lengthM: number; declare adhesionMassKg: number;
  declare brakeServiceMs2: number; declare brakeBuildSec: number; declare traction: string; declare electricSystems: ElectricalSystem[]; declare gauges: number[]; declare loadingGauge: string;
  declare axleLoad: number | null; declare metreLoad: number | null; declare source: string;
  constructor(data: PerformanceProfileInput = {}) {
    ...
    this.referenceComposition = array<AnyRecord>(data.referenceComposition).map((x) => ({ ...x }));
    ...
    const freightLike = [TrainCategory.FREIGHT, TrainCategory.INFRA, TrainCategory.TTX, TrainCategory.TM].includes(this.category as typeof TrainCategory.FREIGHT | typeof TrainCategory.INFRA | typeof TrainCategory.TTX | typeof TrainCategory.TM);
    ...
    this.electricSystems = array<AnyRecord>(data.electricSystems).map((s) => ({ voltage: num(s.voltage, 0), frequency: num(s.frequency, 0) }));
    ...
    this.axleLoad = data.axleLoad == null ? null : Math.max(0, num(data.axleLoad, null) as number);
    this.metreLoad = data.metreLoad == null ? null : Math.max(0, num(data.metreLoad, null) as number);
    this.source = data.source || 'PLAYER';
  }

  // Existing Rame objects are accepted as a *reference composition only*.
  static fromLegacyRame(rame: AnyRecord, category: TrainCategoryValue = TrainCategory.PASSENGER): PerformanceProfile {
    if (!rame) return PerformanceProfile.genericForCategory(category, 160);
    ...
    const details=array<AnyRecord>(rame.elementDetails);
    const poweredMassT = details
      .filter(e => (e.category === 'locomotive' || e.category === 'automotrice') && num(e.power, 0) > 0)
      .reduce((sum, e) => sum + Math.max(0, num(e.mass, e.tonnage as number || 0)), 0);
    const systems: AnyRecord[]=[]; const gauges:number[]=[]; let brakeWeighted=0, brakeMass=0;
    const loadingGauges:string[]=[]; const axleLoads:number[]=[]; const metreLoads:number[]=[];
    if(Array.isArray(rame.electricSystems)) systems.push(...rame.electricSystems);
    if(Array.isArray(rame.gauges)) gauges.push(...rame.gauges); else if(Number.isFinite(Number(rame.gauge))) gauges.push(Number(rame.gauge));
    if(rame.loadingGauge) loadingGauges.push(String(rame.loadingGauge));
    if(Number.isFinite(Number(rame.axleLoad))) axleLoads.push(Number(rame.axleLoad));
    if(Number.isFinite(Number(rame.metreLoad))) metreLoads.push(Number(rame.metreLoad));
    for(const e of details){
      if(Array.isArray(e.electricSystems)) systems.push(...e.electricSystems);
      if(Array.isArray(e.gauges)) gauges.push(...e.gauges); else if(Number.isFinite(Number(e.gauge))) gauges.push(Number(e.gauge));
      if(e.loadingGauge) loadingGauges.push(String(e.loadingGauge));
      if(Number.isFinite(Number(e.axleLoad))) axleLoads.push(Number(e.axleLoad));
      if(Number.isFinite(Number(e.metreLoad))) metreLoads.push(Number(e.metreLoad));
      const em=Math.max(1,num(e.mass,e.tonnage as number||0))*1000, eb=Number(e.brakeServiceMs2);
      if(Number.isFinite(eb)&&eb>0){brakeWeighted+=eb*em;brakeMass+=em;}
    }
    const uniqueSystems: ElectricalSystem[]=[]; const seenSystems=new Set();
    for(const x of systems){const v=num(x?.voltage,0),f=num(x?.frequency,0),k=`${v}/${f}`;if(v>0&&!seenSystems.has(k)){seenSystems.add(k);uniqueSystems.push({voltage:v,frequency:f});}}
    ...
    return new PerformanceProfile({
      ...
      referenceComposition: array<AnyRecord>(rame.elementDetails).map((e, i) => ({
        order: i,
        catalogId: e.id || e.catalogId || '',
        name: e.name || '',
        category: e.category || '',
        mass: num(e.mass, e.tonnage as number || 0),
        power: num(e.power, 0),
  ...
  static genericForCategory(category: TrainCategoryValue, maxSpeed = 160): PerformanceProfile {
    const vmax = Math.max(1, num(maxSpeed, 160));
    const presets: Record<string, AnyRecord> = {
...
export class ValidationIssue {
  declare id: string; declare level: string; declare code: string; declare message: string; declare data: unknown;
  constructor(level: string, code: string, message: string, data: unknown = null) {
...
export class ValidationReport {
  declare timestamp: string; declare ormRevision: string; declare issues: ValidationIssueLike[]; declare deferred: boolean;
  constructor(data: ValidationReportInput = {}) {
    this.timestamp = data.timestamp || new Date().toISOString();
    this.ormRevision = data.ormRevision || '';
    this.issues = array<ValidationIssueLike>(data.issues).map(i => ({ ...i }));
...
export class ScheduleVersion {
  declare id: string; declare version: number; declare createdAt: string; declare state: ScheduleStateValue; declare calendarIds: string[]; declare category: TrainCategoryValue;
  declare performanceProfile: PerformanceProfile; declare locations: ScheduledLocation[]; declare outboundPath: SchedulePath; declare returnPath: SchedulePath | null;
  declare validationReport: ValidationReport | null; declare notes: string; declare automaticMarginSec: number; declare lastRecalculatedAt: string; declare allowShorterThanPhysicalTiming: boolean;
  constructor(data: ScheduleVersionInput = {}) {
    ...
    this.locations = array<AnyRecord>(data.locations).map(l => new ScheduledLocation(l));
  ...
  normalize(): this {
    ...
    this.locations.forEach((l, i:number) => { l.order = i; });
    this.outboundPath.constraints.sort((a,b) => a.order - b.order);
    this.outboundPath.constraints.forEach((c,i:number) => { c.order = i; });
    if (this.returnPath) {
      this.returnPath.constraints.sort((a,b) => a.order - b.order);
      this.returnPath.constraints.forEach((c,i:number) => { c.order = i; });
    }
    return this;
  }
...
export class ScheduleRecord {
  declare id: string; declare number: string; declare name: string; declare createdAt: string; declare versions: ScheduleVersion[]; declare currentVersionId: string;
  constructor(data: ScheduleRecordInput = {}) {
    this.id = data.id || makeV2Id('schedule');
    this.number = data.number || '';
    this.name = data.name || '';
    this.createdAt = data.createdAt || new Date().toISOString();
    this.versions = array<AnyRecord>(data.versions).map(v => new ScheduleVersion(v));
    if (this.versions.length === 0) this.versions.push(new ScheduleVersion());
  ...
  createVersion(options: AnyRecord = {}): ScheduleVersion {
    ...
    if (options.calendarIds) copy.calendarIds = [...options.calendarIds];
...
function roundTripLocationIdentity(loc: ScheduledLocation | null | undefined): string {
  if (!loc) return '';
  if (loc.kind === LocationKind.TECHNICAL && loc.technicalLocationId) return `TECH:${loc.technicalLocationId}`;
  if (loc.stationId) return `STATION:${loc.stationId}`;
  return '';
}

export function validateRoundTripPair(outVersion: ScheduleVersion | null | undefined, returnVersion: ScheduleVersion | null | undefined, { terminalLayoverSec = 300 }: AnyRecord = {}): ValidationIssue[] {
  const issues = [];
  ...
  if (outTerminal && retOrigin && outTerminal !== retOrigin) {
    issues.push(new ValidationIssue('ERROR', 'ROUND_TRIP_TERMINAL_MISMATCH',
      `Le retour doit partir du terminus de l’aller (${outLast?.name || 'terminus aller'}), pas de ${retFirst?.name || 'cet arrêt'}.`,
      { outboundTerminalId: outTerminal, returnOriginId: retOrigin }));
  }
  if (outOrigin && retTerminal && outOrigin !== retTerminal) {
    issues.push(new ValidationIssue('ERROR', 'ROUND_TRIP_ORIGIN_MISMATCH',
      `Le retour doit revenir à l’origine de l’aller (${outFirst?.name || 'origine aller'}), pas à ${retLast?.name || 'cet arrêt'}.`,
      { outboundOriginId: outOrigin, returnTerminalId: retTerminal }));
  }
  ...
  if (Number.isFinite(outArrival) && Number.isFinite(retDeparture) && retDeparture < requiredDeparture) {
    issues.push(new ValidationIssue('ERROR', 'ROUND_TRIP_TURNAROUND_TOO_SHORT',
      'Le retour part avant la fin de l’attente au terminus.',
      { outboundArrivalSec: outArrival, returnDepartureSec: retDeparture, requiredDepartureSec: requiredDeparture }));
  }
  return issues;
}

export class RoundTripGroup {
  declare id: string; declare outboundScheduleId: string; declare returnScheduleId: string; declare terminalLayoverSec: number;
...
function shiftLocationTimes(location: ScheduledLocation, deltaSec: number): void {
  for (const key of ['computedArrivalSec','computedDepartureSec','arrivalSec','departureSec'] as const) {
    if (location[key] != null) location[key] = Number(location[key]) + deltaSec;
  }
}

function rekeyPath(path: SchedulePath | null, locationMap: Map<string,string>): SchedulePath | null {
  if (!path) return null;
  path.id = makeV2Id('path');
  const constraintMap = new Map();
  for (const c of path.constraints || []) {
    const old = c.id; c.id = makeV2Id('via'); constraintMap.set(old, c.id);
  }
...
function rekeyVersionInternals(version: ScheduleVersion, { shiftSec = 0 }: AnyRecord = {}): ScheduleVersion {
  const locationMap = new Map();
  for (const l of version.locations || []) {
    const old = l.id; l.id = makeV2Id('loc'); locationMap.set(old, l.id);
    if (shiftSec) shiftLocationTimes(l, shiftSec);
  }
...
export class ScheduleV2Manager {
  private _batchClones: Map<ScheduleRecord,()=>ScheduleRecord> | null = null;
  private _batchNumbers: LabelAllocator | null = null;
  private _batchNames: LabelAllocator | null = null;
  declare schemaVersion: number; declare schedules: ScheduleRecord[]; declare calendars: OperatingCalendar[]; declare roundTrips: RoundTripGroup[]; declare technicalLocations: TechnicalLocation[];
  constructor() {
  ...
  getSchedule(id: string): ScheduleRecord | null { return this.schedules.find((s) => s.id === id) || null; }
  getVersion(scheduleId: string, versionId: string | null = null): ScheduleVersion | null {
    const rec = this.getSchedule(scheduleId);
    if (!rec) return null;
    if (!versionId) return rec.currentVersion;
  ...
  addRoundTrip(data: AnyRecord = {}): RoundTripGroup {
    ...
    if (!group.outboundScheduleId || !group.returnScheduleId) throw new Error('Un aller-retour doit référencer un horaire aller et un horaire retour.');
  ...
  removeCalendar(id: string, { rotationManager = null }: AnyRecord = {}) {
    id=String(id||'');
    const cal=this.calendars.find((c)=>String(c.id)===id);
    if(!cal)return {ok:false,references:[]};
    const references=[]; const affectedSchedules=new Set(); const affectedRotations=new Set();
    for(const rec of this.schedules)for(const v of rec.versions){
    ...
    for(const r of rotationManager?.rotations||[]){
      if(String(r.calendarId||'')===id){
        ...
        r.enabled=false;
        references.push({kind:'rotation',rotationId:r.id});affectedRotations.add(r.id);
      }
    }
    this.calendars=this.calendars.filter((c)=>String(c.id)!==id);
    for(const sid of affectedSchedules){for(const ref of rotationManager?.findScheduleReferences?.(sid)||[])affectedRotations.add(ref.rotationId);}
    for(const rid of affectedRotations)rotationManager?.recalculateRotation?.(rid);
  ...
  resolveVersion(scheduleId: string, dateStr: string): ScheduleVersion | null {
    ...
    for (const cal of this.calendars) {
      if (!cal.matchesDate(dateStr)) continue;
  ...
  isTrainNumberAvailable(number: unknown, exceptScheduleId = ''): boolean {
    const n=String(number??'').trim();
    if(!n) return false;
    return !this.schedules.some((s)=>s.id!==exceptScheduleId && String(s.number??'').trim()===n);
  }

  uniqueTrainNumber(preferred: unknown, exceptScheduleId = '', step = 1): string {
  ...
  duplicateSchedule(id: string, options: AnyRecord = {}): ScheduleRecord | null {
    ...
    if(this._batchClones&&!factory){factory=prepareEditableClone(src);this._batchClones.set(src,factory);}
    ...
    if (options.name != null) copy.name = String(options.name);
    else copy.name = duplicatedTrainName(src.name, src.number, copy.number, Number(options.numberDelta || 1), this._batchNames || new LabelAllocator(this.schedules.map(s=>s.name),true));
    copy.createdAt = new Date().toISOString();
    ...
    copy.versions = copy.versions.map((v) => {
      ...
      if (v.state === ScheduleState.DRAFT) v.validationReport = null;
      return v;
    });
  ...
  duplicateScheduleFrequency(id: string, options: AnyRecord = {}): Array<ScheduleRecord | null> {
    const src = this.getSchedule(id);
    if (!src) throw new Error('Horaire introuvable.');
    const intervalSec = Math.max(60, Math.round(Number(options.intervalSec || 0)));
    if (!Number.isFinite(intervalSec) || intervalSec < 60) throw new Error('Fréquence invalide.');
    ...
    if (options.untilDepartureSec != null) {
      ...
      if (!Number.isFinite(until) || until <= baseDeparture) throw new Error('Heure de fin invalide.');
      if (until - baseDeparture > 86400) throw new Error('La génération par fréquence est limitée à 24 h.');
      for (let shift = intervalSec; baseDeparture + shift <= until && shifts.length < maxCopies; shift += intervalSec) shifts.push(shift);
    } else {
      ...
      if (copies < 1) return [];
      if (copies > maxCopies) throw new Error(`Trop de duplicatas (${copies}). Maximum : ${maxCopies}.`);
      if (copies * intervalSec > 86400) throw new Error('La génération par fréquence est limitée à 24 h.');
      for (let i = 1; i <= copies; i++) shifts.push(i * intervalSec);
    }
    ...
    try{
      ...
      this.schedules.splice(length);throw err;
    } finally { this._batchNumbers=prevNumbers; this._batchNames=prevNames;this._batchClones=prevClones; }
  }

  duplicateRoundTrip(groupId: string, options: AnyRecord = {}, rotationManager: RotationManagerBridge | null = null) {
    const group = this.roundTrips.find((g) => g.id === groupId);
    if (!group) throw new Error('Aller-retour introuvable.');
    const out = this.getSchedule(group.outboundScheduleId);
    const ret = this.getSchedule(group.returnScheduleId);
    if (!out || !ret) throw new Error('Horaire aller/retour introuvable.');
    ...
    if (options.nextDepartureSec != null) firstShift = Number(options.nextDepartureSec) - baseDeparture;
    if (!intervalSec) intervalSec = Math.max(1, firstShift || 3600);
    if (firstShift <= 0) firstShift = intervalSec;
    const created: Array<{ group: RoundTripGroup; outbound: ScheduleRecord; return: ScheduleRecord; shiftSec: number }> = [];
    ...
    this._batchNames=new LabelAllocator(this.schedules.map(s=>s.name),true);
    let pairIndex = 1;
    try{for (let shift = firstShift; shift <= horizonSec && created.length < maxPairs; shift += intervalSec, pairIndex++) {
      ...
      if(outNumber===out.number)outNumber=`${out.number} ${pairIndex*2}`;
      if(retNumber===ret.number)retNumber=`${ret.number} ${pairIndex*2}`;
      if(outNumber===retNumber)throw new Error('Les numéros aller et retour doivent être distincts.');
      while(this._batchNumbers!.has(outNumber)||this._batchNumbers!.has(retNumber)){
        outNumber=incrementLabel(outNumber,2);retNumber=incrementLabel(retNumber,2);
      }
    ...
    if (rotationManager && options.rotationId) rotationManager.recalculateRotation!(options.rotationId);
    return created;
    }catch(err){
      this.schedules.splice(scheduleLength);this.roundTrips.splice(groupLength);
      if(rotationSnapshot&&rotationManager?.loadFromSave)rotationManager.loadFromSave(rotationSnapshot);
      throw err;
    } finally { this._batchNumbers=prevNumbers; this._batchNames=prevNames;this._batchClones=prevClones; }
  }
  ...
  addTechnicalLocation(data: AnyRecord = {}): TechnicalLocation {
    ...
    if (!loc.name.trim()) throw new Error('Le point technique doit avoir un nom.');
    this.technicalLocations.push(loc);
    return loc;
  }

  removeSchedule(id: string, { cascade = false, rotationManager = null }: AnyRecord = {}) {
    const refs = rotationManager?.findScheduleReferences?.(id) || [];
    if (refs.length && !cascade) return { ok: false, references: refs };
    if (refs.length && cascade) rotationManager.removeScheduleReferences(id);
  ...
  removeTechnicalLocation(id: string, { cascade = false }: AnyRecord = {}) {
    const refs: ScheduleReference[] = [];
    for (const s of this.schedules) for (const v of s.versions) {
      v.locations.forEach((l) => { if (l.technicalLocationId === id) refs.push({ scheduleId: s.id, versionId: v.id, locationId: l.id }); });
    }
    if (refs.length && !cascade) return { ok: false, references: refs };
    if (cascade) {
      for (const schedule of this.schedules) for (const v of schedule.versions) {
        const oldLocations=[...v.locations],removed=new Set();
        oldLocations.forEach((l,i)=>{if(l.technicalLocationId===id)removed.add(i);});
        if(!removed.size)continue;
        const kept=oldLocations.map((_,i)=>i).filter(i=>!removed.has(i));
        const path=v.outboundPath;
        const mapped=[];
        for(const c of path.constraints||[]){
          const oldLeg=Math.max(0,Number(c.legIndex)||0);let newLeg=-1;
          for(let j=0;j<kept.length-1;j++)if(kept[j]<=oldLeg&&oldLeg<kept[j+1]){newLeg=j;break;}
          if(newLeg>=0)mapped.push({c,oldLeg,newLeg,oldOrder:Number(c.order)||0});
        }
        ...
        path.constraints.forEach((c,i:number)=>c.order=i);
        v.locations=oldLocations.filter((_,i)=>!removed.has(i));v.normalize();
  ...
  loadFromSave(data: AnyRecord): boolean {
    ...
    const migrated={...data,schemaVersion:SCHEDULE_V2_SCHEMA,schedules:array<AnyRecord>(data.schedules),calendars:array<AnyRecord>(data.calendars),roundTrips:array<AnyRecord>(data.roundTrips),technicalLocations:array<AnyRecord>(data.technicalLocations)};
    // Transactional load: a malformed/incompatible payload must never erase the live game state.
    let schedules:ScheduleRecord[],calendars:OperatingCalendar[],roundTrips:RoundTripGroup[],technicalLocations:TechnicalLocation[];
    try{
      schedules=migrated.schedules.map((s:AnyRecord) => new ScheduleRecord(s));
      calendars=migrated.calendars.map((c:AnyRecord) => new OperatingCalendar(c));
      roundTrips=migrated.roundTrips.map((r:AnyRecord) => new RoundTripGroup(r));
      technicalLocations=migrated.technicalLocations.map((t:AnyRecord) => new TechnicalLocation(t));
      const unique=<T extends { id?: unknown }>(items:T[],label:string)=>{const ids=new Set();for(const x of items){const id=String(x?.id||'');if(!id||ids.has(id))throw new Error(`${label}: identifiant vide ou dupliqué (${id||'?'})`);ids.add(id);}};
      unique(schedules,'Horaires');unique(calendars,'Calendriers');unique(roundTrips,'Aller-retour');unique(technicalLocations,'Points techniques');
      for(let si=0;si<schedules.length;si++){
        const rec=schedules[si],raw=migrated.schedules[si]||{};unique(rec.versions,`Versions ${rec.id}`);
        if(raw.currentVersionId && !rec.versions.some((v)=>v.id===raw.currentVersionId))throw new Error(`Horaire ${rec.id}: currentVersionId introuvable (${raw.currentVersionId})`);
        for(const v of rec.versions){unique(v.locations,`Arrêts ${rec.id}/${v.id}`);unique(v.outboundPath?.constraints||[],`VIA ${rec.id}/${v.id}`);}
      }
    }catch(err){console.error('Schedule V2 chargement refusé:',err);return false;}
    this.schedules=schedules;this.calendars=calendars;this.roundTrips=roundTrips;this.technicalLocations=technicalLocations;
    return true;
  }
}
