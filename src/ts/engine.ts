...
export class SimulationEngine {
    chronologicalClock: ChronologicalClock | null = null;
    private _replayTimeSecond = -1;
    private _replayTime: ParisTime | null = null;
    enableChronologicalReplay(raw: unknown = null, fallbackMs = Date.now(), nowMs = Date.now()): void {
        this.chronologicalClock = new ChronologicalClock(raw, fallbackMs, nowMs);
        this.paused = !!this.chronologicalClock.failed;
        this._replayTimeSecond = -1; this._replayTime = null;
        this._accumulator = 0; this.discardedPhysicsSeconds = 0;
        this._ptCache = null;
    }
    getSimulationEpochMs(): number { return this.chronologicalClock?.cursorMs ?? Date.now(); }
    getReplayDebtSeconds(): number { return this.chronologicalClock?.debtSeconds ?? 0; }
    toClockSave() { return this.chronologicalClock?.snapshot() ?? null; }
    private _timeAtEpoch(ms: number): ParisTime {
        const second = Math.floor(ms / 1000);
        if (!this._replayTime || second !== this._replayTimeSecond) {
            this._replayTime = this._getRealParisTime(new Date(ms)); this._replayTimeSecond = second;
        }
        return this._replayTime;
    }
    private _updateChronological(): void {
        const clock = this.chronologicalClock!;
        const dateAt = (ms: number) => this._formatDateISO(this._timeAtEpoch(ms).date);
        const timeAt = (ms: number) => { const p = this._timeAtEpoch(ms); return p.hours * 60 + p.minutes + (p.seconds + (ms % 1000) / 1000) / 60; };
        try {
            ...
            clock.advance(targetMs, {
                minute: ms => {
                    ...
                    this.lastMinute = pt.hours * 60 + pt.minutes; this.currentDate = date;
                    this.onTick?.(timeAt(ms), date, pt);
                },
                second: ms => {
                    const pt = this._timeAtEpoch(ms), date = dateAt(ms);
                    this.lastSecondKey = String(Math.floor(ms / 1000)); this.currentDate = date;
    ...
    _getRealParisTime(now = new Date()): ParisTime {
    ...
    getParisTime(): ParisTime {
        if (this.chronologicalClock) return this._timeAtEpoch(this.chronologicalClock.cursorMs);
    ...
    update(): void {
        if (this.paused)
            return;
        if (this.chronologicalClock) { this._updateChronological(); return; }
        const now = performance.now();
        const pt = this.getParisTime();
