...
type FreightContractLike = { active: boolean } & UnknownRecord;
...
type IndustrialSave = {
  clients?: unknown;
  stats?: unknown;
  _nextClientId?: unknown;
  locationOverrides?: unknown;
  customSites?: unknown;
  hiddenStaticKeys?: unknown;
};
...
export class IndustrialClients {
  ...
  railLegProvider: (() => readonly FreightRailLeg[]) | null = null;
  freightAccessProvider: ((stationId: string) => boolean | null) | null = null;
  lastGenerationReport = { generated: 0, clientsWithoutKnownRoute: 0, clientsWithoutFreightAccess: 0, knownDirectedLinks: 0 };
  ...
  setLocationOverride(key: string, lat: unknown, lon: unknown) {
    if (!key) return false;
    const nLat = Number(lat), nLon = Number(lon);
    if (!Number.isFinite(nLat) || !Number.isFinite(nLon) || nLat < -90 || nLat > 90 || nLon < -180 || nLon > 180) return false;
    if (String(key).startsWith('custom|')) {
      const s = this.customSites.find((s: { _key: unknown }) => s._key === key);
      if (!s) return false;
      s.lat = nLat; s.lon = nLon;
      return true;
    }
    this.locationOverrides[key] = { ...(this.locationOverrides[key] || {}), lat: nLat, lon: nLon };
    return true;
  }
  ...
  getAllRealLocations() {
    ...
    for (const ind of INDUSTRY_TYPES) {
      ...
      for (const loc of ind.realLocations) {
        ...
        locs.push({
          ...loc,
          name: ov?.name ?? loc.name,
          country: ov?.country ?? loc.country,
          lat: Number.isFinite(Number(ov?.lat)) ? Number(ov.lat) : loc.lat,
          lon: Number.isFinite(Number(ov?.lon)) ? Number(ov.lon) : loc.lon,
          industryType: ov?.type ?? ind.type,
          industryName: this.getIndustryInfo(ov?.type ?? ind.type)?.name || ind.name,
          industryIcon: this.getIndustryInfo(ov?.type ?? ind.type)?.icon || ind.icon,
          color: INDUSTRY_COLORS[ov?.type ?? ind.type] || '#94a3b8',
          _key: key,
          custom: false,
        });
      }
  ...
  addCustomSite(type: unknown, name: unknown, lat: number, lon: number, country: unknown = 'FR') {
    lat = Number(lat); lon = Number(lon);
    if (!this.getIndustryInfo(type) || !String(name || '').trim() || !Number.isFinite(lat) || !Number.isFinite(lon) || lat < -90 || lat > 90 || lon < -180 || lon > 180) return null;
    const id = `cs-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const _key = `custom|${id}`;
    const s: CustomIndustrialSite = { id, type: String(type), name: String(name).trim(), lat, lon, country: String(country || 'FR'), _key };
    this.customSites.push(s);
    return s;
  }

  removeSite(key: string) {
    if (String(key).startsWith('custom|')) {
      this.customSites = this.customSites.filter((s: { _key: unknown }) => s._key !== key);
    } else {
  ...
  restoreStaticSite(key: unknown) {
    this.hiddenStaticKeys.delete(String(key));
  }

  updateSite(key: string, data: { lat: unknown; lon: unknown; type: unknown; name: unknown; country: unknown }) {
    const validCoord = (v: unknown, min: number, max: number) => v == null || (Number.isFinite(Number(v)) && Number(v) >= min && Number(v) <= max);
    if (!validCoord(data.lat, -90, 90) || !validCoord(data.lon, -180, 180)) return null;
    if (data.type != null && !this.getIndustryInfo(data.type)) return null;
    if (String(key).startsWith('custom|')) {
      const s = this.customSites.find((s: { _key: unknown }) => s._key === key); if (!s) return null;
      if (data.name != null && String(data.name).trim()) s.name = String(data.name).trim();
      if (data.type != null) s.type = String(data.type);
      if (data.country != null) s.country = String(data.country);
      if (data.lat != null) s.lat = Number(data.lat);
      if (data.lon != null) s.lon = Number(data.lon);
      return s;
    }
    const ov = this.locationOverrides[key] = this.locationOverrides[key] || {};
    if (data.name != null && String(data.name).trim()) ov.name = String(data.name).trim();
    if (data.type != null) ov.type = String(data.type);
    if (data.country != null) ov.country = String(data.country);
    if (data.lat != null) ov.lat = Number(data.lat);
    if (data.lon != null) ov.lon = Number(data.lon);
    return this.getSiteByKey(key);
  }

  getSiteByKey(key: unknown): { name?: string; type?: string; lat?: number; lon?: number; country?: string; _key?: string; id?: string } | null {
    const keyStr = String(key);
    if (keyStr.startsWith('custom|')) {
      return this.customSites.find((s) => s._key === keyStr) || null;
    }
    for (const ind of INDUSTRY_TYPES) {
      if (!ind.realLocations) continue;
      for (const loc of ind.realLocations) {
        if (ind.type + '|' + loc.name === keyStr) { const ov=this.locationOverrides[keyStr]||{}; return { ...loc, ...ov, _key:keyStr, type:ov.type || ind.type }; }
      }
    }
    return null;
  }

  attractClient(industryType: unknown, stationId: unknown, depotId: unknown, economy: Economy) {
    const industry = this.getIndustryInfo(industryType);
    if (!industry || !stationId || !depotId) return null;
    if (!economy || !Number.isFinite(Number(economy.balance)) || economy.balance < industry.attractCost) return null;
  ...
  removeClient(clientId: unknown, freightManager: FreightManagerLike | null = null) {
    const clientKey = String(clientId ?? ''); const before = this.clients.length;
    this.clients = this.clients.filter((c) => c.id !== clientKey);
    if (this.clients.length !== before) {
      freightManager?.cancelContractsForIndustrialClient?.(clientKey);
      this.stats.totalClients = this.clients.filter((c: { active: unknown }) => c.active !== false).length;
      return true;
    }
    return false;
  }
  ...
  updateClient(clientId: unknown, data: { name?: unknown; dailyTonnage?: unknown; satisfaction?: unknown }) {
    const clientKey = String(clientId ?? ''); const client = this.clients.find((c) => c.id === clientKey);
    if (!client) return null;
    if (data.name != null) client.name = String(data.name);
    if (data.dailyTonnage != null) client.dailyTonnage = Math.max(0, parseInt(String(data.dailyTonnage)) || 0);
    if (data.satisfaction != null) client.satisfaction = Math.max(0, Math.min(100, parseFloat(String(data.satisfaction)) || 0));
    return client;
  }

  // IND-01 : déplacer un client vers une autre gare avec ITE
  moveClient(clientId: unknown, stationId: unknown, depotId: unknown) {
    const clientKey = String(clientId ?? ''), stationKey = String(stationId ?? ''), depotKey = String(depotId ?? '');
    const client = this.clients.find((c) => c.id === clientKey);
    if (!client || !stationKey || !depotKey) return null;
    client.stationId = stationKey;
    client.depotId = depotKey;
    return client;
  }

  getClientsByStation(stationId: unknown) {
    return this.clients.filter((c: { stationId: unknown; active: unknown }) => c.stationId === stationId && c.active);
  }
  ...
  generateDailyContracts(freightManager: FreightManagerLike, world: World, cargoTypes: CargoTypesLike | null = null) {
    ...
    this.lastGenerationReport = { generated: 0, clientsWithoutKnownRoute: 0, clientsWithoutFreightAccess: 0, knownDirectedLinks: network.edgeCount };
    for (const client of this.clients) {
      ...
      let reachable = destinations.get(client.stationId);
      if (!reachable) { reachable = network.reachableStations(client.stationId); destinations.set(client.stationId, reachable); }
      const otherStations = stations.filter(s => reachable.has(s.id) && this.freightAccessProvider?.(s.id) !== false);
      if (otherStations.length === 0) {
        if (reachable.size === 0) this.lastGenerationReport.clientsWithoutKnownRoute++;
        else this.lastGenerationReport.clientsWithoutFreightAccess++;
        continue;
      }

      const fromStation = stations.find((s: { id: unknown; closed: unknown }) => s.id === client.stationId && s.closed !== true);
      if (this.freightAccessProvider?.(client.stationId) === false) { this.lastGenerationReport.clientsWithoutFreightAccess++; continue; }
      if (!fromStation || !Array.isArray(industry.cargoTypes) || industry.cargoTypes.length === 0) continue;

      // FRT-02/05 : satisfaction + part de marché = plus d'offres
      const satBoost = 1 + Math.max(0, (client.satisfaction - 70)) / 100;
      const marketShare = Number.isFinite(Number(client.marketShare)) ? Math.max(0, Math.min(100, Number(client.marketShare))) : 5;
      const shareBoost = 0.5 + marketShare / 10;
      ...
      while (remainingTonnage > 0 && contractsToday < 10) {
        ...
        const cargoTariff = Number(cargoTypes?.getTransportTariffPerTonne?.(cargoType));
        const effectiveTariff = Number.isFinite(cargoTariff) && cargoTariff > 0 ? cargoTariff : Math.max(0, Number(industry.pricePerTonne) || 0);
        const revenue = Math.floor(contractTonnage * effectiveTariff);

        if (freightManager.contracts.filter((c) => c.active).length >= 30) break;
        const contractData = {
            cargoType,
            cargoName: industry.cargoOut,
            quantity: contractTonnage,
            initialQuantity: contractTonnage,
            unit: 't',
            ...
            toId: toStation.id,
            payment: revenue,
            unitPrice: contractTonnage > 0 ? revenue / contractTonnage : 0,
            active: true,
            progress: 0,
            industrialClientId: client.id,
          };
        let created=null;
        if (typeof freightManager.addContract === 'function') created=freightManager.addContract(contractData);
        else { const fallbackId=`fret-ind-${client.id}-${Date.now()}-${contractsToday}`; created={...contractData,id:fallbackId}; freightManager.contracts.push(created); }
        if(!created) break;
        remainingTonnage -= contractTonnage;
        contractsToday++;
        this.lastGenerationReport.generated++;
        client.contractsGenerated++;
        this.stats.contractsGenerated++;
        // Tonnage/revenue are actual delivered performance, not offered work.
        // They are updated by FreightManager.fulfillAtStation().
      }
  ...
  boostSatisfaction(clientId: unknown, amount: unknown) {
    const client = this.clients.find((c: { id: unknown }) => c.id === clientId);
    const delta = Number(amount);
    if (client && Number.isFinite(delta)) {
      client.satisfaction = Math.max(0, Math.min(100, (Number(client.satisfaction) || 0) + delta));
      return true;
    }
    return false;
  }

  pruneClients(validITEs: unknown = []) {
    const valid = new Set((Array.isArray(validITEs) ? validITEs : []).map((x: unknown) => typeof x === 'string' ? x : (isRecord(x) ? x.id : undefined)).filter(Boolean));
    const before = this.clients.length;
    if (valid.size === 0) {
      this.clients = [];
    } else {
      this.clients = this.clients.filter((c: { active: unknown; depotId: unknown }) => c && c.active !== false && c.depotId && valid.has(c.depotId));
    }
  ...
  render(container: HTMLElement, game: RailEmpire) {
    ...
    container.innerHTML = `
      <div class="dash-section">
        ...
        <p style="font-size:12px;color:var(--text3)">Les offres utilisent uniquement les liaisons ferroviaires déjà connues localement et les gares équipées pour le fret. Une liaison inconnue n’est pas déclarée impossible : validez un horaire pour la rendre disponible. Aucune requête OSM supplémentaire.</p>
        <p style="font-size:11px;color:var(--text3)">Dernière génération : ${this.lastGenerationReport.generated} offre(s) · ${this.lastGenerationReport.clientsWithoutKnownRoute} client(s) sans liaison connue · ${this.lastGenerationReport.clientsWithoutFreightAccess} sans terminal fret utilisable.</p>
      ...
      <div class="dash-section">
        ...
        <div class="dash-train-table">
          ...
          ${activeClients.map((c: IndustrialClient) => {
            const station = game.world?.stations.find((s: { id: unknown }) => s.id === c.stationId);
            ...
            return `
              <div class="dash-train-row" style="grid-template-columns:0.3fr 1fr 0.8fr 0.6fr 0.6fr 0.6fr 0.5fr">
                ...
                <span>${htmlText(c.name)}<br><span style="font-size:9px;color:var(--text3)">${c.totalTonnage.toLocaleString('fr-FR')} t traités</span></span>
                <span>${htmlText(station?.name || '?')}</span>
                <span style="color:#38bdf8">${c.dailyTonnage.toLocaleString('fr-FR')} t</span>
                <span style="color:${htmlText(satColor)}">${Math.floor(c.satisfaction)}%</span>
                <span style="color:#a78bfa">${htmlText(Math.floor(Number.isFinite(Number(c.marketShare)) ? Number(c.marketShare) : 5))}%</span>
                <span style="display:flex;gap:4px;flex-wrap:wrap">
                  <button class="btn-primary industrial-edit" data-id="${htmlText(c.id)}" style="font-size:9px;padding:3px 6px;background:#3b82f6">Modifier</button>
                  <button class="btn-primary industrial-move" data-id="${htmlText(c.id)}" style="font-size:9px;padding:3px 6px;background:#6366f1">Déplacer</button>
                  <button class="btn-primary industrial-remove" data-id="${htmlText(c.id)}" style="font-size:9px;padding:3px 6px;background:#991b1b">Résilier</button>
                </span>
              </div>
            `;
          }).join('')}
      ...
      <div class="dash-section">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
          <select id="industrial-station-select" style="flex:1;min-width:200px;padding:6px;font-size:12px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:4px">
            ...
            ${((game.depotManager?.getITEs?.() || []) as IndustrialITE[]).filter((ite) => ite.built).map((ite) => {
              const station = game.world?.stations.find((s: { id: unknown }) => s.id === ite.stationId);
              return `<option value="${htmlText(ite.stationId)}" data-depot="${htmlText(ite.id)}">${htmlText(station?.name || '?')} — ${htmlText(ite.name)}</option>`;
            }).join('')}
          </select>
        </div>
        ...
        <div id="industrial-types-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:8px">
          ${INDUSTRY_TYPES.map((ind) => {
            ...
            return `<button class="industrial-attract btn-primary" data-type="${htmlText(ind.type)}"
              style="font-size:11px;padding:10px 14px;background:${htmlText(affordable ? color : '#991b1b')};text-align:left;border:1px solid ${htmlText(color)}50"
              ${!affordable ? 'disabled' : ''}>
              ${icon(ind.icon, 14)} <b>${htmlText(ind.name)}</b><br>
              <span style="font-size:9px;opacity:0.8">${htmlText(ind.description)}</span><br>
      ...
      <div class="dash-section">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
          <select id="ind-filter-country" style="padding:4px 8px;font-size:11px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:4px">
            ...
            ${[...totalCountries].sort().map((c) => `<option value="${htmlText(c)}">${htmlText(COUNTRY_NAMES[c] || c)}</option>`).join('')}
          </select>
          <select id="ind-filter-type" style="padding:4px 8px;font-size:11px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:4px">
            ...
            ${INDUSTRY_TYPES.map((ind) => `<option value="${htmlText(ind.type)}">${htmlText(ind.name)}</option>`).join('')}
          </select>
        </div>
    ...
    container.querySelectorAll('.industrial-attract').forEach((node) => {
    ...
    // Bind remove buttons
    container.querySelectorAll('.industrial-remove').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
        if (confirm('Résilier ce client ?')) {
          this.removeClient(btn.dataset.id, game.freightManager);
          game.saveState?.();
          this.render(container, game);
        }
      });
    ...
    container.querySelectorAll('.industrial-edit').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
    ...
    container.querySelectorAll('.industrial-move').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
        const client = this.clients.find((c) => c.id === btn.dataset.id);
        if (!client) return;
        const ites = (game.depotManager?.getITEs?.() || []).filter((ite: { built: unknown }) => ite.built);
        if (ites.length === 0) { alert('Aucune ITE disponible'); return; }
        const opts = (ites as IndustrialITE[]).map((ite) => {
          const st = game.world?.stations.find((s: { id: unknown }) => s.id === ite.stationId);
        ...
        if (!choice) return;
        const token = choice.trim().split(/\s+—\s+/)[0];
        const [stationId, depotId] = token.split('|').map((x) => x.trim());
        const valid = ites.some((ite: { id: unknown; stationId: unknown }) => ite.id === depotId && ite.stationId === stationId);
        if (!valid) return alert('Gare / ITE invalide. Copiez le code avant le premier tiret — ou gardez la valeur proposée.');
    ...
    // Bind filters
    const filterCountry = container.querySelector('#ind-filter-country') as HTMLSelectElement | null;
    const filterType = container.querySelector('#ind-filter-type') as HTMLSelectElement | null;
    ...
    container.querySelector('#btn-use-map-center')?.addEventListener('click', () => {
      ...
      if (!tm) return;
      const latInput = container.querySelector('#new-site-lat') as HTMLInputElement | null;
      const lonInput = container.querySelector('#new-site-lon') as HTMLInputElement | null;
    ...
    container.querySelector('#btn-add-custom-site')?.addEventListener('click', () => {
      const type = (container.querySelector('#new-site-type') as HTMLSelectElement | null)?.value;
      const name = ((container.querySelector('#new-site-name') as HTMLInputElement | null)?.value || '').trim();
      const lat = parseFloat((container.querySelector('#new-site-lat') as HTMLInputElement | null)?.value || '');
      const lon = parseFloat((container.querySelector('#new-site-lon') as HTMLInputElement | null)?.value || '');
      const country = ((container.querySelector('#new-site-country') as HTMLInputElement | null)?.value || 'FR').trim();
      if (!type || !name || !Number.isFinite(lat) || !Number.isFinite(lon)) {
        alert('Remplissez type, nom, latitude et longitude.'); return;
      }
      if (!this.addCustomSite(type, name, lat, lon, country)) return alert('Site invalide.');
      game.saveState?.();
      this.render(container, game);
    });

    container.querySelectorAll('.site-edit').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
        ...
        if (type === null) return;
        const lat = prompt('Latitude :', s.lat == null ? '' : String(s.lat));
        if (lat === null) return;
        const lon = prompt('Longitude :', s.lon == null ? '' : String(s.lon));
        if (lon === null) return;
        const country = prompt('Pays :', s.country || 'FR');
        if (country === null) return;
        const updated=this.updateSite(key, {
        ...
        if (!updated) return alert('Modification invalide.');
        game.saveState?.();
        this.render(container, game);
      });
    });

    container.querySelectorAll('.site-delete').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
        const key = btn.dataset.key;
        if (key && confirm('Supprimer ce site ?')) { this.removeSite(key); game.saveState?.(); this.render(container, game); }
      });
    });

    container.querySelectorAll('.site-reset').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => {
        const key = btn.dataset.key;
        if (!key) return;
    ...
    container.querySelectorAll('.site-restore').forEach((node) => {
      const btn = node as HTMLButtonElement;
      btn.addEventListener('click', () => { this.restoreStaticSite(btn.dataset.key); game.saveState?.(); this.render(container, game); });
    });
  }

  _renderSitesSection(game: RailEmpire) {
    const typeOptions = INDUSTRY_TYPES.map((ind) => `<option value="${htmlText(ind.type)}">${htmlText(ind.name)}</option>`).join('');
    ...
    const customRows = this.customSites.map((s) => {
      ...
      return `
        <div class="dash-train-row" style="grid-template-columns:0.3fr 0.8fr 1.2fr 0.5fr 0.5fr 0.4fr 0.8fr">
          ...
          <span style="font-size:10px;color:${htmlText(color)}">${htmlText(ind?.name || s.type)}</span>
          <span style="font-size:10px">${htmlText(s.name)}</span>
          ...
          <span style="display:flex;gap:4px;flex-wrap:wrap">
            <button class="btn-primary site-edit" data-key="${htmlText(s._key)}" style="font-size:9px;padding:3px 6px;background:#3b82f6">Modifier</button>
            <button class="btn-primary site-delete" data-key="${htmlText(s._key)}" style="font-size:9px;padding:3px 6px;background:#991b1b">Supprimer</button>
          </span>
        </div>
      `;
    }).join('');
    ...
    const movedRows = Object.entries(this.locationOverrides).filter(([k, v]) => !k.startsWith('custom|') && (v.lat != null || v.lon != null)).map(([key, ov]) => {
      ...
      return `
        <div class="dash-train-row" style="grid-template-columns:0.3fr 0.8fr 1.2fr 0.5fr 0.5fr 0.4fr 0.8fr">
          ...
          <span style="font-size:10px">${htmlText(site.name || key)}</span>
          ...
          <span><button class="btn-primary site-reset" data-key="${htmlText(key)}" style="font-size:9px;padding:3px 6px;background:#6366f1">Reset</button></span>
        </div>
      `;
    }).join('');

    const hiddenRows = [...this.hiddenStaticKeys].map((key: unknown) => {
      const site = this.getSiteByKey(key) || {};
      return `
        <div class="dash-train-row" style="grid-template-columns:0.3fr 0.8fr 1.2fr 0.5fr 0.5fr 0.4fr 0.8fr">
          ...
          <span style="font-size:9px;color:var(--text3)">${site.country || ''}</span>
          <span><button class="btn-primary site-restore" data-key="${htmlText(key)}" style="font-size:9px;padding:3px 6px;background:#047857">Restaurer</button></span>
        </div>
      `;
    }).join('');
  ...
  _renderLocationsRows(countryFilter: unknown, typeFilter: unknown) {
    let html = '';
    for (const loc of this.getAllRealLocations()) {
      if (loc.custom) continue;
      if (typeFilter && loc.industryType !== typeFilter) continue;
      if (countryFilter && loc.country !== countryFilter) continue;
      const color = loc.color || INDUSTRY_COLORS[loc.industryType] || '#94a3b8';
      html += `
        <div class="dash-train-row" style="grid-template-columns:0.3fr 0.6fr 1.5fr 0.3fr 0.4fr 0.4fr">
          <span>${icon(loc.industryIcon || 'factory', 12)}</span>
          <span style="font-size:10px;color:${htmlText(color)}">${htmlText(loc.industryName || loc.industryType)}</span>
          <span style="font-size:10px">${htmlText(loc.name)}</span>
          <span style="font-size:9px;color:var(--text3)">${loc.country}</span>
          <span style="font-size:9px;color:var(--text3)">${htmlText(Number(loc.lat).toFixed(3))}</span>
          <span style="font-size:9px;color:var(--text3)">${htmlText(Number(loc.lon).toFixed(3))}</span>
        </div>`;
    }
    return html;
  }
  ...
  loadFromSave(s: unknown) {
    ...
    const save = s as IndustrialSave;
    const finite = (v: unknown, fb = 0) => Number.isFinite(Number(v)) ? Number(v) : fb;
    const rawClients: UnknownRecord[] = (Array.isArray(save.clients) ? save.clients : []).filter(isRecord);
    const clientSeen = new Set<string>();
    this.clients = rawClients
      .filter((c) => {
        const id = String(c.id ?? '');
        return Boolean(id && !clientSeen.has(id) && clientSeen.add(id) && this.getIndustryInfo(c.type) && c.stationId && c.depotId);
      })
      .map((c): IndustrialClient => ({
        id: String(c.id),
        type: String(c.type),
        name: String(c.name ?? this.getIndustryInfo(c.type)?.name ?? 'Client industriel'),
        icon: String(c.icon ?? this.getIndustryInfo(c.type)?.icon ?? 'factory'),
        stationId: String(c.stationId),
        depotId: String(c.depotId),
        active: c.active !== false,
        dailyTonnage: Math.max(0, finite(c.dailyTonnage)),
        satisfaction: Math.max(0, Math.min(100, finite(c.satisfaction, 80))),
        marketShare: Math.max(0, Math.min(100, finite(c.marketShare, 5))),
        contractsGenerated: Math.max(0, Math.floor(finite(c.contractsGenerated))),
        totalTonnage: Math.max(0, finite(c.totalTonnage)),
        totalRevenue: Math.max(0, finite(c.totalRevenue)),
        createdAt: Math.max(0, finite(c.createdAt, Date.now())),
      }));
    const st: UnknownRecord = isRecord(save.stats) ? save.stats : {};
    this.stats = {
      totalClients: this.clients.filter((c) => c.active !== false).length,
      totalTonnage: Math.max(0, finite(st.totalTonnage)),
      totalRevenue: Math.max(0, finite(st.totalRevenue)),
      contractsGenerated: Math.max(0, Math.floor(finite(st.contractsGenerated))),
    };
    if (Number.isInteger(Number(save._nextClientId)) && Number(save._nextClientId) > 0) nextClientId = Number(save._nextClientId);
    for (const c of this.clients) {
      const n = parseInt(c.id.replace(/^client-/, ''), 10);
      if (Number.isFinite(n) && n >= nextClientId) nextClientId = n + 1;
    }
    const rawOverrides: UnknownRecord = isRecord(save.locationOverrides) ? save.locationOverrides : {};
    this.locationOverrides = {};
    for (const [key, ovRaw] of Object.entries(rawOverrides)) {
      if (!isRecord(ovRaw)) continue;
      const clean: IndustrialLocationOverride = {};
      if (typeof ovRaw.name === 'string' && ovRaw.name.trim()) clean.name = ovRaw.name.trim();
      if (typeof ovRaw.country === 'string') clean.country = ovRaw.country;
      if (ovRaw.type != null && this.getIndustryInfo(ovRaw.type)) clean.type = String(ovRaw.type);
      const lat = Number(ovRaw.lat), lon = Number(ovRaw.lon);
      if (Number.isFinite(lat) && lat >= -90 && lat <= 90) clean.lat = lat;
      if (Number.isFinite(lon) && lon >= -180 && lon <= 180) clean.lon = lon;
      this.locationOverrides[key] = clean;
    }
    const rawSites: UnknownRecord[] = (Array.isArray(save.customSites) ? save.customSites : []).filter(isRecord);
    const siteSeen = new Set<string>();
    this.customSites = rawSites
      .filter((x) => this.getIndustryInfo(x.type) && Number.isFinite(Number(x.lat)) && Number.isFinite(Number(x.lon)) && Number(x.lat) >= -90 && Number(x.lat) <= 90 && Number(x.lon) >= -180 && Number(x.lon) <= 180)
      .map((x): CustomIndustrialSite => {
        const key = String(x._key || x.id || `custom|${x.type}|${x.name || ''}|${x.lat}|${x.lon}`);
        return {
          id: x.id == null ? '' : String(x.id),
          type: String(x.type),
          name: String(x.name || 'Site').trim() || 'Site',
          lat: Number(x.lat),
          lon: Number(x.lon),
          country: String(x.country || 'FR'),
          _key: key,
          industryName: typeof x.industryName === 'string' ? x.industryName : undefined,
          icon: typeof x.icon === 'string' ? x.icon : undefined,
        };
      })
      .filter((x) => !siteSeen.has(x._key) && Boolean(siteSeen.add(x._key)));
    this.hiddenStaticKeys = new Set((Array.isArray(save.hiddenStaticKeys) ? save.hiddenStaticKeys : []).filter((x: unknown): x is string => typeof x === 'string'));
  }
}
