type __KPM43 = { "lat": number; "lon": number };
type __KPM44 = { "lat": number; "lon": number };
type __KPA17 = unknown[];
type __KPA18 = unknown[];
type __KPA19 = { "id": unknown; "name": unknown; "lat": number; "lon": number; "type": unknown; "uicRef": unknown; "osmType": unknown; "osmId": unknown; "ref": unknown; "operator": unknown; "network": unknown; "wikidata": unknown; "wheelchair": unknown };

type StationProgress = Record<string, unknown>;
type StationProgressCallback = ((progress: StationProgress) => void) | null;
type QueryRow = {
    station?: unknown; railway?: unknown; name?: unknown; centroid?: unknown; stationKind?: unknown;
    train?: unknown; subway?: unknown; tram?: unknown; lightRail?: unknown; monorail?: unknown;
    uicRef?: unknown; ref?: unknown; operator?: unknown; network?: unknown; wikidata?: unknown; wheelchair?: unknown;
    [key: string]: unknown;
};
type WorldStation = QueryRow & {
    id: string; lat: number; lon: number; type?: string; osmType?: unknown; osmId?: unknown;
    reference?: boolean; source?: unknown; country?: unknown; _normName?: string;
};
type CompactStationRow = unknown[];
...
export async function loadEmbeddedStationPack(onProgress: ((progress: Record<string, unknown>) => void) | null = null) {
    const globalBag = globalThis as typeof globalThis & Record<string, unknown>;
    const pack = globalBag[EMBEDDED_PACK_GLOBAL] as { prepared?: boolean; shards?: unknown[]; source?: string; datasetKind?: string; generatedAt?: string } | undefined;
...
function valueOf(binding: unknown) {
...
function parseWktPoint(wkt: unknown) {
    const s = String(wkt || '');
...
function parseOsmIri(iri: unknown) {
    const s = String(iri || '');
...
function normText(v: unknown) { return String(v || '').trim().toLowerCase(); }
function isUrbanTransitOnly(row: { stationKind?: unknown; train?: unknown; subway?: unknown; tram?: unknown; lightRail?: unknown; monorail?: unknown }) {
...
function approxDistanceMeters(a: __KPM43, b: __KPM44) {
    const lat = (a.lat + b.lat) * 0.5 * Math.PI / 180;
    const dy = (a.lat - b.lat) * 111320;
    const dx = (a.lon - b.lon) * 111320 * Math.max(0.15, Math.cos(lat));
    return Math.hypot(dx, dy);
}
export function dedupeWorldStations(stations: unknown) {
    const byId = new Map<string, WorldStation>();
    const byUic = new Map<string, WorldStation>();
    const grid = new Map<string, number[]>();
    const out: WorldStation[] = [];
    const cell = 0.01; // ~1 km, candidate lookup only.
    const gridKey = (lat:number, lon:number) => `${Math.floor(lat / cell)}:${Math.floor(lon / cell)}`;
    for (const raw of Array.isArray(stations) ? stations : []) {
        if (!raw || typeof raw !== 'object') continue;
        const st = raw as Partial<WorldStation>;
        if (!st.id || typeof st.id !== 'string' || !Number.isFinite(st.lat) || !Number.isFinite(st.lon)) continue;
        const typed = st as WorldStation;
        if (byId.has(typed.id))
            continue;
        const uic = String(typed.uicRef || '').trim();
        if (uic) {
            const existing = byUic.get(uic);
            if (existing) { byId.set(typed.id, existing); continue; }
        }
        ...
        const stored: WorldStation = { ...typed, _normName: nn };
...
export function parseWorldStationResponse(data: unknown) {
    let rows: QueryRow[] = [];
    const root = data && typeof data === 'object' ? data as Record<string, unknown> : {};
    const results = root.results && typeof root.results === 'object' ? root.results as Record<string, unknown> : {};
    if (Array.isArray(results.bindings)) {
        rows = results.bindings.map((b: { station: unknown; railway: unknown; name: unknown; centroid: unknown; stationKind: unknown; train: unknown; subway: unknown; tram: unknown; lightRail: unknown; monorail: unknown; uicRef: unknown; ref: unknown; operator: unknown; network: unknown; wikidata: unknown; wheelchair: unknown }) => ({
        ...
        const names = root.selected.map((x: unknown) => String(x).replace(/^\?/, ''));
        rows = root.res.map((r: __KPA17) => {
            const o: QueryRow = {};
        ...
        rows = root.result.map((r: __KPA18) => {
            const o: QueryRow = {};
...
class WorldStationDB {
    ...
    open() {
        ...
        this._openPromise = new Promise<IDBDatabase | null>((resolve) => {
            try {
...
function compactStations(stations: WorldStation[]) {
...
function expandStations(rows: unknown, source: unknown = 'OpenStreetMap / QLever OSM Planet') {
    return (Array.isArray(rows) ? rows : []).filter(Array.isArray).map((r: CompactStationRow) => ({
    ...
    })).filter((s: { id: unknown; lat: unknown; lon: unknown }) => s.id && Number.isFinite(s.lat) && Number.isFinite(s.lon));
}
async function fetchQLeverJson(query: string, onProgress: StationProgressCallback) {
    let lastError = null;
...
export class GlobalStationCatalog {
    stations: WorldStation[]; loaded: boolean; source: string; cacheTimestamp: number; refreshPromise: Promise<unknown> | null;
    constructor() {
        this.stations = [];
        this.loaded = false;
        this.source = '';
    ...
    async load(onProgress: StationProgressCallback = null, onRefreshed: unknown = null) {
        const embedded = await loadEmbeddedStationPack(onProgress);
        if (!embedded?.stations?.length) {
            const err = new Error('Pack européen RailNet intégré absent ou vide. Réextraire Rail Empire v1.1.14 dans un dossier neuf.') as Error & { code?: string };
    ...
    async refresh(onProgress: StationProgressCallback = null) {
