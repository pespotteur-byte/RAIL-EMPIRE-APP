...
type PresenceService = { id: unknown; active?: unknown; completed?: unknown; cancelled?: unknown; state?: unknown; position?: unknown };
type ServiceLookupFrame = {
    services: PresenceService[];
    length: number;
    indexed: boolean;
    byId: Map<unknown, { service: PresenceService; index: number }>;
    resourceById?: Map<string, { service: PresenceService; index: number }>;
};
function buildServiceLookupFrame(services: PresenceService[]): ServiceLookupFrame {
    const byId: ServiceLookupFrame['byId'] = new Map();
    for (let index = 0; index < services.length; index++) {
        const service = services[index];
        // Match Array.find even for an invalid duplicate-ID legacy save.
        if (!byId.has(service.id)) byId.set(service.id, { service, index });
    }
...
export function timeDiff(a: __KPM474, b: __KPM475) {
    let d = a - b;
    if (d > 720)
        d -= 1440;
    else if (d < -720)
 ...
 */
export function analyzeRoute(route: RoutePoint[], trainMaxSpeed: number) {
 ...
 */
function getBlockLength(speed: number) {
...
export class CantonManager {
    private _serviceLookupFrame: ServiceLookupFrame | null = null;
    ...
    private _generatedTables = new WeakMap<CantonAssignment[], {length:number;ids:Set<unknown>}>();
    private _ensureAssignment(a: CantonAssignment): void {
        if (!a || this.cantons.has(a.cantonId)) return;
        const c = new Canton(a.cantonId,a.startIndex,a.endIndex,a.resourceIds || []);
        this.cantons.set(a.cantonId,c); this._registerCantonResources(c,a.resourceIds || []);
    }
    private _assignmentIndex(assignments: CantonAssignment[], index: number): number {
        const generated = this._generatedTables.get(assignments);
        if (!generated || generated.length !== assignments.length) return assignments.findIndex(a=>index>=a.startIndex&&index<a.endIndex);
        let lo=0,hi=assignments.length;
        while(lo<hi){const mid=(lo+hi)>>>1;if(assignments[mid].endIndex<=index)lo=mid+1;else hi=mid;}
    ...
    findResourceOwner<T extends PresenceService>(services: T[], id: unknown): T | undefined {
        ...
        if (!frame) return services.find(service => String(service?.id) === key);
        ...
        const rebuild = () => {
            const owners = new Map<string, { service: PresenceService; index: number }>();
            for (let index = 0; index < services.length; index++) {
                const service = services[index];
                if (service && !owners.has(String(service.id))) owners.set(String(service.id), { service, index });
            }
        ...
        if (entry && (services[entry.index] !== entry.service || String(entry.service.id) !== key)) {
            owners = rebuild(); entry = owners.get(key);
        }
        if (!entry) {
            ...
            if (current) { owners = rebuild(); entry = owners.get(key); }
        }
        return entry?.service as T | undefined;
    }

    private _lookupPresenceService(services: PresenceService[], id: unknown): PresenceService | undefined {
        let frame = this._serviceLookupFrame;
        if (!frame) return services.find(service => service.id === id);
    ...
    setTime(timeOfDay: unknown) {
        this.currentTime = timeOfDay;
    }
    setTrainSeparation(trainId: unknown, minutes: unknown) {
     ...
     */
    _geoKey(lat1: number, lon1: number, lat2: number, lon2: number, trackSig: unknown = '') {
    ...
    _physicalSegmentKey(a: RoutePoint, b: RoutePoint) {
        if (!a || !b)
            return '';
    ...
    _registerCantonResources(canton: Canton, resourceIds: string[]) {
        if (!canton)
            return;
     ...
     */
    createRouteCantons(route: RoutePoint[]) {
        if (!route || route.length < 2)
            return [];
        ...
        const assignments: CantonAssignment[] = [];
        let blockStartIdx = 0;
        let blockDist = 0;
        let routeKm = 0;
        let blockStartKm = 0;
        for (let i = 0; i < route.length - 1; i++) {
            ...
            if (blockDist >= targetBlockLength || i === route.length - 2) {
                ...
                const wayIds = [...new Set(route.slice(blockStartIdx, endIdx + 1)
                        .map((q) => q?.wayId).filter((v: unknown) => v != null && v !== ''))].map(String).sort();
     ...
     */
    getCantonForSegment(assignments: CantonAssignment[], segmentIndex: number) {
    ...
    occupy(cantonId: unknown, trainId: unknown) {
        const c = this.cantons.get(cantonId);
        if (!c)
            return true;
    ...
    release(cantonId: unknown, trainId: unknown) {
        const c = this.cantons.get(cantonId);
        if (!c)
            return;
    ...
    isAvailable(cantonId: unknown, trainId: unknown) {
        const c = this.cantons.get(cantonId);
        if (!c)
            return true;
    ...
    _isTrainGone(id: unknown) {
        if (globalThis.window?.game?.depotManager?.hasPhysicalRescue?.(id)) return false;
    ...
    transferTrainResources(from: unknown, to: unknown): Set<unknown> {
        ...
        for (const id of ids) {
            ...
            if (c && c.occupiedBy === from) c.occupiedBy = to;
            if (c && c.reservedBy === from) c.reservedBy = to;
            this._trackCanton(to,id);
        }
        this.trainCantons.delete(from);
        return ids;
    }
    getTrackedCantons(trainId: unknown) {
        return new Set(this.trainCantons.get(trainId) || []);
    }
    releaseAll(trainId: unknown) {
     ...
     */
    syncFootprint(trainId: unknown, assignments: CantonAssignment[], frontKm: unknown, lengthM: unknown = 0, retainIds: Iterable<unknown> | null = null) {
        ...
        const eps = 1e-6;
        const generated=this._generatedTables.get(assignments);
        let desired: CantonAssignment[];
        if(generated && generated.length===assignments.length){
            let lo=0,hi=assignments.length;
            while(lo<hi){const mid=(lo+hi)>>>1;if(assignments[mid].endKm+eps<rear)lo=mid+1;else hi=mid;}
            desired=[];for(let i=lo;i<assignments.length && assignments[i].startKm-eps<=front;i++)desired.push(assignments[i]);
        } else desired=assignments.filter(a=>Number(a.endKm)+eps>=rear&&Number(a.startKm)-eps<=front);
        ...
        const desiredIds = new Set(desired.map((a: { cantonId: unknown }) => a.cantonId));
        if (retainIds)
     ...
     */
    reserveNextAhead(assignments: CantonAssignment[], segmentIndex: number, trainId: unknown, frontKm: unknown = 0, horizonM: number = Infinity) {
        if (!Array.isArray(assignments) || !assignments.length)
            return null;
    ...
    firstUnavailableAhead(assignments: CantonAssignment[], segmentIndex: number, trainId: unknown, frontKm: unknown = 0, horizonM: number = Infinity) {
    ...
    cleanup() {
        ...
        for(const table of this.routeCantons.values())for(const a of table)activeCantonIds.add(a.cantonId);
        ...
        for(const service of live)for(const a of service?._cantonAssignments || [])activeCantonIds.add(a.cantonId);
    ...
    getEntrySignalAspect(assignments: CantonAssignment[], entryIndex: number, trainId: unknown): 0 | 30 | null {
        const first=assignments[entryIndex];if(!first)return null;
        this._ensureAssignment(first);
        if(!this.isAvailable(first.cantonId,trainId))return 0;
        const second=assignments[entryIndex+1];
        if(second){this._ensureAssignment(second);if(!this.isAvailable(second.cantonId,trainId))return 30;}
        return null;
    }
    ...
    _trackCanton(trainId: unknown, cantonId: unknown) {
