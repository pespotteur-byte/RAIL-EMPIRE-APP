...
// HOTFIX68 — station names are spoken in the language of the station, not in
// the language of the fixed French announcement. Piper models are downloaded
// on demand by piper-tts-web and cached by the browser when possible. Missing
// Piper languages prefer a same-language system voice. HOTFIX75 adds a final
// locale-hinted browser utterance (without assigning a wrong-language voice) so
// local file:// builds do not become completely silent when Piper/OPFS fails.
type StationVoiceConfig = Readonly<{ voiceId: string | null; label: string; source: string }>;
type PiperProgress = { loaded?: unknown; total?: unknown };
type PiperVoiceSession = { predict(text: string): unknown };
type PiperSessionFactory = {
  _instance?: PiperVoiceSession | null;
  create(options: { voiceId: string; allowLocalModels: boolean; fallbackStrategy: string; progress: (ev: PiperProgress) => void; logger: () => void }): Promise<PiperVoiceSession>;
};
type PiperModule = {
...
type PiperVoiceOptions = {
  enabled?: boolean; voiceId?: string; moduleUrl?: string;
  moduleLoader?: ((url: string) => Promise<PiperModule> | PiperModule) | null;
  prepareTimeoutMs?: number; initialPredictTimeoutMs?: number; predictTimeoutMs?: number;
};
type StationRecord = { id?: string; name?: unknown; country?: unknown; lat?: unknown; lon?: unknown };
type SpeechWorld = {
...
type SpeechStop = {
  stationId?: string; locationName?: unknown; name?: unknown; country?: unknown;
  lat?: unknown; latitude?: unknown; lon?: unknown; longitude?: unknown; type?: unknown;
};
type SpeechService = {
  id?: string; currentStopIndex?: unknown; state?: string; isReturnLeg?: boolean;
  stops?: SpeechStop[]; returnStops?: SpeechStop[]; getCurrentStops?: () => SpeechStop[] | null | undefined;
};
type StationSpeechMeta = {
  name: string; speech: string; country: string; lat?: number | null; lon?: number | null;
  locale: string; voiceId: string | null; voiceLabel?: string; voiceSource?: string;
};
type LivemapAnnouncement = {
  destination: string; destinationSpeech: string; destinationSpeechMeta: StationSpeechMeta;
  futureStops: string[]; futureStopsSpeech: string[]; futureStopsSpeechMeta: StationSpeechMeta[]; text: string;
};
type LivemapAnnouncerOptions = {
  enabled?: boolean; useRecordedPhrases?: boolean; rate?: number; pitch?: number; volume?: number;
  useNeuralFrenchVoice?: boolean; useNeuralStationVoices?: boolean; neuralReadyTimeoutMs?: number;
  neuralFirstUseWaitMs?: number; stationNeuralReadyTimeoutMs?: number; neuralTts?: PiperFrenchNeuralVoice;
  neuralVoiceId?: string; neuralModuleUrl?: string; neuralModuleLoader?: PiperVoiceOptions['moduleLoader'];
  neuralPrepareTimeoutMs?: number; neuralInitialPredictTimeoutMs?: number; neuralPredictTimeoutMs?: number;
  voiceReadyTimeoutMs?: number; voicePollMs?: number; allowLanguageFallback?: boolean;
  speechWatchdogMs?: number; mediaWatchdogMs?: number;
};
...
function normalizeCountryCode(value: unknown) {
  ...
  return (COUNTRY_NAME_ALIASES as Readonly<Record<string,string>>)[key] || upper;
}

function normLang(value: unknown) { return String(value || '').trim().replace(/_/g,'-'); }
function langFamily(value: unknown) { return normLang(value).toLowerCase().split('-')[0] || ''; }

export function resolveStationSpeechLocale(country: unknown, lat: unknown = null, lon: unknown = null, name: unknown = '') {
  ...
  if (c === 'CH') {
    if (Number.isFinite(y) && Number.isFinite(x) && y < 46.68 && x > 8.15) return 'it-IT';
    if (Number.isFinite(y) && Number.isFinite(x) && ((x < 7.25) || (y < 46.45 && x < 7.78))) return 'fr-FR';
    return 'de-DE';
  }
  // Belgium: coarse linguistic regions; Brussels remains Dutch/French-capable via system fallback.
  if (c === 'BE') {
    if (Number.isFinite(x) && x > 5.85) return 'de-DE';
    if (Number.isFinite(y) && y < 50.75) return 'fr-FR';
    return 'nl-BE';
  }
  // Spain: improve pronunciation in the two strongest non-Castilian rail regions.
  if (c === 'ES' && Number.isFinite(y) && Number.isFinite(x)) {
    if (y > 42.45 && x > -3.35 && x < -1.45) return 'eu-ES';
    if (x > -0.15 && y > 40.45) return 'ca-ES';
  }
  // Wales: local Welsh pronunciation where the coordinate is clearly in Wales.
  if (c === 'GB' && Number.isFinite(y) && Number.isFinite(x) && y > 51.25 && y < 53.55 && x < -2.5 && x > -5.5) return 'cy-GB';
  return (COUNTRY_DEFAULT_LOCALES as Readonly<Record<string,string>>)[c] || 'fr-FR';
}

export function stationVoiceProfile(country: unknown, lat: unknown = null, lon: unknown = null, name: unknown = '') {
  const locale = resolveStationSpeechLocale(country, lat, lon, name);
  const cfg = (PIPER_STATION_VOICES as Readonly<Record<string, StationVoiceConfig>>)[locale] || Object.freeze({voiceId:null,label:`System ${locale}`,source:'system'});
  return { locale, country:normalizeCountryCode(country), voiceId:cfg.voiceId || null, label:cfg.label, source:cfg.source };
}

function withTimeout<T>(promise: Promise<T>, timeoutMs: number, code: string = 'TIMEOUT') {
...
export class PiperFrenchNeuralVoice {
  ...
  set ready(v: unknown) { if (v) this._readyVoices.add(this.voiceId); else this._readyVoices.delete(this.voiceId); }
  isVoiceReady(voiceId: unknown = this.voiceId) { return this._readyVoices.has(String(voiceId || this.voiceId)); }
  ...
  async _probeOpfsCache() {
    ...
    if (this.moduleLoader) return true;
    const nav = globalThis.navigator;
    if (!nav?.storage?.getDirectory) return false;
    if (!this._opfsProbePromise) {
      this._opfsProbePromise = withTimeout(
        Promise.resolve().then(() => nav.storage.getDirectory()),
        1500,
        'PIPER_OPFS_PROBE_TIMEOUT',
  ...
  async _loadModule() {
    if (!this.enabled) throw new Error('PIPER_DISABLED');
    if (!this._modulePromise) {
      this._modulePromise = (async () => {
        if (this.moduleLoader) return await this.moduleLoader(this.moduleUrl);
        const dynamicImport = new Function('url', 'return import(url)');
        const urls = [this.moduleUrl, PIPER_WEB_MODULE_FALLBACK_URL, PIPER_WEB_MODULE_LEGACY_URL].filter((u: string, i: number, a: string[]) => u && a.indexOf(u) === i);
        let last = null;
        for (const url of urls) {
          try { return await dynamicImport(url); }
          catch (err: unknown) { last = err; }
        }
        throw last || new Error('PIPER_MODULE_IMPORT_FAILED');
      })().then((mod: PiperModule) => {
        if (!mod || typeof mod.predict !== 'function') throw new Error('PIPER_MODULE_INVALID');
        return mod;
      }).catch((err: unknown) => {
        this._modulePromise = null;
        throw err;
  ...
  _onProgress(event: PiperProgress) {
  ...
  async _getVoiceSession(mod: PiperModule, voiceId: unknown) {
    const requested = String(voiceId || this.voiceId);
    const TtsSession = mod?.TtsSession;
    if (!requested || !TtsSession || typeof TtsSession.create !== 'function') return null;
    if (this._sessions.has(requested)) return this._sessions.get(requested);
    if (this._sessionPromises.has(requested)) return await this._sessionPromises.get(requested);
    ...
    const create = async () => {
      ...
      try {
        ...
        const session = await TtsSession.create({
          ...
          progress: (ev: PiperProgress) => this._onProgress(ev),
          logger: () => {},
        });
        if (!session || typeof session.predict !== 'function') throw new Error('PIPER_SESSION_INVALID');
        this._sessions.set(requested, session);
        return session;
      } finally {
        try { TtsSession._instance = previous; } catch (_) { /* package compatibility */ }
      }
    };
    ...
    finally { this._sessionPromises.delete(requested); }
  }

  async synthesize(text: unknown, voiceId: unknown = this.voiceId) {
    const clean = normalizeSpace(text);
    const requested = String(voiceId || this.voiceId);
    if (!clean || !this.enabled || !requested) return null;
    ...
    if (session) predictPromise = Promise.resolve(session.predict(clean));
    else if (typeof mod.predict === 'function') predictPromise = Promise.resolve(mod.predict({ text: clean, voiceId: requested }, (ev: PiperProgress) => this._onProgress(ev)));
    else throw new Error('PIPER_PREDICT_UNAVAILABLE');
    const wav = await withTimeout(
      predictPromise,
      wasReady ? this.predictTimeoutMs : this.initialPredictTimeoutMs,
      'PIPER_PREDICT_TIMEOUT',
    );
    if (!wav || typeof wav !== 'object') throw new Error('PIPER_EMPTY_AUDIO');
  ...
  prepareVoice(voiceId: unknown = this.voiceId, warmText: unknown = 'gare') {
    const requested = String(voiceId || this.voiceId);
    if (!this.enabled || !requested) return Promise.resolve(false);
    if (this.isVoiceReady(requested)) return Promise.resolve(true);
    if (this._preparePromises.has(requested)) return this._preparePromises.get(requested);
    if (requested === this.voiceId) this.preparing = true;
    this.lastError = '';
    const prepareJob = (async () => {
      ...
      if (canCache && typeof mod.download === 'function') {
        try { await mod.download(requested, (ev: PiperProgress) => this._onProgress(ev)); }
    ...
    const promise = withTimeout(prepareJob, this.prepareTimeoutMs, 'PIPER_PREPARE_TIMEOUT')
      ...
      .catch((err: unknown) => {
        ...
        this.lastError = String((err as {code?: unknown; message?: unknown})?.code || (err as {message?: unknown})?.message || 'PIPER_PREPARE_FAILED');
        return false;
      })
      .finally(() => {
        if (requested === this.voiceId) this.preparing = false;
...
export function normalizeStationForSpeech(name: unknown, locale: unknown = 'fr-FR') {
  ...
  if (family === 'fr') s = s.replace(/\bGare\s+TGV\b/gi, 'Gare T G V');
  return s;
}

function nearestCountryForPoint(world: SpeechWorld | null | undefined, lat: number, lon: number) {
  const y = Number(lat), x = Number(lon);
  if (!Number.isFinite(y) || !Number.isFinite(x) || !world?.getStationsNear) return '';
  const near = world.getStationsNear(y, x, 6) || [];
  let best = null, bestD = Infinity;
  for (const st of near) {
    if (!st?.country || st.country === 'OTHER') continue;
...
function stationSpeechMetaForStop(stop: SpeechStop | null | undefined, world: SpeechWorld | null | undefined): StationSpeechMeta {
  if (!stop) return { name:'', speech:'', country:'', locale:'fr-FR', voiceId:PIPER_FRENCH_VOICE_ID };
  let st = null;
  if (stop.stationId && world?.getStationById) st = world.getStationById(stop.stationId) || null;
  ...
  if (!country || country === 'OTHER' || country === 'UNMAPPED') country = normalizeCountryCode(nearestCountryForPoint(world, lat, lon) || '');
...
function stationNameForStop(stop: SpeechStop | null | undefined, world: SpeechWorld | null | undefined) { return stationSpeechMetaForStop(stop, world).name; }

function isPassengerStop(stop: SpeechStop | null | undefined) {
  if (!stop) return false;
  // V2 uses arret for booked stops. Legacy/custom saves may omit type.
  return stop.type == null || stop.type === 'arret' || stop.type === 'stop';
}

function dedupeConsecutive(names: string[]) {
  const out = [];
  for (const name of names) {
    if (!name) continue;
    if (out[out.length - 1]?.toLocaleLowerCase('fr') === name.toLocaleLowerCase('fr')) continue;
    out.push(name);
  }
  return out;
}

function futureStopStartIndex(svc: SpeechService, stops: SpeechStop[]) {
  ...
  if ((svc?.state === 'waiting' || svc?.state === 'predeparture') && idx === 0 && stops.length > 1) return 1;
...
export function joinFrenchList(items: unknown[] | null | undefined) {
...
export function buildLivemapAnnouncement(svc: SpeechService | null | undefined, world: SpeechWorld | null | undefined): LivemapAnnouncement | null {
  if (!svc) return null;
  ...
  for (const x of futureMeta) {
    if (deduped.at(-1)?.name?.toLocaleLowerCase('fr') === x.name.toLocaleLowerCase('fr')) continue;
    deduped.push(x);
  }
  futureMeta = deduped;
  ...
  if (futureMeta.length === 0 && destination) futureMeta = [destinationMeta];
...
function isFrenchVoice(voice: SpeechSynthesisVoice | null | undefined) {
  const lang = String(voice?.lang || '').trim().toLowerCase().replace(/_/g, '-');
  return lang === 'fr' || lang.startsWith('fr-');
}

function scoreVoice(voice: SpeechSynthesisVoice) {
  if (!isFrenchVoice(voice)) return -Infinity;
  const lang = String(voice?.lang || '').toLowerCase().replace(/_/g, '-');
  const name = String(voice?.name || '').toLowerCase();
  let score = 0;
  if (lang === 'fr-fr') score += 120;
  else if (lang.startsWith('fr-')) score += 100;
  else if (lang === 'fr') score += 90;
  if (FEMALE_FR_NAME_HINTS.some((h) => name.includes(h))) score += 35;
...
function scoreLocaleVoice(voice: SpeechSynthesisVoice, locale: unknown) {
  const want = normLang(locale).toLowerCase();
  const wantFamily = langFamily(want);
  const got = normLang(voice?.lang).toLowerCase();
  if (!wantFamily || langFamily(got) !== wantFamily) return -Infinity;
  let score = 0;
  if (got === want) score += 140;
  else if (got.startsWith(`${wantFamily}-`)) score += 110;
  else if (got === wantFamily) score += 100;
  if (wantFamily === 'fr' && FEMALE_FR_NAME_HINTS.some((h) => String(voice?.name || '').toLowerCase().includes(h))) score += 35;
...
export class LivemapTrainAnnouncer {
  declare game: { world?: SpeechWorld };
  declare enabled: boolean;
  declare useRecordedPhrases: boolean;
  declare rate: number;
  declare pitch: number;
  declare volume: number;
  declare useNeuralFrenchVoice: boolean;
  declare useNeuralStationVoices: boolean;
  declare neuralReadyTimeoutMs: number;
  declare neuralFirstUseWaitMs: number;
  declare stationNeuralReadyTimeoutMs: number;
  declare neuralTts: PiperFrenchNeuralVoice;
  declare voiceReadyTimeoutMs: number;
  declare voicePollMs: number;
  declare allowLanguageFallback: boolean;
  declare speechWatchdogMs: number;
  declare mediaWatchdogMs: number;
  declare voice: SpeechSynthesisVoice | null;
  declare _voices: SpeechSynthesisVoice[];
  declare lastError: string;
  declare _runToken: number;
  declare _currentAudio: HTMLAudioElement | null;
  declare _currentAudioFinish: ((ok: boolean) => void) | null;
  declare _mediaElement: HTMLAudioElement | null;
  declare _audioContext: AudioContext | null;
  declare _audioUnlocked: boolean;
  declare _currentBufferSource: AudioBufferSourceNode | null;
  declare _currentBufferFinish: ((ok: boolean) => void) | null;
  declare _currentObjectUrl: string | null;
  declare _lastServiceId: string | null;
  declare _audioDiag: Record<string, unknown>;
  constructor(game: unknown, options: LivemapAnnouncerOptions = {}) {
    this.game = game as { world?: SpeechWorld };
    this.enabled = options.enabled !== false;
    ...
    this.useNeuralFrenchVoice = (options.useNeuralFrenchVoice !== false && (options.neuralTts || typeof globalThis.document !== 'undefined')) as boolean;
    ...
    if (synth?.addEventListener) synth.addEventListener('voiceschanged', this._loadVoices);
    this._loadVoices();
  ...
  _publishAudioDiagnostics(stage: unknown, extra: Record<string, unknown> = {}) {
    ...
    const data = {
      ...
      voices: (this._voices || []).map((v: SpeechSynthesisVoice) => ({name:String(v?.name||''),lang:String(v?.lang||''),local:!!v?.localService})),
    ...
    try { (globalThis as typeof globalThis & { __RAIL_EMPIRE_AUDIO_DIAG__?: unknown }).__RAIL_EMPIRE_AUDIO_DIAG__ = data; } catch (_) { /* no-op */ }
    return data;
  }
  ...
  prepareNeuralVoice() {
    if (!this.useNeuralFrenchVoice || !this.neuralTts?.prepare) return Promise.resolve(false);
    return Promise.resolve(this.neuralTts.prepare()).then((ok: unknown) => {
      if (!ok && this.neuralTts?.lastError) this.lastError = `PIPER:${this.neuralTts.lastError}`;
      this._publishAudioDiagnostics(ok ? 'piper-ready' : 'piper-unavailable');
      return !!ok;
    }).catch((err: unknown) => {
      this.lastError = `PIPER:${String((err as {message?: unknown})?.message || err || 'PREPARE_FAILED')}`;
  ...
  _loadVoices() {
    ...
    this.voice = french.sort((a: SpeechSynthesisVoice, b: SpeechSynthesisVoice) => scoreVoice(b) - scoreVoice(a))[0] || null;
    this.lastError = this.voice ? '' : 'NO_FRENCH_VOICE';
    return this.voice;
  }

  _systemVoiceForLocale(locale: unknown) {
    this._loadVoices();
    const family = langFamily(locale);
    if (!family) return null;
    const candidates = (this._voices || []).filter((v: SpeechSynthesisVoice) => langFamily(v?.lang) === family);
    candidates.sort((a: SpeechSynthesisVoice,b: SpeechSynthesisVoice)=>scoreLocaleVoice(b, locale)-scoreLocaleVoice(a, locale));
    return candidates[0] || null;
  }

  async _waitForLocaleVoice(locale: unknown, token: unknown, timeoutMs: number = this.voiceReadyTimeoutMs) {
    ...
    while (token === this._runToken && Date.now() < deadline) {
      await new Promise<void>((resolve) => setTimeout(resolve, this.voicePollMs));
  ...
  async _waitForFrenchVoice(token: number, timeoutMs: number = this.voiceReadyTimeoutMs) {
    const v = await this._waitForLocaleVoice('fr-FR', token, timeoutMs);
    if (v && isFrenchVoice(v)) { this.voice = v; return v; }
    if (token === this._runToken) this.lastError = 'NO_FRENCH_VOICE';
    return null;
  }
  ...
  _getMediaElement() {
    ...
    if (!this._mediaElement) {
      ...
      } catch (_) { this._mediaElement = null; }
    }
  ...
  _unlockAudioFromGesture() {
    ...
    const Ctx = globalThis.AudioContext || (globalThis as typeof globalThis & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
    if (typeof Ctx !== 'function') return false;
    try {
      if (!this._audioContext || this._audioContext.state === 'closed') this._audioContext = new Ctx();
      ...
      this.lastError = `AUDIO_UNLOCK:${String((err as {message?: unknown})?.message || err || 'FAILED')}`;
      return false;
    }
  }

  _blobArrayBuffer(blob: Blob) {
    if (blob?.arrayBuffer) return blob.arrayBuffer();
    const FileReaderCtor = globalThis.FileReader;
    if (typeof FileReaderCtor !== 'function') return Promise.reject(new Error('BLOB_ARRAYBUFFER_UNAVAILABLE'));
  ...
  _decodeAudioBuffer(ctx: BaseAudioContext, arrayBuffer: ArrayBuffer): Promise<AudioBuffer> {
    return new Promise<AudioBuffer>((resolve, reject) => {
      let settled = false;
      const ok = (value: AudioBuffer) => { if (!settled) { settled = true; resolve(value); } };
      const bad = (err: unknown) => { if (!settled) { settled = true; reject(err || new Error('AUDIO_DECODE_FAILED')); } };
      try {
        const out = ctx.decodeAudioData(arrayBuffer, ok, bad);
        if (out?.then) out.then(ok, bad);
      } catch (err: unknown) { bad(err); }
    });
  }

  async _playNeuralViaContextAsync(blob: Blob, token: number) {
    const ctx = this._audioContext as AudioContext | null;
    if (token !== this._runToken || !this._audioUnlocked || !ctx?.decodeAudioData || !ctx?.createBufferSource) return false;
    try {
      if (ctx.state === 'suspended') await Promise.resolve(ctx.resume?.()).catch(() => false);
      if (token !== this._runToken || ctx.state === 'closed') return false;
      const bytes = await this._blobArrayBuffer(blob);
      if (token !== this._runToken) return false;
      const audioBuffer = await this._decodeAudioBuffer(ctx, bytes);
      if (token !== this._runToken || !audioBuffer) return false;
      return await new Promise((resolve) => {
        let source: AudioBufferSourceNode | null = null, gain: GainNode | null = null, done = false, watchdog: ReturnType<typeof setTimeout> | null = null;
        const finish = (ok: boolean) => {
          if (done) return;
          done = true;
          if (watchdog) clearTimeout(watchdog);
          if (this._currentBufferSource === source) this._currentBufferSource = null;
          if (this._currentBufferFinish === finish) this._currentBufferFinish = null;
        ...
        try {
          ...
          watchdog = setTimeout(() => { try { source?.stop(); } catch (_) {} finish(false); }, Math.max(this.mediaWatchdogMs, durationMs + 5000));
          source.start(0);
  ...
  _playMediaSrcAsync(src: unknown, token: unknown) {
    return new Promise((resolve) => {
      if (token !== this._runToken) return resolve(false);
      const a = this._getMediaElement();
      if (!a) return resolve(false);
      if (this._currentAudioFinish) {
        try { this._currentAudioFinish(false); } catch (_) {}
      }
      this._currentAudio = a;
      a.preload = 'auto';
      a.volume = Math.min(1, Math.max(0, this.volume));
      let done = false, watchdog: ReturnType<typeof setTimeout> | null = null;
      const finish = (ok: boolean) => {
        if (done) return;
        done = true;
        if (watchdog) clearTimeout(watchdog);
        if (this._currentAudio === a) this._currentAudio = null;
        if (this._currentAudioFinish === finish) this._currentAudioFinish = null;
  ...
  cancel() {
    this._runToken++;
    if (this._currentAudioFinish) { try { this._currentAudioFinish(false); } catch (_) {} }
    if (this._currentBufferFinish) { try { this._currentBufferFinish(false); } catch (_) {} }
    if (this._currentBufferSource) { try { this._currentBufferSource.stop?.(); } catch (_) {} this._currentBufferSource = null; }
    if (this._currentAudio) {
      try { this._currentAudio.pause(); this._currentAudio.currentTime = 0; } catch (_) { /* no-op */ }
      this._currentAudio = null;
    }
    if (this._currentObjectUrl) {
      try { globalThis.URL?.revokeObjectURL?.(this._currentObjectUrl); } catch (_) { /* no-op */ }
      this._currentObjectUrl = null;
    }
    try { globalThis.speechSynthesis?.cancel?.(); } catch (_) { /* no-op */ }
  }

  _makeUtterance(text: string) {
    const Utterance = globalThis.SpeechSynthesisUtterance;
    if (typeof Utterance !== 'function') return null;
    const frenchVoice = this.voice && isFrenchVoice(this.voice) ? this.voice : null;
    if (!frenchVoice && !this.allowLanguageFallback) return null;
  ...
  async _speakAsync(text: string, token: number) {
    if (token !== this._runToken) return false;
    if (!this.voice || !isFrenchVoice(this.voice)) {
      ...
      if (!this.voice && this.allowLanguageFallback) this.lastError = 'FR_LANG_FALLBACK';
    }
    return await new Promise((resolve) => {
      if (token !== this._runToken) return resolve(false);
      const synth = globalThis.speechSynthesis;
      const u = this._makeUtterance(text);
      if (!synth?.speak || !u) return resolve(false);
      let done = false;
      ...
      watchdog = setTimeout(() => {
        try { synth.cancel?.(); } catch (_) { /* no-op */ }
        finish(false);
  ...
  _makeLocaleUtterance(text: string, locale: unknown, voice: SpeechSynthesisVoice | null | undefined) {
    const Utterance = globalThis.SpeechSynthesisUtterance;
    if (typeof Utterance !== 'function' || !voice) return null;
    const wantFamily = langFamily(locale);
    if (!wantFamily || langFamily(voice?.lang) !== wantFamily) return null;
    const u = new Utterance(text);
  ...
  _makeLocaleHintUtterance(text: string, locale: unknown) {
    const Utterance = globalThis.SpeechSynthesisUtterance;
    if (typeof Utterance !== 'function') return null;
    const u = new Utterance(text);
  ...
  async _speakLocaleHintAsync(text: string, locale: unknown, token: number) {
    if (token !== this._runToken || !this.allowLanguageFallback) return false;
    const synth = globalThis.speechSynthesis;
    const u = this._makeLocaleHintUtterance(text, locale);
    if (!synth?.speak || !u) return false;
    ...
    return await new Promise((resolve) => {
      let done=false, watchdog: ReturnType<typeof setTimeout> | null=null;
      const finish=(ok: boolean)=>{ if(done)return; done=true; if(watchdog)clearTimeout(watchdog); resolve(ok && token===this._runToken); };
      u.onend=()=>finish(true);
      u.onerror=()=>finish(false);
      watchdog=setTimeout(()=>{ try{synth.cancel?.();}catch(_){} finish(false); },this.speechWatchdogMs);
      try { synth.resume?.(); synth.speak(u); } catch (_) { finish(false); }
    });
  }

  async _speakLocaleSystemAsync(text: string, locale: unknown, token: number, readyTimeoutMs: number = this.voiceReadyTimeoutMs) {
    if (token !== this._runToken) return false;
    const voice = await this._waitForLocaleVoice(locale, token, readyTimeoutMs);
    if (token !== this._runToken || !voice) return false;
    return await new Promise((resolve) => {
      const synth = globalThis.speechSynthesis;
      const u = this._makeLocaleUtterance(text, locale, voice);
      if (!synth?.speak || !u) return resolve(false);
      let done=false, watchdog: ReturnType<typeof setTimeout> | null=null;
      const finish=(ok: boolean)=>{ if(done)return; done=true; if(watchdog)clearTimeout(watchdog); resolve(ok); };
  ...
  _neuralVoiceReady(voiceId: unknown) {
    if (!voiceId || !this.neuralTts) return false;
    if (typeof this.neuralTts.isVoiceReady === 'function') return !!this.neuralTts.isVoiceReady(voiceId);
    return voiceId === PIPER_FRENCH_VOICE_ID && !!this.neuralTts.ready;
  }

  _prepareNeuralVoiceId(voiceId: unknown, warmText: unknown = 'gare') {
    if (!this.useNeuralStationVoices || !voiceId || !this.neuralTts) return Promise.resolve(false);
    if (this._neuralVoiceReady(voiceId)) return Promise.resolve(true);
    if (typeof this.neuralTts.prepareVoice === 'function') return Promise.resolve(this.neuralTts.prepareVoice(voiceId, warmText));
    if (voiceId === PIPER_FRENCH_VOICE_ID && typeof this.neuralTts.prepare === 'function') return Promise.resolve(this.neuralTts.prepare());
    return Promise.resolve(false);
  }

  async _speakNeuralVoiceIdAsync(text: string, voiceId: string, token: number) {
    if (token !== this._runToken || !this.useNeuralStationVoices || !voiceId || !this.neuralTts?.synthesize) return false;
    try {
      const blob = await this.neuralTts.synthesize(text, voiceId) as Blob | null;
      if (token !== this._runToken || !blob) return false;
      const ok = await this._playNeuralBlobAsync(blob, token);
      if (!ok && token === this._runToken) this.lastError = `PIPER_AUDIO_PLAYBACK_FAILED:${voiceId}`;
      return ok;
    } catch (err) {
      if (token === this._runToken) this.lastError = `PIPER:${voiceId}:${String((err as {code?: unknown; message?: unknown})?.code || (err as {message?: unknown})?.message || 'PREDICT_FAILED')}`;
      return false;
    }
  }

  _prewarmStationVoices(announcement: LivemapAnnouncement) {
    if (!this.useNeuralStationVoices) return;
    ...
    for (const m of metas) {
      ...
      if (!id || id === PIPER_FRENCH_VOICE_ID || ids.includes(id)) continue;
      ids.push(id);
      if (ids.length >= 2) break; // old/low-memory Chromium: never warm a fleet of ONNX models at once.
    }
    let chain: Promise<unknown>=Promise.resolve();
    for (const id of ids) chain=chain.then(()=>this._prepareNeuralVoiceId(id,'gare')).catch(()=>false);
    void chain;
  }

  async _speakStationMetaAsync(meta: StationSpeechMeta, token: number, frenchMode: 'auto' | 'neural' | 'system' = 'auto') {
    const speech=normalizeSpace(meta?.speech || meta?.name || '');
    const locale=normLang(meta?.locale || 'fr-FR') || 'fr-FR';
    if (!speech || token !== this._runToken) return false;
    if (langFamily(locale) === 'fr') return await this._speakDynamicAsync(speech, token, frenchMode);
    ...
    if (voiceId && this._neuralVoiceReady(voiceId)) {
      if (await this._speakNeuralVoiceIdAsync(speech, voiceId, token)) return true;
    }
    const systemVoice=this._systemVoiceForLocale(locale);
    if (systemVoice) return await this._speakLocaleSystemAsync(speech, locale, token, 0);
    ...
    if (voiceId) {
      let ok=false;
      try { ok=(await withTimeout(this._prepareNeuralVoiceId(voiceId, speech), this.stationNeuralReadyTimeoutMs, 'PIPER_STATION_NOT_READY')) as boolean; }
      catch (_) { ok=false; }
      if (ok && token === this._runToken) return await this._speakNeuralVoiceIdAsync(speech, voiceId, token);
    }
    ...
    if (await this._speakLocaleHintAsync(speech, locale, token)) return true;
    this.lastError=`NO_MATCHING_STATION_VOICE:${locale}`;
    this._publishAudioDiagnostics('station-voice-missing', { missingLocale:locale });
    return false;
  }

  async _speakStationSequenceAsync(metas: StationSpeechMeta[], token: number, frenchMode: 'auto' | 'neural' | 'system' = 'auto') {
    ...
    const groups: Array<{locale:string; rows:StationSpeechMeta[]}>=[];
    for (const row of rows) {
      const loc=normLang(row.locale || 'fr-FR');
      const g=groups.at(-1);
      if (g && g.locale === loc) g.rows.push(row);
      else groups.push({locale:loc,rows:[row]});
    }
  ...
  async _playNeuralBlobAsync(blob: Blob, token: number) {
    if (token !== this._runToken || !blob) return false;
    ...
    if (await this._playNeuralViaContextAsync(blob, token)) return true;
    if (token !== this._runToken) return false;

    const URLApi = globalThis.URL;
    if (!URLApi?.createObjectURL) return false;
    let url = null;
    try { url = URLApi.createObjectURL(blob); }
    catch (_) { return false; }
    this._currentObjectUrl = url;
    try {
      return await this._playMediaSrcAsync(url, token);
    } finally {
      if (this._currentObjectUrl === url) this._currentObjectUrl = null;
      try { URLApi.revokeObjectURL(url); } catch (_) { /* no-op */ }
    }
  }

  async _speakNeuralAsync(text: string, token: number) {
    if (token !== this._runToken || !this.useNeuralFrenchVoice || !this.neuralTts?.synthesize) return false;
    try {
      const blob = await this.neuralTts.synthesize(text) as Blob | null;
      if (token !== this._runToken || !blob) return false;
      const ok = await this._playNeuralBlobAsync(blob, token);
      if (!ok && token === this._runToken) this.lastError = 'PIPER_AUDIO_PLAYBACK_FAILED';
      return ok;
    } catch (err) {
      if (this.neuralTts && 'ready' in this.neuralTts) { try { this.neuralTts.ready = false; } catch (_) { /* no-op */ } }
      if (token === this._runToken) this.lastError = `PIPER:${String((err as {code?: unknown; message?: unknown})?.code || (err as {message?: unknown})?.message || 'PREDICT_FAILED')}`;
      return false;
    }
  }

  async _speakDynamicAsync(text: string, token: number, mode: 'auto' | 'neural' | 'system' = 'auto') {
    if (token !== this._runToken) return false;
    ...
    if (wantsNeural && this.neuralTts?.ready) {
      if (await this._speakNeuralAsync(text, token)) return true;
  ...
  _playOneRecordedAsync(src: string, token: number) {
    return this._playMediaSrcAsync(src, token);
  }

  async _playRecordedAsync(sources: string | readonly string[], token: number) {
    const list = Array.isArray(sources) ? sources : [sources];
    for (const src of list) {
      if (token !== this._runToken) return false;
      if (await this._playOneRecordedAsync(src, token)) return true;
    }
    return false;
  }

  async _runHybrid(announcement: LivemapAnnouncement, token: number) {
    const phrase = async (sources: string | readonly string[], fallbackText = '') => {
      if (token !== this._runToken) return false;
      const played = await this._playRecordedAsync(sources, token);
      if (played) return token === this._runToken;
    ...
    if (!await phrase(RECORDED_PHRASES.attention, 'Attention.')) return;
    ...
    let dynamicMode: '' | 'neural' | 'system' = this.neuralTts?.ready ? 'neural' : '';
    if (!dynamicMode && this.useNeuralFrenchVoice) {
      try {
        ...
        if (ok || this.neuralTts?.ready) dynamicMode = 'neural';
    ...
    if (!dynamicMode) {
      try {
        ...
        if (v) dynamicMode = 'system';
      } catch (_) { /* language hint below */ }
    }
    if (!dynamicMode) dynamicMode = 'system'; // _speakAsync => fr-FR language hint when no explicit voice.
    await this._speakStationMetaAsync(
      {...announcement.destinationSpeechMeta, speech:`${announcement.destinationSpeech}.`},
      token,
      dynamicMode,
    );
    if (token !== this._runToken) return;
  ...
  async _runTtsOnly(announcement: LivemapAnnouncement, token: number) {
    if (this.useNeuralFrenchVoice) void this.prepareNeuralVoice();
    const dynamicMode = this.neuralTts?.ready ? 'neural' : 'system';
    if (token !== this._runToken) return;
    if (!await this._speakDynamicAsync('Attention.', token, dynamicMode)) return;

    const destinationMeta=announcement.destinationSpeechMeta || {};
    if (!await this._speakDynamicAsync('Ce train a pour destination', token, dynamicMode)) return;
    await this._speakStationMetaAsync({...destinationMeta,speech:`${announcement.destinationSpeech}.`}, token, dynamicMode);
    if (token !== this._runToken) return;

    if (!await this._speakDynamicAsync("Il s'arrêtera en gare de", token, dynamicMode)) return;
    await this._speakStationSequenceAsync(announcement.futureStopsSpeechMeta || [], token, dynamicMode);
    this._publishAudioDiagnostics('announcement-complete-tts');
  }

  announce(svc: SpeechService) {
    if (!this.enabled || !svc) return false;
    ...
    try { globalThis.speechSynthesis?.resume?.(); } catch (_) { /* no-op */ }
    this._loadVoices();
