...
export class RollingStockItem {
  ...
  constructor(data: Record<string, unknown> = {}) {
    ...
    if (this.category === 'automotrice' && this.traction === 'electrique' && /^X\s?\d{3,5}\b/.test(this.name)) {
      ...
      if (/sncf|trains-europe\.fr\/sncf/.test(origin)) this.traction = 'diesel';
    }
    ...
    if ((this.category === 'locomotive' || this.category === 'automotrice') && this.power > 0 && this.power < 10) {
      if (/remorque|voiture d/i.test(this.name)) this.power = 0;
      else if (/^X\s?73900\b/.test(this.name)) this.power = 630;
      else if (/^X\s?73500\b/.test(this.name)) this.power = 514;
      else if (/^Z\s?6400\b/.test(this.name)) this.power = 1180;
      else this.power = defaultPower;
    }
