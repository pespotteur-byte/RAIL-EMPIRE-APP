...
import type { FreightContract } from './freight.js';
type __KPM525 = { "_v2DirectRameId"?: unknown; "rame"?: unknown };
type __KPM546 = unknown[];
type __KPM561 = number;
type __KPM565 = string;
type __KPM568 = number;
type __KPM570 = { "stationId": unknown; "name": unknown };
type __KPM571 = { "serviceType": unknown; "category": unknown; "train": Record<string, unknown> };
...
type LivemapDisplayStop = { s: LivemapStop; origIdx: number };
type RameClickArg = boolean | { ctrlKey?: boolean } | null | undefined;
type ScheduleStopData = { stationId?: unknown; voiePointId?: unknown; type?: unknown; stopCode?: unknown; arrivalTime?: number; departureTime?: number; platform?: unknown };
type ScheduleStopEdit = { stationId?: unknown; voiePointId?: unknown; type?: unknown; stopCode?: unknown; arrTimeMin?: number; depTimeMin?: number; platform?: unknown };
...
type AlertBannerItem = { text: string; icon: string };
type WorksLocationSource = { startStation?: { name?: unknown } | null; endStation?: { name?: unknown } | null; stationA?: unknown; stationB?: unknown };
type RuntimeDiagItem = { level?: string; label?: unknown; status?: unknown };
...
type InfogareFieldExtra = { color?: string; fontSize?: number; weight?: number; align?: string; textTransform?: string; bg?: string; alpha?: number; bitmapFont?: string; letterSpacing?: number; style?: string };
type InfogareImageBlock = { y: number; h: number; bg?: string; viaY?: number; viaH?: number; remarkY?: number; remarkH?: number; time?: InfogareFieldSpec; type?: InfogareFieldSpec; num?: InfogareFieldSpec; dest?: InfogareFieldSpec; provenance?: InfogareFieldSpec; via?: InfogareFieldSpec; status?: InfogareFieldSpec; remark?: InfogareFieldSpec; voie?: InfogareFieldSpec };
type InfogareImageLayout = { file: string; width: number; height: number; bg?: string; scale?: number; header?: { bg?: string; color?: string }; headerFields?: InfogareFieldSpec[]; blocks: InfogareImageBlock[] };
...
const LVM_CAT_COLORS: Record<string, string> = { voyageur: '#3b82f6', fret: '#22c55e', travaux: '#f59e0b', machine: '#a855f7' };
const LVM_CAT_LABELS: Record<string, string> = { voyageur: 'Voyageur', fret: 'Fret', travaux: 'Travaux', machine: 'Machine' };
const LVM_CAT_ICONS: Record<string, string> = { voyageur: 'img/livemap/train_voyageur.png', fret: 'img/livemap/train_fret.png', travaux: 'img/livemap/train_travaux.png', machine: 'img/livemap/train_generic.png' };
...
const STOCK_DETAIL_GROUPS: { label: string; items: [string, string][] }[] = [
...
function _foldStock(v: unknown = '') {
  return String(v ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[’']/g, "'");
}
function _stockFilterMeta(item: Partial<RollingStockItem> = {}) {
  ...
  const has = (re: __S3Struct465) => re.test(txt);
  ...
  if (!powered) tractions.add('none');
  if (isElectric) tractions.add('electric');
  if (isDiesel) tractions.add('diesel');
  if (isSteam) tractions.add('steam');
  if (isBimode) tractions.add('bimode');
  if (isBattery) tractions.add('battery');
  if (isHydrogen) tractions.add('hydrogen');
  if (voltageHits >= 2 || /multisystem|multi[- ]?system|tricourant|quadricourant/.test(txt)) tractions.add('multisystem');
  ...
  if (root === 'locomotive') {
    if (isElectric) details.add('loco-electric');
    if (isDiesel) details.add('loco-diesel');
    if (isSteam) details.add('loco-steam');
    if (isBimode) details.add('loco-bimode');
    if (/locotracteur|shunter|shunting|rangier|\bkof\b|\bköf\b|\by\s*\d{3,5}\b/.test(txt)) details.add('loco-shunter');
    if (highSpeed || /motrice|power car|triebkopf/.test(txt)) details.add('loco-highspeed');
    if (/industrial|industriel|mine|works|travaux|infra/.test(txt)) details.add('loco-industrial');
  }
  if (root === 'automotrice') {
    if (isElectric) details.add('mu-electric');
    if (isDiesel || /autorail|railcar|\bdmu\b/.test(txt)) details.add('mu-diesel');
    if (isBimode) details.add('mu-bimode');
    if (isBattery) details.add('mu-battery');
    if (isHydrogen) details.add('mu-hydrogen');
    if (highSpeed) details.add('mu-highspeed');
    if (/\brer\b|s[- ]?bahn|suburban|banlieue|transilien/.test(txt)) details.add('mu-suburban');
    if (/tram[- ]?train|dualis|regiocitadis|citylink/.test(txt)) details.add('mu-tramtrain');
    if (/\bter\b|\bagc\b|regio\s*2n|\bflirt\b|\bkiss\b|coradia|talent|desiro|minuetto|regiolis|régiolis/.test(txt)) details.add('mu-regional');
    if (/motrice|motor car|power car|triebkopf|powered/.test(txt) || (powered && /(^|[_\s-])[ml][0-9]?([_\s-]|$)/.test(txt))) details.add('mu-motor-car');
    if (/intermedia|intermediate|mittelwagen|middle car/.test(txt)) details.add('mu-intermediate');
    if (!powered || /remorque|trailer|steuerwagen/.test(txt)) details.add('mu-trailer');
  }
  if (root === 'voiture') {
    const driving = !!item.isDrivingTrailer || /voiture pilote|driving trailer|steuerwagen|cab car|pilote/.test(txt);
    if (driving) details.add('coach-driving');
    ...
    if (first && !mixed) details.add('coach-first');
    if (second && !mixed) details.add('coach-second');
    if (mixed) details.add('coach-mixed-class');
    if (/voiture[- ]?lits|sleeping|schlafwagen|(^|[\s_-])wl([\s_-]|$)/.test(txt)) details.add('coach-sleeper');
    if (/couchett|liegewagen/.test(txt)) details.add('coach-couchette');
    if (/restaurant|dining|speisewagen|(^|[\s_-])wr([\s_-]|$)/.test(txt)) details.add('coach-restaurant');
    if (/\bbar\b|bistro|buffet/.test(txt)) details.add('coach-bar');
    if (/fourgon|bagage|baggage|gepack|gepäck/.test(txt)) details.add('coach-baggage');
    if (postal) details.add('coach-postal');
    if (/salon|pullman|parlour|lounge/.test(txt)) details.add('coach-salon');
    if (doubleDeck) details.add('coach-doubledeck');
    if (highSpeed) details.add('coach-highspeed');
    if (/\bciwl\b|orient express/.test(txt)) details.add('coach-ciwl');
    if (/\bpmr\b|wheelchair|fauteuil|velo|vélo|bicycle|fahrrad|multifonction/.test(txt)) details.add('coach-accessible');
  }
  if (root === 'wagon') {
    const sub = _foldStock(item.wagonSubCategory);
    if (sub === 'citerne' || /citerne|tank wagon|kesselwagen|(^|[\s_-])zac?n?s?([\s_-]|$)/.test(txt)) details.add('wagon-tank');
    if (sub === 'gaz' || /gazier|gas wagon|gaz|lpg|lng/.test(txt)) details.add('wagon-gas');
    if (sub === 'tombereau' || /tombereau|open wagon|eaos|eanos/.test(txt)) details.add('wagon-open');
    if (sub === 'tremie' || /tremie|trémie|hopper|fals|falns|talns/.test(txt)) details.add('wagon-hopper');
    if (sub === 'cerealier' || /cereal|céré|grain|farine/.test(txt)) details.add('wagon-grain');
    if (['ciment','silos'].includes(sub) || /ciment|cement|silo|pulverulent/.test(txt)) details.add('wagon-cement');
    if (sub === 'couvert' || /couvert|boxcar|covered wagon|gedeckt|habbi|gbs\b/.test(txt)) details.add('wagon-covered');
    if (sub === 'bache' || /bache|bâch|tarpaulin|telescop|shimmns/.test(txt)) details.add('wagon-tarpaulin');
    if (sub === 'plat' || /wagon plat|flat wagon|flatcar|rils\b|rnoos/.test(txt)) details.add('wagon-flat');
    if (sub === 'intermodal' || /container|conteneur|intermodal|sggr|sgrss|megafret|poche mobile/.test(txt)) details.add('wagon-intermodal');
    if (sub === 'porte-auto' || /porte[- ]?auto|car carrier|autorack|laaers/.test(txt)) details.add('wagon-auto');
    if (/frigor|reefer|refriger|kühl|kuhl/.test(txt)) details.add('wagon-reefer');
    if (/coil|bobine|acier|steel|stahl|poutrelle/.test(txt)) details.add('wagon-steel');
    if (/bois|grume|timber|wood|holz/.test(txt)) details.add('wagon-timber');
    if (/ballast|schotter/.test(txt)) details.add('wagon-ballast');
    if (['infra','ttx'].includes(sub) || service) details.add('wagon-infra');
    if (sub === 'speciaux' || /grue|crane|secours|rescue|special/.test(txt)) details.add('wagon-crane');
    if (/betail|bétail|livestock|cattle|animaux|animal/.test(txt)) details.add('wagon-livestock');
  }

  if (powered) details.add('role-powered'); else details.add('role-unpowered');
  if (passenger) details.add('role-passenger');
  if (freight) details.add('role-freight');
  if (passenger && freight) details.add('role-mixed');
  if (item.isDrivingTrailer || /driving trailer|steuerwagen|cab car|voiture pilote/.test(txt)) details.add('role-driving-cab');
  if (service) details.add('role-service');
  if (postal) details.add('role-postal');
  if (night) details.add('role-night');
  if (doubleDeck) details.add('role-doubledeck');

  const vmax = Number(item.maxSpeed);
  if (Number.isFinite(vmax) && vmax > 0) {
    if (vmax <= 90) details.add('speed-le90');
    else if (vmax <= 120) details.add('speed-100-120');
    else if (vmax <= 140) details.add('speed-121-140');
    else if (vmax <= 160) details.add('speed-141-160');
    else if (vmax <= 200) details.add('speed-161-200');
    else details.add('speed-gt200');
  }
  ...
  if (item && typeof item === 'object') _stockFilterMetaCache.set(item, meta);
  return meta;
}
function _rameKindCodes(rame: Rame | Record<string, unknown> = {}) {
  const out = new Set();
  const details = Array.isArray(rame.elementDetails) ? rame.elementDetails : [];
  const roots = new Set(details.map((e: { category: unknown }) => _foldStock(e.category)));
  ...
  if (passenger && freight) out.add('mixed');
  else if (passenger) out.add('passenger');
  else if (freight) out.add('freight');
  else if (power) out.add('traction-only');
  if (roots.has('automotrice')) out.add('multiple-unit');
  if (roots.has('locomotive') && roots.has('voiture')) out.add('loco-hauled-passenger');
  if (roots.has('locomotive') && roots.has('wagon')) out.add('loco-hauled-freight');
  const allCodes = new Set(details.flatMap((e: unknown) => [..._stockFilterMeta(e as Partial<RollingStockItem>).details]));
  if (allCodes.has('mu-highspeed') || allCodes.has('coach-highspeed') || allCodes.has('loco-highspeed')) out.add('high-speed');
  if (allCodes.has('role-doubledeck')) out.add('double-deck');
  if (allCodes.has('role-night')) out.add('night');
  if (allCodes.has('role-service') || allCodes.has('wagon-infra')) out.add('infra');
  if (allCodes.has('role-driving-cab')) out.add('driving-trailer');
  return out;
}
...
const PAGE_PARENT: Record<string, unknown> = {};
const PAGE_GROUPS: Array<Array<[string, string]>> = [];
...
export class UI {
  ...
  constructor(game: unknown) {
    ...
    setTimeout(() => {
      if (this.activePage === 'map') void this.trainAnnouncer?.prepareNeuralVoice?.();
    }, 2500);
    ...
    this._approachInterval = setInterval(() => {
      if (this.activePage !== 'map' || !this._hasApproachBlink) return;
  ...
  _displayRameForService(svc: __KPM525) {
  ...
  _livemapImageSignature(rame: LivemapRame | null | undefined) {
    if (!rame) return '';
    const parts = (rame.elementDetails || []).map((e, i:number) =>
      `${e.catalogId || e.id || e.instanceName || e.name || i}:${e.imageData || ''}:${e.flipped ? 1 : 0}`
    );
  ...
  _hydratePersistentTrainImages(container: HTMLElement, services: Array<__S3Struct469 & __KPM525>, preserved: Map<string, { signature: string; node: Element }> = new Map()) {
    if (!container) return;
    const byId = new Map((services || []).map((svc: __S3Struct469 & __KPM525) => [String(svc.id), svc]));
    container.querySelectorAll('.tc-images-scroll[data-service-id]').forEach((slot) => {
      ...
      for (const e of (rame?.elementDetails || [])) {
        ...
        if (e.flipped) img.style.transform = 'scaleX(-1)';
        img.loading = 'lazy';
        img.decoding = 'async';
  ...
  refreshAll() {
    ...
    else if (this.activePage === 'qg') this.renderQGPage();
    ...
    else if (this.activePage === 'marketing') this.renderMarketingPage();
  }
  ...
  setupPageHelpButtons() {
    if(!this._pageHelpDelegated){
      ...
      document.addEventListener('click',(e)=>{
        const btn=(e.target as Element | null)?.closest?.('[data-page-help]') as HTMLElement | null;
        if(!btn)return;
        const page=btn.dataset.pageHelp||this.activePage||'map';
        try{this.game.tutorial?.startPage?.(page,this.game);}catch(err){console.warn('Page tutorial error:',err);}
      });
    }
    document.querySelectorAll('.page[id^="page-"]').forEach((root) =>this._ensurePageHelpButton(root.id.slice(5)));
  }

  _ensurePageHelpButton(page: string) {
    const root=document.getElementById(`page-${page}`);if(!root)return null;
    if(root.querySelector(`[data-page-help="${page}"]`))return root.querySelector(`[data-page-help="${page}"]`);
    const b=document.createElement('button');b.type='button';b.className='btn-secondary page-help-btn';b.dataset.pageHelp=page;b.innerHTML='<span aria-hidden="true">❓</span> Aide / Tutoriel';b.title=`Aide détaillée — ${page}`;
    if(page==='map'){const host=root.querySelector('.map-controls')||root;host.appendChild(b);return b;}
    const header=root.querySelector(':scope > .page-content > .page-header')||root.querySelector('.rv3-head');
    if(header){header.appendChild(b);return b;}
    b.classList.add('page-help-floating');root.appendChild(b);return b;
  }
  ...
  switchPage(page: string) {
    ...
    if (page !== 'map' && this._threeDFollowActive) this.disable3DFollow();
    ...
    if (page !== 'depots' && this._iteMapInterval) { clearInterval(this._iteMapInterval); this._iteMapInterval = null; }
    if (page !== 'lines' && this._sillonMapInterval) { clearInterval(this._sillonMapInterval); this._sillonMapInterval = null; }
    if (page !== 'incidents' && this._worksMapInterval) { clearInterval(this._worksMapInterval); this._worksMapInterval = null; }
    ...
    if (page === 'map') {
      ...
      if (Array.isArray(this.game._visibleBuf)) this.game._visibleBuf.length = 0;
      if (Array.isArray(this.game._stoppedWithPosBuf)) this.game._stoppedWithPosBuf.length = 0;
    ...
    if (page === 'rolling-stock') { this._populateStockSubcatFilter(); this.renderStockList(); }
    if (page === 'rames') this.renderRamesList();
    if (page === 'liveries') this.renderLiveriesPage();
    if (page === 'schedules') { this._ensureScheduleV2Editor(); this.renderSchedulesList(); }
    if (page === 'rotations') { this._ensureRotationV2Editor(); this.rotationV2Editor?.render(); }
    if (page === 'lines') this.renderLinesList();
    if (page === 'depots') this.renderDepotsList();
    if (page === 'incidents') this.renderIncidentsPage();
    if (page === 'qg') this.renderQGPage();
    ...
    if (page === 'marketing') this.renderMarketingPage();
  ...
  setupMapEvents() {
    ...
    const finishObjectDrag = () => {
      ...
      if (this._draggingStation) {
        ...
        if (this.game.world) this.game.world._stationSpatialDirty = true;
      ...
      if (changed) this.game.saveState?.();
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      if (this._draggingVoiePoint && this.game.renderer) {
        ...
        if (this.game.voiePointManager) this.game.voiePointManager._spatialDirty = true;
        this.game.renderer?.invalidateStatic?.();
        return;
      }
      ...
      if (this.game.renderer) {
        ...
        if (!this._hoverRAF) {
          this._hoverRAF = requestAnimationFrame(() => {
            this._hoverRAF = 0;
            const hp = this._pendingHover; this._pendingHover = null;
            if (hp) this.handleMapHover(hp.x, hp.y);
          });
        }
      }
    });
    ...
    canvas.addEventListener('mouseup', (e) => {
      ...
      if (this.isDragging && !this.dragMoved && this.game.renderer) {
        ...
        if (this.industryCreationMode) {
          ...
          const typeList = types.map((t: { type: unknown; name: unknown }) => `${t.type} - ${t.name}`).join('\n');
          const typeInput = prompt(`Type d'industrie :\n${typeList}`) || '';
          const type = typeInput.split(' - ')[0].trim();
          if (!types.find((t: { type: unknown }) => t.type === type)) { this.isDragging = false; return; }
    ...
    canvas.addEventListener('dblclick', (e: unknown) => {
    ...
    let touchStart: { x: number; y: number } | null = null, touchDist: number | null = null, touchMoved = false;
    ...
    canvas.addEventListener('touchend', (e: unknown) => {
  ...
  _livemapEsc(value: unknown) {
    return String(value ?? '').replace(/[&<>"']/g, (c) => (HTML_ESCAPE_MAP[c]));
  }

  _livemapClock(value:number, showDay: unknown = false) {
  ...
  _livemapWorkLocation(item: LivemapWorkItem) {
    const stationName = (value:number | undefined) => value ? (this.game.world.getStationById?.(value)?.name || String(value)) : '';
    ...
    if(a&&b)return a===b?`à ${a}`:`entre ${a} et ${b}`;
    return a||b?`à ${a||b}`:'';
  }

  _livemapWorkTooltipHtml(item: LivemapWorkItem) {
    if(!item)return '';
    const esc=(v: unknown) =>this._livemapEsc(v);
  ...
  handleMapHover(x:number, y:number) {
    ...
    if (station) {
      const typeLabels: Record<string, unknown> = { voyageur: 'Voyageurs', marchandise: 'Marchandises', ite: 'ITE', depot: 'Depot', mixed: 'Mixte' };
  ...
  _findServiceAtScreen(x:number, y:number) {
    const renderer = this.game.renderer;
  ...
  selectService(svc: __S3Struct474) {
  ...
  _livemapPayloadText(svc: ActiveServiceLike): string {
    ...
    if (cat === 'voyageur' || rame.totalCapacity > 0) return `${Math.max(0, Math.round(Number(svc._onboardPax) || 0))} passagers à bord`;
    return '';
  }
  ...
  _syncLivemapPanel() {
    ...
    if (!svc || !svc.train || !this.game.scheduleCreator?.services.includes(svc)) {
      if (this._threeDFollowActive) this.disable3DFollow();
      this.selectedService = null;
      this._lvpKey = null;
    ...
    const stops: LivemapStop[] = typeof svc.getCurrentStops === 'function'
      ? svc.getCurrentStops()
    ...
    if (payload) payload.textContent = this._livemapPayloadText(svc);
    const dl = document.getElementById('lvp-delay');
    if (dl) {
      const d = operationalDelayMinutes(Number.isFinite(t.delay as number) ? (t.delay as number) : 0);
    ...
    const reasonSig = (Array.isArray(t.incidentDelayReasons) ? t.incidentDelayReasons : [])
      .map((r: { incidentId: unknown; contributed: unknown; active: unknown; text: unknown }) => `${r.incidentId || ''}:${r.contributed ? 1 : 0}:${r.active ? 1 : 0}:${r.text || ''}`).join('|');
    ...
    if (stateKey !== this._lvpStateKey) {
      ...
      if (reasonsHtml) {
        if (reasonsEl) reasonsEl.outerHTML = reasonsHtml;
        else if (sitEl) sitEl.insertAdjacentHTML('afterend', reasonsHtml);
      } else if (reasonsEl) {
        reasonsEl.remove();
      }
  ...
  selectServiceById(id:string) {
    const svc = this.game.scheduleCreator?.services.find((s: { id: unknown }) => s.id === id);
  ...
  applyCameraFollow() {
    ...
    if (!svc || !this.game.renderer?.tileMap) return;
    ...
    if (svc.completed || svc.cancelled || svc.state === 'completed' || svc.state === 'cancelled' || !this.game.scheduleCreator?.services.includes(svc)) {
      this._followService = null;
      if (this._threeDFollowActive) this.disable3DFollow();
      if (this.selectedService === svc) this.deselectService();
      return;
    }
    if (!svc.position) return;
    const renderer = this.game.renderer;
    const tm = renderer.tileMap;
    if (this._threeDFollowActive) this._sync3DCameraHeading(svc);
    ...
    if (this._threeDFollowActive && this._threeDReliefEnabled && terrain?.enabled) {
      ...
      if (p && Number.isFinite(p.x) && Number.isFinite(p.y)) {
        ...
        if (safe) this._set3DMapPan(dx, dy);
        else this._threeDMapNeedsReanchor = true;
      }
    } else if (viewCenter) {
    ...
    if (this._threeDFollowActive) this._sync3DAtmosphere();
  }

  _set3DMapPan(dx: unknown = 0, dy: unknown = 0) {
  ...
  prepare3DMapAnchorForRender(force:boolean = false) {
    if (!this._threeDFollowActive || this._threeDReliefEnabled) return false;
    if (!force && !this._threeDMapNeedsReanchor) return false;
  ...
  _sync3DCameraHeading(svc: unknown, force:boolean = false) {
    if (!this._threeDFollowActive || !svc) return;
    const renderer = this.game.renderer;
  ...
  setup3DFollowView() {
    if (this._threeDFollowBound) return;
    ...
    main?.addEventListener('click', (e) => {
      ...
      const btn = clicked?.closest?.('[data-re3d-action]') as HTMLElement | null;
      if (!btn) return;
      const action = btn.dataset.re3dAction;
      if (action === 'toggle') this.toggle3DFollow();
      else if (action === 'zoom-in') this.adjust3DZoom(1);
      else if (action === 'zoom-out') this.adjust3DZoom(-1);
      else if (action === 'recenter' || action === 'follow') this.recenter3DFollow();
      else if (action === 'labels') this.toggle3DLabels();
      else if (action === 'relief') this.toggle3DRelief();
      else if (action === 'exit') this.disable3DFollow();
    });
    main?.addEventListener('change', (e) => {
      ...
      this._sync3DAtmosphere(true);
      this.game.renderer?.requestRender?.();
      if (this.game) this.game._last3DMapRender = 0;
    });
    main?.addEventListener('wheel', (e) => {
      if (!this._threeDFollowActive) return;
  ...
  _set3DLightingFilter(value: unknown) {
  ...
  toggle3DFollow() {
    if (this._threeDFollowActive) this.disable3DFollow();
    else this.enable3DFollow();
  }

  _ensure3DServicePosition(svc: ActiveService) {
    if (!svc || svc.completed || svc.cancelled || svc.state === 'completed' || svc.state === 'cancelled') return null;
    if (svc.position && Number.isFinite(Number(svc.position.lat)) && Number.isFinite(Number(svc.position.lon))) return svc.position;
  ...
  enable3DFollow() {
    ...
    if (this._threeDFollowActive) { this.recenter3DFollow(); return true; }
    ...
    if (weatherFilter) weatherFilter.value = this._threeDWeatherFilter || 'auto';
    ...
    if (initialViewCenter) { tm.centerLat = initialViewCenter.lat; tm.centerLon = initialViewCenter.lon; tm.markDirty?.(); }
    this._sync3DAtmosphere(true);
    this._sync3DPanelButton();
    this._bindTerrainStatus();
    if (this._threeDReliefEnabled) {
      try { renderer.terrain3D?.enable?.(renderer.canvas); }
      catch (e) { console.warn('[RE3D] Relief DEM indisponible, pseudo-3D conservée', e); }
    }
  ...
  disable3DFollow() {
    ...
    if (atmosphere) { atmosphere.style.backgroundColor = ''; atmosphere.style.boxShadow = ''; }
    renderer?.clear3DMarkers?.();
    if (tm && prev) {
      if (prev.basic !== tm.basicMode) tm.toggleBasic();
      ...
      if (Number.isFinite(prev.zoom)) tm.zoomLevel = prev.zoom;
    ...
    this._sync3DPanelButton();
    requestAnimationFrame(() => {
      if (this.activePage === 'map') renderer?.resize?.();
      renderer?.requestRender?.();
    });
  }
  ...
  _sync3DToggleInputs() {
    const tm = this.game.renderer?.tileMap;
    if (!tm) return;
    const sat = document.getElementById('toggle-satellite'); if (sat) sat.checked = !!tm.satelliteEnabled;
    const orm = document.getElementById('toggle-orm'); if (orm) orm.checked = !!tm.railEnabled;
    const bas = document.getElementById('toggle-basic'); if (bas) bas.checked = !!tm.basicMode && !tm.satelliteEnabled && !tm.nightMapEnabled;
    const nit = document.getElementById('toggle-night') as HTMLInputElement | null; if (nit) nit.checked = !!tm.nightMapEnabled;
    const wea = document.getElementById('toggle-weather'); if (wea) wea.checked = !!tm.weatherEnabled;
  }

  _threeDViewCenterForService(svc: ActiveServiceLike, heading: unknown = null) {
  ...
  adjust3DZoom(direction: number) {
    if (!this._threeDFollowActive) return;
    const tm = this.game.renderer?.tileMap; if (!tm) return;
  ...
  recenter3DFollow() {
    const svc = this.selectedService; const tm = this.game.renderer?.tileMap;
    const pos = this._ensure3DServicePosition(svc);
    if (!pos || !tm) return;
    this._followService = svc;
    this._sync3DCameraHeading(svc, true);
    const viewCenter = this._threeDViewCenterForService(svc, this._threeDCameraHeading);
    if (viewCenter) { tm.centerLat = viewCenter.lat; tm.centerLon = viewCenter.lon; tm.markDirty?.(); }
    this._threeDMapNeedsReanchor = false;
    this._set3DMapPan(0, 0);
    if (this.game) this.game._last3DMapRender = 0;
    this.game.renderer?.terrain3D?.requestRebuild?.(true);
  ...
  _bindTerrainStatus() {
    if (this._terrainStatusUnsub || !this.game.renderer?.terrain3D) return;
    this._terrainStatusUnsub = this.game.renderer.terrain3D.onStatus((status:string, detail: unknown) => {
      const main = document.getElementById('main-area');
  ...
  _sync3DAtmosphere(force:boolean = false) {
    if (!this._threeDFollowActive) return;
    const main = document.getElementById('main-area');
    const canvas = document.getElementById('game-canvas');
    ...
    if (!force && key === this._threeDAtmosphereKey) return;
    this._threeDAtmosphereKey = key;
    for (const c of ['rain','snow','storm','fog','heat']) main.classList.toggle(`re3d-${c}`, type === c);
    ...
    let dim = night * .06;
    if (type === 'rain') dim += .07;
    if (type === 'storm') dim += .16;
    if (type === 'fog') dim = Math.max(.08, dim * .55);
    if (type === 'snow') dim = Math.max(0, dim - .08);
    ...
    const labels: Record<string, string> = {clear:'Dégagé',rain:'Pluie',snow:'Neige',storm:'Orage',fog:'Brouillard',heat:'Canicule'};
    const icons: Record<string, string> = {clear:'☀',rain:'☔',snow:'❄',storm:'⚡',fog:'≋',heat:'♨'};
    ...
    if (chip) chip.innerHTML = visualFilter === 'auto'
    ...
    if (badge) badge.textContent = `SUIVI 3D GPS · ${nightActive ? 'NUIT' : 'JOUR'}${lighting.mode === 'auto' ? ' AUTO' : ' FORCÉ'}${type !== 'clear' ? ' · ' + (labels[type] || type).toUpperCase() : ''}`;
  ...
  _lvpRouteDistanceKm(route: unknown) {
  ...
  _lvpRouteForLeg(svc: ActiveServiceLike | ActiveService, legIndex:number) {
    if (!svc || legIndex < 0) return null;
    if (svc.isReturnLeg) {
      if (Array.isArray(svc._returnRoutes) && svc._returnRoutes.length) return svc._returnRoutes[legIndex] || null;
  ...
  _lvpCurrentLegFraction(svc: ActiveServiceLike, route: unknown) {
    const st = svc?._state;
    if (st?.cachedRoute && st.cachedRoute.length >= 2 && st.segDists && st.cumDist) {
      ...
      if (total > 1e-9) return Math.max(0, Math.min(1, travelled / total));
    }
    ...
    if (svc?.position && Array.isArray(route) && route.length >= 2) {
      ...
      for (let i = 1; i < route.length; i++) {
        ...
        if (da < bestDist) { bestDist = da; bestAlong = along; }
        if (db < bestDist) { bestDist = db; bestAlong = along + seg; }
        along += seg;
      }
      if (total > 1e-9) return Math.max(0, Math.min(1, bestAlong / total));
    }
    return 0;
  }
  ...
  _livemapDistanceToStopKm(svc: UiActiveService, targetIndex: number): number | null {
    ...
    if (!['moving', 'departing'].includes(svc?.state) || !Number.isInteger(current) || current < 1 ||
        !Number.isInteger(targetIndex) || targetIndex < current) return null;
    const route = svc._state?.cachedRoute;
    if (!route || route.length < 2 || typeof svc._getRemainingDistance !== 'function') return null;
    ...
    for (let leg = current; leg < targetIndex; leg++) {
      ...
      if (!nextRoute || nextRoute.length < 2) return null;
  ...
  _livemapArrowProgress(svc: ActiveServiceLike, stops: LivemapStop[] | null = null, displayStops: LivemapDisplayStop[] | null = null) {
    stops = stops || (typeof svc?.getCurrentStops === 'function' ? svc.getCurrentStops() : []) || [];
    const isArret = (x: LivemapStop) => x && x.type === 'arret' && x.stationId;
    displayStops = displayStops || stops!.map((s: LivemapStop, i: number): LivemapDisplayStop => ({ s, origIdx: i })).filter(({ s }: LivemapDisplayStop) => isArret(s));
    if (!displayStops.length) return null;
    ...
    if (svc?.state !== 'moving' && svc?.state !== 'departing') {
      ...
      for (let i = 0; i < displayStops.length; i++) if (displayStops[i].origIdx <= stationOrig) d = i;
    ...
    for (let i = 0; i < displayStops.length; i++) {
      if (displayStops[i].origIdx <= currentLeg) fromDisplayIndex = i;
      else break;
    }
    let toDisplayIndex = fromDisplayIndex;
    for (let i = fromDisplayIndex + 1; i < displayStops.length; i++) {
      if (displayStops[i].origIdx >= curIdx) { toDisplayIndex = i; break; }
    }
    if (toDisplayIndex === fromDisplayIndex) return { fromDisplayIndex, toDisplayIndex, fraction: 0 };
    ...
    for (let leg = fromOrig; leg < toOrig; leg++) {
      ...
      total += dist;
      if (leg < currentLeg) travelled += dist;
      else if (leg === currentLeg) travelled += dist * this._lvpCurrentLegFraction(svc, route);
    }
    const fraction = total > 1e-9 ? Math.max(0, Math.min(1, travelled / total)) : 0;
    return { fromDisplayIndex, toDisplayIndex, fraction };
  }

  _syncLivemapProgressArrow(svc: ActiveServiceLike) {
    const panel = document.getElementById('livemap-train-panel');
    const inner = panel?.querySelector('.lvp-stops-inner');
    const arrow = inner?.querySelector('.lvp-arrow-floating');
    if (!inner || !arrow || !svc) return;
    const stops: LivemapStop[] = typeof svc.getCurrentStops === 'function' ? svc.getCurrentStops() : (svc.isReturnLeg ? svc.returnStops : svc.stops) || [];
    const displayStops = stops.map((s: LivemapStop, i: number): LivemapDisplayStop => ({ s, origIdx: i })).filter(({ s }: LivemapDisplayStop) => s && s.type === 'arret' && s.stationId);
    const p = this._livemapArrowProgress(svc, stops, displayStops);
    if (!p) { arrow.style.opacity = '0'; return; }
    const rows = inner.querySelectorAll('.lvp-stop[data-display-idx]');
    const a = rows[p.fromDisplayIndex], b = rows[p.toDisplayIndex];
    if (!a || !b) { arrow.style.opacity = '0'; return; }
    const center = (row: unknown) => (row as HTMLElement).offsetTop + (row as HTMLElement).offsetHeight / 2;
  ...
  _buildLivemapPanelContent(svc: ActiveServiceLike) {
    ...
    const fmt = (m: unknown) => this.minToTimeStr(((Math.round(m as number) % 1440) + 1440) % 1440);
    const curIdx = svc.currentStopIndex || 0;

    // currentStopIndex semantics: moving = target stop; stopped/waiting = next leg, current station is previous
    let prevIdx, curStationIdx: number, nextIdx: number;
    if (svc.state === 'moving') {
      prevIdx = curIdx - 1;
    ...
    const cat = svc.category as string || t.category as string || 'voyageur';
    const catColor = LVM_CAT_COLORS[cat] || (t.color as string) || '#22d3ee';

    // Annex 5 — planned (grey crossed-out) vs recalculated times.
    const isArret = (s: LivemapStop) => s && s.type === 'arret' && s.stationId;
    const buildTimes = (s: LivemapStop, isFirst: unknown, isLast: unknown, origIdx: number) => {
    ...
    const displayStops = stops.map((s: LivemapStop, i: number): LivemapDisplayStop => ({ s, origIdx: i })).filter(({ s }: LivemapDisplayStop) => isArret(s));
    ...
    let rows = displayStops.map(({ s, origIdx }: LivemapDisplayStop, idx: number) => {
      const isFirst = idx === 0;
    ...
    const bandeauStartIdx = displayStops.findIndex(({ origIdx }: LivemapDisplayStop) => origIdx >= curIdx);
    const bandeauStops = displayStops.slice(bandeauStartIdx >= 0 ? bandeauStartIdx : 0);
    const bandeau = bandeauStops.length ? `Prochains arrêts : ${bandeauStops.map(({ s }: LivemapDisplayStop) => world.getStationById(s.stationId)?.name).filter(Boolean).join('  •  ')}` : 'Service terminé';

    const stopName = (s: LivemapStop) => s?.stationId ? (world.getStationById(s.stationId)?.name || '—') : (s ? 'Waypoint' : '—');
    const nextArretFrom = (fromIndex: number) => {
    ...
    const nextArrLabel = nextArrTime != null
      ? (d !== 0
          ...
          : ` · Arr. ${fmt(nextArrTime)}`)
      : '';
    const findArretStop = (start: number, dir: number) => {
      for (let i = start; dir > 0 ? i < stops.length : i >= 0; i += dir) if (isArret(stops[i])) return stops[i];
      return null;
    };
    const arretStationName = (s: LivemapStop) => s?.stationId ? (world.getStationById(s.stationId)?.name || '—') : '—';
    ...
    const arretLabel = (st: LivemapStop) => st?.stationId
  ...
  _updateBandeauMarquee() {
    ...
    const step = (now: number) => {
      if (!track.parentElement || document.getElementById('livemap-train-panel')?.classList.contains('hidden')) { this._lvpMarqueeRaf = null; this._lvpMarqueeText = null; return; }
      if (track.scrollWidth <= wrapper.clientWidth) return;
  ...
  _livemapDelayReasonLines(svc: ActiveServiceLike, delayMin: unknown) {
    const d = Number(delayMin || 0);
    if (d < 5 || !svc?.train) return [];
    const t = svc.train;
    const lines: string[] = [];
    const history = Array.isArray(t.incidentDelayReasons) ? t.incidentDelayReasons : [];
    for (const r of history) {
      if (!r?.contributed || !String(r.text || '').trim()) continue;
  ...
  _livemapDelayReasonsHtml(svc: ActiveServiceLike, delayMin: unknown) {
    const lines = this._livemapDelayReasonLines(svc, delayMin);
    if (!lines.length) return '';
    const esc = (v: unknown) => String(v ?? '').replace(/[&<>"']/g, (c) => (HTML_ESCAPE_MAP[c]));
    ...
    return `<div class="lvp-delay-reasons" id="lvp-delay-reasons">
      ...
      ${lines.map((line: unknown) => `<div class="lvp-delay-reason-item">${esc(line)}</div>`).join('')}
    </div>`;
  }
  ...
  setupTabs() {
    document.querySelectorAll('.tab-btn').forEach((btn: HTMLButtonElement) => {
      btn.addEventListener('click', () => {
        const tab = btn.dataset.tab;
        btn.closest('.tab-bar')?.querySelectorAll('.tab-btn').forEach((b: __S3Struct485) => b.classList.remove('active'));
        btn.classList.add('active');
        btn.closest('aside')?.querySelectorAll('.tab-content').forEach((t: __S3Struct486) => t.classList.remove('active'));
  ...
  openStationCreationModal(lat: number, lon: number) {
    this.stationCreationMode = false;
    const btn = document.getElementById('btn-create-station');
    ...
    if (lineSelect) {
      lineSelect.innerHTML = '<option value="">Aucune</option>' +
        this.game.lineManager.getAll().map((l: { id: unknown; name: unknown; code: unknown }) =>
          `<option value="${htmlText(l.id)}">${htmlText(l.name)}${htmlText(l.code ? ' (' + l.code + ')' : '')}</option>`
        ).join('');
    }
    ...
    if (connectSelect) {
      const existing = this.game.world.stations.map((s: __S3Struct487) => {
        const d = Math.sqrt(
          Math.pow((s.lat - lat) * 111, 2) +
          Math.pow((s.lon - lon) * 111 * Math.cos(lat * Math.PI / 180), 2)
        );
        return { ...s, dist: d };
      }).sort((a: __S3Struct488, b: __S3Struct489) => a.dist - b.dist);
      ...
      const renderOptions = (filter: string = '') => {
        ...
        const filtered = existing.filter((s: __S3Struct491) => {
          const name = s.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
          return name.includes(term);
        });
        connectSelect.innerHTML = '<option value="">Aucune connexion</option>' +
          filtered.map((s: __S3Struct492) =>
            `<option value="${htmlText(s.id)}">${htmlText(s.name)} (${Math.round(s.dist)} km)</option>`
          ).join('');
      };
      renderOptions();
      connectSelect.value = '';
    ...
    if (terminusLineSelect) {
      const openLines = this.game.lineManager.getAll().filter((l: __S3Struct493) => l.stops.length > 0);
      terminusLineSelect.innerHTML = '<option value="_new">Creer une nouvelle ligne</option>' +
        openLines.map((l: __S3Struct494) => `<option value="${htmlText(l.id)}">Terminer : ${htmlText(l.name)}${htmlText(l.code ? ' (' + l.code + ')' : '')} (${l.stops.length} gares)</option>`).join('');
  ...
  async saveStation() {
    ...
    const platformNames = platformNamesRaw ? platformNamesRaw.split(',').map((s: __S3Struct495) => s.trim()).filter((s: unknown) => s) : [];
    ...
    if (connectChoice === '') {
      ...
      const candidates = typeof this.game.world.getStationsNear === 'function'
        ? this.game.world.getStationsNear(lat, lon, 50).filter((s: { id: unknown }) => s.id !== station.id)
        : this.game.world.stations.filter((s: { id: unknown }) => s.id !== station.id);
      let nearestDist = Infinity;
      for (const s of candidates) {
        const d = haversineDistance(s.lat, s.lon, lat, lon);
        if (d < nearestDist) { nearestDist = d; connectTo = s; }
      }
      if (nearestDist >= 50) connectTo = null;
    ...
    let pendingOrmTrack: unknown = null;
    ...
    if (connectTo) {
      ...
      const schedule = () => { refine().catch((e: unknown) => console.warn('Deferred station route failed:', e)); };
  ...
  handlePickConnection(station: __S3Struct496) {
  ...
  openEditStationModal(station: Station) {
    ...
    document.getElementById('station-platforms').value = (station.platforms || 4) as unknown as string;
    ...
    if (lineSelect) {
      lineSelect.innerHTML = '<option value="">Aucune</option>' +
        this.game.lineManager.getAll().map((l: __S3Struct498) =>
          `<option value="${htmlText(l.id)}" ${(station.lineIds || []).includes(l.id) ? 'selected' : ''}>${htmlText(l.name)}${htmlText(l.code ? ' (' + l.code + ')' : '')}</option>`
        ).join('');
    }
  ...
  deleteStation(stationId: unknown) {
  ...
  setupRollingStockPage() {
    ...
    const catalogInput = document.getElementById('stock-catalog-file') as HTMLInputElement | null;
    ...
    document.getElementById('btn-clear-stock-catalog')?.addEventListener('click', async () => {
      if (!confirm('Retirer le catalogue externe mémorisé ? Les fiches natives de Rail Empire seront restaurées au rechargement.')) return;
      try {
    ...
    drop?.addEventListener('drop', (e) => {
      ...
      const file = e.dataTransfer?.files[0]; if (file) this.loadStockImage(file);
    });
  ...
  _populateStockSubcatFilter() {
    ...
    const distinct = new Set(this.game.rollingStock.getAll().filter((i: { category: unknown; wagonSubCategory: unknown }) => i.category === 'wagon' && i.wagonSubCategory).map((i: { wagonSubCategory: unknown }) => i.wagonSubCategory));
    let html = '<option value="">Tous les wagons</option>';
    for (const [val, label] of Object.entries(labels)) {
      if (distinct.has(val)) html += `<option value="${htmlText(val)}">${htmlText(label)}</option>`;
    }
    sel.innerHTML = html;
    if (Array.from(sel.options).some((o: unknown) => (o as HTMLOptionElement).value === current)) sel.value = current;
  }

  _populateMaterialAdvancedFilters(prefix: string, force: unknown = false) {
    ...
    if (!force && this._materialAdvancedFilterSig[prefix] === sig) return;
    ...
    for (const item of all) {
      ...
      for (const code of meta.details) detailCounts.set(code, (detailCounts.get(code) || 0) + 1);
      for (const code of meta.tractions) tractionCounts.set(code, (tractionCounts.get(code) || 0) + 1);
      if (meta.country) countryCounts.set(meta.country, (countryCounts.get(meta.country) || 0) + 1);
      if (meta.operator) operatorCounts.set(meta.operator, (operatorCounts.get(meta.operator) || 0) + 1);
      if (meta.mlg) mlgCounts.set(meta.mlg, (mlgCounts.get(meta.mlg) || 0) + 1);
    }
    ...
    if (detailSel) {
      ...
      if (Array.from(detailSel.querySelectorAll('option')).some((o) => o.value === current && !o.disabled)) detailSel.value = current;
    }
    ...
    if (tractionSel) {
      ...
      if (Array.from(tractionSel.options).some((o: unknown) => (o as HTMLOptionElement).value === current)) tractionSel.value = current;
    }
    const fillDynamic = (idSuffix: string, firstLabel: string, counts: Map<string, number>) => {
      ...
      for (const [value, count] of values) sel.appendChild(new Option(`${value} (${count.toLocaleString('fr-FR')})`, value));
      if (Array.from(sel.options).some((o: unknown) => (o as HTMLOptionElement).value === current)) sel.value = current;
    };
  ...
  _filterMaterialAdvanced(items: RollingStockItem[], prefix: unknown): RollingStockItem[] {
    ...
    return items.filter((item) => {
      ...
      if (subcat && item.wagonSubCategory !== subcat) return false;
      const meta = _stockFilterMeta(item);
      if (detail && !meta.details.has(detail)) return false;
      if (traction && !meta.tractions.has(traction)) return false;
      if (country && meta.country !== country) return false;
      if (operator && meta.operator !== operator) return false;
      if (mlg && meta.mlg !== mlg) return false;
      return true;
    });
  }
  ...
  _setStockField(id: string, val: unknown) {
    const el = document.getElementById(id);
    if (el) el.value = val as string;
  }

  _setStockTraction(values: string[]) {
    const checkboxes = document.querySelectorAll('.stock-traction-cb');
    const set = new Set(values.map((v: __S3Struct505) => v.toLowerCase()));
  ...
  _setStockWagonSubcat(val: unknown) {
  ...
  editStock(id: unknown) {
    ...
    this._setStockTraction((item.traction || 'none').split('+').map((s: __S3Struct506) => s.trim()).filter(Boolean));
  ...
  _populateCargoTypesCheckboxes() {
    ...
    let html = '';
    for (const [catKey, cat] of (Object.entries(cargoTypes.categories as any) as any)) {
  ...
  saveStock() {
    ...
    if (!name) return alert('Nom requis');
    const category = document.getElementById('stock-category')?.value || 'locomotive';
    if (!['locomotive','automotrice','voiture','wagon'].includes(category)) return alert('Catégorie de matériel invalide.');
    const read = (id: string) => Number(document.getElementById(id)?.value);
    ...
    if (!Number.isFinite(maxSpeed) || maxSpeed <= 0) return alert('Vitesse maximale invalide.');
    if (!Number.isFinite(mass) || mass <= 0) return alert('Masse à vide invalide.');
    if (!Number.isFinite(length) || length <= 0) return alert('Longueur invalide.');
  ...
  renderStockList() {
    ...
    if (q) items = items.filter((i: RollingStockItem) => {
      const meta = _stockFilterMeta(i);
      const detailText = [...meta.details].map((c: string) => STOCK_DETAIL_LABELS.get(c) || '').join(' ').toLowerCase();
    ...
    container.innerHTML = view.map((item: RollingStockItem) => {
      const subLabel = item.wagonSubCategory ? ` — ${item.wagonSubCategory}` : '';
      const detailLabels = [..._stockFilterMeta(item).details].map((c: string) => STOCK_DETAIL_LABELS.get(c)).filter(Boolean).slice(0, 4);
      const powerTxt = item.power ? ` · ${item.power} kW` : '';
      const cargoTxt = item.cargoTypes?.length ? ` · ${item.cargoTypes.map((ct: unknown) => { const info = this.game.cargoTypes?.getTypeInfo?.(ct); return info?.name || ct; }).join(', ')}` : '';
      const compatCargoTxt = item.technicallyCompatibleCargoTypes?.length ? item.technicallyCompatibleCargoTypes.map((ct: unknown) => { const info = this.game.cargoTypes?.getTypeInfo?.(ct); return info?.name || ct; }).join(', ') : '';
      return `
  ...
  stockPageGo(p: unknown) {
    this._stockPage = p;
    this.renderStockList();
    document.getElementById('stock-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  deleteStock(id: unknown) {
    const item = this.game.rollingStock.getById(id);
    if (!item) return;
    if (item._catalog) return alert('Cette fiche appartient au catalogue de base. Elle peut être modifiée, mais pas supprimée depuis la page Matériel.');
    const usedBy = this.game.rameManager.getAll().filter((r: Rame) => (r.elements || []).includes(String(item.id)));
    if (usedBy.length) return alert(`Impossible de supprimer cet engin : il est utilisé dans ${usedBy.length} rame(s), dont ${usedBy.slice(0,3).map((r: { name: unknown }) =>r.name).join(', ')}.`);
  ...
  resetLiveryEditor() {
    if(!this._liveryEditor)return;
    try{this._liveryEditor.dispose();this._liveryEditor=null;
      const root=document.getElementById('page-liveries');if(root)root.innerHTML='';
      if(this.activePage==='liveries')this.renderLiveriesPage();
    }catch(error){console.warn('Livrées : rafraîchissement après import',error);}
  }
  renderLiveriesPage() {
    const root=document.getElementById('page-liveries');if(!root)return;
    if(!this._liveryEditor)this._liveryEditor=new LiveryEditor(root,{
      library:this.game.liveries,
      catalog:()=>this.game.rollingStock.getAll(),
      targets:()=>this.game.liveryTargets(),
      commit:(change:()=>void)=>this.game.commitLiveryChange(change),
      ensureCatalog:()=>this.game._loadBatch186FullCatalogInBackground?.(),
    });
    else this._liveryEditor.show();
  }
  selectRameLivery(index: number, id: string) {
    const element=this.currentRameElements[index] as LiveryTarget | undefined;if(!element)return;
    try{const catalogId=element.catalogId||element.stockId||'';this.game.liveries.select(element,id,this.game.rollingStock.getById(catalogId)?.imageData);this.renderRameAssembly();}
  ...
  _rameLiverySelect(element: LiveryTarget, index: number): string {
    ...
    if(!rows.length&&!element.liveryId)return '';
    return `<label class="liv-rame-select">Livrée<select data-rame-livery="${index}" aria-label="Livrée du véhicule ${index+1}"><option value="">Image d’origine</option>${rows.map((r: LiveryDefinition)=>`<option value="${htmlText(r.id)}" ${r.id===element.liveryId?'selected':''}>${htmlText(r.label)}</option>`).join('')}</select></label>`;
  }
  ...
  openRameModal() {
    ...
    const title=document.getElementById('rame-modal-title');if(title)title.textContent='Composer une rame';
    const saveBtn=document.getElementById('btn-save-rame');if(saveBtn)saveBtn.textContent='Enregistrer la rame';
    ...
    for (const id of ['rame-detail-filter','rame-traction-filter','rame-country-filter','rame-operator-filter','rame-mlg-filter']) { const el = document.getElementById(id); if (el) el.value = ''; }
    ...
    if (depotSel) {
      ...
      depotSel.innerHTML = '<option value="">— Aucun —</option>' + depots.map((d: { id: unknown; name: unknown }) => `<option value="${htmlText(d.id)}">${htmlText(d.name)}</option>`).join('');
    }
  ...
  openRameEditor(id: unknown) {
    ...
    this._rameRandomWagonSelection = new Set();
    this._rameMultiWagonMode = false;
    this.currentRameElements=(rame.elementDetails||[]).map((d: { catalogId: unknown; elementId: unknown },i: number)=>{
    ...
    this._editingOriginalElementIds=new Set(this.currentRameElements.map((e: { elementId: unknown }) =>e.elementId).filter(Boolean));
    this._editingOriginalComposition={elements:[...(rame.elements||[])],elementDetails:(rame.elementDetails||[]).map((e: Rame['elementDetails'][number]) =>({...e}))};
    document.getElementById('rame-name').value=rame.name||'';
    const ser=document.getElementById('rame-serial');if(ser)ser.value=rame.serialNumber||'';
    const s=document.getElementById('rame-search');if(s)s.value='';
    const c=document.getElementById('rame-cat-filter');if(c)c.value='';
    const sc=document.getElementById('rame-subcat-filter');if(sc)sc.value='';
    for (const id of ['rame-detail-filter','rame-traction-filter','rame-country-filter','rame-operator-filter','rame-mlg-filter']){const el=document.getElementById(id);if(el)el.value='';}
    ...
    this._populateRameSubcatFilter();
    const depotSel=document.getElementById('rame-depot');
    if(depotSel){const depots=this.game.depotManager.getDepots();depotSel.innerHTML='<option value="">— Aucun —</option>'+depots.map((d: { id: unknown; name: unknown }) =>`<option value="${htmlText(d.id)}">${htmlText(d.name)}</option>`).join('');depotSel.value=rame.depotId||'';}
    const title=document.getElementById('rame-modal-title');if(title)title.textContent=`Éditer la rame — ${rame.name}`;
    const saveBtn=document.getElementById('btn-save-rame');if(saveBtn)saveBtn.textContent='Enregistrer les modifications';
    document.getElementById('rame-edit-hint')?.classList.remove('hidden');
    document.getElementById('modal-rame')?.classList.remove('hidden');
    this.renderRamePicker();this.renderRameAssembly();
  }

  duplicateRame(id: unknown) {
    const rame=this.game.rameManager.getById(id);
    if(!rame)return;
    ...
    const candidate=new LabelAllocator(allRames.map((r: {name: string})=>r.name),true).next(rame.name);
    const serial=rame.serialNumber ? new LabelAllocator(allRames.map((r: {serialNumber: string})=>r.serialNumber)).next(rame.serialNumber) : '';
    this.currentRameElements=(rame.elementDetails||[]).map((d: { catalogId: unknown },i: number)=>{
      const stockId=rame.elements?.[i]||d.catalogId||'';
      const stock=this.game.rollingStock.getById(stockId);
      return {...(stock||{}),...d,stockId,elementId:this._newRameElementId()};
    });
    document.getElementById('rame-name').value=candidate;
    const ser=document.getElementById('rame-serial');if(ser)ser.value=serial;
    const depot=document.getElementById('rame-depot');if(depot)depot.value=rame.depotId||'';
    const title=document.getElementById('rame-modal-title');if(title)title.textContent=`Dupliquer la rame — ${rame.name}`;
    const saveBtn=document.getElementById('btn-save-rame');if(saveBtn)saveBtn.textContent='Créer la rame dupliquée';
    this.renderRameAssembly();
  }
  ...
  renderRamePicker() {
    ...
    if (query) items = items.filter((i: RollingStockItem) => {
      const meta = _stockFilterMeta(i);
      const detailText = [...meta.details].map((c: string) => STOCK_DETAIL_LABELS.get(c) || '').join(' ').toLowerCase();
  ...
  _updateRameRandomControls() {
    ...
    const hasLoco = this.currentRameElements.some((e: { category: unknown }) => e.category === 'locomotive');
    ...
    if (countEl) countEl.textContent = `${count} modèle${count > 1 ? 's' : ''} sélectionné${count > 1 ? 's' : ''}`;
  }
  ...
  onRamePickerItemClick(stockId: unknown, ev: RameClickArg) {
    const item = this.game.rollingStock.getById(stockId);
    if (!item) return;
    if (this._rameMultiWagonMode && item.category === 'wagon') {
      const id = String(item.id);
      if (this._rameRandomWagonSelection.has(id)) this._rameRandomWagonSelection.delete(id);
      else this._rameRandomWagonSelection.add(id);
      this._updateRameRandomControls();
      this.renderRamePicker();
      return;
    }
  ...
  _shuffleRameRandomPool(items: RollingStockItem[]) {
    const out = [...items];
  ...
  _makeRameElementFromStock(item: { seriesName: unknown; name: unknown; id: unknown }, flipped: unknown = false) {
  ...
  _setRameRandomPolicyControls(enabled: boolean, maximum: number | null) {
  ...
  addToRame(stockId: unknown, evOrFlipped: RameClickArg) {
    ...
    let currentLength = this.currentRameElements.reduce((s: number, e: { length: number }) => s + e.length, 0);
    let added = 0;
  ...
  moveRameElement(index: unknown, delta: unknown) {
    const from=Number(index),to=from+Number(delta);
    if(!Number.isInteger(from)||!Number.isInteger(to)||from<0||to<0||from>=this.currentRameElements.length||to>=this.currentRameElements.length)return;
    const [el]=this.currentRameElements.splice(from,1);
    this.currentRameElements.splice(to,0,el);
    this.renderRameAssembly();
  }

  onRameElementClick(index: unknown, ev: { ctrlKey: unknown }) {
    if (ev?.ctrlKey) {
      this.flipRameElement(index as number);
    } else {
      this.removeFromRame(index);
    }
  }

  flipRameElement(index: number) {
  ...
  renderRameAssembly() {
    ...
    if (this.currentRameElements.length === 0) {
      ...
      container.innerHTML = '<div class="rame-assembly-images">' + this.currentRameElements.map((el: HTMLElement, i: unknown) => {
        ...
        return `<div class="rame-element-wrap">${imgHtml}${this._rameLiverySelect(el as unknown as LiveryTarget, Number(i))}<span class="rame-element-label">${htmlText(label)}</span><div class="rame-element-move"><button type="button" class="rame-move-btn" ${i===0?'disabled':''} onclick="game.ui.moveRameElement(${htmlJsValue(i)},-1)" title="Déplacer vers la gauche">←</button><button type="button" class="rame-move-btn" ${i===last?'disabled':''} onclick="game.ui.moveRameElement(${htmlJsValue(i)},1)" title="Déplacer vers la droite">→</button></div></div>`;
      }).join('') + '</div>';
    }

    (container.querySelectorAll('[data-rame-livery]') as NodeListOf<HTMLSelectElement>).forEach(select=>select.addEventListener('change',()=>this.selectRameLivery(Number(select.dataset.rameLivery),select.value)));

    const totalLen = this.currentRameElements.reduce((s: number, e: { length: number }) => s + e.length, 0);
    const totalTon = this.currentRameElements.reduce((s: number, e: { tonnage: number }) => s + e.tonnage, 0);
    const totalCap = this.currentRameElements.reduce((s: number, e: { passengerCapacity: number }) => s + e.passengerCapacity, 0);
    const totalPrice = this.currentRameElements.reduce((s: number, e: { purchasePrice?: number }) => s + (e.purchasePrice || 0), 0);
    const vmax = this.currentRameElements.length > 0 ? Math.min(...this.currentRameElements.map((e: { maxSpeed: unknown }) => e.maxSpeed)) : 0;
    const tractors = this.currentRameElements.filter((e: { category: unknown }) => e.category === 'locomotive' || e.category === 'automotrice');
    const traction = tractors.length > 0 ? [...new Set(tractors.map((t: { traction: unknown }) => t.traction))].join('+') : '-';
  ...
  saveRame() {
    ...
    const totalLength = this.currentRameElements.reduce((sum: number,e: { length: unknown })=>sum+(Number(e.length)||0),0);
    if (!Number.isFinite(totalLength) || totalLength <= 0 || totalLength > 750) return alert('Composition invalide : longueur totale hors limite (0–750 m).');
    if (this.currentRameElements.some((e: { maxSpeed: unknown; mass: unknown; tonnage: unknown }) => !Number.isFinite(Number(e.maxSpeed)) || Number(e.maxSpeed) <= 0 || !Number.isFinite(Number(e.mass ?? e.tonnage)) || Number(e.mass ?? e.tonnage) <= 0)) {
      return alert('Composition invalide : au moins un véhicule possède une vitesse ou une masse non valide.');
    }

    let randomMaxVehicles: number | null;
    ...
    if(isEdit&&!existing)return alert('Rame introuvable.');
    const oldIds=this._editingOriginalElementIds||new Set();
    const structuralChanged=isEdit&&JSON.stringify(this.currentRameElements.map((e: { elementId: unknown }) =>e.elementId))!==JSON.stringify((this._editingOriginalComposition?.elementDetails||[]).map((e: { elementId: unknown }) =>e.elementId));
    ...
    if(enteredSerial&&enteredSerial!==existing?.serialNumber&&this.game.rameManager.getAll().some((r: Rame)=>r.id!==existing?.id&&r.serialNumber===enteredSerial))return alert('Ce numéro de rame est déjà utilisé. Choisissez un numéro libre.');
    const billedElements=isEdit?this.currentRameElements.filter((e: { elementId: unknown }) =>!oldIds.has(e.elementId)):this.currentRameElements;
    const billedPrice=billedElements.reduce((sum: number,e: { purchasePrice?: number })=>sum+(e.purchasePrice||0),0);
    if(billedPrice>0){
      if(this.game.economy.balance<billedPrice)return alert(`Solde insuffisant ! Coût des nouveaux engins: ${billedPrice.toLocaleString('fr-FR')} € — Solde: ${Math.round(this.game.economy.balance).toLocaleString('fr-FR')} €`);
    ...
    const data={
      ...
      elements:this.currentRameElements.map((e: { stockId: unknown }) =>e.stockId),
      elementDetails:this.currentRameElements.map((e: { liveryId?: string; originalImageData?: string; seriesName: unknown; elementId: unknown; name: unknown; instanceName: unknown; instanceNumber: unknown; category: unknown; traction: unknown; maxSpeed: unknown; tonnage: unknown; mass: unknown; power: unknown; passengerCapacity: unknown; freightCapacity: unknown; length: unknown; imageData: unknown; purchasePrice: unknown; wagonSubCategory: unknown; electricSystems: unknown; gauges: unknown; gauge: unknown; isDrivingTrailer: unknown; flipped: unknown }) =>{
    ...
    if(isEdit){
      const previous=this._editingOriginalComposition||{elements:[...(existing.elements||[])],elementDetails:(existing.elementDetails||[]).map((e: Rame['elementDetails'][number]) =>({...e}))};
      ...
      if(sync?.rotationsBlocked){alert(`${sync.rotationsBlocked} roulement(s) lié(s) ont été bloqués car la nouvelle composition contient du matériel qui n’a pas encore été créé dans « Rame → matériel ». Créez ces engins/coupons puis réaffectez ou recalculez le roulement.`);}
    }else{
    ...
    this.editingRameId=null;this._duplicatingRameSourceId=null;this._editingOriginalElementIds=new Set();this._editingOriginalComposition=null;
  ...
  renderRamesList() {
    ...
    let items = all.filter((r: Rame) => {
      if (kind && !_rameKindCodes(r).has(kind)) return false;
      if (tractionFilter) {
        const codes = new Set((r.elementDetails || []).flatMap((e: unknown) => [..._stockFilterMeta(e as Partial<RollingStockItem>).tractions]));
        if (!codes.has(tractionFilter)) return false;
      }
      if (statusFilter) {
    ...
    if (q) {
      items = items.filter((r: Rame) =>
        (r.name || '').toLowerCase().includes(q) ||
        (r.serialNumber || '').toLowerCase().includes(q) ||
        (r.elementDetails || []).some((e: __S3Struct522) => (e.instanceName || e.name || '').toLowerCase().includes(q))
      );
    }
    ...
    container.innerHTML = view.map((r: Rame) => `
      <div class="rame-card">
        ...
        <div class="card-info" style="font-size:10px;color:var(--text3)">
          <b>Mise en service:</b> ${htmlText(r.createdDate)} | <b>Km parcourus:</b> ${Math.round(r.totalKmRun || 0).toLocaleString('fr-FR')} km${r.elementDetails.some((e: { purchasePrice: unknown }) => e.purchasePrice) ? ` | <b>Valeur:</b> ${htmlText(r.elementDetails.reduce((s: number,e: { purchasePrice: number }) => s + (e.purchasePrice || 0), 0).toLocaleString('fr-FR'))} €` : ''}
          ${r.depotId ? `| <b>Port d'attache:</b> ${htmlText((this.game.depotManager.getAll().find((d: { id: unknown }) => d.id === r.depotId)?.name || r.depotId))}` : ''}
  ...
  ramesPageGo(p: unknown) {
    this._ramesPage = p;
    this.renderRamesList();
    document.getElementById('rames-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  _rameLocationLabel(r: Rame) {
    const loc = r.currentLocation || {};
    if (loc.serviceId) {
      const svc = this.game.scheduleCreator.services.find((s: { "id": unknown }) => s.id === loc.serviceId);
  ...
  deleteRame(id: unknown) {
    const rame = this.game.rameManager.getById(id);
    if (!rame) return;
    if (rame.currentLocation?.serviceId) return alert('Impossible de supprimer une rame actuellement en service.');
    if (rame.inMaintenance || this.game.depotManager?.isRameInMaintenance?.(rame.id)) return alert('Impossible de supprimer une rame actuellement en maintenance.');
    const rotationLinks = (this.game.rotationV2?.rotations || []).filter((r: { assignedRameId: unknown }) => String(r.assignedRameId || '') === String(rame.id));
    if (rotationLinks.length) return alert(`Impossible de supprimer cette rame : elle est encore affectée à ${rotationLinks.length} ligne(s) de roulement.`);
  ...
  _ensureScheduleV2Editor() {
    if (!this.scheduleV2Editor) this.scheduleV2Editor = new ScheduleV2Editor(this.game, this);
    return this.scheduleV2Editor;
  }

  _ensureWorksV2Editor() {
    if (!this.worksV2Editor) this.worksV2Editor = new WorksV2Editor(this.game, this);
    return this.worksV2Editor;
  }

  _ensureInfrastructureV2Editor() {
    if (!this.infrastructureV2Editor) this.infrastructureV2Editor = new InfrastructureV2Editor(this.game, this);
    return this.infrastructureV2Editor;
  }

  _ensureDepotITEPointEditor() {
    if (!this.depotITEPointEditor) this.depotITEPointEditor = new DepotITEPointEditor(this.game, this);
  ...
  openScheduleModal(editService: unknown[]) {
    this._editingScheduleId = editService?.id || null;
    if (editService) {
      this._forwardStops = editService.stops.map((s: unknown) => this._stopDataToEditObj(s as ScheduleStopData));
      this._forwardManualRoutes = (editService.routes || []).map((r: unknown[] | null) => (r && r.length >= 2 ? [...r] : null));
      this._returnStops = (editService._returnStopsData || []).map((s: unknown) => this._stopDataToEditObj(s as ScheduleStopData));
      this._returnManualRoutes = (editService._returnRoutes || []).map((r: unknown[] | null) => (r && r.length >= 2 ? [...r] : null));
    ...
    rameSelect.innerHTML = rames.map((r: { id: unknown; name: unknown; maxSpeed: unknown }) => `<option value="${htmlText(r.id)}">${htmlText(r.name)} (${r.maxSpeed} km/h)</option>`).join('');
  ...
  _renderContractPicker(selectedId: unknown = '') {
    ...
    const hasFreight = rame && (rame.totalFreightCapacity > 0 || rame.elementDetails?.some((e: { cargoTypes: unknown }) => Array.isArray(e.cargoTypes) && e.cargoTypes.length > 0));
    row.style.display = (type === 'fret' || (type === 'passager' && hasFreight)) ? 'flex' : 'none';

    const contracts = this.game.freightManager?.getAllActive() || [];
    const opts = contracts.map((c: __S3Struct531) => `<option value="${htmlText(c.id)}" ${c.id === selectedId ? 'selected' : ''}>${htmlText(c.from)} → ${htmlText(c.to)} : ${htmlText(c.cargoName)} (${c.quantity}${htmlText(c.unit)}) — ${c.payment.toLocaleString()} €</option>`).join('');
  ...
  _calcAutoAR() {
    ...
    document.getElementById('sched-multi-departures').value = maxAR as unknown as string;
  ...
  setupSchedMap() {
    ...
    const drawMap = () => {
      ...
      const selectedIds = new Set(this.schedStops.map((s: { stationId: unknown }) => s.stationId));
      ...
      if (this.schedStops.length > 1) {
        ...
        const drawArrow = (x: unknown, y: unknown, angle: unknown, size: number = 5, color: unknown = '#ff00ff') => {
          ctx.save(); ctx.translate(x, y); ctx.rotate(angle);
          ctx.fillStyle = color;
          ctx.beginPath(); ctx.moveTo(size, 0); ctx.lineTo(-size / 2, -size / 2); ctx.lineTo(-size / 2, size / 2); ctx.closePath(); ctx.fill();
          ctx.restore();
        };
        const drawGeom = (geom: Array<{lat:number;lon:number}>) => {
      ...
      if (vpm) {
        ...
        if (tileMap.zoomLevel >= 6) {
          const usedVPIds = new Set(this.schedStops.filter((s: { voiePointId: unknown }) => s.voiePointId).map((s: { voiePointId: unknown }) => s.voiePointId));
    ...
    let schedDrag = false, schedDragStart: { x: number; y: number } | null = null, totalDragDist = 0;
  ...
  _updateManualUI() {
    ...
    const hasTrace = this._manualRoutes && this._manualRoutes.some((r: __S3Struct532) => r && r.length >= 2);
  ...
  _stopNameFor(stationId: unknown, voiePointId: unknown) {
    if (stationId) {
  ...
  _removeTraceControl(leg: number, control: ManualRoutePoint) {
    if (!control) return;
    const route = this._manualRoutes[leg];
    if (!route) return;
    const controls = this._extractRouteControls(route as Array<{ control?: unknown; lat: number; lon: number }>);
  ...
  _addManualPoint(lat: number, lon: number) {
    if (this._manualStartCoords) {
  ...
  _finishManualLeg(endStop: ScheduleUiStop, maxSpeed: unknown = 30) {
    if (!this._manualMode || !this._manualStartCoords) return;
    const endCoords = this._getStopCoords(endStop) as { lat: number; lon: number; maxSpeed?: unknown } | null;
    if (!endCoords) return;
  ...
  _startManualRetrace(leg: number, routeIndex: number) {
    const route = this._manualRoutes[leg];
  ...
  _buildManualRoute(start: ManualRoutePoint, controls: ManualRoutePoint[], end: ManualRoutePoint, maxSpeed: unknown = 30) {
    const points = [{ ...start, maxSpeed, control: true }, ...controls.map((p: ManualRoutePoint) => ({ ...p, maxSpeed, control: true })), { ...end, maxSpeed, control: true }];
    return this._densifyRoute(points, 0.05);
  }

  // --- Sillon picker (Section V integration) ---
  openSillonPicker(sillons: Array<{ name: string; fromStationName: string; toStationName: string; distance: number; maxSpeed: number; electrified?: boolean }>, fromName: unknown, toName: unknown) {
  ...
  _resolveSillonPicker(index: unknown) {
    const modal = document.getElementById('modal-sillon-picker');
  ...
  async _pickSillonForLeg(prevStop: { stationId: unknown; stationName: unknown }, newStop: { stationId: unknown; stationName: unknown }, legIdx: number) {
    ...
    const path = (paths as any)[choice as any];
    if (!path || !path.route?.length) return null;

    // Densify to 50 m points like manual trace.
    const route = this._densifyRoute(path.route.map((p: { lat: unknown; lon: unknown; maxSpeed: unknown }) => ({ lat: p.lat, lon: p.lon, maxSpeed: p.maxSpeed || path.maxSpeed })));
  ...
  async addSchedStop(station: { closed: unknown; id: unknown; name: unknown }) {
    if (station.closed) {
  ...
  async addSchedVoiePointStop(voiePoint: { voie: unknown; stationId: unknown; id: unknown }) {
  ...
  async _addMapWaypoint(lat: number, lon: number) {
  ...
  minToTimeStr(m: unknown) {
    const h = Math.floor((m as number) / 60) % 24;
    const min = (m as number) % 60;
    return `${String(h).padStart(2, '0')}:${String(min).padStart(2, '0')}`;
  }

  timeStrToMin(s: unknown) {
    const [h, m] = (s as string).split(':').map(Number);
    return h * 60 + (m || 0);
  }

  incrementTime(timeStr: unknown, minutes: unknown) {
    return this.minToTimeStr(this.timeStrToMin(timeStr) + (minutes as number));
  }
  ...
  renderSchedStops() {
    ...
    container.innerHTML = header + this.schedStops.map((stop: __S3Struct537, i: number) => {
      const isFirst = i === 0;
    ...
    if (rtChecked && this.schedStops.length >= 2 && !this._isReturnEditMode) {
      ...
      const fmt = (t: unknown) => this.minToTimeStr(((Math.round(t as number) % 1440) + 1440) % 1440);
      const retTimes: Array<{ arr: number; dep: number }> = [];
      for (let j = 0; j < n; j++) {
  ...
  _renderRouteSummary() {
    ...
    const fmt = (t: unknown) => this.minToTimeStr(((Math.round(t as number) % 1440) + 1440) % 1440);
    const row = (name: unknown, time: unknown, cls: unknown = '') => `<div class="ss-row ${htmlText(cls)}"><span class="ss-name">${htmlText(name)}</span><span class="ss-time">${time}</span></div>`;

    const allerRows = this.schedStops.map((stop: { type: unknown; arrTimeMin: unknown; depTimeMin: unknown; stationName: unknown }, i: unknown) => {
      const isFirst = i === 0;
      const isLast = i === this.schedStops.length - 1;
      let t = '';
  ...
  async updateSchedStop(index: number, field: unknown, value: string) {
  ...
  updateReturnPlatform(stationId: string, value: unknown) {
    if (!this._schedReturnPlatforms) this._schedReturnPlatforms = {};
    if (value) {
  ...
  _approxRailDistance(lat1: number, lon1: number, lat2: number, lon2: number) {
  ...
  _densifyRoute(route: ManualRoutePoint[], spacingKm: number = 0.05) {
  ...
  _resampleRoute(route: unknown, spacingKm: unknown = 0.05) {
    return this._densifyRoute(route as ManualRoutePoint[], spacingKm as number);
  }

  _findNearestTracePoint(x: number, y: number, tileMap: { worldToScreen: (lat: unknown, lon: unknown, width: number, height: number) => { x: number; y: number } }, canvas: HTMLCanvasElement) {
    if (!this._manualRoutes || this._manualRoutes.length === 0) return null;
    let best: { leg: number; index: number; pt: unknown } | null = null, bestDist = Infinity;
    for (let leg = 0; leg < this._manualRoutes.length; leg++) {
      const route = this._manualRoutes[leg];
      if (!route) continue;
      for (let i = 0; i < route.length; i++) {
  ...
  _extractRouteControls<T extends { control?: unknown; lat: number; lon: number }>(route: T[]): T[] {
  ...
  _insertTracePoint(leg: number, index: number, lat: number, lon: number) {
  ...
  _removeTracePoint(leg: number, index: number) {
    const route = this._manualRoutes[leg];
  ...
  _moveTracePoint(leg: number, control: ManualRoutePoint, lat: number, lon: number) {
    if (!control) return;
    const snapped = this._snapToTrack(lat, lon);
  ...
  _moveManualControlPoint(index: number, lat: number, lon: number) {
  ...
  async _recalcAfterTraceEdit(leg: number) {
  ...
  _adjustManualRoutesForInsert(stopIndex: number) {
    if (!this._manualRoutes) this._manualRoutes = [];
  ...
  _adjustManualRoutesForRemove(stopIndex: number) {
  ...
  async _resolveRouteForLeg(stopA: ScheduleUiStop, stopB: ScheduleUiStop) {
    const ca = this._getStopCoords(stopA), cb = this._getStopCoords(stopB);
    ...
    if (sa && sb) {
      ...
      if (track && track.route && track.route.length > 1) {
        ...
        if (track.stationA === sa.id && track.stationB === sb.id) return track.route;
      }
    }
    ...
    try {
      ...
      }).filter((p): p is { latA: number; lonA: number; latB: number; lonB: number } => p !== null);
      const opts: { maxSpeed: number; avoidStationPairs?: Array<{ latA: number; lonA: number; latB: number; lonB: number }> } = { maxSpeed: routingSpeed };
  ...
  async _getSegmentTravelTime(prevStop: ScheduleUiStop, curStop: ScheduleUiStop, rameSpeed: number, rame: Rame | null = null, legIndex: number | null = null) {
    let route = null;
    ...
    const isPass = (s: unknown) => (s as { type?: unknown } | null | undefined)?.type === 'waypoint' || (s as { type?: unknown } | null | undefined)?.type === 'passage';
  ...
  async recalcStopsFrom(fromIndex: number) {
  ...
  removeSchedStop(index: unknown) {
    this.schedStops.splice(index, 1);
    this._adjustManualRoutesForRemove(index as number);
    this.recalcStopsFrom(index as number);
    this.renderSchedStops();
    this._recalcPreviewRoutes();
  }

  startInsertStop(index: unknown) {
  ...
  async _insertStopFromMap(afterIndex: __KPM561, item: __S3Struct548) {
  ...
  async _buildSaveRoutes(stops: ScheduleUiStop[], manualRoutes: Array<ManualRoutePoint[] | null>) {
    const routePromises = [];
  ...
  async saveSchedule() {
    ...
    const stops = forwardStops.map((s: unknown) => this._stopEditToData(s as ScheduleStopEdit));
    const returnStopsData = returnStops.length >= 2 ? returnStops.map((s: unknown) => this._stopEditToData(s as ScheduleStopEdit)) : [];
    ...
    if (serviceType === 'hlp') {
      const locoCount = rame.elementDetails.filter((e: { category: unknown }) => e.category === 'locomotive' || e.category === 'automotrice').length;
    ...
    if (assignedContractId) {
      const contract = this.game.freightManager.contracts.find((c: FreightContract) => c.id === assignedContractId);
    ...
    const runDates = runDatesStr ? runDatesStr.split(',').map((d: string) => d.trim()).filter((d: string) => /^\d{4}-\d{2}-\d{2}$/.test(d)) : [];
    ...
    if (this._editingScheduleId) this.game.scheduleCreator.removeService(this._editingScheduleId);
  ...
  editSchedule(id: unknown) {
    const svc = this.game.scheduleCreator.services.find((s: { id: unknown }) => s.id === id);
    if (svc) this.openScheduleModal(svc);
  }

  renderSchedulesList() {
    if (this.scheduleV2Editor) return this.scheduleV2Editor.renderList();
    ...
    if (sortMode === 'departure') {
      ...
      sorted.sort((a: __S3Struct552, b: __S3Struct553) => (a.id || '').localeCompare(b.id || ''));
    } else if (sortMode === 'rame') {
      sorted.sort((a: { rameId: unknown }, b: { rameId: unknown }) => {
      ...
      sorted.sort((a: __S3Struct554, b: __S3Struct555) => a.name.localeCompare(b.name));
    ...
    const getGroupKey = (svc: ActiveServiceLike) => {
      if (sortMode === 'rame') {
    ...
    const dayNames: string[] = ['Di','Lu','Ma','Me','Je','Ve','Sa'];
    const typeLabels: Record<string, string> = { passager: 'Voy', w: 'W', hlp: 'HLP', tm: 'TM', evo: 'EVO', work: 'Travaux' };

    const rows = pageItems.map((svc: unknown[]) => {
      ...
      const daysLabel = rd.length === 7 ? 'TLJ' : rd.map((d: number) => dayNames[d]).join(' ');
      ...
      // Detail content
      const stopsPreview = svc.stops.map((s: { stationId: unknown; arrivalTime: unknown; departureTime: unknown; type: unknown }) => {
        ...
        if (s.type === 'waypoint') return `<span class="sched-stop-tag waypoint" style="opacity:0.5;font-style:italic">(via ${htmlText(name)})</span>`;
      ...
      const passagePreview = (svc._passageStops?.length)
        ? `<div class="sched-stops-preview" style="margin-top:4px"><span style="color:#22c55e;font-size:9px;margin-right:4px">Passages :</span>${svc._passageStops.map((p: { time: unknown; name: unknown }) => `<span class="sched-stop-tag passage">${this.minToTimeStr(p.time)} ${htmlText(p.name)}</span>`).join('<span style="color:var(--text3)"> → </span>')}</div>`
        : '';

      let returnPreview = '';
      if (svc.roundTrip) {
        const retStops = svc.buildReturnStops();
        const retStr = retStops.map((s: { stationId: unknown; arrivalTime: unknown; departureTime: unknown; type: unknown }) => {
          const st = this.game.world.getStationById(s.stationId);
          const name = st ? st.name : s.stationId;
          const arr = this.minToTimeStr(s.arrivalTime);
          const dep = this.minToTimeStr(s.departureTime);
      ...
      if (delayReason) bilanRows.push(`<span style="color:#f59e0b">Retard : ${htmlText(delayReason)}</span>`);
      if (breakdown?.type) bilanRows.push(`<span style="color:#ef4444">Panne : ${breakdown.type}</span>`);
      if (incident?.name || incident?.effect) bilanRows.push(`<span style="color:#ef4444">Incident : ${htmlText(incident.name || incident.effect)}</span>`);
      ...
      // Group header row
      const gk = getGroupKey(svc as unknown as ActiveServiceLike);
      let groupHeader = '';
  ...
  duplicateSchedulePrompt(id: unknown) {
    const svc = this.game.scheduleCreator.services.find((s: { id: unknown }) => s.id === id);
    if (!svc) return;
  ...
  setupLinePage() {
    ...
    if (sillonsList && !sillonsList._delegated) {
      ...
      sillonsList.addEventListener('mousedown', (e) => {
        const target=e.target as Element | null; const btn = target?.closest?.('[data-delete-sillon]') as HTMLElement | null;
  ...
  async saveStationFromLines() {
    ...
    if (connectChoice === '_nearest') {
      ...
      for (const s of this.game.world.stations) {
        ...
        if (Number.isFinite(d) && d < nearestDistKm) { nearestDistKm = d; connectTo = s; }
      }
      if (nearestDistKm >= 3) connectTo = null;
    } else if (connectChoice) {
      connectTo = this.game.world.getStationById(connectChoice);
    }
  ...
  openLineModal(editLine: Line | null = null) {
    this.lineStops = [];
    this._editingLineId = null;
    this.lineMapCenter = null;
    this.lineMapScale = null;
    ...
    if (editLine) {
      ...
      this.lineStops = editLine.stops.map((stId: unknown) => {
  ...
  _populateLineStationSelect(filter: __KPM565 = '') {
    const select = document.getElementById('line-station-select');
    if (!select) return;
    const term = filter.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const stations = (this.game.world.stations || [])
      .filter((st: __S3Struct562) => {
        const name = (st.name || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        return name.includes(term);
      })
      .sort((a: __S3Struct563, b: __S3Struct564) => a.name.localeCompare(b.name));
    select.innerHTML = stations.map((st: { id: unknown; name: unknown }) => `<option value="${htmlText(st.id)}">${htmlText(st.name)}</option>`).join('');
    select.dataset.stations = JSON.stringify(stations.map((s: { id: unknown; name: unknown }) => ({ id: s.id, name: s.name })));
  }
  ...
  setupLineMap() {
    ...
    const getSegmentStations = (i: number) => {
    ...
    const drawMap = () => {
      ...
      for (const st of world.stations) {
        ...
        const isSelected = this.lineStops.some((s: { stationId: unknown }) => s.stationId === st.id);
    ...
    let drag = false, dragStart: { x: number; y: number } | null = null, totalDragDist = 0;

    const findNearestManualPoint = (x: number, y: number) => {
    ...
    const isNearStation = (x: number, y: number, radius: number = 14) => {
      for (const st of world.stations) {
        const p = project(st.lat, st.lon);
  ...
  addLineStop(station: { id: unknown; name: unknown }) {
    if (this._lineManualMode) {
  ...
  _rebuildLineManualRoute() {
    ...
    if (!sa || !sb) return;
    const start: ManualRoutePoint = { lat: sa.lat, lon: sa.lon, maxSpeed: 160, control: true };
    const end: ManualRoutePoint = { lat: sb.lat, lon: sb.lon, maxSpeed: 160, control: true };
    const controls = this._lineManualPoints.map((p: { lat: number; lon: number }) => ({ lat: p.lat, lon: p.lon, maxSpeed: 160, control: true }));
  ...
  _toggleLineManual() {
    if (this._lineManualMode) {
      ...
      if (route && route.length >= 2) {
        this._lineManualPoints = route.filter((p: { control: unknown }, i: unknown) => p.control && i !== 0 && i !== route.length - 1).map((p: { lat: unknown; lon: unknown }) => ({ lat: p.lat, lon: p.lon }));
      } else {
        this._lineManualPoints = [];
      }
  ...
  renderLineStops() {
    ...
    container.innerHTML = this.lineStops.map((stop: { stationId: unknown; stationName: unknown }, i: number) => {
  ...
  async saveLine() {
    ...
    const stops = this.lineStops.map((s: { stationId: unknown }) => s.stationId);
    ...
    if (this._editingLineId) {
      ...
      if (line) {
        ...
        const stagedTrackIds: unknown[] = [...candidate.trackIds];
        this.game.lineManager.removeLine(candidate.id);
        const oldStops: unknown[] = [...line.stops];
        for (const oldStId of oldStops) {
          const st = this.game.world.getStationById(oldStId);
          if (st) st.lineIds = (st.lineIds || []).filter((lid: unknown) => lid !== line.id);
        }
        line.name = name;
        line.color = color;
        line.code = code;
        ...
        for (const stId of stops) {
          const st = this.game.world.getStationById(stId);
          if (!st) continue;
          if (!Array.isArray(st.lineIds)) st.lineIds = [];
  ...
  renderLinesList() {
      ...
      if (stations.length > 0) {
        stationsContainer.innerHTML = `
          ...
          <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px">
            ${stations.map((st: __S3Struct565) => {
              const typeLabel = ({ voyageur: 'Voy', marchandise: 'Fret', mixed: 'Mix', depot: 'Dep', ite: 'ITE' } as any)[st.type] || '';
    ...
    if (networkContainer) {
      ...
      const avgWear = troncons.length > 0 ? (troncons.reduce((s: number, t: { wear?: number }) => s + (t.wear || 0), 0) / troncons.length).toFixed(1) : '0';
      const maxWear = troncons.length > 0 ? Math.max(...troncons.map((t: { wear: unknown }) => t.wear || 0)).toFixed(1) : '0';
      const closedTracks = troncons.filter((t: { closed: unknown }) => t.closed).length;
      const lineRows = this.game.lineManager.getAll().map((line: __S3Struct567) => {
        ...
        const tracks = line.trackIds.map((id: unknown) => this.game.world.tracks.find((t: { id: unknown }) => t.id === id) || this.game.voiePointManager?.getTronconById(id)).filter(Boolean);
        const wear = tracks.length ? (tracks.reduce((s: unknown, t) => s + (t.wear || 0), 0) / tracks.length).toFixed(1) : '-';
    ...
    container.innerHTML = lines.map((line: unknown[]) => {
      const stopsPreview = line.stops.map((stId: unknown) => {
        const st = this.game.world.getStationById(stId);
      ...
      const totalDist = line.trackIds.reduce((sum: unknown, trkId: unknown) => {
        if (!trkId) return sum;
        const track = this.game.world.tracks.find((t: { id: unknown }) => t.id === trkId);
      ...
      return `
        <div class="line-item" style="border-left:4px solid ${htmlText(line.color)}">
          ...
          <div class="line-route-preview">
            ${stopsPreview.map((name: unknown, i: number) =>
              `<span class="line-stop-tag">${htmlText(name)}</span>${i < stopsPreview.length - 1 ? '<span style="color:var(--text3)"> → </span>' : ''}`
            ).join('')}
          </div>
        </div>
      `;
  ...
  editStationFromLines(stationId: unknown) {
  ...
  deleteLine(id: unknown) {
    ...
    if (line) {
      ...
      for (const stId of line.stops) {
        const st = this.game.world.getStationById(stId);
        if (st) st.lineIds = (st.lineIds || []).filter((lid: unknown) => lid !== id);
      }
    }
  ...
  openSillonCreator() {
    ...
    const opts = this.game.world.stations.map((st: { id: unknown; name: unknown }) => `<option value="${htmlText(st.id)}">${htmlText(st.name)}</option>`).join('');
  ...
  async saveSillon() {
    ...
    if (this._sillonManualRoute && this._sillonManualRoute.length >= 2) {
      ...
      const speeds = route.filter((r: { maxSpeed: unknown }) => r.maxSpeed).map((r: { maxSpeed: unknown }) => r.maxSpeed);
      maxSpeed = speeds.length ? Math.round(speeds.reduce((a: unknown, b: unknown) => (a as number) + (b as number), 0) / speeds.length) : 160;
      electrified = route.some((r: { electrified: unknown }) => r.electrified === false) ? false : true;
      route = this.game.orm.getRouteSegments(route).map((s: __S3Struct570) => ({ lat: s.from.lat, lon: s.from.lon, maxSpeed: s.maxSpeed })).concat([{ lat: route[route.length - 1].lat, lon: route[route.length - 1].lon, maxSpeed: route[route.length - 1].maxSpeed || 160 }]);
    }
    ...
    if (route && route.length >= 2) {
      ...
      const speeds = route.map((r: { maxSpeed: unknown }) => Number(r.maxSpeed)).filter((v: number) => Number.isFinite(v) && v > 0);
      maxSpeed = speeds.length ? Math.round(speeds.reduce((a: number, b: number) => a + b, 0) / speeds.length) : 160;
      electrified = route.some((r: { electrified: unknown }) => r.electrified === false) ? false : true;
  ...
  renderSillonsList() {
    ...
    container.innerHTML = sillons.map((s: __S3Struct571) => `
  ...
  _rebuildSillonManualRoute() {
    ...
    const controls = this._sillonManualPoints.map((p: { lat: unknown; lon: unknown }) => ({ lat: p.lat, lon: p.lon, maxSpeed: 160 }));
  ...
  setupSillonMap() {
    ...
    const drawMap = () => {
      ...
      if (!trace && this._sillonManualStart && this._sillonManualEnd) {
        const start = { lat: this._sillonManualStart.lat, lon: this._sillonManualStart.lon, maxSpeed: 160 };
        const end = { lat: this._sillonManualEnd.lat, lon: this._sillonManualEnd.lon, maxSpeed: 160 };
        const controls = this._sillonManualPoints.map((p: Record<string, unknown>) => ({ ...p, maxSpeed: 160 }));
  ...
  openItemModal(lat: unknown, lon: unknown) {
    const modal = document.getElementById('modal-ite');
    if (!modal) return;
    document.getElementById('ite-lat').value = lat as string;
    document.getElementById('ite-lon').value = lon as string;
    document.getElementById('ite-name').value = '';
    document.getElementById('ite-type').value = 'ite-fret';
    document.getElementById('ite-tracks').value = 2 as unknown as string;
    document.getElementById('ite-cost').value = 50000 as unknown as string;
  ...
  _finishITETrack() {
    ...
    const route = this._densifyRoute(this._iteTrackPoints.map((p: { lat: unknown; lon: unknown }) => ({ lat: p.lat, lon: p.lon, maxSpeed: 30 })));
  ...
  _renderITEPendingTracks() {
    ...
    list.innerHTML = this._pendingITE.tracks.length === 0
      ...
      : this._pendingITE.tracks.map((t: { name: unknown; length: unknown }, i: unknown) => `
  ...
  _removeITEPendingTrack(index: unknown) {
  ...
  saveITE() {
    ...
    const createdITE = this.game.depotManager.add({
      ...
      tracks,
      cost,
      iteTracks: (this._pendingITE?.tracks || []).map((t: { name: unknown; length: unknown }) => ({ name: t.name, length: t.length, cargoType: '' })),
  ...
  openDepotModal() {
    ...
    if (select) select.innerHTML = '<option value="">—</option>' + this.game.world.stations.map((s: { id: unknown; name: unknown }) => `<option value="${htmlText(s.id)}">${htmlText(s.name)}</option>`).join('');
  ...
  _renderITETrackList() {
    ...
    if (!list) return;
    list.innerHTML = (this._pendingITETracks || []).map((t: { name: unknown; length: unknown; cargoType: unknown }, i: unknown) => `
  ...
  saveDepot() {
    ...
    const data = {
      ...
      iteCargoTypes: type.startsWith('ite') ? [...new Set((this._pendingITETracks || []).map((t: { cargoType: unknown }) => t.cargoType).filter(Boolean))] : [],
    };
    const created = this.game.depotManager.add(data, this.game.economy);
    if (!created) return alert('Fonds insuffisants : le dépôt / ITE n’a pas été créé.');
  ...
  renderDepotsList() {
    ...
    if(!this._selectedDepotPageId||!depots.some((d: { id: unknown }) =>d.id===this._selectedDepotPageId))this._selectedDepotPageId=depots[0]?.id||'';
    if(!this._depotPageTab)this._depotPageTab='overview';
    ...
    const services=this.game.scheduleCreator?.getActiveServices?.()||[];
    const activeFor=(r: __S3Struct572) =>services.some((s: __S3Struct573) =>s?.rame?.id===r.id&&!s.completed&&!s.cancelled&&['moving','departing','stopped_at_station'].includes(String(s.state||'')));
    const pct=(v: unknown,c: number)=>c>0?Math.round(Math.max(0,Math.min(100,Number(v||0)/c*100))):0;
    const money=(n: unknown) =>(Number(n)||0).toLocaleString('fr-FR',{maximumFractionDigits:0})+' €';
    const qty=(n: unknown,d: number=0)=>(Number(n)||0).toLocaleString('fr-FR',{maximumFractionDigits:d});
    const homeName=(id: unknown) =>this.game.depotManager.getDepotById(id)?.name||'—';
    const staffName=(id: string) =>(DEPOT_STAFF_CATALOG as any)[id]?.label||id.replace(/_/g,' ');
    const equipmentName=(id: unknown) =>(DEPOT_EQUIPMENT_CATALOG as any)[id as string]?.label||id;
    const allTractionElements: Array<{ rame: Rame; el: Rame['elementDetails'][number] }> = [];for (const r of allRames)for (const e of r.elementDetails||[])if(['locomotive','automotrice'].includes(e.category))allTractionElements.push({rame:r,el:e});
    const searchInput=(placeholder: unknown,selector: unknown,key: unknown)=>`<input class="depot-search" placeholder="${esc(placeholder)}" data-depot-filter="${esc(selector)}" data-depot-key="${esc(key)}">`;

    const depotList=depots.map((d: { id: unknown; equipmentInventory: unknown; name: unknown; stationId: unknown }) =>{const occ=this.game.depotManager.getDepotOccupancy(d.id),assigned=allRames.filter((r: { depotId: unknown }) =>r.depotId===d.id).length,ops=this.game.depotManager.getOperationsForDepot(d.id).length,equip=Object.values(d.equipmentInventory||{}).reduce((a,b: unknown)=>a+Number(b||0),0);return `<button type="button" class="depot-nav-card ${htmlText(d.id===this._selectedDepotPageId?'active':'')}" data-depot-search="${esc(norm(d.name+' '+(this.game.world.getStationById(d.stationId)?.name||'')+' '+equip+' équipements'))}" onclick="game.ui.selectDepotDetail('${htmlJsString(d.id)}')"><span class="depot-nav-icon">▣</span><span><b>${esc(d.name)}</b><small>${occ.used}/${occ.capacity} voies · ${assigned} affectée(s) · ${ops} opération(s) · ${equip} équipement(s)</small></span></button>`;}).join('');
    ...
    const renderMaterial=(d: DepotView) =>{
      const rows=allRames.map((r: Rame) =>{
        const assigned=r.depotId===d.id,present=r.currentLocation?.depotId===d.id,busy=activeFor(r),op=!!r.depotOperationId;
        const elems=(r.elementDetails||[]).filter((e: __S3Struct582) =>['locomotive','automotrice','voiture','wagon'].includes(e.category)).map((e: __S3Struct583) =>`<div class="depot-element-row"><span>${esc(e.instanceName||e.name)} <small>${esc(e.seriesName||e.category)}</small></span><span>Port d'attache élément : <b>${esc(homeName(e.homeDepotId))}</b></span><button class="btn-sm" onclick="game.ui.assignDepotElement('${htmlJsString(d.id)}','${htmlJsString(r.id)}','${htmlJsString(e.elementId)}')">Affecter ici</button></div>`).join('');
        const search=norm([r.name,r.serialNumber,homeName(r.depotId),present?'présent':'',busy?'service':'',...(r.elementDetails||[]).flatMap((e: { instanceName: unknown; name: unknown; seriesName: unknown; category: unknown; traction: unknown }) =>[e.instanceName,e.name,e.seriesName,e.category,e.traction])].join(' '));
      ...
      const physicalRows=rotationVehicles.map((v: { number: unknown; name: unknown; category: unknown; traction: unknown; homeDepotId: unknown; id: unknown }) =>`<div class="depot-element-row depot-rotation-material" data-rotation-search="${esc(norm([v.number,v.name,v.category,v.traction,homeName(v.homeDepotId)].join(' ')))}"><span><b>${esc(v.number||v.name)}</b> <small>${esc(v.category||'matériel physique')} · ${esc(v.traction||'')}</small></span><span>Port d'attache : <b>${esc(homeName(v.homeDepotId))}</b></span>${v.homeDepotId===d.id?`<button class="btn-sm" onclick="game.ui.clearRotationVehicleDepot('${htmlJsString(v.id)}')">Désaffecter</button>`:`<button class="btn-sm" onclick="game.ui.assignRotationVehicleDepot('${htmlJsString(d.id)}','${htmlJsString(v.id)}')">Affecter ici</button>`}</div>`).join('');
      const couponRows=rotationCoupons.map((c: __S3Struct584) =>`<div class="depot-element-row depot-rotation-material" data-rotation-search="${esc(norm(c.name+' coupon '+c.vehicleIds.map((id: unknown) =>this.game.rotationV2?.getVehicle?.(id)?.number||'').join(' ')+' '+homeName(c.homeDepotId)))}"><span><b>${esc(c.name||'Coupon')}</b> <small>Coupon · ${c.vehicleIds.length} véhicule(s)</small></span><span>Port d'attache : <b>${esc(homeName(c.homeDepotId))}</b></span>${c.homeDepotId===d.id?`<button class="btn-sm" onclick="game.ui.clearRotationCouponDepot('${htmlJsString(c.id)}')">Désaffecter</button>`:`<button class="btn-sm" onclick="game.ui.assignRotationCouponDepot('${htmlJsString(d.id)}','${htmlJsString(c.id)}')">Affecter ici</button>`}</div>`).join('');
      const rescue=d.rescueLocos.map((x: { deployed: unknown; stockName: unknown; stockId: unknown }) =>`<div class="depot-rescue-current"><span>${x.deployed?'🔴':'🟢'} ${esc(x.stockName)}</span><button class="btn-sm danger" onclick="game.ui.removeRescueLoco('${htmlJsString(d.id)}','${htmlJsString(x.stockId)}')">Retirer</button></div>`).join('')||'<div class="depot-muted">Aucune machine de secours.</div>';
    ...
    const renderTracks=(d: { id: unknown }) =>{const occ=this.game.depotManager.getDepotOccupancy(d.id);return `<section class="depot-panel"><div class="depot-section-title"><div><h4>Occupation des voies de garage</h4><span class="depot-muted">Capacité réelle : une rame présente occupe une voie.</span></div><b>${occ.used}/${occ.capacity}</b></div><div class="depot-track-actions"><span>Extension : <b>${money(DEPOT_TRACK_EXPANSION_COST)}</b> / voie</span><button class="btn-sm" onclick="game.ui.addDepotTracks('${htmlJsString(d.id)}',1)">+ 1 voie</button><button class="btn-sm" onclick="game.ui.addDepotTracks('${htmlJsString(d.id)}',5)">+ 5 voies</button></div>${searchInput('Rechercher une voie, une rame ou une opération…','.depot-track','trackSearch')}<div class="depot-track-grid">${occ.tracks.map((t: { rameId: unknown; operationId: unknown; track: unknown }) =>{const r=allRames.find((x: { id: unknown }) =>x.id===t.rameId);const op=this.game.depotManager.depotOperations.find((o: { id: unknown }) =>o.id===t.operationId);const search=norm(`voie ${t.track} ${r?.name||'libre'} ${op?.label||'garage'}`);return `<div class="depot-track ${htmlText(t.rameId?'occupied':'free')}" data-track-search="${esc(search)}"><strong>Voie ${t.track}</strong>${r?`<b>${esc(r.name)}</b><span>${htmlText(op?(op.label):'Garage')}</span>${op?`<small>${Math.ceil(op.remainingMin)} min restantes</small>`:''}`:'<span>Libre</span>'}</div>`;}).join('')}</div></section>`;};

    const opButtons=(d: DepotView,r: __S3Struct586,groups: string[])=>Object.entries(DEPOT_OPERATION_CATALOG).filter(([,def])=>groups.includes(def.group)).map(([id,def])=>{
      const c=r.consumables||{},defects=r.pendingDefects||[];
      const recommended=(id==='routine_service'&&(Number(r.wearLevel||0)>=20||Number(r.kmSinceLastMaint||0)>=5000))||(id==='wheel_reprofile'&&(Number(r.wearLevel||0)>=40||Number(r.kmSinceLastMaint||0)>=15000))||(id==='engine_replace'&&defects.includes('moteur'))||(id==='traction_motor_replace'&&defects.some((x: string) =>['traction','moteur_traction'].includes(x)))||(id==='pantograph_replace'&&defects.includes('pantographe'))||(id==='brake_overhaul'&&defects.includes('freins'))||(id==='compressor_replace'&&defects.includes('compresseur'))||(id==='hvac_replace'&&defects.includes('climatisation'))||(id==='door_repair'&&defects.includes('portes'))||(id==='battery_service'&&defects.includes('batterie'))||(id==='fluid_service'&&['oil','coolant','gearboxOil','hydraulicOil'].some((k: unknown) =>Number(c[k+'CapacityL']||0)>0&&Number(c[k+'L']||0)/Number(c[k+'CapacityL']||1)<.6))||(id==='washer_fill'&&Number(c.washerCapacityL||0)>0&&Number(c.washerL||0)/Number(c.washerCapacityL||1)<.35)||(id==='refuel'&&Number(c.fuelCapacityL||0)>0&&Number(c.fuelL||0)/Number(c.fuelCapacityL||1)<.3)||(id==='sand_fill'&&Number(c.sandCapacityKg||0)>0&&Number(c.sandKg||0)/Number(c.sandCapacityKg||1)<.4)||(id==='exterior_wash'&&Number(r.cleanliness?.exterior??100)<70)||(id==='interior_clean'&&Number(r.cleanliness?.interior??100)<70)||(id==='deep_disinfection'&&Number(r.cleanliness?.interior??100)<35);
      const need=this.game.depotManager._operationNeeds?.(r,id);
      const resourceText=Object.entries(need?.resources||{}).filter(([,v])=>Number(v)>0).map(([k,v])=>{const rd=(DEPOT_RESOURCE_CATALOG as any)[k];return rd?`${qty(v,rd.unit==='kg'||rd.unit==='m³'?1:0)} ${rd.unit} ${rd.label}`:'';}).filter(Boolean).join(' · ');
      ...
      const equipment=(('equipment' in def && Array.isArray(def.equipment)) ? def.equipment : []).map((k: unknown) =>({id:k,label:equipmentName(k),...this.game.depotManager.getEquipmentAvailability(d.id,k)}));
      const equipmentText=equipment.map((x) =>`${x.label} ${x.free}/${x.installed} libre`).join(' · ');
      const missingResource=Object.entries(need?.resources||{}).some(([k,v])=>(DEPOT_RESOURCE_CATALOG as any)[k]?.stock&&Number(d.resourceStocks[k]||0)<Number(v||0));
      ...
      const staffAdvanced=this.game.realismSettings?.personnelRequired===true;const staffCheck=staffAdvanced?(this.game.staffManager?.checkDepotStaff?.(d.id,def.staff||{})||{ok:true,shortages:[]}):{ok:true,shortages:[]};const missingStaff=staffAdvanced&&!staffCheck.ok;
      const shortage=missingResource||missingPart||missingEquipment||busyEquipment||missingStaff;
      const directCost=Object.entries(need?.resources||{}).reduce((sum,[k,v])=>{const rd=(DEPOT_RESOURCE_CATALOG as any)[k];return sum+(!rd?.stock?Number(v||0)*Number(rd?.price||0):0);},0);
      const stockValue=Object.entries(need?.resources||{}).reduce((sum,[k,v])=>{const rd=(DEPOT_RESOURCE_CATALOG as any)[k];return sum+(rd?.stock?Number(v||0)*Number(rd?.price||0):0);},0)+Object.entries(need?.parts||{}).reduce((sum,[k,v])=>sum+Number(v||0)*Number(DEPOT_PART_CATALOG.find((x) =>x.id===k)?.price||0),0);
      const staffText=Object.entries(def.staff||{}).map(([k,v])=>{const a=this.game.staffManager?.getDepotStaffAvailability?.(d.id,k);return `${v} ${staffName(k)}${a?` (${a.free} libre${a.free>1?'s':''})`:''}`;}).join(', ')||'—';const staffModeLabel=staffAdvanced?'personnel':'personnel facultatif';const duration=d.getMaintenanceDuration?.(def.duration)||def.duration;const reason=missingEquipment?'ÉQUIPEMENT MANQUANT':busyEquipment?'ÉQUIPEMENT OCCUPÉ':missingStaff?'PERSONNEL MANQUANT':missingResource||missingPart?'STOCK INSUFFISANT':'';
    ...
    const renderEquipment=(d: { id: unknown }) =>{
    ...
    const renderStaff=(d: { id: unknown }) =>{
      const ops=this.game.depotManager.getOperationsForDepot(d.id),needs: Record<string, number>={};for (const o of ops)for (const [role,n] of Object.entries(o.staff||{}))needs[role]=(needs[role]||0)+Number(n||0);
      const operationByRole: Record<string, string[]>={};for (const def of Object.values(DEPOT_OPERATION_CATALOG))for (const role of Object.keys(def.staff||{}))(operationByRole[role]||(operationByRole[role]=[])).push(def.label);
    ...
    const renderDetail=(d: DepotView | null | undefined) =>{if(!d)return '<div class="depot-empty">Créez un dépôt pour commencer.</div>';const station=this.game.world.getStationById(d.stationId),occ=this.game.depotManager.getDepotOccupancy(d.id),tabs: [string, string][]=[['overview','Vue générale'],['material','Matériel'],['tracks','Voies'],['equipment','Équipements'],['maintenance','Maintenance'],['cleaning','Nettoyage'],['stocks','Stocks & pièces'],['staff','Personnel']];let body='';if(this._depotPageTab==='material')body=renderMaterial(d);else if(this._depotPageTab==='tracks')body=renderTracks(d);else if(this._depotPageTab==='equipment')body=renderEquipment(d);else if(this._depotPageTab==='maintenance')body=renderMaintenance(d);else if(this._depotPageTab==='cleaning')body=renderCleaning(d);else if(this._depotPageTab==='stocks')body=renderStocks(d);else if(this._depotPageTab==='staff')body=renderStaff(d);else body=renderOverview(d);
      const rescued=this.game.depotManager.getRescuedFormations?.(d.id)||[];
      if(rescued.length && ['overview','material','tracks','maintenance'].includes(String(this._depotPageTab||'overview'))) body+=`<section class="depot-panel"><h4>Formations physiques remorquées</h4>${rescued.map((f: {id:string;name:string;vehicleIds:string[];repairing:boolean})=>`<p><b>${esc(f.name)}</b> — ${f.vehicleIds.length} engin(s), voie ${esc(this.game.depotManager.getDepotById(d.id)?.findRameTrack(f.id)?.track||'?')} : ${f.repairing?'réparation en cours':'réparation terminée — acheminement explicite depuis ce dépôt requis'}</p>`).join('')}<small>Ces engins conservent leur identité dans les roulements. Aucune rame supplémentaire n’est créée ; la voie est libérée après le départ constaté de tous les engins.</small></section>`;
      return `<div class="depot-detail-head"><div><span class="depot-detail-icon">▣</span><span><h3>${esc(d.name)}</h3><small>${esc(station?.name||'Rattachement technique')} · ${occ.used}/${occ.capacity} voies · ${htmlText(d.location?Number(d.location.lat).toFixed(5)+', '+Number(d.location.lon).toFixed(5):'position héritée')}</small></span></div><button class="btn-sm danger" onclick="game.ui.deleteDepot('${htmlJsString(d.id)}')">Supprimer</button></div><nav class="depot-tabs">${tabs.map(([id,label]: [string,string])=>`<button class="${htmlText(this._depotPageTab===id?'active':'')}" onclick="game.ui.setDepotDetailTab('${htmlJsString(id)}')">${htmlText(label)}</button>`).join('')}</nav><div class="depot-tab-body">${body}</div>`;};

    if(depotsContainer)depotsContainer.innerHTML=`<div class="depot-page-toolbar">${searchInput('Rechercher un dépôt…','.depot-nav-card','depotSearch')}<span>${depots.length} dépôt(s)</span></div><div class="depot-master"><aside class="depot-nav">${depotList||'<div class="depot-muted">Aucun dépôt.</div>'}</aside><main class="depot-detail">${renderDetail(selected)}</main></div>`;
    if(iteContainer)iteContainer.innerHTML=`<div class="depot-page-toolbar">${searchInput('Rechercher une ITE…','.ite-modern-card','iteSearch')}<span>${ites.length} ITE</span></div><div class="ite-modern-grid">${ites.map((d: __S3Struct595) =>{const st=this.game.world.getStationById(d.stationId);return `<article class="ite-modern-card" data-ite-search="${esc(norm(d.name+' '+(st?.name||'')+' '+d.getTypeLabel()+' '+(d.iteCargoTypes||[]).join(' ')))}"><div class="ite-modern-icon">▥</div><div><h4>${esc(d.name)}</h4><p>${esc(d.getTypeLabel())} · ${d.tracks} voie(s)</p><small>${htmlText(d.location?Number(d.location.lat).toFixed(5)+', '+Number(d.location.lon).toFixed(5):(st?.name||''))}</small>${d.iteCargoTypes?.length?`<div class="depot-badges">${d.iteCargoTypes.map((x: unknown) =>`<span>${esc(x)}</span>`).join('')}</div>`:''}</div><button class="btn-sm danger" onclick="game.ui.deleteDepot('${htmlJsString(d.id)}')">Supprimer</button></article>`;}).join('')||'<div class="depot-muted">Aucune ITE.</div>'}</div>`;
    this._wireDepotSearchInputs(depotsContainer);this._wireDepotSearchInputs(iteContainer);
  }

  selectDepotDetail(id: unknown){this._selectedDepotPageId=id;this._depotPageTab='overview';this.renderDepotsList();}
  setDepotDetailTab(tab: unknown){this._depotPageTab=tab;this.renderDepotsList();}
  _wireDepotSearchInputs(root: HTMLElement){if(!root)return;root.querySelectorAll('.depot-search[data-depot-filter]').forEach((input) =>{if(input._depotSearchWired)return;input._depotSearchWired=true;input.addEventListener('input',()=>this.filterDepotRows(input,input.dataset.depotFilter,input.dataset.depotKey));});}
  filterDepotRows(input: HTMLElement,selector: string,dataKey: string){const q=String(input?.value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();const root=input?.closest('.depot-panel,.depot-master,.page-content')||document;let visible=0;root.querySelectorAll(selector).forEach((el: HTMLElement) =>{const fromData=el.dataset&&dataKey?el.dataset[dataKey]:'';const raw=String(fromData||el.textContent||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();const show=!q||raw.includes(q);el.style.display=show?'':'none';if(show)visible++;});input.setAttribute('data-result-count',String(visible));}
  addDepotTracks(depotId: unknown,count: unknown){const res=this.game.depotManager.addDepotTracks(depotId,count,this.game.economy);if(!res.ok)return alert(res.reason||'Extension impossible.');this.game.saveState();this.renderDepotsList();}
  buyDepotEquipment(depotId: unknown,equipmentId: unknown){const res=this.game.depotManager.buyEquipment(depotId,equipmentId,this.game.economy);if(!res.ok)return alert(res.reason||'Achat équipement impossible.');this.game.saveState();this.renderDepotsList();}

  _syncRameRotationDepotLinks(rame: Rame){if(!rame||!this.game.rotationV2)return;const byId: Map<string, any>=new Map((rame.elementDetails||[]).map((e: __S3Struct598) =>[String(e.elementId||''),e]));for (const v of this.game.rotationV2.vehicles||[]){if(v.sourceRameId!==rame.id)continue;const e=byId.get(String(v.sourceRameElementId||''));if(e)v.homeDepotId=String(e.homeDepotId||'');}for (const c of this.game.rotationV2.coupons||[]){if(c.sourceRameId!==rame.id)continue;const homes: unknown[]=[...new Set((c.vehicleIds||[]).map((id: unknown) =>this.game.rotationV2.getVehicle?.(id)?.homeDepotId||'').filter(Boolean))];c.homeDepotId=homes.length===1?homes[0]:'';}}
  assignDepotHome(depotId: unknown,rameId: unknown){const r=this.game.rameManager.getById(rameId);if(!this.game.depotManager.assignRameHome(r,depotId))return alert('Affectation impossible.');this._syncRameRotationDepotLinks(r);this.game.saveState();this.renderDepotsList();}
  assignDepotElement(depotId: unknown,rameId: unknown,elementId: unknown){const r=this.game.rameManager.getById(rameId);if(!this.game.depotManager.assignElementHome(r,elementId,depotId))return alert('Affectation élément impossible.');this._syncRameRotationDepotLinks(r);this.game.saveState();this.renderDepotsList();}
  clearDepotHome(rameId: unknown){const r=this.game.rameManager.getById(rameId);if(!r)return;this.game.depotManager.clearRameHome(r);this._syncRameRotationDepotLinks(r);this.game.saveState();this.renderDepotsList();}
  _syncRotationVehicleSourceHome(vehicle: { sourceRameId: unknown; sourceRameElementId: unknown; homeDepotId: unknown }){if(!vehicle?.sourceRameId||!vehicle.sourceRameElementId)return;const r=this.game.rameManager.getById(vehicle.sourceRameId),e=r?.elementDetails?.find((x: { elementId: unknown }) =>String(x.elementId||'')===String(vehicle.sourceRameElementId));if(e)e.homeDepotId=String(vehicle.homeDepotId||'');}
  assignRotationVehicleDepot(depotId: unknown,vehicleId: unknown){const d=this.game.depotManager.getDepotById(depotId),v=this.game.rotationV2?.getVehicle?.(vehicleId);if(!d||d.type!=='depot'||!v)return alert('Affectation impossible.');this.game.rotationV2.setVehicleHomeDepot(vehicleId,depotId);this._syncRotationVehicleSourceHome(v);for (const c of this.game.rotationV2.coupons||[]){if(!(c.vehicleIds||[]).includes(v.id))continue;const homes: unknown[]=[...new Set(c.vehicleIds.map((id: unknown) =>this.game.rotationV2.getVehicle(id)?.homeDepotId||'').filter(Boolean))];c.homeDepotId=homes.length===1?homes[0]:'';}this.game.saveState();this.renderDepotsList();}
  clearRotationVehicleDepot(vehicleId: unknown){const v=this.game.rotationV2?.getVehicle?.(vehicleId);if(!v)return;this.game.rotationV2.setVehicleHomeDepot(vehicleId,'');this._syncRotationVehicleSourceHome(v);for (const c of this.game.rotationV2.coupons||[]){if((c.vehicleIds||[]).includes(v.id))c.homeDepotId='';}this.game.saveState();this.renderDepotsList();}
  assignRotationCouponDepot(depotId: unknown,couponId: unknown){const d=this.game.depotManager.getDepotById(depotId),c=this.game.rotationV2?.getCoupon?.(couponId);if(!d||d.type!=='depot'||!c)return alert('Affectation coupon impossible.');this.game.rotationV2.setCouponHomeDepot(couponId,depotId);for (const id of c.vehicleIds||[])this._syncRotationVehicleSourceHome(this.game.rotationV2.getVehicle(id));this.game.saveState();this.renderDepotsList();}
  clearRotationCouponDepot(couponId: unknown){const c=this.game.rotationV2?.getCoupon?.(couponId);if(!c)return;this.game.rotationV2.setCouponHomeDepot(couponId,'');for (const id of c.vehicleIds||[])this._syncRotationVehicleSourceHome(this.game.rotationV2.getVehicle(id));this.game.saveState();this.renderDepotsList();}
  enterDepot(depotId: unknown,rameId: unknown){const r=this.game.rameManager.getById(rameId);const res=this.game.depotManager.enterRame(depotId,r,this.game.scheduleCreator?.getActiveServices?.()||[]);if(!res.ok)return alert(res.reason||'Entrée impossible.');this.game.saveState();this.renderDepotsList();}
  leaveDepot(depotId: unknown,rameId: unknown){const r=this.game.rameManager.getById(rameId);const res=this.game.depotManager.leaveRame(depotId,r,this.game.scheduleCreator?.getActiveServices?.()||[]);if(!res.ok)return alert(res.reason||'Sortie impossible.');this.game.saveState();this.renderDepotsList();}
  resumeRescueRouting(id: string) {
    if(!confirm('Réessayer ce routage ? Un refus du fournisseur peut persister ; aucun service alternatif ne sera utilisé pour le contourner.'))return;
    if(!this.game.depotManager.resumeRescueRouting(id))return alert('Reprise indisponible : temporisation en cours, calcul déjà lancé ou mission terminée.');
    this.game.saveState();this.renderDepotsList();
  }

  addRescueElement(depotId: string,rameId: unknown,elementId: unknown){const r=this.game.rameManager.getById(rameId),el=r?.elementDetails?.find((e: { elementId: unknown }) =>e.elementId===elementId);if(!el)return;const name=`${el.instanceName||el.name} — ${r.name}`;if(!this.game.depotManager.addRescueLoco(depotId,el.elementId,name,el.traction))return alert('Impossible : maximum 2 secours ou matériel déjà affecté comme secours.');this.game.saveState();this.renderDepotsList();}
  buyDepotResource(depotId: unknown,key: unknown){const inp=document.getElementById(`res-${depotId}-${key}`),n=Number(inp?.value);const res=this.game.depotManager.buyResource(depotId,key,n,this.game.economy);if(!res.ok)return alert(res.reason||'Commande impossible.');this.game.saveState();this.renderDepotsList();}
  buyDepotPart(depotId: unknown,partId: unknown){const inp=document.getElementById(`part-${depotId}-${partId}`),n=Number(inp?.value);const res=this.game.depotManager.buyPart(depotId,partId,n,this.game.economy);if(!res.ok)return alert(res.reason||'Commande impossible.');this.game.saveState();this.renderDepotsList();}
  startDepotOp(depotId: unknown,rameId: unknown,opId: unknown){const r=this.game.rameManager.getById(rameId),staff=this.game.realismSettings?.personnelRequired===true?this.game.staffManager:null,res=this.game.depotManager.startDepotOperation(depotId,r,opId,this.game.economy,staff);if(!res.ok)return alert(res.reason||'Opération impossible.');this.game.saveState();this.renderDepotsList();}

  addRescueLoco(depotId: unknown) {
    const select = document.getElementById(`rescue-stock-${depotId}`);
    if (!select || !select.value) return;
    const stock = this.game.rollingStock.getAll().find((s: { id: unknown }) => s.id === select.value);
    if (!stock) return;
  ...
  removeRescueLoco(depotId: unknown, stockId: unknown) {
    if (!this.game.depotManager.removeRescueLoco(depotId, stockId)) return alert('Impossible : cette locomotive de secours est actuellement engagée.');
    this.game.saveState();
    this.renderDepotsList();
  }

  buySparePart(depotId: unknown, type: string, qty: __KPM568) {
    qty = Number(qty);
    if (!Number.isInteger(qty) || qty <= 0) return alert('Quantité de pièces invalide.');
    const depot = this.game.depotManager.getDepotById(depotId);
    if (!depot) return;
    const prices: Record<string, number> = { moteur: 5000, freins: 3000, climatisation: 2000, portes: 1500, fanaux: 1000 };
  ...
  deleteDepot(id: unknown) {
    if (!confirm('Supprimer ce d\u00e9p\u00f4t ?')) return;
    const depot=this.game.depotManager.getDepotById(id);
    if (!depot) return;
    if (depot.type === 'depot') {
      const linked=(this.game.rameManager.getAll?.()||[]).filter((r: Rame) =>r.depotId===id||r.currentLocation?.depotId===id||(r.elementDetails||[]).some((e: __S3Struct600) =>e.homeDepotId===id));
      const rotationLinked=(this.game.rotationV2?.vehicles||[]).filter((v: { homeDepotId: unknown }) =>v.homeDepotId===id).length+(this.game.rotationV2?.coupons||[]).filter((c: { homeDepotId: unknown }) =>c.homeDepotId===id).length;
      if(linked.length||rotationLinked) return alert(`Suppression impossible : ${linked.length+rotationLinked} matériel(s) sont encore affectés ou présents. Désaffectez-les et sortez-les d’abord.`);
    }
    if (depot.type?.startsWith('ite') && this.game.industrialClients?.clients?.some((c: { active: unknown; depotId: unknown }) => c?.active !== false && c.depotId === id)) {
    ...
    if (!this.game.depotManager.remove(id)) return alert('Suppression impossible : ce site a encore un secours, une réparation ou une maintenance en cours.');
    if (depot.type?.startsWith('ite')) this.game.iteModules?.removeInstallation?.(id);
    this.game.saveState();
    this.renderDepotsList();
  }

  _renderDepotQueueSection(depot: { id: unknown }) {
    const dm = this.game.depotManager;
    const repairs = dm.repairQueue.filter((r: { depotId: unknown }) => r.depotId === depot.id);
    const maint = dm.maintenanceQueue.filter((m: { depotId: unknown }) => m.depotId === depot.id);
    if (repairs.length === 0 && maint.length === 0) return '';
    const items: unknown[] = [
      ...repairs.map((r: __S3Struct601) => `<div style="font-size:10px;padding:2px 0"><span style="color:#ef4444">Reparation</span> ${htmlText(r.serviceName)} — ${Math.ceil(r.remainingMin)} min</div>`),
      ...maint.map((m: __S3Struct602) => `<div style="font-size:10px;padding:2px 0"><span style="color:#3b82f6">Entretien</span> ${htmlText(m.rameName)} — ${Math.ceil(m.remainingMin)} min</div>`),
    ];
  ...
  _renderMaintenanceButton(depot: { id: unknown }) {
    // List all RAMES not already in maintenance (preventive maintenance available for any rame)
    const allRames = this.game.rameManager.getAll();
    const dm = this.game.depotManager;
    const available = allRames.filter((r: { inMaintenance: unknown; id: unknown }) =>
  ...
  sendRameToMaintenance(depotId: unknown) {
    const select = document.getElementById(`maint-rame-${depotId}`);
    if (!select || !select.value) return;
    const rameId = select.value;
    const rame = this.game.rameManager.getById(rameId);
    if (!rame) return;
    if (!this.game.depotManager.sendRameToMaintenance(rameId, rame.name, depotId)) return alert('Impossible d’envoyer cette rame en maintenance.');
  ...
  bulkSendToMaintenance(depotId: unknown) {
    const rames = this.game.rameManager.getAll().filter((r: { recommendedMaintenance: unknown; id: unknown }) => r.recommendedMaintenance && !this.game.depotManager.isRameInMaintenance(r.id));
    let count = 0;
    for (const rame of rames) {
      if (!this.game.depotManager.sendRameToMaintenance(rame.id, rame.name, depotId)) continue;
      rame.inMaintenance = true;
      const services = this.game.scheduleCreator.getActiveServices();
      for (const svc of services) {
  ...
  setupIncidentPage() {
    ...
    document.getElementById('works-impact')?.addEventListener('change', (e) => {
      document.getElementById('works-speed-group').style.display = (e.target as HTMLSelectElement).value === 'slow' ? 'block' : 'none';
    });
    document.getElementById('works-recurrence')?.addEventListener('change', (e) => {
      document.getElementById('works-days-group').style.display = (e.target as HTMLSelectElement).value === 'weekly' ? 'block' : 'none';
    });
  ...
  async saveWorks() {
    ...
    if (this._worksManualRoute && this._worksManualRoute.length >= 2) {
      ...
      route = this.game.orm.getRouteSegments(route).map((s: __S3Struct603) => ({ lat: s.from.lat, lon: s.from.lon, maxSpeed: s.maxSpeed })).concat([{ lat: route[route.length - 1].lat, lon: route[route.length - 1].lon, maxSpeed: route[route.length - 1].maxSpeed || 160 }]);
    }
  ...
  renderIncidentsPage() {
    ...
    if (activeList) {
      ...
      const nowMinute = Number.isFinite(Number(this.game.timeOfDay))
        ? Number(this.game.timeOfDay)
        : (() => { const pt=this.game.engine?.getParisTime?.(); return pt ? pt.hours*60 + pt.minutes + (pt.seconds||0)/60 : 0; })();
      const clock = (value: unknown, showDay: unknown = false) => {
        const raw = Number(value);
        if (!Number.isFinite(raw)) return '--:--';
      ...
      const services = this.game.scheduleCreator?.getActiveServices?.() || [];
      const serviceById: Map<string, any> = new Map(services.map((s: __S3Struct604) => [String(s?.id ?? ''), s]));
      activeList.innerHTML = active.length === 0
        ? '<div class="no-incidents">Aucun incident en cours</div>'
        : active.map((inc: __S3Struct605) => {
            const elapsed = Math.max(0, Number(inc.duration || 0) - Number(inc.remaining || 0));
            let start = Number(inc.startTime);
            ...
            if (svc && this.game.incidentManager?._serviceLocationDescriptor) {
              ...
              if (loc?.text) trainPosition = loc.text;
              if (loc?.stationA && loc?.stationB) trainAtStation = String(loc.stationA) === String(loc.stationB);
            }
    ...
    if (worksCount) {
      const activeWorks = works.filter((w: { active: unknown }) => w.active).length;
    ...
    if (worksList) {
      const worksEsc = (v: unknown) => String(v ?? '').replace(/[&<>"']/g, (c) => (HTML_ESCAPE_MAP[c]));
      const stationName = (value: unknown) => {
        if (!value) return '';
        const st = this.game.world.getStationById?.(value);
        return st?.name || String(value);
      };
      const locationFor = (z: WorksLocationSource, w: WorksLocationSource) => {
      ...
      worksList.innerHTML = works.length === 0
        ...
        : works.map((w: { "scope": unknown; "zones": { "length": number }; "stationName": unknown; "station": { "name": unknown }; "stationId": unknown; "stationA": unknown; "affectsTraffic": unknown; "stationImpact": unknown; "stationSpeedLimit": unknown; "manualRoute": unknown; "active": unknown; "name": unknown; "id": unknown; "getDateRange": (...args: unknown[]) => unknown; "recurrence": unknown; "startDate": string; "daysOfWeek": { "join": (...args: unknown[]) => unknown }; "startTime": unknown; "endTime": unknown }) => {
            ...
            const locations = stationOnly
              ...
              : [...new Set(zones.map((z: WorksLocationSource) => locationFor(z, w)).filter(Boolean))];
            ...
            const zoneSummary = stationOnly
              ...
              : (trafficSummary || (isV2
                  ? zones.map((z: { impact: unknown; speedLimit: unknown; distanceKm: unknown; direction: unknown; manual: unknown }, i: number) => {
  ...
  _rebuildWorksManualRoute() {
    ...
    const start: ManualRoutePoint = { lat: this._worksManualStart.lat, lon: this._worksManualStart.lon, maxSpeed: 160 };
    const end: ManualRoutePoint = { lat: this._worksManualEnd.lat, lon: this._worksManualEnd.lon, maxSpeed: 160 };
    const controls = this._worksManualPoints.map((p: { lat: number; lon: number }) => ({ lat: p.lat, lon: p.lon, maxSpeed: 160 }));
  ...
  setupWorksMap() {
    ...
    const drawMap = () => {
      ...
      if (!trace && this._worksManualStart && this._worksManualEnd) {
        ...
        const controls = this._worksManualPoints.map((p: Record<string, unknown>) => ({ ...p, maxSpeed: 160 }));
        trace = this._densifyRoute([start, ...controls, end], 0.05);
      }
      if (trace && trace.length >= 2) {
  ...
  setupEconomyPage() {
    ...
    logoInput?.addEventListener('change', (e) => { const f = (e.target as HTMLInputElement | null)?.files?.[0]; if (f) this._loadLogo(f); });
  ...
  _applyLogo(dataUrl: unknown) {
    ...
    if (drop) drop.innerHTML = `<img src="${htmlText(dataUrl)}" style="width:100%;height:100%;object-fit:contain">`;
  }
  ...
  _generateBulletin() {
    ...
    const noAcc = (s: unknown) => typeof s === 'string' ? s.normalize('NFD').replace(/[\u0300-\u036f]/g, '') : String(s);
    ...
    const drawLine = (yPos: unknown) => { doc.setDrawColor(180); doc.line(10, yPos, pw - 10, yPos); };
    // Helper: page footer
    const addFooter = () => {
    ...
    const sections: { num: string; title: string; page: number }[] = [
    ...
    const tracks = this.game.world.tracks || [];
    let trackKm = Math.round(tracks.reduce((s: number, t: { distance?: number }) => s + (t.distance || 0), 0));
    if (this.game.voiePointManager) {
      trackKm += Math.round(this.game.voiePointManager.getAllTroncons().reduce((s: number, t: { distance?: number }) => s + (t.distance || 0), 0));
    }
    ...
    const rames = this.game.rameManager.getAll();
    const totalTrainKm = Math.round(rames.reduce((s: number, r: Rame) => s + (r.totalKmRun || 0), 0));
    ...
    for (const svc of services) {
      ...
      doc.setFont(undefined, 'normal');
      const stopsStr = svc.stops.map((s: { stationId: unknown }) => {
        const st = this.game.world.getStationById(s.stationId);
        return st ? st.name : '?';
      }).join('  >  ');
    ...
    doc.addPage();
    y = 20;
    const newRames = lastBulletin ? rames.filter((r: __S3Struct613) => r.createdDate >= lastBulletin) : rames;
    ...
    for (const r of (newRames.length > 0 ? newRames : rames)) {
      ...
      doc.text(noAcc(`Mise en service: ${htmlText(r.createdDate)} | Km: ${Math.round(r.totalKmRun || 0).toLocaleString('fr-FR')} km | Traction: ${r.traction}`), 18, y); y += 5;
      // Images
      let imgX = 18;
    ...
    } catch (err) { console.error('Bulletin PDF error:', err); alert('Erreur generation PDF: ' + (err as { message?: string }).message); }
  }
  ...
  _generateFicheHoraire(stationId: unknown) {
    const { jsPDF } = window.jspdf || {};
    if (!jsPDF) return alert('jsPDF non charge');
    try {
    const noAcc = (s: unknown) => typeof s === 'string' ? s.normalize('NFD').replace(/[\u0300-\u036f]/g, '') : String(s);
    const minToStr = (m: unknown) => { const h = Math.floor((m as number) / 60) % 24; const mi = Math.round((m as number) % 60); return `${String(h).padStart(2,'0')}:${String(mi).padStart(2,'0')}`; };
    ...
    for (const svc of allServices) {
      ...
      for (let i = 0; i < stops.length; i++) {
        ...
        const intermediates: { name: string; depTime: string }[] = [];
    ...
    for (const entry of entries) {
      ...
      if (entry.intermediates.length > 0) {
        ...
        const intText = entry.intermediates.map((s: { name: unknown; depTime: unknown }) => `${s.name} (${s.depTime})`).join('  |  ');
    ...
    } catch (err) { console.error('Fiche horaire PDF error:', err); alert('Erreur generation PDF: ' + (err as { message?: string }).message); }
  }
  ...
  renderEconomyPage() {
    ...
    if (el('eco-track-km')) {
      const tracks = this.game.world.tracks || [];
      let totalTrackKm = tracks.reduce((s: number, t: { distance?: number }) => s + (t.distance || 0), 0);
      if (this.game.voiePointManager) {
        totalTrackKm += this.game.voiePointManager.getAllTroncons().reduce((s: number, t: { distance?: number }) => s + (t.distance || 0), 0);
      }
    ...
    if (histEl) {
      histEl.innerHTML = eco.history.slice().reverse().slice(0, 40).map((e: { type: unknown; description: unknown; category: unknown; amount: unknown }) => `
  ...
  update(activeServices: unknown) {
    ...
    try {
      ...
      if (!this._lastWeatherWidgetAt || perfNow - this._lastWeatherWidgetAt >= 1000) {
        ...
        if (ww) {
          ...
          if (ww._lastWeatherHtml !== html) { ww._lastWeatherHtml = html; ww.innerHTML = html; }
        }
      }
    } catch(e) { /* graceful */ }
    ...
    if (this.activePage === 'map') {
      ...
      this.updateTrainsList(activeServices as UiActiveService[]);
  ...
  exportMovementDiagnostics() {
    ...
    link.href=url;link.download=`Rail_Empire_${RELEASE}_mouvement_${Date.now()}.json`;
  ...
  updateV2RuntimeSidebar() {
    ...
    const perfNow=typeof performance!=='undefined'?performance.now():Date.now();
    if(this._lastV2SidebarDiagAt && perfNow-this._lastV2SidebarDiagAt<2000)return;
    ...
    const escHtml=(v: unknown) =>String(v??'').replace(/[&<>"']/g,(c) =>(HTML_ESCAPE_MAP[c]));
    const icon: Record<string, string>={error:'🔴',warn:'🟠',ok:'🟢',info:'🔵'};
    const rows=(diag.items||[]).slice(0,6).map((x: RuntimeDiagItem) =>`<div style="margin-top:4px;padding-top:4px;border-top:1px solid #1e293b"><b>${icon[x.level as string]||'•'} ${escHtml(x.label)}</b><br><span style="font-weight:500;color:#94a3b8">${escHtml(x.status)}</span></div>`).join('');
    const failures = this.game.diagnostics?.snapshot?.() || [];
    const failureRows = failures.slice(0,3).map((f: {code: string; count: number; message: string}) =>
      `<div class="re-operational-failure" style="margin-top:5px;color:#fbbf24"><b>${escHtml(f.code)}</b> · ${f.count} erreur(s) observée(s)<br>${escHtml(f.message)}</div>`).join('');
    ...
    if(box._lastV2Html!==html){box._lastV2Html=html;box.innerHTML=html;}
    box.style.borderColor=(diag.items||[]).some((x: { level: unknown }) =>x.level==='error')?'#7f1d1d':(diag.items||[]).some((x: { level: unknown }) =>x.level==='warn')?'#78350f':'#334155';
  }

  updateTrainsList(services: UiActiveService[]) {
    const container = document.getElementById('trains-list');
    if (!container) return;
    const perfNow = typeof performance !== 'undefined' ? performance.now() : Date.now();
    if (this._lastTrainListAt && perfNow - this._lastTrainListAt < 500) return;
    ...
    for (const svc of services) {
      ...
      if (!svc || svc.completed || svc.cancelled || ['completed','cancelled'].includes(svc.state) || !svc.train || svc.train.inMaintenance || svc.train.inDepot || displayRame?.inMaintenance || displayRame?.currentLocation?.depotId) continue;
      const t = svc.train;
      if (
        svc.isRescue ||
    ...
    const html = activeTrains.map((svc) => {
      ...
      let delayDisplay: unknown, delayClass: unknown;
      if (delayVal > 0) {
      ...
      if (svc.isRescue) {
        const stateLabels: Record<string, string> = { routing: 'Recherche du tracé aller', routing_return: 'Recherche du tracé retour', en_route: 'En route', recovering: 'Préparation du secours', returning: 'Retour dépôt' };
      ...
      const curIdx = svc.currentStopIndex || 0;
      const isArretStop = (s: { type: unknown; stationId: unknown }) => s && s.type === 'arret' && s.stationId;
      ...
      if (svc._atTerminus && svc._nextDepartureTime != null) {
        ...
        const fmtTime = (m: unknown) => this.minToTimeStr(((Math.round(m as number) % 1440) + 1440) % 1440);
      ...
      if (t.platform) {
        ...
        let voieName: unknown;
        if (stoppedStation?.platformNames?.length! >= (t.platform as number)) {
          voieName = stoppedStation!.platformNames![(t.platform as number) - 1];
        } else {
      ...
      let imageHtml = '';
      if (!svc.isWorkTrain && svc.serviceType !== 'work' && displayRame?.elementDetails?.some((e: { imageData: unknown }) => e.imageData)) {
  ...
  garageService(svcId: unknown) {
    const svc = this.game.scheduleCreator.services.find((s: { id: unknown }) => s.id === svcId);
    if (!svc) return;
  ...
  resumeFromGarage(svcId: unknown) {
    const svc = this.game.scheduleCreator.services.find((s: { id: unknown }) => s.id === svcId);
    if (!svc) return;
  ...
  updateFreightTab() {
    ...
    if (!container) return;
    const perfNow=typeof performance!=='undefined'?performance.now():Date.now();
    if(this._lastFreightSidebarAt && perfNow-this._lastFreightSidebarAt<1000)return;
    this._lastFreightSidebarAt=perfNow;
    const contracts = this.game.freightManager.contracts;
    const active = contracts.filter((c: FreightContract) => c.active);
    const completed = contracts.filter((c: FreightContract) => !c.active && c.quantity <= 0);
    const cancelled = contracts.filter((c: FreightContract) => !c.active && c.quantity > 0);
    const esc = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, c => HTML_ESCAPE_MAP[c]);
    const html = `
      <div class="section-title">Contrats actifs (${active.length})</div>
      ${active.map((c: FreightContract) => {
      ...
      ${cancelled.filter((c: FreightContract)=>c.inTransitQuantity>0).map((c: FreightContract)=>`<div class="contract-item"><b>${esc(c.cargoName)}</b> · ${c.inTransitQuantity} ${esc(c.unit)} à restituer au prochain arrêt compatible (sans recette).</div>`).join('')}
    `;
    if(container._lastFreightHtml!==html){container._lastFreightHtml=html;container.innerHTML=html;}
  }
  ...
  updateAlertBanner() {
    ...
    const interruptions: AlertBannerItem[] = [];
    const slowdowns: AlertBannerItem[] = [];
    const worksItems: { text: string; icon: string }[] = [];
    ...
    for (const inc of incidents) {
      ...
      const item: AlertBannerItem = { text: inc.effect === 'stop' ? label : `${label} - ${inc.speedLimit || 30} km/h`,
    ...
    const worksStationName = (value: unknown) => {
      if (!value) return '';
      const st = this.game.world.getStationById?.(value);
      return st?.name || String(value);
    };
    const worksLocation = (source: WorksLocationSource) => {
    ...
    if (displayWorks.length) {
      ...
      for (const r of displayWorks) {
        ...
        seen.add(key);
        let suffix = '';
        if (r.affectsTraffic === false) suffix = ' — sans impact circulation';
        else if (r.impact === 'slow') suffix = ` — LTV ${r.speedLimit || 40} km/h`;
        else if (r.impact === 'power-off') suffix = ' — coupure caténaire';
        worksItems.push({
    ...
    const stopTicker = (el: HTMLElement, reset: unknown = false) => {
      if (!el) return;
      if (el._alertTickerRaf) cancelAnimationFrame(el._alertTickerRaf);
      el._alertTickerRaf = null;
      el._alertTickerLast = null;
      if (reset) el._alertTickerX = null;
    };

    const runTicker = (el: HTMLElement, track: HTMLElement) => {
      if (!el || !track || el._alertTickerRaf) return;
      const SPEED_PX_PER_SEC = 52; // v1.1.72: nettement plus rapide que l'ancien ~18-25 px/s
      if (!Number.isFinite(el._alertTickerX)) el._alertTickerX = el.clientWidth;
      const step = (now: number) => {
        ...
        if (width <= viewport) {
          ...
          if (el._alertTickerX + width <= 0) el._alertTickerX = viewport;
          if (el._alertTickerX > viewport) el._alertTickerX = viewport;
    ...
    const setBanner = (el: HTMLElement, items: AlertBannerItem[]) => {
      if (!el) return;
      if (items.length === 0) {
      ...
      const contentKey = items.map((i: { icon: unknown; text: unknown }) => `${i.icon}|${i.text}`).join('||');
      const html = items.map((i: { icon: unknown; text: unknown }) =>
        `<span class="alert-item"><span class="alert-icon op-icon ${htmlText(i.icon)}" aria-hidden="true"></span>${esc(i.text)}</span>`
  ...
  _populateVpStationDropdown(selectedStationId: unknown, lat: number, lon: number) {
  ...
  openVoiePointModal(lat: number, lon: number) {
  ...
  openEditVoiePointModal(vp: VoiePoint) {
    ...
    if (troncons.length > 0) {
      trcList.innerHTML = '<div style="font-size:10px;color:var(--text2);margin-bottom:4px;font-weight:600">Troncons connectes:</div>' +
        troncons.map((trc: __S3Struct621) => {
  ...
  async _handleTronconClick(x: number, y: number) {
    const renderer = this.game.renderer;
    const vpm = this.game.voiePointManager;
    const world = this.game.world;
    ...
    if (!this._tronconPointA) {
      ...
      try {
        ...
        if (!Number.isFinite(distance) || distance <= 0) throw new Error('Distance ferroviaire invalide.');
      } catch (e) {
        this._showPickHint(`Tronçon non créé : ${(e as { message?: string })?.message || 'routage ORM impossible'}. Utilisez « Tracé manuel » si vous créez réellement une nouvelle infrastructure.`);
        this._tronconPointA = null;
        return;
      }
  ...
  _handleManualTronconClick(x: number, y: number) {
    const renderer = this.game.renderer;
    const vpm = this.game.voiePointManager;
    const world = this.game.world;
  ...
  _finalizeManualTroncon(endPoint: __S3Struct622) {
    const vpm = this.game.voiePointManager;
    const ptA = this._manualTronconPointA;
    const coarseRoute = this._manualTronconWaypoints.map((wp: { lat: unknown; lon: unknown; maxSpeed: unknown }) => ({
  ...
  async _handleTracerLigneClick(x: number, y: number) {
    const renderer = this.game.renderer;
    const worldPos = renderer.tileMap.screenToWorld(x, y, renderer.logicalWidth, renderer.logicalHeight);
    ...
    if (!this._tracerLignePointA) {
      ...
      try {
        ...
        let addedStations = 0;
        const uiYield = () => new Promise<void>((resolve) => {
        ...
        const stationCellKey = (lat: number, lon: number) => `${Math.floor(lat / stCell)},${Math.floor(lon / stCell)}`;
        const addStationToGrid = (st: __S3Struct624) => {
          const k = stationCellKey(st.lat, st.lon);
          if (!stationGrid.has(k)) stationGrid.set(k, []);
          stationGrid.get(k).push(st);
        };
        for (const st of world.stations) addStationToGrid(st);

        const nearbyStation = (lat: number, lon: number, maxKm: number = 0.5, sameName: unknown = null) => {
          const iy = Math.floor(lat / stCell), ix = Math.floor(lon / stCell);
          let best = null, bestD = Infinity;
          for (let dy=-1;dy<=1;dy++) for (let dx=-1;dx<=1;dx++) {
            for (const st of (stationGrid.get(`${iy+dy},${ix+dx}`) || [])) {
              if (sameName != null && (st.name || '').toLowerCase() !== sameName) continue;
              const d = Math.sqrt(Math.pow((lat - st.lat) * 111, 2) + Math.pow((lon - st.lon) * 111 * Math.cos(lat * Math.PI / 180), 2));
              if (d < maxKm && d < bestD) { best = st; bestD = d; }
            }
          }
          return best;
        };

        const addImportedStations = async (stations: OSMStationImport[]) => {
          let added = 0, work = 0;
          const accepted = [];
          for (const osmSt of (stations || [])) {
            const stableId = `osm-station-${osmSt.id}`;
            if (world.getStationById(stableId)) { accepted.push(world.getStationById(stableId)); continue; }
            const nameKey = (osmSt.name || '').toLowerCase();
            if (nearbyStation(osmSt.lat, osmSt.lon, 0.15, nameKey)) continue;
        ...
        try {
          ...
          const gridKey = (lat: number, lon: number) => `${Math.floor(lat / vpCell)},${Math.floor(lon / vpCell)}`;
          let existingWork = 0;
          for (const v of vpm.getAll()) {
            const k = gridKey(v.lat, v.lon);
            if (!existingVpGrid.has(k)) existingVpGrid.set(k, []);
            existingVpGrid.get(k).push(v);
            if ((++existingWork % 2500) === 0) await uiYield();
          }
          const findExistingVP = (vpData: __S3Struct626) => {
            const iy = Math.floor(vpData.lat / vpCell), ix = Math.floor(vpData.lon / vpCell);
            for (let dy=-1; dy<=1; dy++) for (let dx=-1; dx<=1; dx++) {
              const arr = existingVpGrid.get(`${iy+dy},${ix+dx}`) || [];
              for (const v of arr) {
                if (v.voie !== vpData.voie) continue;
          ...
          let vpChunk: unknown[] = [];
          const flushVP = async () => {
            if (!vpChunk.length) return;
            if (typeof vpm.addVoiePointsBulk === 'function') vpm.addVoiePointsBulk(vpChunk);
            else for (const v of vpChunk) vpm.addVoiePoint(v);
            vpChunk = [];
          ...
          for (const vpData of result.voiePoints) {
            ...
            vpDone++;
            if (vpChunk.length >= 700) await flushVP();
          }
          await flushVP();

          let addedTrc = 0, trcDone = 0, trcChunk: unknown[] = [];
          const flushTrc = async () => {
            if (!trcChunk.length) return;
            if (typeof vpm.addTronconsBulk === 'function') vpm.addTronconsBulk(trcChunk);
            else for (const t of trcChunk) vpm.addTroncon(t);
          ...
          for (const trcData of result.troncons) {
            if (remap.has(trcData.pointA)) trcData.pointA = remap.get(trcData.pointA);
            if (remap.has(trcData.pointB)) trcData.pointB = remap.get(trcData.pointB);
            trcData.lineGroupId = lineGroupId;
            trcDone++;
            if (trcData.pointA === trcData.pointB) continue;
            trcChunk.push(trcData);
            if (trcChunk.length >= 500) await flushTrc();
          }
          await flushTrc();
          ...
          try { vpm.deleteLineGroup?.(lineGroupId); } catch (_) { /* best effort */ }
          throw insertionError;
        } finally {
          vpm.endBatch?.();
        }

        const linkImportedStations = async (stations: { length: unknown }) => {
          if (!stations?.length) return;
          // One pass over imported VPs. No stations × all-VPs Cartesian product.
          const importedVPs = [];
          let scan = 0;
          for (const v of vpm.getAll()) {
            if (v.lineGroupId === lineGroupId) importedVPs.push(v);
            if ((++scan % 3000) === 0) await uiYield();
          }
          let changed = 0;
          for (let i = 0; i < importedVPs.length; i++) {
            ...
            if (!vp.stationId) {
              ...
              if (st) { vp.stationId = st.id; changed++; }
            }
            if ((i % 800) === 0) await uiYield();
          }
          if (changed) vpm.markDirty?.();
        };
        ...
        if (result.stationsPromise && typeof result.stationsPromise.then === 'function') {
          result.stationsPromise.then(async (stations: unknown) => {
            try {
              const r = await addImportedStations(stations as OSMStationImport[]);
              addedStations += r.added;
              await linkImportedStations(r.accepted);
              this.game.renderer?.invalidateStatic?.();
              this.game.saveState();
          ...
          }).catch((e: unknown) => console.warn('Auto-stations skipped safely:', e));
        }
  ...
  setupLivemapPanelDrag() {
    ...
    panel.addEventListener('mousedown', (e) => {
      if ((e.target as Element | null)?.closest('.lvp-close, button, a')) return;
      dragging = true;
      startX = e.clientX;
      startY = e.clientY;
  ...
  setupMobileNav() {
    ...
    nav.addEventListener('click', (e: MouseEvent) => { if ((e.target as Element).classList.contains('nav-btn')) nav.classList.remove('open'); });
  }
  ...
  _normalizeInfogareSearch(value: unknown = '') {
    return String(value || '')
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
  ...
  _rebuildInfogareStationIndex() {
    const stations = (this.game.world.stations || []).filter((st: { closed: unknown; id: unknown; name: unknown }) => !st.closed && st?.id && st?.name);
    this._infogareStationIndex = stations.map((st: __S3Struct627) => ({
      station: st,
      search: this._normalizeInfogareSearch(st.name)
    }));
  }

  _findInfogareStations(query: unknown, limit: number = 12) {
    const q = this._normalizeInfogareSearch(query);
    if (!q) return [];
    if (!this._infogareStationIndex?.length) this._rebuildInfogareStationIndex();
    const words = q.split(' ').filter(Boolean);
    return this._infogareStationIndex
      .map((entry: InfogareStationIndexEntry) => {
        const n = entry.search;
        if (!words.every((w: string) => n.includes(w))) return null;
        let score = 4;
        if (n === q) score = 0;
        else if (n.startsWith(q)) score = 1;
        else if (n.split(' ').some((part: string) => part.startsWith(q))) score = 2;
        else if (n.includes(q)) score = 3;
        return { ...entry, score };
      })
      .filter(Boolean)
      .sort((a: __S3Struct630, b: __S3Struct631) => a.score - b.score || a.station.name.localeCompare(b.station.name, 'fr', { sensitivity: 'base' }))
      .slice(0, Math.max(1, limit))
      .map((entry: { station: unknown }) => entry.station);
  }

  _escapeInfogareText(value: unknown = '') {
    return String(value).replace(/[&<>"']/g, (ch) => (HTML_ESCAPE_MAP[ch]));
  }

  _renderInfogareStationSuggestions(query: unknown) {
    ...
    box.innerHTML = matches.map((st: Station, idx: number) => {
    ...
    box.classList.remove('hidden');
    box.querySelectorAll('.infogare-station-suggestion').forEach((btn: HTMLButtonElement) => {
      btn.onmousedown = (e: MouseEvent) => e.preventDefault();
      btn.onclick = () => this._selectInfogareStation(btn.dataset.stationId, true);
    });
    return matches;
  }

  _selectInfogareStation(stationId: unknown, showBoard: unknown = false) {
    const input = document.getElementById('infogare-station');
    const box = document.getElementById('infogare-station-suggestions');
    const station = this.game.world.getStationById(stationId);
    if (!input || !station || station.closed) return false;
    this._infogareSelectedStationId = station.id;
    input.dataset.stationId = station.id;
    input.value = station.name;
    if (box) { box.innerHTML = ''; box.classList.add('hidden'); }
    this._infogarePage = 0;
    if (showBoard) this._showInfogareBoard();
    return true;
  }

  _commitInfogareStationSearch(showBoard: unknown = true) {
    const input = document.getElementById('infogare-station');
    ...
    if (current && this._normalizeInfogareSearch(current.name) === typed) {
      if (showBoard) { this._infogarePage = 0; this._showInfogareBoard(); }
      return true;
    }
  ...
  _rebuildInfogareServiceIndex() {
    const rows: InfogareServiceIndexRow[] = [];
    const mgr = this.game.scheduleV2;
    for (const rec of mgr?.schedules || []) {
      const ver = rec?.currentVersion;
      if (!rec?.id || !ver?.locations?.length) continue;
      if (!this._isInfogarePassengerCategory(ver.category || '')) continue;
      const stationIds = ver.locations.map((l: { stationId: unknown }) => l?.stationId).filter(Boolean);
      const first = ver.locations.find((l: { stationId: unknown }) => l?.stationId);
    ...
    for (const svc of this.game.scheduleCreator?.services || []) {
      if (!svc?.id || svc._v2OccurrenceId) continue;
      if (!this._isInfogarePassengerService(svc)) continue;
      const stops = svc.getCurrentStops?.() || svc.stops || [];
      const stationIds = stops.map((s: { stationId: unknown }) => s?.stationId).filter(Boolean);
  ...
  _findInfogareServices(query: unknown, limit: number = 12) {
    const q = this._normalizeInfogareSearch(query);
    if (!q) return [];
    if (!this._infogareServiceIndex?.length) this._rebuildInfogareServiceIndex();
    ...
    return this._infogareServiceIndex
      .map((entry: __S3Struct633) => {
        if (stationId && !entry.stationIds.includes(stationId)) return null;
        if (!entry.search.includes(q) && !q.split(' ').every((w: unknown) => entry.search.includes(w))) return null;
        let score = 4;
        const n = this._normalizeInfogareSearch(entry.name);
        const num = this._normalizeInfogareSearch(entry.number);
        if (n === q || num === q) score = 0;
        else if (n.startsWith(q) || num.startsWith(q)) score = 1;
        else if (entry.search.startsWith(q)) score = 2;
        else score = 3;
        return { ...entry, score };
      })
      .filter(Boolean)
      .sort((a: __S3Struct634,b: __S3Struct635) => a.score-b.score || a.name.localeCompare(b.name,'fr',{sensitivity:'base'}))
      .slice(0, Math.max(1,limit));
  }

  _renderInfogareServiceSuggestions(query: unknown) {
    const box = document.getElementById('infogare-service-suggestions');
    if (!box) return [];
    const matches = this._findInfogareServices(query, 12);
    const q = this._normalizeInfogareSearch(query);
    if (!q) { box.innerHTML=''; box.classList.add('hidden'); return matches; }
    if (!matches.length) {
      box.innerHTML='<div class="infogare-station-empty">Aucun service trouvé</div>';
      box.classList.remove('hidden');
      return matches;
    }
    box.innerHTML = matches.map((svc: { key: unknown; name: unknown; number: unknown; route: unknown },idx: unknown) =>
      `<button type="button" class="infogare-station-suggestion${htmlText(idx===0?' is-first':'')}" data-service-key="${this._escapeInfogareText(svc.key)}">`
      ...
      + `</button>`).join('');
    box.classList.remove('hidden');
    box.querySelectorAll('[data-service-key]').forEach((btn: HTMLButtonElement) => {
      btn.onmousedown=(e: MouseEvent) =>e.preventDefault();
  ...
  _getInfogareServiceEntry(key: unknown = this._infogareSelectedServiceKey) {
    if (!this._infogareServiceIndex?.length) this._rebuildInfogareServiceIndex();
    return this._infogareServiceIndex.find((x: { key: unknown }) => x.key === key) || null;
  }

  _selectInfogareService(key: unknown, showBoard: unknown = false) {
    ...
    if (currentStation && !entry.stationIds.includes(currentStation)) return false;
    if (!currentStation) {
      const first=entry.stationIds[0];
      if (first) this._selectInfogareStation(first, false);
    }
    this._infogarePage=0;
    if (showBoard) this._showInfogareBoard();
    return true;
  ...
  _commitInfogareServiceSearch(showBoard: unknown = true) {
    const input=document.getElementById('infogare-service');
    if (!input) return false;
    const typed=this._normalizeInfogareSearch(input.value);
  ...
  _infogareDateDiffDays(fromDate: unknown, toDate: unknown) {
  ...
  _infogareAddDays(dateStr: unknown, days: unknown) {
    const t = Date.parse(`${dateStr}T00:00:00Z`);
    if (!Number.isFinite(t)) return dateStr;
    return new Date(t + Number(days || 0) * 86400000).toISOString().slice(0, 10);
  }

  _infogareStationName(location: __KPM570) {
    if (!location) return '';
    if (location.stationId) return this.game.world.getStationById(location.stationId)?.name || location.name || '';
    return location.name || '';
  }

  _isInfogarePassengerCategory(category: unknown = '') {
    const c = String(category || '').trim().toUpperCase();
    if (!c) return true; // older passenger schedules often had no explicit category
    return ['PASSENGER','PASSAGER','PASSAGERS','VOYAGEUR','VOYAGEURS'].includes(c);
  }

  _isInfogarePassengerService(service: __KPM571) {
    if (!service) return false;
    const type = String(service.serviceType || '').trim().toLowerCase();
    if (type) {
      if (['fret','w','hlp','m-','evo','tm','infra','ttx','work'].includes(type)) return false;
      if (['passager','passenger','voyageur','voyageurs'].includes(type)) return true;
    }
    return this._isInfogarePassengerCategory(service.category || service.train?.category || '');
  }

  _collectInfogareV2Trains(stationId: unknown, currentDate: unknown, nowMin: number, horizonMin: number = 1440) {
    const runtime = this.game.scheduleV2Runtime;
    if (!runtime || !this.game.scheduleV2 || !this.game.rotationV2) return [];
    const rotations = runtime._runtimeRotations?.() || this.game.rotationV2.rotations || [];
    const liveById: Map<string, any> = new Map((this.game.scheduleCreator?.services || []).map((svc: __S3Struct638) => [String(svc.id), svc]));
    const rows: InfogareRow[] = [];
    ...
    for (const rotation of rotations) {
      ...
      for (const baseDate of baseDates) {
        let plans: unknown[][] = [];
        try { plans = runtime.planRotationForDate?.(rotation, baseDate) || []; }
        catch (err) { console.warn('Infogare V2 planning skipped:', err); continue; }
        const baseOffsetMin = this._infogareDateDiffDays(currentDate, baseDate) * 1440;
        for (const plan of plans) {
          if (!plan?.ver || !Array.isArray(plan.locations) || !plan.locations.length) continue;
          ...
          for (let i = 0; i < locs.length; i++) {
            ...
            if (!loc || loc.stationId !== stationId || loc.kind === 'PASS' || loc.type === 'passage' || loc.stopCode === 'NONE' && i > 0 && i < locs.length - 1) continue;
            const isFirst = i === 0;
            ...
            const waits = [depWaitMin, arrWaitMin].filter((v): v is number => Number.isFinite(v));
            if (!waits.some((v) => v >= -3 && v <= horizonMin)) continue;
            const waitMin = isDeparture ? depWaitMin : arrWaitMin;

            const servedStations: string[] = [];
            const servedStationIds: unknown[] = [];
            ...
            const fromStations: string[] = [];
            for (let j = 0; j < i; j++) {
              const n = locs[j]?.location;
              if (n?.stationId && (j===0 || n.stopCode!=='NONE') && n.type!=='passage' && n.kind!=='PASS') fromStations.push(this._infogareStationName(n));
            }
            const nextStops: Array<{ name: string; time: number | null }> = [];
            for (let j = i + 1; j < locs.length; j++) {
              const n = locs[j]?.location;
              if (!n?.stationId || j!==locs.length-1 && n.stopCode==='NONE' || n?.type==='passage' || n?.kind==='PASS') continue;
  ...
  _collectInfogareLegacyTrains(stationId: unknown, currentDate: unknown, nowMin: number, horizonMin: number = 1440) {
    const results: InfogareRow[] = [];
    for (const svc of this.game.scheduleCreator?.services || []) {
      if (!svc?.active || svc._v2OccurrenceId) continue;
      ...
      for (const run of runs) {
        ...
        for(let i=0;i<stops.length;i++) {
          const stop=stops[i];
          if(stop.stationId!==stationId || stop.type==='waypoint' || stop.type==='passage') continue;
          ...
          if(![depWaitMin,arrWaitMin].some(v=>v!=null&&v>=-3&&v<=horizonMin)) continue;
          ...
          const name=(id:unknown)=>String(this.game.world.getStationById(id)?.name||'?');
  ...
  _getInfogareTrains(stationId: unknown, mode: unknown, horizonMin: number = 1440) {
    const pt = this.game.engine.getParisTime();
    ...
    const passengerResults = results.filter((r: InfogareRow) => this._isInfogarePassengerCategory(r.category || ''));
    const wantArrivals = mode === 'sncf-arr' || mode === 'afl-arrivee' || mode === 'cati-ar';
    const filtered = passengerResults.filter((r: InfogareRow) => wantArrivals
      ...
      .map((r: InfogareRow): InfogareRow => ({ ...r, waitMin: wantArrivals ? (r.arrWaitMin ?? r.waitMin) : (r.depWaitMin ?? r.waitMin) }));
    // One physical/scheduled occurrence appears once even if it is already spawned in runtime.
    const seen = new Set();
    const unique: InfogareRow[] = [];
  ...
  _updateInfogareMeta(station: { name: unknown }, trains: Array<{ state?: string; isCancelled?: boolean; delay?: number }> = [], displayType: unknown = '') {
    const el=document.getElementById('infogare-live-meta'); if(!el) return;
    const esc=(v: unknown) =>this._escapeInfogareText(v);
    const live=(trains||[]).filter((t: { state?: string }) =>t.state && t.state!=='planned').length;
    const cancelled=(trains||[]).filter((t: { isCancelled?: boolean }) =>t.isCancelled).length;
    const delayed=(trains||[]).filter((t: { delay?: number }) =>Number(t.delay)>0).length;
  ...
  _showInfogareBoard(_refreshOnly: unknown = false) {
    const input = document.getElementById('infogare-station');
  ...
  _animateInfogareBoard(board: HTMLElement, displayType: string) {
  ...
  _updateInfogareClocks(board: HTMLElement) {
    const pt = this.game.engine.getParisTime();
  ...
  _rerRatpWaitText(waitMin: unknown) {
    if (!Number.isFinite(Number(waitMin))) return '';
    const raw = Number(waitMin);
    if (raw <= 0) return '0 min';
    const mins = Math.max(1, Math.ceil(raw));
    if (mins <= 59) return `${mins} min`;
    const h = Math.floor(mins / 60);
    const m = mins % 60;
    return `${h}:${String(m).padStart(2, '0')}`;
  }

  _rerRatpTopDirections(trains: InfogareRow[]) {
    const stats = new Map();
    let order = 0;
    for (const t of trains || []) {
      if (!t?.isDeparture || !t.destination) continue;
      const w = Number(t.waitMin);
      if (!Number.isFinite(w) || w < -1 || w > 1440) continue;
  ...
  _rerRatpIncidentText(stationId: unknown, trains: InfogareRow[]) {
    const im = this.game.incidentManager;
    const active = im?.getActiveIncidents?.() || [];
    if (!active.length) return 'Circulation normale.';
    const relevantStations = new Set([stationId]);
    const relevantServices = new Set();
    for (const t of trains || []) {
      if (!t?.isDeparture) continue;
      const w = Number(t.waitMin);
      if (!Number.isFinite(w) || w < -1 || w > 1440) continue;
      for (const id of t.servedStationIds || []) if (id) relevantStations.add(id);
      if (t.svcId) relevantServices.add(t.svcId);
    }
    const relevant = active.filter((inc: { active: unknown; serviceId: unknown; stationA: unknown; stationB: unknown }) => {
      if (!inc?.active) return false;
      if (inc.serviceId && relevantServices.has(inc.serviceId)) return true;
      return (inc.stationA && relevantStations.has(inc.stationA)) ||
             (inc.stationB && relevantStations.has(inc.stationB));
    });
    if (!relevant.length) return 'Circulation normale.';
    return relevant.slice(0,2).map((inc: { stationAName: unknown; stationBName: unknown; trackName: unknown; name: unknown }) => {
  ...
  _renderRerRatp(station: { id: unknown; name: unknown }, trains: InfogareRow[], nowStr: unknown, _page: unknown = 0) {
    const esc = (value: unknown) => String(value ?? '')
      .replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;')
      .replaceAll('"','&quot;').replaceAll("'",'&#39;');
    const all24h = (trains || []).filter((t: { isDeparture: unknown; waitMin: unknown }) => t?.isDeparture && Number.isFinite(Number(t.waitMin)) && Number(t.waitMin) >= -1 && Number(t.waitMin) <= 1440);
    ...
    const rows = Array.from({length:5}, (_: unknown,i) => {
      const t = use[i] || null;
    ...
    const directions = dirs.length
      ? dirs.map((d: unknown) => `<div>${esc(d)}</div>`).join('')
  ...
  _renderRerSncf(station: unknown, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    ...
    const all = trains.filter((t: { isDeparture: unknown }) => t.isDeparture);
    ...
    const ys: number[]=[32.1,39.4,46.6,53.8,61.0,68.2];
    for(let i=0;i<6;i++){
      const t=use[i], y=ys[i];
      const line=t?.lineCode || '';
      const code=t ? String(t.name || t.trainNumber || '').split(/\s+/)[0].slice(0,6).toUpperCase() : '';
      let wait='';
      if(t){ if(t.isCancelled)wait='supprimé'; else if(t.waitMin!<=1)wait="à l'approche"; else wait=t.waitMin!<60?`${Math.round(t.waitMin!)} min`:this._fmtWait(t.waitMin!); }
      ...
      if(t?.svcId) html += `<div class="ig-image-hit" data-svc-id="${htmlText(t.svcId)}" style="left:7.5%;width:40.5%;top:${htmlText(y)}%;height:6.4%"></div>`;
    }
    if(all.length>6) html += `<div class="ig-image-field ig-photo-page" style="left:43%;top:72.5%;width:4%;height:2%;font-size:${htmlText(5*scale)}px;color:#555;justify-content:flex-end">${page+1}/${Math.ceil(all.length/6)}</div>`;
    html += `</div>`;
    return html;
  }

  // --- SNCF DEPARTS (blue) --- exact replica with voie badges
  _renderSncfDep(station: { name: unknown }, trains: InfogareRow[], nowStr: unknown) {
    let rows = '';
    for (const t of trains) {
      ...
      rows += `<div class="ig-sncf-row" data-svc-id="${htmlText(t.svcId)}">
        <div class="ig-sncf-main">
          <span class="ig-sncf-time">${htmlText(this._fmtTime(t.depTime as number))}</span>
  ...
  _renderSncfArr(station: { name: unknown }, trains: InfogareRow[], nowStr: unknown) {
    let rows = '';
    for (const t of trains) {
      const m = (t.name || '').match(/^([A-Za-z]+)(.*)$/);
      const type = m ? m[1] : (t.seriesName || 'TER');
      ...
      rows += `<div class="ig-sncf-row" data-svc-id="${htmlText(t.svcId)}">
        <div class="ig-sncf-main">
          <span class="ig-sncf-time">${htmlText(this._fmtTime(t.arrTime as number))}</span>
          <span class="ig-sncf-trainid">
            <span class="ig-sncf-type">${htmlText(type || t.seriesName || 'TER')}</span>
            <span class="ig-sncf-trainnum">${htmlText(num || t.trainNumber || t.name)}</span>
          </span>
  ...
  _renderOldSncf(station: unknown, trains: InfogareRow[], nowStr: string) {
    let rows = '';
    for (const t of trains) {
      ...
      rows += `<div class="ig-solari-row" data-svc-id="${htmlText(t.svcId)}">
        <span class="ig-solari-cell ig-solari-time">${htmlText(this._fmtTime(t.depTime as number).replace('h', '.'))}</span>
  ...
  _renderFlashCirculation(station: unknown, nowStr: string) {
    const im=this.game.incidentManager;
    const bulletins=im?.getBulletins?.()||[];
    const services=this.game.scheduleCreator?.services||[];
    const delayed=services.filter((s: __KPM571 & { active?: boolean; delay: number }) =>s.active&&s.delay>=15&&this._isInfogarePassengerService(s)).slice(0,4);
    const pt=this.game.engine?.getParisTime?.();
    ...
    for (const w of works)items.push(`Travaux : ${w.name||w.type||'chantier'}`);
  ...
  _renderCATI3_3(station: Record<string, unknown> & { id?: string; name?: string }, trains: InfogareRow[], nowStr: unknown) {
    const dep = trains.filter((r: { isDeparture: unknown }) => r.isDeparture).slice(0, 6);
    const mid = Math.ceil(dep.length / 2);
    const left = dep.slice(0, mid);
    const right = dep.slice(mid);
    const cell = (t: __S3Struct645) => {
      ...
      return `<div class="ig-cati-33-row" data-svc-id="${htmlText(t.svcId)}">
        ...
        <span class="ig-cati-33-time">${htmlText(this._fmtTime(t.depTime as number))}</span>
        <span class="ig-cati-33-dest">${htmlText(t.destination)}</span>
      </div>`;
    };
    const col = (items: InfogareRow[]) => items.length ? items.map(cell).join('') : '<div class="ig-cati-empty">Aucun départ</div>';
    const first = dep[0];
    const topInfo = first ? `${this._fmtTime(first.depTime as number)} ${first.destination}` : 'Aucun train';
  ...
  _renderAFLDepart(station: Record<string, unknown> & { id?: string; name?: string }, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    const deps = trains.filter((r: { isDeparture: unknown }) => r.isDeparture);
    const t = deps[page] || deps[0];
    if (!t) return `<div class="ig-afl-board"><div class="ig-afl-station">${htmlText(station?.name || '')}</div><div class="ig-afl-msg">Aucun départ prévu</div></div>`;
    ...
    return `<div class="ig-afl-board" data-svc-id="${htmlText(t.svcId)}">
      ...
      <div class="ig-afl-line">${htmlText(this._fmtTime(t.depTime as number))} ${delay}</div>
  ...
  _renderAFLArrivee(station: Record<string, unknown> & { id?: string; name?: string }, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    const arrs = trains.filter((r: { isArrival: unknown }) => r.isArrival);
    const t = arrs[page] || arrs[0];
    if (!t) return `<div class="ig-afl-board ig-afl-arr"><div class="ig-afl-station">${htmlText(station?.name || '')}</div><div class="ig-afl-msg">Aucune arrivée prévue</div></div>`;
    ...
    return `<div class="ig-afl-board ig-afl-arr" data-svc-id="${htmlText(t.svcId)}">
      ...
      <div class="ig-afl-line">${htmlText(this._fmtTime(t.arrTime as number))} — ${htmlText(status)}</div>
      <div class="ig-afl-details">Train ${htmlText(t.name)} — Voie ${htmlText(t.voie || '—')}</div>
    </div>`;
  }

  // --- PLATFORM DISPLAY (click on a train) ---
  _showPlatformDisplay(svcId: unknown, stationId: unknown) {
    const svc = this.game.scheduleCreator?.services?.find((s: { id: unknown }) => s.id === svcId);
    const station = this.game.world.getStationById(stationId);
    if (!svc) {
      const row=(this._infogareVisibleTrains || []).find((r: { svcId: unknown }) =>r.svcId===svcId);
      if (row && station) { this._showPlannedInfogareDetail(row, station); }
      return;
    }
    ...
    const stopIdx = stops.findIndex((s: { stationId: unknown }) => s.stationId === stationId);
    const isGrandeLigne = (svc.rame?.maxSpeed || 0) >= 160;

    const servedAfter: PlatformStopLabel[] = [];
  ...
  _showPlannedInfogareDetail(row: __S3Struct651, station: __S3Struct652) {
    const board=document.getElementById('infogare-board'); if(!board) return;
    const pt=this.game.engine.getParisTime();
    ...
    const esc=(v: unknown) =>this._escapeInfogareText(v);
    const stops=(row.nextStops||[]).map((n: { name: unknown; time: unknown }) =>`<span>${esc(n.name)}${Number.isFinite(Number(n.time))?` <small>${esc(this._fmtTime(n.time as number))}</small>`:''}</span>`).join('');
    const vehicles=formation.map((v: { traction: unknown; number: unknown; name: unknown }) =>`<span class="ig-planned-vehicle${htmlText((v.traction&&v.traction!=='none')?' is-powered':'')}">${esc(v.number||v.name||'Véhicule')}</span>`).join('');
    ...
    board.innerHTML=`<div class="ig-planned-detail">
      ...
      <div class="ig-planned-main"><div><small>Départ prévu</small><b>${esc(this._fmtTime((row.depTime ?? row.arrTime) as number))}</b>${effective!=null?`<em>${esc(this._fmtTime(effective as number))} (+${htmlText(Math.round(Number(row.delay)))} min)</em>`:''}</div><div><small>Destination</small><b>${esc(row.destination||'?')}</b><em>${row.isCancelled?'SUPPRIMÉ':esc(row.state==='planned'?'Horaire planifié':'Service en cours')}</em></div></div>
  ...
  _renderPlatformGL(svc: ActiveService, station: Station, destStation: Station, servedAfter: PlatformStopLabel[], depTime: unknown, delayStr: unknown, nowStr: unknown, numCars: number) {
    const stopsHtml = servedAfter.map((s: { isLast: unknown; name: unknown }) =>
      `<div class="ig-pgl-stop ${htmlText(s.isLast ? 'ig-pgl-terminus' : '')}"><span class="ig-pgl-bullet">\u25CF</span>${htmlText(s.name)}</div>`
    ).join('');
    ...
    return `<div class="ig-pgl-board">
      ...
      <div class="ig-pgl-header">
        <div class="ig-pgl-left">
          ...
          <div class="ig-pgl-time">${htmlText(this._fmtTime(depTime as number))}</div>
  ...
  _renderPlatformBanlieue(svc: __S3Struct657, station: unknown, destStation: __S3Struct658, servedAfter: PlatformStopLabel[], depTime: number | null | undefined, delayStr: unknown, nowStr: unknown, numCars: number) {
    const half = Math.ceil(servedAfter.length / 2);
    const col1 = servedAfter.slice(0, half);
    const col2 = servedAfter.slice(half);
    const col1Html = col1.map((s: { isLast: unknown; name: unknown }) => `<div class="ig-pban-stop-item"><span class="ig-pban-stop-dot">\u25CF</span><span class="ig-pban-stop-name${htmlText(s.isLast ? ' ig-pgl-terminus' : '')}">${htmlText(s.name)}</span></div>`).join('');
    const col2Html = col2.map((s: { isLast: unknown; name: unknown }) => `<div class="ig-pban-stop-item"><span class="ig-pban-stop-dot">\u25CF</span><span class="ig-pban-stop-name${htmlText(s.isLast ? ' ig-pgl-terminus' : '')}">${htmlText(s.name)}</span></div>`).join('');

    let crowdHtml = '';
    const crowdClasses: string[] = ['ig-pban-car-green', 'ig-pban-car-green', 'ig-pban-car-orange', 'ig-pban-car-red'];
    const crowdIcons: string[] = ['\u{1F9CD}', '\u{1F9CD}', '\u{1F9CD}\u{1F9CD}', '\u{1F9CD}\u{1F9CD}\u{1F9CD}'];
  ...
  _renderCATIComplet(station: Record<string, unknown> & { id?: string; name?: string }, trains: InfogareRow[], nowStr: unknown) {
    const dep = trains.filter((r: { isDeparture: unknown }) => r.isDeparture).slice(0, 12);
    ...
    const cell = (t: __S3Struct661) => `<div class="ig-cati-row" data-svc-id="${htmlText(t.svcId)}">
      <span class="ig-cati-logo">SNCF</span>
      <span class="ig-cati-num">${htmlText(t.trainNumber || t.name.split(' ')[0] || t.name)}</span>
      <span class="ig-cati-time">${htmlText(this._fmtTime(t.depTime as number))}</span>
  ...
  _renderCATIAr(station: Record<string, unknown> & { id?: string; name?: string }, trains: InfogareRow[], nowStr: unknown) {
    const arr = trains.filter((r: { isArrival: unknown }) => r.isArrival).slice(0, 12);
    const cell = (t: __S3Struct664) => {
      let status = '';
      ...
      return `<div class="ig-cati-row" data-svc-id="${htmlText(t.svcId)}">
        ...
        <span class="ig-cati-time">${htmlText(this._fmtTime(t.arrTime as number))}</span>
  ...
  _renderEcranQuai(station: unknown, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    const deps = trains.filter((r: { isDeparture: unknown }) => r.isDeparture);
    ...
    const time=t?this._fmtTime(t.depTime as number):'';
    ...
    if(t?.svcId) html += `<div class="ig-image-hit" data-svc-id="${htmlText(t.svcId)}" style="left:0;top:7%;width:100%;height:83%"></div>`;
    if(deps.length>1) html += `<div class="ig-image-field ig-photo-page" style="left:2%;top:4%;width:12%;height:3%;color:#084a96;font-size:9px">${page+1}/${deps.length}</div>`;
    html += `</div>`;
    return html;
  }

  // --- Palette SNCF moderne (image annexe INFOGARE : 2 colonnes, horloge, défilant) ---
  _renderPalette(station: unknown, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    const perPage=20;
    const all=trains.filter((r: { isDeparture: unknown }) =>r.isDeparture).slice(page*perPage,page*perPage+perPage);
    ...
    const draw=(t: InfogareRow | null,base: number,y: number)=>{
      const type=t?(t.seriesName || (t.category==='PASSENGER'?'TER':t.category||'Train')).toUpperCase():'';
      const num=t?(t.trainNumber||''):'';
      const time=t?this._fmtTime(t.depTime as number).replace('h',':'):'';
      ...
      if(t?.svcId)h+=`<div class="ig-image-hit" data-svc-id="${htmlText(t.svcId)}" style="left:${htmlText(base)}%;width:48%;top:${htmlText(y)}%;height:5.2%"></div>`;
      return h;
    };
    ...
    if(trains.filter((r: { isDeparture: unknown }) =>r.isDeparture).length>perPage){
      const n=Math.ceil(trains.filter((r: { isDeparture: unknown }) =>r.isDeparture).length/perPage);
  ...
  _dbQuaiClock(min: unknown) {
    if (!Number.isFinite(Number(min))) return '--:--';
    const total=((Math.round(Number(min))%1440)+1440)%1440;
    return `${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`;
  }
  _dbQuaiIncidentText(stationId: unknown, stationIds: unknown[] = [], serviceId: unknown = '') {
    const active=this.game.incidentManager?.getActiveIncidents?.() || [];
    if (!active.length) return '';
    const ids=new Set([stationId,...stationIds].filter(Boolean));
    const relevant=active.filter((inc: { active: unknown; serviceId: unknown; stationA: unknown; stationB: unknown }) => inc?.active && ((serviceId && inc.serviceId===serviceId) || (inc.stationA && ids.has(inc.stationA)) || (inc.stationB && ids.has(inc.stationB))));
    return relevant.slice(0,2).map((inc: { stationAName: unknown; stationBName: unknown; trackName: unknown; name: unknown }) =>{
  ...
  _dbQuaiFormation(scheduleId: unknown, versionId: unknown, occurrenceId: unknown = '', liveServiceId: unknown = '') {
    const rm=this.game.rotationV2;
    const live=liveServiceId ? (this.game.scheduleCreator?.services || []).find((s: { id: unknown }) =>s.id===liveServiceId) : null;
    let members=(live?._v2FormationMembers || []).map((m: { vehicleId: unknown; role: unknown },i: unknown)=>({vehicleId:m.vehicleId,role:m.role,order:i}));
    if (!members.length && rm) {
      for (const rot of rm.rotations || []) {
        const occ=(rot.occurrences || []).find((o: { id: unknown; scheduleId: unknown }) => (occurrenceId && o.id===occurrenceId) || (!occurrenceId && o.scheduleId===scheduleId));
        if (occ?.formation?.members?.length) { members=occ.formation.members.map((m: Record<string, unknown>) =>({...m})); break; }
      }
    }
    if (!members.length && rm) {
      const a=rm.getDirectAssignment?.(scheduleId,versionId);
      if (a?.formation?.members?.length) members=a.formation.members.map((m: Record<string, unknown>) =>({...m}));
    }
    const vehicles=members.sort((a: __S3Struct668,b: __S3Struct669)=>(a.order||0)-(b.order||0)).map((m: __S3Struct670) =>{
    ...
    if (vehicles.length) {
      ...
      if (leadAtStart && !leadAtEnd) vehicles.reverse();
      return vehicles;
    }
    const direct=rm?.getDirectAssignment?.(scheduleId,versionId);
    const rameId=direct?.rameId || '';
    const rame=rameId ? this.game.rameManager?.getById?.(rameId) : null;
    return (rame?.elementDetails || []).map((e: { elementId: unknown; number: unknown; serialNumber: unknown; name: unknown; category: unknown; traction: unknown; length: unknown },i: unknown)=>({id:e.elementId||String(i),number:e.number||e.serialNumber||'',name:e.name||'',role:(e.category==='locomotive'||e.category==='automotrice')?'LEAD':'COACH',category:e.category||'',traction:e.traction||'none',lengthM:(Number.isFinite(Number(e.length))&&Number(e.length)>0?Number(e.length):25)}));
  }
  _getDbQuaiData(stationId: unknown, serviceKey: unknown) {
    const entry=this._getInfogareServiceEntry(serviceKey);
    if (!entry) return { error:'Sélectionnez un service.' };
    const pt=this.game.engine.getParisTime();
    const nowMin=pt.hours*60+pt.minutes+Number(pt.seconds||0)/60;
    const currentDate=this.game.engine.currentDate || this.game.engine.getParisDate?.() || '';
    if (entry.kind==='v2') {
      const rec=this.game.scheduleV2?.getSchedule?.(entry.id); const ver=rec?.currentVersion;
      if (!rec || !ver?.locations?.length) return {error:'Horaire introuvable.'};
      const idx=ver.locations.findIndex((l: { stationId: unknown }) =>l?.stationId===stationId); if (idx<0) return {error:'Ce service ne dessert pas cette gare.'};
      const loc=ver.locations[idx]; const last=[...ver.locations].reverse().find((l) =>l?.stationId);
      const middle=ver.locations.slice(idx+1).filter((l: { stationId: unknown }) =>l?.stationId && l.stationId!==last?.stationId);
      ...
      const rawDelay=Number(row?.delay); const delay=Number.isFinite(rawDelay)?Math.max(0,rawDelay):0; const servedIds=ver.locations.slice(idx+1).map((l: { stationId: unknown }) =>l?.stationId).filter(Boolean); const serviceId=row?.svcId || '';
      return {serviceName:String(rec.name || rec.number || 'Service'),scheduledTime:this._dbQuaiClock(plannedMin),updatedTime:delay>0 ? this._dbQuaiClock(plannedMin+delay) : '',delay,destination:this._infogareStationName(last) || last?.name || '?',stops:middle.map((l: __KPM570) =>this._infogareStationName(l) || l.name || '').filter(Boolean),traffic:this._dbQuaiIncidentText(stationId,servedIds,serviceId),formation:this._dbQuaiFormation(rec.id,ver.id,row?.occurrenceId||'',serviceId),svcId:serviceId};
    }
    const svc=(this.game.scheduleCreator?.services || []).find((s: { id: unknown }) =>s.id===entry.id); if (!svc) return {error:'Service introuvable.'};
    const stops=svc.getCurrentStops?.() || svc.stops || []; const idx=stops.findIndex((s: { stationId: unknown }) =>s?.stationId===stationId); if (idx<0) return {error:'Ce service ne dessert pas cette gare.'};
    const loc=stops[idx], last=stops.at(-1); const middle=stops.slice(idx+1,-1).filter((s: { stationId: unknown }) =>s?.stationId).map((s: { stationId: unknown }) =>this.game.world.getStationById(s.stationId)?.name||'').filter(Boolean);
    const planned=Number(loc.departureTime ?? loc.arrivalTime ?? 0); const rawDelay=Number(svc.delay); const delay=Number.isFinite(rawDelay)?Math.max(0,rawDelay):0; const dest=this.game.world.getStationById(last?.stationId)?.name || '?'; const servedIds=stops.slice(idx+1).map((s: { stationId: unknown }) =>s.stationId).filter(Boolean);
    const formation=(svc.rame?.elementDetails || []).map((e: { elementId: unknown; number: unknown; serialNumber: unknown; name: unknown; category: unknown; traction: unknown; length: unknown },i: unknown)=>({id:e.elementId||String(i),number:e.number||e.serialNumber||'',name:e.name||'',role:(e.category==='locomotive'||e.category==='automotrice')?'LEAD':'COACH',category:e.category||'',traction:e.traction||'none',lengthM:(Number.isFinite(Number(e.length))&&Number(e.length)>0?Number(e.length):25)}));
  ...
  _renderDbAbfahrt(station: unknown, trains: InfogareRow[], nowStr: unknown, page: number = 0) {
    ...
    html += `</div>`;
    if(all.length>10)html+=`<div class="ig-image-field ig-photo-page" style="left:63%;top:59%;width:4%;height:2.5%;font-size:${htmlText(7*scale)}px;color:#fff;justify-content:flex-end">${page+1}/${Math.ceil(all.length/10)}</div>`;
    html += `</div>`;
    return html;
  }
  ...
  renderDashboard() {
    try {
      ...
      this._dashboardInterval = setInterval(() => {
        ...
        if (document.querySelector('[data-re-interacting="true"]') || active?.closest?.('.re-chart-controls,.re-financial-panel') || document.querySelector('.re-financial-panel details[open]')) return;
  ...
  openConnectionsDialog() {
    ...
    const modal=document.createElement('div');modal.id='re-connections-dialog';
    modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');
    modal.setAttribute('aria-label','Correspondances voyageurs');modal.tabIndex=-1;
    ...
    const close=()=>{modal.remove();if(previous instanceof HTMLElement)previous.focus();};
    ...
    this.game.connections.render(modal.querySelector('[data-connections-content]') as HTMLElement,this.game);
    modal.focus();
  }
  ...
  openCargoTypeModal() {
    ...
    if (catSel) {
      catSel.innerHTML = Object.entries(this.game.cargoTypes.categories)
        .map(([key, cat]) => `<option value="${htmlText(key)}">${htmlText((cat as { name: string }).name)}</option>`).join('')
  ...
  renderMarketingPage() {
    ...
    } catch(e) { console.warn('Marketing render error:', e); }
  }

  _igPhotoMask(color: unknown, alpha: unknown = 0.90) {
  ...
  _igPhotoFrame(file: unknown, width: number, height: unknown, scale: __KPM593 = 1, extraClass: unknown = '') {
  ...
  _renderImageMode(displayType: string, station: Station, trains: InfogareRow[], nowStr: string, page: number = 0) {
    const layout = IG_IMAGE_LAYOUTS[displayType as keyof typeof IG_IMAGE_LAYOUTS] as InfogareImageLayout | undefined;
    if (!layout) return '';
    ...
    for (const f of layout.headerFields || []) {
      ...
      else continue; // static labels/logos already exist in the supplied image
    ...
    const mask = (spec: InfogareFieldSpec | undefined, bg: unknown, y: number, content: unknown = '', extra: InfogareFieldExtra = {}) => {
      if (!spec) return '';
    ...
    for (let i = 0; i < perPage; i++) {
      ...
      if (b.voie) html += mask(b.voie,rowBg,b.y + (b.voie?.yOff || 0),t.voie || '',{color:b.voie.color || '#fff',weight:900,align:b.voie.align || 'center',bg:b.voie.bg || rowBg,alpha:0.94});
      if (b.remark) html += mask(b.remark,rowBg,b.remarkY ?? b.y,remark,{color:b.remark?.color || '#facc15',weight:b.remark?.weight});
      if (t.svcId) html += `<div class="ig-image-hit" data-svc-id="${htmlText(t.svcId)}" style="top:${htmlText(b.y)}%;height:${htmlText(b.h)}%"></div>`;
    }
  ...
  _infogareAflCommercialType(t: Record<string, unknown> = {}) {
    ...
    if (known) return known[1].toUpperCase().replace('INTERCITES','INTERCITÉS').replace('S BAHN','S-BAHN');
    return 'TRAIN';
  }

  _infogareAflTrainNumber(t: Record<string, unknown> = {}) {
    if (String(t.trainNumber || '').trim()) return String(t.trainNumber).trim();
    const m = String(t.name || '').match(/\b(\d{2,7})\b/);
    return m?.[1] || '';
  }

  _getInfogareBitmapImage(key: string) {
    const cfg = (INFOGARE_BITMAP_FONTS as any)[key];
    if (!cfg) return null;
    this._infogareBitmapImages ||= new Map();
    if (this._infogareBitmapImages.has(key)) return this._infogareBitmapImages.get(key);
    const img = new Image();
    const rec = { img, loaded:false, failed:false, waiters:[] };
    img.onload = () => { rec.loaded=true; const q=rec.waiters.splice(0); q.forEach((fn: () => void) =>fn()); };
  ...
  _paintInfogareBitmapFields(board: HTMLElement) {
    if (!board) return;
    board.querySelectorAll('canvas.ig-bitmap-field').forEach((canvas) => {
      ...
      if (rec.loaded || rec.img.complete && rec.img.naturalWidth) { rec.loaded=true; paint(); }
      else rec.waiters.push(paint);
    });
  }

  _paintInfogareBitmapField(canvas: HTMLCanvasElement) {
    if (!canvas?.isConnected) return;
    const key=canvas.dataset.bitmapFont || 'regular';
    const cfg=(INFOGARE_BITMAP_FONTS as any)[key];
    const rec=this._getInfogareBitmapImage(key);
    if (!cfg || !rec?.loaded) return;
    ...
    let total=0;
    for (const ch of text){const g=glyphs[ch]||fallback;if(g)total+=Number(g[4]||0)*scale;}
    ...
    for (const ch of text){
      const g=glyphs[ch]||fallback;if(!g)continue;
  ...
  _igField(spec: InfogareFieldSpec, content: unknown, extra: InfogareFieldExtra = {}, yOff: number = 0) {
    if (!spec) return '';
    const common = [
...
// S3_STRUCT_V2_TEMP
type __S3Struct465 = { "test": (...args: unknown[]) => unknown };
type __S3Struct469 = { "id": string };
type __S3Struct474 = { "position": { "lat": number; "lon": number } };
type __S3Struct485 = { "classList": { "remove": (...args: unknown[]) => unknown } };
type __S3Struct486 = { "classList": { "remove": (...args: unknown[]) => unknown } };
type __S3Struct487 = { "lat": number; "lon": number };
type __S3Struct488 = { "dist": number };
type __S3Struct489 = { "dist": number };
type __S3Struct491 = { "name": string };
type __S3Struct492 = { "id": string; "name": string; "dist": number };
type __S3Struct493 = { "stops": unknown[] };
type __S3Struct494 = { "id": string; "name": string; "code": string; "stops": unknown[] };
type __S3Struct495 = { "trim": (...args: unknown[]) => unknown };
type __S3Struct496 = { "id": string; "name": string };
type __S3Struct498 = { "id": string; "name": string; "code": string };
type __S3Struct505 = { "toLowerCase": (...args: unknown[]) => unknown };
type __S3Struct506 = { "trim": (...args: unknown[]) => unknown };
type __S3Struct522 = { "instanceName": string; "name": string };
type __S3Struct531 = { "id": string; "from": unknown; "to": unknown; "cargoName": string; "quantity": unknown; "unit": unknown; "payment": { "toLocaleString": (...args: unknown[]) => unknown } };
type __S3Struct532 = { "length": number };
type __S3Struct534 = { "lon": number; "lat": number };
type __S3Struct535 = { "lon": number; "lat": number };
type __S3Struct537 = { "arrTimeMin": number; "voiePointId": string; "platform": unknown; "type": string; "stationId": string; "arrTimeStr": unknown; "depTimeStr": unknown; "depTimeMin": number; "stationName": string; "stopCode": string };
type __S3Struct548 = { "voie": unknown; "stationId": string; "id": number; "lat": { "toFixed": (...args: unknown[]) => unknown }; "lon": { "toFixed": (...args: unknown[]) => unknown }; "vpId": string; "name": string; "closed": unknown };
type __S3Struct549 = { "trim": (...args: unknown[]) => unknown };
type __S3Struct552 = { "id": string };
type __S3Struct553 = { "id": string };
type __S3Struct554 = { "name": string };
type __S3Struct555 = { "name": string };
type __S3Struct562 = { "name": string };
type __S3Struct563 = { "name": string };
type __S3Struct564 = { "name": string };
type __S3Struct565 = { "type": string; "closed": unknown; "lat": { "toFixed": (...args: unknown[]) => unknown }; "lon": { "toFixed": (...args: unknown[]) => unknown }; "platforms": unknown; "id": string; "name": string };
type __S3Struct567 = { "stops": unknown[]; "trackIds": unknown[]; "name": string };
type __S3Struct570 = { "from": { "lat": number; "lon": number }; "maxSpeed": number };
type __S3Struct571 = { "name": string; "fromStationName": string; "toStationName": string; "distance": number; "maxSpeed": number; "electrified": boolean; "id": string };
type __S3Struct572 = { "id": string };
type InfogareServiceIndexRow = {
  key: string;
  kind: 'v2' | 'legacy';
  id: string;
  name: string;
...
type InfogareRow = Record<string, unknown> & {
  ...
  fromStations: string[];
  nextStops: Array<{ name: string; time: number | null }>;
  delay: number;
  delayReason: string;
  isCancelled: boolean;
  isFull: boolean;
...
type __S3Struct573 = { "rame": { "id": string }; "completed": number; "cancelled": number; "state": string };
type __S3Struct575 = { "depotId": string };
type __S3Struct576 = { "currentLocation": { "depotId": string } };
type __S3Struct577 = { "cleanliness": { "exterior": unknown; "interior": unknown } };
type __S3Struct579 = { "label": unknown; "amount": unknown };
type __S3Struct582 = { "category": string };
type __S3Struct583 = { "instanceName": string; "name": string; "seriesName": string; "category": string; "homeDepotId": string; "elementId": string };
type __S3Struct584 = { "name": string; "vehicleIds": unknown[]; "homeDepotId": string; "id": string };
type __S3Struct586 = { consumables: Record<string, number | undefined>; pendingDefects: string[]; wearLevel: number; kmSinceLastMaint: number; cleanliness: { exterior: number; interior: number }; id: string };
type __S3Struct588 = { "currentLocation": { "depotId": string } };
type __S3Struct591 = { "currentLocation": { "depotId": string } };
type __S3Struct595 = { "stationId": string; "name": string; "getTypeLabel": (...args: unknown[]) => unknown; "iteCargoTypes": unknown[]; "tracks": number; "location": { "lat": number; "lon": number }; "id": string };
type __S3Struct598 = { "elementId": string };
type __S3Struct600 = { "homeDepotId": string };
type __S3Struct601 = { "serviceName": string; "remainingMin": number };
type __S3Struct602 = { "rameName": string; "remainingMin": number };
type __S3Struct603 = { "from": { "lat": number; "lon": number }; "maxSpeed": number };
type __S3Struct604 = { "id": string };
type __S3Struct605 = { "duration": number; "remaining": number; "startTime": unknown; "locationText": unknown; "trackName": string; "stationAName": string; "stationBName": string; "serviceId": string; "trainName": string; "trainId": string; "stationA": unknown; "stationB": unknown; "effect": string; "speedLimit": unknown; "source": string; "triggerText": unknown; "weatherLevel": unknown; "name": string; "id": string };
type __S3Struct613 = { "createdDate": number };
type __S3Struct621 = { "pointA": unknown; "pointB": unknown; "distance": number; "id": string };
type __S3Struct622 = { "id": string };
type __S3Struct623 = { "phase": string; "total": number; "done": number; "ways": number; "troncons": unknown; "round": unknown; "stations": unknown[]; "failed": boolean };
type __S3Struct624 = { "lat": number; "lon": number };
type __S3Struct626 = { "lat": number; "lon": number; "voie": unknown };
type __S3Struct627 = { "name": string };
type __S3Struct629 = { "startsWith": (...args: unknown[]) => unknown };
type __S3Struct630 = { "score": number; "station": { "name": string } };
type __S3Struct631 = { "score": number; "station": { "name": string } };
type __S3Struct633 = { "stationIds": unknown[]; "search": unknown[]; "name": string; "number": unknown };
type __S3Struct634 = { "score": number; "name": string };
type __S3Struct635 = { "score": number; "name": string };
type __S3Struct638 = { "id": string };
type __S3Struct639 = { "isArrival": boolean; "arrTime": unknown; "arrWaitMin": number; "waitMin": number; "isDeparture": boolean; "depTime": unknown; "depWaitMin": number };
type __S3Struct645 = { "servedStations": unknown[]; "svcId": string; "depTime": unknown; "destination": unknown };
type __S3Struct651 = { "scheduleId": string; "versionId": string; "occurrenceId": string; "svcId": string; "nextStops": Array<{ name: unknown; time: unknown }>; "delay": number; "depTime": unknown; "arrTime": unknown; "name": string; "trainNumber": unknown; "voie": unknown; "destination": unknown; "isCancelled": boolean; "state": string };
type __S3Struct652 = { name: unknown; time: unknown };
type __S3Struct657 = { "train": { "color": string; "seriesName": unknown[]; "platform": unknown }; "state": string; "name": string };
type __S3Struct658 = { "name": string };
type __S3Struct661 = { "svcId": string; "trainNumber": unknown; "name": string; "depTime": unknown; "destination": unknown };
type __S3Struct664 = { "isCancelled": boolean; "delay": number; "fromStations": unknown[]; "svcId": string; "arrTime": unknown; "origin": unknown };
type __S3Struct668 = { "order": number };
type __S3Struct669 = { "order": number };
type __S3Struct670 = { "vehicleId": string; "role": unknown };
