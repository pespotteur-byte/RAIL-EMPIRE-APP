...
export class HeadquartersPage {
    ...
    private row(row: FleetRow, old?: HTMLElement): HTMLElement {
        ...
        const strip = (node.querySelector('.qg-consist') as HTMLElement);
        ...
        if (this.compositions.get(strip) !== signature) {
            ...
            for (const e of row.elements) {
                ...
                if (src) {
                    const img = document.createElement('img'); img.src = src; img.alt = label; img.title = label;
                    img.loading = 'lazy'; img.decoding = 'async'; if (e.flipped) img.style.transform = 'scaleX(-1)';
                    fragment.appendChild(img);
                } else { const missing = document.createElement('span'); missing.className = 'qg-missing-sprite'; missing.textContent = label; fragment.appendChild(missing); }
            }
            if (!row.elements.length) { const empty = document.createElement('span'); empty.textContent = 'Composition non disponible'; fragment.appendChild(empty); }
            strip.replaceChildren(fragment); this.compositions.set(strip, signature); strip.scrollLeft = scroll;
        }
        return node;
    }
    refresh(reset = false): void {
        ...
        setText(this.root, '#qg-active', String(d.rows.length)); setText(this.root, '.qg-company', d.company); setText(this.root, '.qg-clock', d.clock);
        ...
        (this.root.querySelector('#qg-empty') as HTMLElement).hidden = !!rows.length;
        this.list.setRows(rows, reset);
    }
    setActive(active: boolean): void {
        if (this.timer) { clearInterval(this.timer); this.timer = null; }
        if (active) { this.refresh(); this.timer = setInterval(() => this.refresh(), 1000); }
    }
