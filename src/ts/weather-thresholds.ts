 ...
 */
export interface WeatherRailLevel { id:string; label:string; rank:number; color:string; }
export type WeatherThresholdDirection = 'high'|'low';
export interface WeatherThreshold { label:string; unit:string; direction:WeatherThresholdDirection; bands:number[]; effects:string[]; }
export interface RailWeatherState { [key:string]: unknown; precipitation?:number; rain?:number; snowfall?:number; rain6hMm?:number; precipitation6h?:number; rain24hMm?:number; precipitation24h?:number; snowDepthCm?:number; windSpeed?:number; windGust?:number; visibilityM?:number; visibility?:number; temperature?:number; weatherCode?:number; humidity?:number; type?:string; }
export interface RailWeatherMetric { id:string; value:unknown; rank:number; level:WeatherRailLevel; [key:string]:unknown; }
export interface RailWeatherEvaluation { rank:number; level:WeatherRailLevel; metrics:RailWeatherMetric[]; tags:string[]; reasons:string[]; brakeFactor:number; speedCap:number; hazards:Record<string,number>; }

export const WEATHER_RAIL_LEVELS: WeatherRailLevel[] = [
...
export const WEATHER_RAIL_THRESHOLDS: Record<string, WeatherThreshold> = {
  wind_kmh: {
...
function bandLevel(value: unknown, cfg: WeatherThreshold): number {
  const n=Number(value);
  if(!Number.isFinite(n)) return 0;
  if(cfg.direction==='low') {
    let rank=0;
    for(let i=0;i<cfg.bands.length;i++) if(n <= cfg.bands[i]) rank=i+1;
    return rank;
  }
  let rank=0;
  for(let i=0;i<cfg.bands.length;i++) if(n >= cfg.bands[i]) rank=i+1;
...
function metric(id: string, value: unknown, extra: Record<string, unknown> = {}): RailWeatherMetric {
 ...
 */
export function evaluateRailWeather(state: RailWeatherState = {}, trainSpeedKmh = 0, realism = 1): RailWeatherEvaluation {
  ...
  let forcedRank=0;
  const tags=[];
  if([95,96,99].includes(code)){ forcedRank=Math.max(forcedRank,3); tags.push('Orage'); }
  if([56,57,66,67].includes(code)){ forcedRank=Math.max(forcedRank,3); tags.push('Pluie verglaçante'); }
  if(code===48){ forcedRank=Math.max(forcedRank,2); tags.push('Brouillard givrant'); }
  if([82,86].includes(code)){ forcedRank=Math.max(forcedRank,3); tags.push('Averses violentes'); }
  ...
  if(snow>=0.2 || state.type==='snow' || [71,73,75,77,85,86].includes(code)) {
    ...
    brakeFactor=Math.min(brakeFactor,1-(1-base)*mult);
    if(Number(trainSpeedKmh)>=140) speedCap=Math.min(speedCap, Math.max(80,Number(trainSpeedKmh)-20*mult));
  ...
  if(visM<=1000 || state.type==='fog') {
    ...
    brakeFactor=Math.min(brakeFactor,1-(1-base)*mult);
    if(visM<=200) speedCap=Math.min(speedCap,Math.max(40,120-30*mult));
    else if(visM<=500) speedCap=Math.min(speedCap,Math.max(60,160-20*mult));
    reasons.push(`visibilité ${Math.round(visM)} m`);
  }
  if(gust>=100) speedCap=Math.min(speedCap,Math.max(60,120-20*mult));
  if(gust>=120) speedCap=Math.min(speedCap,Math.max(30,80-20*mult));
  if([95,96,99,82].includes(code) || state.type==='storm') brakeFactor=Math.min(brakeFactor,1-(1-0.60)*mult);
  ...
  const hazards={
    ...
    rollingStock: Math.min(1,Math.max(
      Number.isFinite(temp)&&temp>=40 ? (temp-37)/15 : 0,
      Number.isFinite(temp)&&temp<=-12 ? (-temp-8)/25 : 0,
      snow>=3 ? snow/8 : 0, freezingRain?0.85:0
    )),
  };
