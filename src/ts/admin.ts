...
import { AuxiliaryJsonStorage, AUXILIARY_KEYS, type AuxiliaryKey } from './storage-auxiliary.js';
const adminStorage = new AuxiliaryJsonStorage();
const adminSnapshots = new Map<string, unknown>();
let adminReady = false;
let adminWriting = false;
function readAdminSnapshot(key:string):unknown {
  const value=adminSnapshots.get(key) ?? [];
  return JSON.parse(JSON.stringify(value)) as unknown;
}
async function persistAdmin(key:AuxiliaryKey,value:unknown):Promise<boolean> {
  try { await adminStorage.save(key,value);adminSnapshots.set(key,JSON.parse(JSON.stringify(value)) as unknown);return true; }
  catch(error){alert(error instanceof Error?error.message:String(error));return false;}
}
...
function adminMutation(action:()=>Promise<void>):()=>Promise<void> {
  return async()=>{if(!adminReady||adminWriting)return;adminWriting=true;
    try{await action();}catch(error){alert(error instanceof Error?error.message:String(error));}
    finally{adminWriting=false;}};
}

/** Admin application: TypeScript source; js/admin.js is compiler output.
 * This entry also passes its own typecheck WITHOUT legacy DOM compatibility. */
import type { CatalogEntry } from './catalog-contracts.js';
import { catalogRecord } from './catalog-contracts.js';

interface AdminIncident {
  id: string; name: string; description: string; effect: string; severity: string;
  duration: number; probability: number; hourStart: number; hourEnd: number;
}
interface ZipEntry { dir: boolean; async(type: 'blob'): Promise<Blob>; }
interface ZipLibrary { loadAsync(data: Blob): Promise<{ files: Record<string, ZipEntry> }>; }
declare global {
  interface Window {
...
type AdminElementId = keyof typeof ADMIN_TAGS;
type AdminElement<K extends AdminElementId> = HTMLElementTagNameMap[(typeof ADMIN_TAGS)[K]];
function byId<K extends AdminElementId>(id: K): AdminElement<K> {
  ...
  return element as AdminElement<K>;
}
...
function catalogEntries(value: unknown): CatalogEntry[] {
  ...
  for (const raw of value) {
    ...
    const entry = { ...source, id: source.id } as CatalogEntry;
    for (const key of ['name','seriesName','category','traction','imageData','_source'] as const) {
      if (source[key] != null) entry[key] = String(source[key]);
    }
    for (const key of ['maxSpeed','power','mass','length','passengerCapacity','freightCapacity','purchasePrice'] as const) {
      if (source[key] != null) { const value = Number(source[key]); entry[key] = Number.isFinite(value) ? value : 0; }
    }
    if (source.cargoTypes != null) entry.cargoTypes = Array.isArray(source.cargoTypes) ? source.cargoTypes.filter((x): x is string => typeof x === 'string') : [];
    entries.push(entry);
  }
  return entries;
}
function readCatalogStorage(key: string): CatalogEntry[] { return catalogEntries(readStoredJson(key)); }
function readStringStorage(key: string): string[] {
  const value = readStoredJson(key);
  return Array.isArray(value) ? value.filter((x): x is string => typeof x === 'string') : [];
}
function readIncidents(): AdminIncident[] {
  const value = readStoredJson('admin_incidents');
  if (!Array.isArray(value)) return [];
  return value.flatMap(raw => {
    const item = catalogRecord(raw);
    if (!item || typeof item.id !== 'string') return [];
    const num = (key: string, fallback: number) => Number.isFinite(Number(item[key])) ? Number(item[key]) : fallback;
...
let allItems: CatalogEntry[] = [];
let filteredItems: CatalogEntry[] = [];
let currentPage = 0;
let pageSize = 500;
let sortCol = 'name';
let sortAsc = true;
const selectedIds = new Set<string>();
let editingItem: CatalogEntry | null | undefined = null;
...
function loadCatalogs() {
  ...
  if (Array.isArray(CATALOG)) {
    for (const c of CATALOG) allItems.push({...c, _source: c._source || 'MLG'});
  }
  // Pack RE
  if (Array.isArray(CATALOG_PACK_RE)) {
    for (const c of CATALOG_PACK_RE) allItems.push({...c, _source: c._source || 'PACK RE'});
  }
  ...
  for (const item of readCatalogStorage('admin_catalog_imported')) allItems.push(item);
...
function updateStats() {
  ...
  for (const item of allItems) {
    ...
    if (item.category === 'locomotive' || item.category === 'automotrice') locomotives++;
    if (item.category === 'wagon' || item.category === 'voiture') wagons++;
  }
  ...
  all.value = ''; all.textContent = 'Toutes sources'; fragment.appendChild(all);
...
function applyFilters() {
  ...
  filteredItems = allItems.filter(i => {
    ...
    if (src && (i._source || '') !== src) return false;
    if (trac && (i.traction || '') !== trac) return false;
    if (q) {
...
byId('page-prev').addEventListener('click', () => { if (currentPage > 0) { currentPage--; renderPage(); byId('table-wrap').scrollTop = 0; }});
byId('page-next').addEventListener('click', () => { const tp = Math.ceil(filteredItems.length / pageSize); if (currentPage < tp-1) { currentPage++; renderPage(); byId('table-wrap').scrollTop = 0; }});
...
byId('select-all').addEventListener('change', e => {
  ...
  const end = Math.min(start + pageSize, filteredItems.length);
  for (let i = start; i < end; i++) {
    if (byId('select-all').checked) selectedIds.add(filteredItems[i].id);
    else selectedIds.delete(filteredItems[i].id);
  }
  renderPage();
});
...
byId('btn-delete-selected').addEventListener('click', adminMutation(async () => {
  if (selectedIds.size === 0) return alert('Aucun engin sélectionné');
  if (!confirm(`Supprimer ${selectedIds.size} engin(s) ?`)) return;
  const deletedIds = new Set(readStringStorage('admin_catalog_deleted'));
  for (const id of selectedIds) deletedIds.add(id);
  if(!await persistAdmin('admin_catalog_deleted',[...deletedIds]))return;
...
// ============ EDIT MODAL ============
window._editItem = function(id: string) {
...
byId('edit-save').addEventListener('click', adminMutation(async () => {
  ...
  if (idx >= 0) mods[idx] = updatedItem; else mods.push(updatedItem);
  if(!await persistAdmin('admin_catalog_mods',mods))return;
...
let importedEntries: CatalogEntry[] = [];
...
dropZone.addEventListener('drop', e => { e.preventDefault(); dropZone.classList.remove('drag'); const file = e.dataTransfer?.files[0]; if (file) void processImportFile(file); });
importFile.addEventListener('change', () => { const file = importFile.files?.[0]; if (file) void processImportFile(file); });
...
async function processImportFile(file: File) {
  ...
  try {
    if (!window.JSZip) throw new Error('Le module ZIP n’a pas pu être chargé.');
    ...
    // Group by folder
    const folders: Record<string, Array<{path: string; fileName: string}>> = {};
    ...
    if (importedEntries.length > 50) html += `<tr><td colspan="8" style="text-align:center;color:var(--text2)">...et ${importedEntries.length-50} autres</td></tr>`;
    html += '</tbody></table>';
    wrap.innerHTML = html;
...
function autoDetectSpecs(folder: string, name: string, lengthM: number) {
  ...
  if (/^z/i.test(nl)) return {category:'wagon',traction:'none',power:0,vmax:100,mass:24,freightCapacity:65,price:6500,cargoTypes:['hydrocarbures']};
  if (/^s[gn]/i.test(nl)) return {category:'wagon',traction:'none',power:0,vmax:120,mass:22,freightCapacity:60,price:6000,cargoTypes:['conteneurs']};
  if (/^e/i.test(nl) && lengthM < 20) return {category:'wagon',traction:'none',power:0,vmax:100,mass:20,freightCapacity:55,price:5500,cargoTypes:['charbon','minerai']};
  if (/^h/i.test(nl)) return {category:'wagon',traction:'none',power:0,vmax:120,mass:30,freightCapacity:25,price:2500,cargoTypes:['véhicules']};
  if (/^f/i.test(nl)) return {category:'wagon',traction:'none',power:0,vmax:100,mass:20,freightCapacity:60,price:6000,cargoTypes:['céréales','sable']};
  if (/^t/i.test(nl) && lengthM < 20) return {category:'wagon',traction:'none',power:0,vmax:100,mass:20,freightCapacity:60,price:6000,cargoTypes:['céréales']};
...
byId('import-confirm').addEventListener('click', adminMutation(async () => {
  ...
  if(!await persistAdmin('admin_catalog_imported',imported))return;
...
document.querySelectorAll('.modal-overlay').forEach(m => {
  m.addEventListener('click', (e: Event) => { if (e.target === m) m.classList.remove('active'); });
});

// ============ INCIDENTS MANAGEMENT ============
let incidents: AdminIncident[] = [];
let editingIncident: AdminIncident | null | undefined = null;
...
function renderIncidentsList() {
  ...
  for (const inc of incidents) {
    const effectLabels: Record<string, string> = {speed_reduction:'Vitesse -50%',line_blocked:'Ligne bloquée',extra_cost:'Surcoût +30%',delay:'Retard +15min',revenue_bonus:'Bonus +20%'};
    const sevColors: Record<string, string> = {low:'var(--green)',medium:'var(--orange)',high:'var(--red)'};
    html += `<tr>
...
window._editIncident = function(id: string) {
...
window._deleteIncident = function(id: string) {
  void adminMutation(async()=>{
    if (!confirm('Supprimer cet incident ?')) return;
    const next = incidents.filter(i => i.id !== id);
    if(!await persistAdmin('admin_incidents',next))return;
    incidents=next;renderIncidentsList();
  })();
};
...
byId('publish-confirm').addEventListener('click', async () => {
  ...
  if (!token) return alert('Token GitHub requis');
...
byId('tbody').addEventListener('dblclick', event => {
  const row = event.target instanceof Element ? event.target.closest<HTMLTableRowElement>('tr[data-id]') : null;
  if (row?.dataset.id) window._editItem(row.dataset.id);
});
byId('incidents-list').addEventListener('click', event => {
  const button = event.target instanceof Element ? event.target.closest<HTMLButtonElement>('button') : null;
  if (button?.dataset.editIncident) window._editIncident(button.dataset.editIncident);
  if (button?.dataset.deleteIncident) window._deleteIncident(button.dataset.deleteIncident);
});
...
// ============ INIT ============
const adminBusyNotice=document.createElement('p');adminBusyNotice.id='admin-storage-status';
adminBusyNotice.textContent='Chargement des données administrateur…';
adminBusyNotice.setAttribute('role','status');document.body.prepend(adminBusyNotice);
document.addEventListener('click',event=>{
  if(adminReady&&!adminWriting)return;
  if(event.target instanceof Element&&event.target.closest('button,input,select')){event.preventDefault();event.stopImmediatePropagation();}
},true);
async function initializeAdmin():Promise<void> {
  try{
    for(const key of AUXILIARY_KEYS)adminSnapshots.set(key,await adminStorage.load(key));
    incidents=readIncidents();loadCatalogs();adminReady=true;
    adminBusyNotice.textContent=adminStorage.lastWarning||'Stockage administrateur compact prêt.';
  }catch(error){adminBusyNotice.textContent='Données non chargées : '+errorMessage(error)+'. Aucune donnée effacée ; fermez les autres onglets et rechargez.';}
}
