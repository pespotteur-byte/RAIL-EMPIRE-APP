...
export function acquireImportUiLock(): () => void {
  ...
  return () => {
    if (fallback) document.removeEventListener('keydown', blockKeys, true);
    dialog.removeEventListener('cancel', cancel);
    try { if (dialog.open) dialog.close(); } catch { /* legacy dialog shim */ }
    dialog.remove();
  };
}
