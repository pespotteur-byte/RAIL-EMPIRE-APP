...
export function civilDayIndex(value: unknown): number | null {
...
export function resolveLegacyOperatingDay(q: LegacyDayQuery): string | null {
  const today = civilDayIndex(q.date);
  if (today == null || !Number.isFinite(q.minute) || !Number.isFinite(q.departure) || !Number.isFinite(q.maxRuntime)) return null;
  ...
  const allowed = (index: number, pinned = false): string | null => {
    if (completed != null && index <= completed) return null;
    const elapsed = now - (index * 1440 + dep);
    if (elapsed < -1 || elapsed > maxRuntime) return null;
    ...
    if (!pinned && (!q.weekdays.includes(((index + 4) % 7 + 7) % 7) || (q.dates.length > 0 && !q.dates.includes(day)))) return null;
    return day;
  };
  const pinned = civilDayIndex(q.pinnedDay);
  if (pinned != null) { const day = allowed(pinned, true); if (day) return day; }
  ...
  for (let d = 0; d <= lookback; d++) { const day = allowed(today - d); if (day) return day; }
