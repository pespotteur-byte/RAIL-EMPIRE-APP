...
interface AnchorLike {
  ...
  wayId?: unknown; segmentIndex?: unknown; osmSnapshot?: { segmentIndex?: unknown; [key: string]: unknown } | null;
}
export interface RoutePointLike {
  [key: string]: unknown;
  lat?: unknown; lon?: unknown; wayId?: unknown; segmentIndex?: unknown;
  maxSpeed?: unknown; maxSpeedSource?: string | null; maxSpeedForward?: unknown; maxSpeedBackward?: unknown;
  electrified?: unknown; electrifiedMode?: string | null; voltage?: unknown; frequency?: unknown; gauge?: unknown;
  loadingGauge?: string | null; axleLoad?: unknown; metreLoad?: unknown; tracks?: unknown; trafficMode?: string | null;
  usage?: string | null; service?: string | null; railway?: string | null; railwayLifecycle?: string | null; railwayBaseType?: string | null;
  preferredDirection?: string | null; bidirectional?: string | null; oneway?: string | null; signalRestrictedDirection?: unknown;
  travelDirection?: string | null; _againstPreferredDirection?: unknown; trackRef?: string | null; name?: string | null; ref?: string | null;
  trainProtection?: Record<string, unknown> | null; fallback?: unknown; tags?: { incline?: unknown; [key: string]: unknown } | null;
  pickerSource?: string | null;
}
interface CompactRoutePoint extends RoutePointLike {
  lat: number; lon: number; wayId: string; segmentIndex: number | null;
  maxSpeed: unknown; maxSpeedSource: string; maxSpeedForward: unknown; maxSpeedBackward: unknown;
  electrified: unknown; electrifiedMode: string; voltage: unknown[]; frequency: unknown[]; gauge: unknown[];
  loadingGauge: string; axleLoad: unknown; metreLoad: unknown; tracks: unknown; trafficMode: string;
  usage: string; service: string; railway: string; railwayLifecycle: string; railwayBaseType: string;
  preferredDirection: string; bidirectional: string; oneway: string; signalRestrictedDirection: boolean;
  travelDirection: string; _againstPreferredDirection: boolean; trackRef: string; name: string; ref: string;
  trainProtection: Record<string, unknown>; fallback: boolean; tags?: { incline?: unknown };
}
interface AnchorCoords { lat: number; lon: number; wayId: string; segmentIndex: number | null; osmSnapshot: AnchorLike['osmSnapshot']; }
export type RouteArray = RoutePointLike[] & { _longRangeWindowed?: boolean };
interface RailGraphPackLike { ready?: () => Promise<unknown> | unknown; prepared?: unknown; }
interface RoutingFailureLike { requiredBytes?: unknown; budgetBytes?: unknown; }
...
interface CodedRoutingError extends Error { code?: string; requiredBytes?: number; budgetBytes?: number; }
...
function structuredCloneSafe<T>(v:T): T | null {try{return typeof structuredClone==='function'?structuredClone(v):JSON.parse(JSON.stringify(v));}catch{return v||null;}}


function documentedSpeedMeta(p:RoutePointLike | null | undefined): {speed:number;source:string}|null {
...
function segmentSpeedMeta(a:RoutePointLike,b:RoutePointLike): {speed:number;source:string} {
...
function normalizeSameWayRoutePointSpeeds(points:CompactRoutePoint[]): CompactRoutePoint[] {
...
function anchorCoords(a:AnchorLike): AnchorCoords {
  return {
...
function compactRoutePoint(p:RoutePointLike={}): CompactRoutePoint {
  const incline=p?.tags?.incline ?? p?.incline;
  const out:CompactRoutePoint={
  ...
  if(incline!=null&&incline!=='')out.tags={incline};
  else out.tags={};
  return out;
}
...
export class ScheduleV2Router {
  ...
  constructor(orm:ORMRouterLike) { this.orm = orm; }

  async chooseTrackCandidatesAtCursor(lat:number, lon:number, options:RoutingOptions = {}): Promise<RoutePointLike[]> {
  ...
  async chooseTrackCandidates(lat:number, lon:number, options:RoutingOptions = {}): Promise<RoutePointLike[]> {
    if (!this.orm) throw new Error('ORM indisponible.');
    ...
    let local:RoutePointLike[]=[];
    try{ local=await this.orm.getTrackCandidates(lat, lon, pickerOpts) || []; }catch(e:unknown){ if((e as {name?:string}|null|undefined)?.name==='AbortError')throw e; }
    if(local.length) return local;
    let packPrepared=false;
    try{ await this.orm?._railGraphPack?.ready?.(); packPrepared=!!this.orm?._railGraphPack?.prepared; }catch{}
    if(packPrepared || options.allowNetworkPickerFallback===false) return local;
    const fallback=await this.orm.getTrackCandidates(lat, lon, { ...options, localOnly:false });
    return (fallback||[]).map((c:RoutePointLike)=>({ ...c, pickerSource:c?.pickerSource||'OSM_TINY_CLICK_FALLBACK' }));
  }

  async routeBetweenBindings(fromTrack:AnchorLike, toTrack:AnchorLike, constraints:AnchorLike[] = [], opts:RoutingOptions = {}): Promise<RouteArray> {
    if (!this.orm) throw new Error('ORM indisponible.');
    const all = [fromTrack, ...(constraints || []), toTrack].filter(Boolean);
    if (all.length < 2) throw new Error('Points de départ/arrivée manquants.');
    for (const a of all) {
      const p = anchorCoords(a);
      if (!Number.isFinite(p.lat) || !Number.isFinite(p.lon)) throw new Error('Position de voie invalide.');
    }
    ...
    let route:RouteArray|null|undefined;
    ...
    if (!route || route.length < 2 || this.orm.isFallbackRoute(route)) {
      ...
      if(reason==='RAILGRAPH_DATA_MISSING'){
        const err=new Error('Données vectorielles OSM de routage absentes et préparation du corridor mondial indisponible pour cette zone. Le fond OpenRailwayMap peut rester visible même quand les vecteurs de routage ne sont pas encore chargés.') as CodedRoutingError;
        err.code='RAILGRAPH_DATA_MISSING';throw err;
      }
      if(reason==='SOURCE_TOPOLOGY_GAP'){
        const err=new Error('Continuité interrompue dans les données ferroviaires ORM locales entre les points sélectionnés. Aucun raccordement artificiel n’a été créé.') as CodedRoutingError;
        err.code='ORM_SOURCE_TOPOLOGY_GAP';throw err;
      }
      if(reason==='NETWORK_UNAVAILABLE'){
        const err=new Error('Données ferroviaires ORM incomplètes ou serveur temporairement indisponible. Réessayez le calcul : aucun verdict « pas de chemin » n’a été enregistré.') as CodedRoutingError;
        err.code='ORM_NETWORK_UNAVAILABLE';throw err;
      }
      if(reason==='NO_CONNECTED_PATH'){
        const err=new Error('Aucun tracé ferroviaire ORM réel connecté n’existe entre les points sélectionnés dans les données complètes chargées.') as CodedRoutingError;
        err.code='ORM_NO_CONNECTED_PATH';throw err;
      }
      if(reason==='TIME_BUDGET'){
        const err=new Error('Le calcul exact du sillon a atteint sa limite de temps. Réessayez le calcul ; aucun faux verdict « aucun chemin » n’a été enregistré.') as CodedRoutingError;
        err.code='ORM_TIME_BUDGET';throw err;
      }
      if(reason==='RAILGRAPH_MEMORY_BUDGET_EXCEEDED'){
        ...
        const err=new Error(`Ce sillon exact dépasse le budget mémoire RailGraph${needed&&budget?` (${Math.ceil(needed/1048576)} Mo nécessaires / ${Math.floor(budget/1048576)} Mo autorisés)`:''}. Aucun itinéraire simplifié n’a été utilisé.`) as CodedRoutingError;
        err.code='RAILGRAPH_MEMORY_BUDGET_EXCEEDED';err.requiredBytes=needed;err.budgetBytes=budget;throw err;
      }
      const err=new Error('Le graphe ferroviaire ORM local n’a pas pu résoudre ce sillon.') as CodedRoutingError;
      err.code='ORM_LOCAL_ROUTE_UNRESOLVED';throw err;
    }
    return route;
  }

  snapshotRoute(route:RouteArray): {routePoints:CompactRoutePoint[];segments:RouteSegmentSnapshot[];distanceKm:number} {
    ...
    if((route as RouteArray)?._longRangeWindowed || route.length>12000){
      for(let i=0;i<routePoints.length;i++)route[i]=routePoints[i];
