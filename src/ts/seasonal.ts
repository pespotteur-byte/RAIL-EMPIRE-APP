...
interface SeasonDate { month: number; day: number; }
interface SeasonConfig { active?: boolean; freq?: number; }
type ServiceSeasonOverrides = Partial<Record<SeasonMode, SeasonConfig>>;
type PeakService = Record<string, unknown> & { id: string; frequency?: number };
interface SeasonalService { id?: string; name?: string; _v2ScheduleId?: string; [key: string]: unknown; }
interface SeasonalGame {
    scheduleCreator?: { services?: SeasonalService[]; _invalidateActiveCache?: () => unknown };
    scheduleV2Runtime?: { forceSync?: (timeOfDay: number, date?: string) => unknown };
    timeOfDay?: number;
    _currentDate?: string;
    engine?: { getParisDate?: () => string };
    saveState?: () => unknown;
}
...
export class SeasonalSchedule {
    ...
    checkSeason(dateStr: string | null | undefined): void {
        ...
        const newMode: SeasonMode = isSummer ? 'summer' : 'winter';
    ...
    render(container: HTMLElement | null, game: SeasonalGame): void {
        ...
        const services = rawServices.filter((svc) => {
            ...
            if (!key || seenServiceKeys.has(key)) return false;
      ...
      <div class="dash-section">
        ...
        const autoCheck = container.querySelector('#seasonal-auto') as HTMLInputElement | null;
        if (autoCheck) {
        ...
        container.querySelectorAll('.seasonal-toggle').forEach((node) => {
            ...
            btn.addEventListener('click', () => {
                ...
                if (season !== 'normal' && season !== 'summer' && season !== 'winter') return;
    ...
    toSave(): { mode: SeasonMode; autoSwitch: boolean; summerStart: SeasonDate; summerEnd: SeasonDate; serviceOverrides: Record<string, ServiceSeasonOverrides>; peakServices: PeakService[] } {
    ...
    loadFromSave(rawSave: unknown): void {
        ...
        if (isRecord(s.serviceOverrides)) {
            for (const [id, raw] of Object.entries(s.serviceOverrides)) {
                ...
                const clean: ServiceSeasonOverrides = {};
                for (const season of ['normal', 'summer', 'winter'] as const) {
                    const rawSeason = raw[season];
                    if (isRecord(rawSeason)) {
                        const f = Number(rawSeason.freq);
                        clean[season] = { active: rawSeason.active !== false };
                        if (Number.isFinite(f) && f > 0) clean[season]!.freq = f;
                    }
                }
        ...
        this.peakServices = (Array.isArray(s.peakServices) ? s.peakServices : [])
            .filter((x): x is UnknownRecord => isRecord(x) && !!x.id)
            .map((x) => {
                const out: PeakService = { ...x, id: String(x.id) };
                if ('frequency' in out) {
