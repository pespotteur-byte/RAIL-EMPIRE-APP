...
export class Sillon {
    constructor(input = {}) {
        ...
        this.route = Array.isArray(data.route) ? data.route.filter((p) => isRecord(p) && Number.isFinite(Number(p.lat)) && Number.isFinite(Number(p.lon)) && Number(p.lat) >= -90 && Number(p.lat) <= 90 && Number(p.lon) >= -180 && Number(p.lon) <= 180 && !p.fallback && !p.synthetic).map((p) => ({ ...p, lat: Number(p.lat), lon: Number(p.lon), maxSpeed: Number.isFinite(Number(p.maxSpeed)) ? Number(p.maxSpeed) : undefined, electrified: p.electrified !== false })) : []; // real ORM points only
...
export class SillonManager {
    ...
    add(data) {
        ...
        if (!sillon.isValid || !(sillon.distance > 0))
            return null;
        if (this.sillons.some((s) => s.id === sillon.id))
    ...
    getBetween(fromStationId, toStationId) {
        ...
        return this.sillons.filter((s) => s.fromStationId === fromStationId && s.toStationId === toStationId);
    }
    ...
    findPaths(fromStationId, toStationId, maxHops = 4) {
        ...
        const addResult = (entry) => {
            if (entry && !resultIds.has(entry.id)) {
