...
function reload(oldGame,time){
  const save=snapshot(oldGame), world=oldGame.world, stock=oldGame.stock;
...
test('A-B-C-D survives two reloads, ITE dwell, freight, staff and works without zombie stop',()=>{
  let game=initialFixture();
  for(let m=0;m<=2;m++) tick(game,m);
  assert.equal(svc(game).state,'stopped_at_station');
  assert.equal(svc(game).train.stoppedAt?.id,'B');
  assert.equal(svc(game).currentStopIndex,2);
  const beforeB={lat:svc(game).position.lat,lon:svc(game).position.lon};

  game=reload(game,2);
  assert.equal(svc(game).state,'stopped_at_station','B stop state survives F5');
  assert.equal(svc(game).currentStopIndex,2,'next stop remains C after F5 at B');
  assert.deepEqual(svc(game).position,beforeB,'physical B position survives F5');

  for(let m=3;m<=9;m++) tick(game,m);
  assert.equal(svc(game).state,'stopped_at_station');
  assert.equal(svc(game).train.stoppedAt?.id,'C');
  assert.equal(svc(game).currentStopIndex,3,'next stop is D while stopped at C');
  assert.ok(svc(game)._iteDwellExtra>=1,'ITE adds real dwell instead of hard-blocking');
  assert.equal(game.freightManager.contracts.find(c=>c.id==='F1')?.active,false,'freight contract delivered at C');
  assert.equal(game.freightManager.contracts.find(c=>c.id==='F1')?.quantity,0);
  const revenueAtC=game.economy.revenueByCategory.fret||0;
  assert.ok(revenueAtC>=2000,'freight revenue booked');
  const stoppedClock=svc(game).train._stoppedSinceGameTime;

  game=reload(game,9);
  assert.equal(svc(game).state,'stopped_at_station','C stop survives second F5');
  assert.equal(svc(game).currentStopIndex,3,'D remains next stop after F5 at C');
  assert.equal(svc(game).train._stoppedSinceGameTime,stoppedClock,'operational dwell clock survives F5');
  assert.ok(svc(game)._iteDwellExtra>=1,'ITE dwell survives F5');

  for(let m=10;m<=25 && !svc(game).completed;m++) tick(game,m);
  assert.equal(svc(game).completed,true,'service reaches D and completes');
  assert.equal(svc(game).state,'completed');
  assert.equal(svc(game).train.state,'completed');
  assert.equal(svc(game).train.stoppedAt,null,'completed service despawns from LiveMap at final terminus');
  assert.equal(game.rameManager.getById('R')?.currentLocation?.stationId,'D','rolling stock remains recorded at final terminus');
  assert.equal(game.freightManager.contracts.find(c=>c.id==='F1')?.quantity,0,'delivered freight does not respawn after reload');
  assert.ok((game.economy.revenueByCategory.fret||0)>=revenueAtC,'freight revenue is not lost on reload');
});
