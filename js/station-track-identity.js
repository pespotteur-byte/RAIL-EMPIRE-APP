...
export function canonicalTrackRef(value) {
...
export function normalizeStationTrackIdentity(value) {
    const data = record(value);
    if (!data || (data.kind !== 'native' && data.kind !== 'osm'))
        return null;
...
export function stationTrackIdentityFromBinding(value) {
    const data = record(value);
    if (!data)
        return null;
    ...
    if (!nativeId && !wayId)
        return null; // An old display label is not physical evidence.
...
export function resolveNativeStationTrack(identity, stationId, tracks) {
    ...
    const candidates = local.filter(track => {
        if (ref && canonicalTrackRef(track.voie) !== ref)
            return false;
