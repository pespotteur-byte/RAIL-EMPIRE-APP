...
| MNT-03 | Pannes bénignes (clim, portes) : pas de technicentre obligatoire | P1 | A6.2 | [x] PR7 (limité à 80 km/h en route, réparé en gare avec +5 min retard, testé) |
| MNT-04 | Pannes moteur/freins : arrêt urgence (plus long si freins) → DDS | P1 | A6.2 | [x] PR7 (arrêt + appel DDS ; annulation si secours absent après max runtime, testé) |
...
| INC-03 | Effets hors zone (bouchons en accordéon) | P1 | A10.1 | [x] PR7 (corrigé : incident stop en gare bloque le départ immédiat + filtrage par segment parcouru ; incident stop sur la ligne annule le service après blocage prolongé) |
...
| TRV-03 | Fermeture de voie : cocher case + tracer portion ; trains reroutés en sécurité | P1 | A10.2 | [x] PR? (PlannedWorks stationA/stationB + reroutage ORM avoidStationPairs) |
...
| TRV-06 | Toutes voies fermées : aucun train ; itinéraires déroutement A/B/C/D ou renoncer (alerte) | P1 | A10.2 | [x] PR? (détection fermeture + ORM essaie itinéraires alternatifs ; UI alerte si échec) |
...
| ARC-04 | Option carte basique (sans tracés ORM) + satellite + .org/.app | P2 | A13 | [x] PR? (toggle-satellite + livemap controls, base Carto + Esri satellite) |
| ARC-05 | Streaming/chargement progressif ORM par zone | P1 | A13 | [x] PR? (ORMClient.fetchArea par bbox + cache + reloadAreas depuis save) |
