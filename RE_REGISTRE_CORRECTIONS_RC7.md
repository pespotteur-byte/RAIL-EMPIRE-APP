# Rail Empire — registre des corrections RC7

**74 dossiers clos sur 87 : 85,1 %.** Base RC6R : 71/84. Six dossiers restent partiels, sept ouverts.

Trois nouvelles causes indépendantes ajoutées et closes. Les compléments de physique, voies et sécurité renforcent des familles existantes sans point supplémentaire. Le périmètre général LM06 reste partiel. Ne mesure pas la proportion de tout le code sans défaut.

## État détaillé

| ID | Famille | Anomalie | RC6R | RC7 | Preuve ou limite |
|---|---|---|---|---|---|
...
| SC20 | SC / exploitation | Voie physique exacte remplacée par un nom modifiable au pont SC-runtime | Partiel | Partiel | Partiel. RC7 refuse une voie native explicitement demandée absente/étrangère plutôt que de réserver une autre voie libre. Alias numériques conservés. Le pont SC-runtime doit encore transmettre partout une identité canonique indépendante du nom affiché. |
...
| LM05 | LiveMap / ressources | Voies alphanumériques remplacées par une voie numérique libre | Clos | Clos | Complément RC7 dans VoiePointManager : A, 1 bis, V1M indisponibles ne deviennent plus une voie libre différente ; point appartenant à une autre gare refusé ; alias V1/voie 1 et choix automatique sans préférence conservés. Identité OSM générale : SC20 reste partiel. |
...
| LM18 | LiveMap / passages | Freinage jusqu’à l’arrêt imposé aux passages et points de voie sans arrêt commercial | Non suivi | Clos | Tests RC7-PASS full/macro : franchissement direct, arrêt réel conservé, anticipation de la limitation/du prochain arrêt, route manquante ou discontinue bloquée, occupation et restrictions de queue conservées. Essai navigateur d’un passage à 30 km/h. Ne certifie pas tout le replay hors ligne. |
| LM19 | LiveMap / changement de liaison | Ancienne clé de route conserve le train dans un groupe LOD bas après un changement de liaison | Non suivi | Clos | Tests réels moveTick à 1 et 601 services (pool factice pour les autres), remise à zéro de clé et reconstitution de la bonne liaison. Chromium : deux observations successives après le passage montrent distance croissante, vitesse 30, LOD high ; pas d’attente du cycle macro de 3 secondes. Différent du blocage durable LM03. |
| ECO12 | Économie / trajet retour | Distance et métadonnées facturées du retour indépendant prises sur l’itinéraire aller | Non suivi | Clos | Test RC7-ECO-return sur une route aller de 1 km et un retour distinct de plus de 20 km : le callback économique reçoit les vrais kilomètres/points du retour. Aucun tarif n’est modifié. |

## Sources des preuves

Les clôtures héritées conservent les suites RC1–RC6R. Les nouveaux tests sont `js/__tests__/re-rc7-movement.test.mjs`, `re-rc7-tail-index.test.mjs` et `re-rc7-build-version.test.mjs`. Journaux et résultats : `QA/RE_REPAIR_RC7/`. Aucun dossier partiel n’est compté comme clos.
