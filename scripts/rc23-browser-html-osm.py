...
root=Path(__file__).resolve().parents[1];out=root/'QA/RE_REPAIR_RC23/browser-osm-isolated';out.mkdir(parents=True,exist_ok=True)
im=Image.new('RGBA',(256,256),(235,231,222,255));d=ImageDraw.Draw(im)
d.rectangle((0,0,127,127),fill=(128,184,126,255));d.rectangle((128,0,255,127),fill=(116,185,220,255));d.line([(0,220),(255,150)],fill=(243,194,83,255),width=16);d.text((8,8),'RC23 TEST TILE - NOT OSM',fill='black')
...
async def main():
 ...
 try:
  async with async_playwright() as p:
   ...
   await page.fill('#login-name','RC23 OSM isolated test');await page.click('#btn-new-game');await page.wait_for_function('game.running && game.ui');await page.wait_for_timeout(1000);print('game running',flush=True)
