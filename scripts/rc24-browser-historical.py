...
root=Path(os.environ.get('RE_GAME_ROOT',str(Path(__file__).resolve().parents[1]))).resolve();out=Path(os.environ.get('AUDIT_OUT',str(Path(__file__).resolve().parents[1]/'QA/RE_REPAIR_RC24')))/'browser-historique';out.mkdir(parents=True,exist_ok=True)
async def main():
 async with async_playwright()as p:
  ...
  print('login ready',errors,console[-3:],flush=True);await page.fill('#login-name','RE RC24 — Essai navigateur isolé');await page.click('#btn-new-game')
  ...
  for item in pages:
   ident=item['id']
   if ident in ['qg','map','rolling-stock','liveries','rames','schedules','rotations','lines','depots','infogare','staff','weather','dashboard','incidents','graph-marche','cargo-types','industrial-clients']:
    try:
  ...
  assert diagnosticExport['build']=='RC24' and diagnosticExport['counts']['moving']==600 and diagnosticExport['sampled']==512 and diagnosticExport['truncated']==88 and diagnosticExport['positionsPresent'] and not diagnosticExport['containsRawRoutes'],diagnosticExport
