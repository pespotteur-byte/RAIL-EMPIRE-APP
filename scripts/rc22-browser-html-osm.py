"""Actual game bundles and Canvas/Image/DOM. HTTP assets/storage and file protocol
are simulated because platform policy blocks native file navigation. Tile requests
use native browser fetch from opaque Origin:null and intercepted synthetic responses.
No request reaches OSM/ORM. Browser HTTP cache behavior not certified (routing).
"""
...
root=Path(__file__).resolve().parents[1];out=root/'QA/RE_REPAIR_RC22/browser-osm-isolated';out.mkdir(parents=True,exist_ok=True)
im=Image.new('RGBA',(256,256),(235,231,222,255));d=ImageDraw.Draw(im)
d.rectangle((0,0,127,127),fill=(128,184,126,255));d.rectangle((128,0,255,127),fill=(116,185,220,255));d.line([(0,220),(255,150)],fill=(243,194,83,255),width=16);d.text((8,8),'RC22 TEST TILE - NOT OSM',fill='black')
 ...
 try:
  async with async_playwright() as p:
   ...
   async def intercept(route):
    if urlparse(route.request.url).hostname=='tile.openstreetmap.org':
     status=await page.evaluate('window.tileStatus||200')
     native_requests.append({'url':route.request.url,'method':route.request.method,'headers':await route.request.all_headers(),'status':status})
     return await route.fulfill(status=status,body=bytes(tile) if status==200 else b'blocked',headers={'Content-Type':'image/png' if status==200 else 'text/plain','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'X-Requested-With','Access-Control-Allow-Methods':'GET,OPTIONS','Access-Control-Expose-Headers':'Retry-After,x-blocked','Cache-Control':'public, max-age=604800','Retry-After':'120'})
   ...
   await page.evaluate('''(bytes)=>{
    ...
    window.fetch=async(src,options)=>{if(String(src).startsWith('https://tile.openstreetmap.org/')){tileCalls.push({url:String(src),options:{cache:options?.cache,referrerPolicy:options?.referrerPolicy,credentials:options?.credentials,headers:options?.headers??null}});return nativeFetch(src,options);}const text=await __readProvidedAsset(String(src));return new Response(text,{headers:{'Content-Type':String(src).includes('.json')?'application/json':'text/plain'}});};
   ...
   await page.evaluate('''()=>{
    ...
    TileMap.prototype._protocol=()=> 'file:'; // Model HTML-file mode; actual native fetch has opaque Origin:null.
   ...
   assert calls[0]['options']=={'cache':'default','referrerPolicy':'strict-origin-when-cross-origin','credentials':'omit','headers':{'X-Requested-With':'RailEmpire'}},calls
   ...
   await page.fill('#login-name','RC22 OSM isolated test');await page.click('#btn-new-game');await page.wait_for_function('game.running && game.ui');await page.wait_for_timeout(1000);print('game running',flush=True)
   ...
   result['nativeTileRequests']=native_requests
   assert native_requests and all(q['headers'].get('x-requested-with')=='RailEmpire' and q['headers'].get('origin')=='null' and not q['headers'].get('referer') for q in native_requests),native_requests
