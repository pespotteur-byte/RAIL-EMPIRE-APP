"""Package a complete RC19 tree and verify every archived file against a SHA-256 manifest."""
from pathlib import Path
from datetime import datetime, timezone
import hashlib, json, zipfile, os

root=Path(__file__).resolve().parents[1]
output=root.parent/'Rail_Empire_S3_GAMEPLAY_REPAIR_RC19.zip'
partial=output.with_suffix('.zip.partial')
manifest_name='RC19_SHA256_MANIFEST.json'
sha=lambda data:hashlib.sha256(data).hexdigest()
summary=json.loads((root/'QA/RE_REPAIR_RC19/SUMMARY.json').read_text())
assert summary['pass'] and summary['independentBuild']['pass']
freeze=json.loads((root/'QA/RE_REPAIR_RC19/SOURCE_FREEZE.json').read_text())
