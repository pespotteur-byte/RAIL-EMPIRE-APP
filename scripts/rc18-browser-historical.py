...
async def main():
 async with async_playwright()as p:
  ...
  (out/'rc18-export-control.json').write_text(json.dumps(exportControl,ensure_ascii=False,indent=2))
  ...
  await download.save_as(str(out/'rc18-movement-export.json'))
  snapshot=json.loads((out/'rc18-movement-export.json').read_text())
  diagnosticExport={'suggestedFilename':download.suggested_filename,'bytes':(out/'rc18-movement-export.json').stat().st_size,'schemaVersion':snapshot['schemaVersion'],'build':snapshot['build'],'counts':snapshot['counts'],'sampled':snapshot['sampled'],'truncated':snapshot['truncated'],'positionsPresent':all(x['position'] is not None for x in snapshot['services']),'containsRawRoutes':any('cachedRoute' in x for x in snapshot['services'])}
  assert diagnosticExport['build']=='RC18' and diagnosticExport['counts']['moving']==600 and diagnosticExport['sampled']==512 and diagnosticExport['truncated']==88 and diagnosticExport['positionsPresent'] and not diagnosticExport['containsRawRoutes'],diagnosticExport
  ...
  await page.screenshot(path=str(out/'rc18-osm-panel.png'))
  ...
  await page.screenshot(path=str(out/'rc18-osm-panel-768.png'))
  ...
  await page.screenshot(path=str(out/'rc18-rescue-controls-768.png'))
  ...
  await page.screenshot(path=str(out/'rc18-storage-before-768.png'))
  ...
  await download.save_as(str(out/'rc18-storage-diagnostic-download.json'))
  downloadReport=json.loads((out/'rc18-storage-diagnostic-download.json').read_text())
  ...
  for width in [768,375]:
   ...
   await page.screenshot(path=str(out/f'rc18-storage-panel-{width}.png'))
  ...
  await page.set_viewport_size({'width':768,'height':1024})
  (out/'rc18-storage-browser.json').write_text(json.dumps({'harness':'Actual Chromium release bundle and native gzip APIs, supplied assets injected, external network denied, memory Web Storage fallback, native IndexedDB not certified.','checks':storageChecks,'statsText':storageReportText,'progressText':progressText,'layouts':layouts,'download':downloadReport,'keyboardEscapeFocusRestored':True},ensure_ascii=False,indent=2))
  ...
  (out/'rc18-rescue-tow.json').write_text(json.dumps({'harness':'Actual release bundle, real main movement loop and depot UI in Chromium. Deterministic supplied route; no live railway-provider validation.', 'checks':rescueTow},ensure_ascii=False,indent=2))
  await page.screenshot(path=str(out/'rc18-rescue-depot.png'))
  ...
  replayCheck=await page.evaluate("""async()=>{
    ...
    try{
      ...
      if(!(after>before&&after-before<=5000&&debt>3590&&!game.engine.paused))throw Error(JSON.stringify({before,after,debt,paused:game.engine.paused}));
      if(!status.includes('Rattrapage chronologique'))throw Error(status);
      game._saveStateNow();if(game._saveWritePromise)await game._saveWritePromise;
