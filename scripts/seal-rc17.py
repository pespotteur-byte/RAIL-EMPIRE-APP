"""Seal a qualified RC17 FULL tree, then validate every archived byte.

Run after all build, tests and report-writing processes have finished:
    python scripts/seal-rc17.py
...
OUT = ROOT.parent
QA = ROOT / 'QA' / 'RE_REPAIR_RC17'
MANIFEST = ROOT / 'QA' / 'FILE_SHA256_MANIFEST.txt'
DEST = OUT / 'Rail_Empire_S3_GAMEPLAY_REPAIR_RC17.zip'
TEMP = DEST.with_suffix('.zip.tmp')
RECORD = OUT / 'RE_RC17_PACK_INTEGRITY.json'
...
def main() -> None:
    ...
    candidates = sorted(p for p in ROOT.rglob('*') if '.git' not in p.relative_to(ROOT).parts)
    ...
    print(f'Building RC17 FULL: {len(files)} files.', flush=True)
    ...
    final_files = sorted(p for p in ROOT.rglob('*') if p.is_file() and '.git' not in p.relative_to(ROOT).parts)
    ...
    print('RC17 FULL sealed and verified.', flush=True)
