"""Seal a qualified RC15 FULL tree, then validate every archived byte.

Run after all build, tests and report-writing processes have finished:
    python scripts/seal-rc15.py
...
OUT = ROOT.parent
QA = ROOT / 'QA' / 'RE_REPAIR_RC15'
MANIFEST = ROOT / 'QA' / 'FILE_SHA256_MANIFEST.txt'
DEST = OUT / 'Rail_Empire_S3_GAMEPLAY_REPAIR_RC15.zip'
TEMP = DEST.with_suffix('.zip.tmp')
RECORD = OUT / 'RE_RC15_PACK_INTEGRITY.json'
...
def main() -> None:
    ...
    print(f'Building RC15 FULL: {len(files)} files.', flush=True)
    ...
    print('RC15 FULL sealed and verified.', flush=True)
