...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   await page.goto((r/'AIDE_CARTE_OSM.html').as_uri(),wait_until='domcontentloaded',timeout=10000)
   result['url']=page.url;result['title']=await page.title();result['text']=(await page.locator('body').inner_text())[:500]
   result['pass']=await page.evaluate('location.protocol === "file:" && document.title.includes("RC22")')
